...
| Araç | Ne zaman |
|---|---|
| `Tools > VortexArena > Build > Configure All Build Elements` | **Build öncesi tek düğme: Hepsini Çalıştır.** Aktif sahne bir arena kutusuysa önce onun `MapDefinition`'ını yazar, sonra HER durumda eşitler (`GameCatalog` + `ModeDefinition.maps` + Build Settings + `maps.json` + **silah kiti** + **rastgele silah havuzları** (`ModeDefinition.loadout`) + **net eşya kataloğu**, ayrıca HMD katmanları **yalnız bayatsa**); idempotent, sahne açık olmadan da koşar. **Hazırlık** satırları yalnız OKUR — her satırın tooltip'i ne zaman gerektiğini yazar. ⚠️ Ayrı "Arena Id" alanı YOKTUR. ⚠️ Hazırlık'ta kalan ✗ **insan adımıdır** |
