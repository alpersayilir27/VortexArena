  ...
  Assets\
    _Shared\                 ← "ikinci bir mod/arena bunu aynen kullanır mı?" → EVET olan her şey
      ...
      Arsenal\ FX\ Environments\ Scenes\             (kod-dışı ortak içerik + Boot/Lobby +
                             Scenes\WeaponCalibration — kavrama yakalama tezgâhı; Build
                             Settings'e GİRMEZ, yalnız editörde açılır)
...
| Sınıf | Görevi |
|---|---|
| `AppBoot` | Rol çözümü: Android → player; masaüstü → `--role` > `VORTEX_ROLE` > admin. **Sahne her rolde `Lobby`'dir** (admin'in ayrı kabuğu yok). ⚠️ **`weapon` rolü buradan yüklenemez** — kalibrasyon sahnesi Build Settings'te değildir (yalnız editör); o rol Play'e `DevBootstrap` üzerinden girer. ⚠️ Bu dala düşülürse `AppBoot` **Lobby'ye GİTMEZ**: hata basıp durur — devam etseydi kullanıcı kalibrasyon yerine bağlantı arayan Lobby ekranını görür ve sebebin rol yönlendirmesi olduğunu anlamasına yarayacak hiçbir ipucu kalmazdı. **Adres çözümü:** `--server-ip` / `--server-port`'u **her rolde** okuyup `AppSession`'a yazar (player'da keşif zincirinin en üstü; admin'de tek kaynak — yoksa uyarı loglar). `AppSession.RoleResolved` doluysa hiçbir şey yazmaz → editörde `DevSession` kazanır. **Inspector'da rol/IP override alanı YOKTUR** (kaldırıldı: sahneyi kirletiyordu) |
...
| `AppSession` | Oturum: rol + sunucu adresi (`ServerIp`/`ServerPort`/`HasServerEndpoint`) — `AppBoot` yazar, controller'lar okur. ⚠️ **`IsWeaponCalibration`** = oturum kapısının okunduğu yer; okuyan taraf `AppSingletons`'tır |
| `AppSingletons` | **App tekillerinin TEK kurulum noktası**: tek bir `RuntimeInitializeOnLoadMethod(AfterSceneLoad)` taşır ve listeyi çağırır (`ConnectionOverlay` · `LoadingOverlay` · `MatchResultOverlay` · `IdentifyOverlay` · `KickedShutdown` · `SceneRouter` · `AdminSpectator`). ⚠️ **Tekillerin kendi `Install`'ları KOŞULSUZDUR** — "bu oturumda gerekli mi" sorusunu yalnız burası cevaplar. Gerekçe: kapı tekillere dağılsaydı yeni bir oturum türü (test/araç kipi, yeni rol) eklemek N dosyayı tek tek düzenlemek olurdu ve biri atlandığında hata değil, o tekilin beklenmedik bir yerde belirmesi olarak görünürdü. Bugünkü tek koşul silah kalibrasyonudur: orada bağlanacak sunucu yoktur, yani bağlantı kartı kaçınılmaz olarak "sunucu bulunamadı" der ve ölçüm ekranını kapatır; ⚠️ ikisi ayrıca **koruyucudur** — `SceneRouter` sunucu mesajıyla sahneyi altımızdan alır, `AdminSpectator` ölçülecek eli taşıyan rig'i kapatır. ⚠️ **Sıra önemsizdir ve öyle kalmalı** (tekiller `Install` anında birbirini çağırmaz, olaya abone olur); ⚠️ Core tekilleri (`HandGripPoser`, `WeaponGranter` …) buraya GİRMEZ — Core App'i referanslamaz, onlar kendi bayraklarıyla susturulur (`HandGripPoser.Suspended`). Sahneye konan `LobbyController` merkezden kurulamaz, o yüzden `Awake`'te kendi kapısına bakar |
...
| Sınıf | Görevi |
|---|---|
| `DevWindow` | `Tools > VortexArena > Development > Dev` penceresi: "Dev enjeksiyonu açık" onayı · **Rol** (Player/Admin/**Silah**) · **Silah** rolünde ayrıca bir `WD_*` seçicisi (seçim `WeaponGripCalibrationSession.SelectedWeaponPath` üzerinden `EditorPrefs`'te kişisel kalır) ve hedef/sandbox/başlangıç blokları **sönük çizilir** — o rolde sunucuya hiç bağlanılmaz · **Sunucusuz sandbox** (+ mod seçicisi; açıkken Hedef bloğu devre dışı) · **Hedef** (`dev-targets.json` + "Özel…" IP/Port + Tazele) · **Başlangıç** (Boot'tan / Açık sahneden) · alttaki "Seçim: …" özeti. **Sunucuya hiç dokunmaz** — ne başlatır, ne durdurur, ne derler (§6.1). Maç parametresi taşımaz — mod/takım/süre/limit sunucudan gelir. **Modal dialog kullanmaz** (Unity CLI doğrulamasını kilitliyor); geri bildirim konsol + `HelpBox` |
...
| `DevBootstrap` | Editör kancaları: "Boot'tan" kipinde `EditorSceneManager.playModeStartScene`'i Boot sahnesine ayarlar (sahne **Build Settings'ten** bulunur, sabit yol gömülmez); **Silah** rolünde aynı kanca `_Shared/Scenes/WeaponCalibration.unity`'i açar — o sahne Build Settings'te olmadığı için ad araması `AssetDatabase`'ten yapılır ve `AppBoot` onu yükleyemez; `Ctrl+Alt+R` kısayolunu kurar (rol player → admin → weapon). ⚠️ Bu yol sessizce düşerse (sahne bulunamadı, atama ezildi) Unity **açık sahneden** başlar; onu yakalayan şey `DevSession`'ın açık sahne adını doğrulayan hata satırıdır. **Hiçbir süreç öldürmez** — sunucu kasıtlı olarak yaşar (üretimde de ayrı makinede sürekli açık) |
...
DEĞİL, tabandaki `ItemDefinition`'ın yakalanmış kavrama kayıtlarındadır) + `WeaponAudio` (Meta XR spatializer'lı namlu AudioSource:
...
| `Combat/WeaponGripCalibrationSession` | Ölçülecek silahın seçimi (`EditorPrefs`, `VortexArena.Dev.WeaponAsset`). Anahtar burada, **tek yerde** tanımlıdır çünkü iki assembly okur: dev penceresi (`VortexArena.App.Editor`) yazar, kalibrasyon bileşeni (`VortexArena.Core`) okur — iki yerde yazılmış bir string "seçtim ama gelmedi" biçiminde sapardı. ⚠️ Yol taşınmış/silinmiş olabilir (`EditorPrefs` asset takibi yapmaz), çağıran `null` ihtimalini karşılar |
| `Combat/ItemGripAuthority` | "Bu eşya bu elde nasıl durur" sorusunun **etkin** cevabı (saf statik): `TryResolvePrimaryGrip(definition, rightHand, out pos, out rot)` → **ROTASYON `identity`** (eşya eksenleri kumanda anchor'ıyla birebir; yakalanan pozun açısı eşyayı DÖNDÜRMEZ, o açı el MODELİNİ sürer: `HandGripPoser` bileği kayda kilitler), **KONUM `anchorToWrist.position − capture.position`** — yani yakalanan bilek noktası canlı bileğin noktasına oturtulur; anchor→bilek deltası sabit değil **canlı ölçümdür** (`HandGripPoser.TryGetAnchorToWrist`). Eşyayı yakalanan elin açısıyla döndürmek "kumandayı uzatınca namlu başka yöne bakıyor" demekti. ⚠️ **Sol ve sağ AYRI çözülür** — kayıtlar el başınadır, kabza simetrik olmadığı için iki elin bilek noktası eşyanın farklı yerlerine düşer. ⚠️ **Çözülemediğinde `false` döner ve çağıran `ItemDefinition.PrimaryGripPosition(bool)`'a düşer**: kavraması yakalanmamış silah, rig'i olmayan uç (admin gözlemci, editör) ve delta ölçülmeden önceki ilk kare o yoldan geçer — fallback bir hata değil normal bir yoldur ve **aynı kayıttan** türer (`−capture.position`), yani iki yol arasında sapma olamaz. ⚠️ **Sıra her çağıranda AYNI olmak zorunda** (`Weapon.ApplyCanonicalGrip`, `WeaponGranter.SolveInitialGrip`, `RemoteAvatar.ApplyGrip`/`TryResolveGripPalm`): biri canlı çözüme, öteki fallback'e bakarsa silah verildiği karede bir duruşta belirip sonraki karede ötekine zıplar — ya da el silahın yanında yüzer. Ters yön (`GripPointOnItem`) de buradan alınır. ⚠️ **Kayıt METRE ve ölçeksizdir**, geri bileşim daima `item.position + item.rotation * p`'dir — `TransformPoint` DEĞİL (§7). ⚠️ **Protokolde karşılığı YOKTUR, tel formatı değişmez** (§6.6): duruş yine telde gitmiyor, iki uç da aynı `WD_*`'ı okuyup aynı deltayı ölçüyor (delta kumanda sürümlü el pozlarından türer, yani her başlıkta aynıdır) |
| `Combat/HandGripPoser` | Elde tutulan silahın kavrama pozunu ISDK'nın **sentetik eline** uygular — oyuncunun gözlükte gördüğü el budur: bilek `SyntheticHand.LockWristPose` ile yakalanan kavrama noktasına kilitlenir — ⚠️ **YALNIZ KONUM** (`WristLockMode.Position`), **dönüş kilitlenmez ve kilitlenmeyecek**: eşyanın dönüşü zaten kumanda anchor'ının dönüşüdür, bileğe ayrıca yakalanmış bir dönüş yazmak eli kumandadan koparır — oyuncu kumandayı dosdoğru ileri tutarken elini yakalama anındaki bilek eğikliğiyle yamuk görür ve silah, dosdoğru duruyor olmasına rağmen yamuk tutuluyormuş gibi okunur; kilit kalkınca el kumandayla döner ve el+silah tek parça durur (elin silah üstündeki AÇISI kalibrasyondan değil donanımın anchor→bilek sözleşmesinden gelir, yani her başlıkta aynıdır). ⚠️ Poz `worldPose: true` ile verilir — çeviriyi `LockWristPose` yapar, doğrudan `LockWristPosition` çağırmak izleme→dünya çevirisini atlar ve el rig'in dünyadaki yerine göre sessizce kayar. ⚠️ **parmaklara poz YAZILMAZ** — beş parmağa açıkça `JointFreedom.Free` yazılır, yani parmaklar izlemeden/kumandadan gelir (`controllerDrivenHandPosesType = Natural`: grip parmakları kapatır, tetik parmağı serbest kalır). ⚠️ **`Free`'yi açıkça yazmak zorunludur**: boş elde parmaklar KİLİTLİ sürülüyor ve o kilit devralınırsa el silah gelince donar. Eşya başına parmak duruşu diye bir authored veri YOKTUR — silahın nasıl sarıldığı elin kendi girdisinden çıkar. **Kendini önyükleyen kalıcı tekil** (`WeaponGranter` kalıbı) — sahneye konmaz, yeni arenaya kurulum adımı doğurmaz; rig yoksa (admin gözlemci, sahne henüz yüklenmedi) sessizce hiçbir şey yapmaz. ⚠️ **Silahın pozuna DOKUNMAZ**: silahın dünya pozunun tek yazarı `Weapon.ApplyCanonicalGrip` + `ItemGripSolver`'dır; iki yazar aynı silahı kendi ekranında başka, karşı ekranda başka gösterirdi. ⚠️ **Yalnız YEREL oyuncuda koşar** — uzak avatarın eli ağdan gelen iskeletten çizilir, bu sınıfın ağ tarafında hiçbir işi yoktur ve **protokolde karşılığı yoktur**. ⚠️ **Bilek kilidi KOŞULSUZDUR**, mesafe/açı kapısı yoktur ve eklenmez: bedeli fiziksel kumanda kavrama noktasından uzaklaşınca elin kolla arasının görsel olarak gerilmesidir, kazancı oyuncu grip tuşunu bırakmadıkça elin silahtan kopmamasıdır — mesafeye bakan bir kapı özellikle ön kabzada eli oyuncu hiçbir şey yapmadan bırakır ve "iki elle tuttum ama ikinci el havada" hissi üretir. Bu bir üründür, eksik değil. `[DefaultExecutionOrder(100)]`: `Weapon.LateUpdate` silahın pozunu yazıyor, bilek ona kilitleniyor — daha erken koşan bir kilit bir kare gerideki silaha sarılır ve hızlı harekette titrer. Sentetik el **adla** süzülür (`SyntheticHandData`): rig'in altında mesafeli kavramanın hayalet elleri de aynı tipte, filtresiz arama oyuncunun elini bırakıp odanın öbür ucundaki hayaleti sarardı. Eli serbest bırakmak yalnız **kilitli → serbest geçişinde** yapılır (her karede koşulsuz `FreeWrist` ISDK'nın kendi kilitlerini de iptal ederdi). Kavraması yakalanmamış silahta davranış değişmez (el kumanda duruşunda kalır) + oturum başına tek uyarı. ⚠️ **İkinci işi ölçüdür: anchor → izlenen bilek deltasını her karede, el başına ölçer** (`TryGetAnchorToWrist`) — silahın duruşunu çözen taraf (`ItemGripAuthority`) elin ANCHOR pozunu bilir, kayıt ise bileği tarif eder; ikisi arasındaki köprü bu deltadır ve onsuz kavrama tahmin sabitlerine düşer. ⚠️ **İzlenen bileği veren tek kapı `TryGetTrackedWrist`'tir** ve kalibrasyon aracı da ondan okur: yakalamanın ölçtüğü bilek ile oyunun kullandığı bilek aynı kaynaktan gelmezse iki uç sessizce sapar (§7). `GetSynthetic(bool)` sentetik eli, `Suspended` ise **poz uygulamasını** askıya alır (kalibrasyon boyunca açıktır: bilek ve parmak kilidi çözülür, el ham izlemeye bırakılır) — ⚠️ askı yalnız UYGULAMAYI durdurur, el çözümü ve bilek ölçümü sürer; sürmezse kalibrasyon eli hiç bulamaz. Ölçü sentetik elin **KAYNAĞINDAN** alınır (`ModifyDataFromSource`, izleme uzayı → dünya çevirisi `ITrackingToWorldTransformer` ile), sentetik elin kendisinden DEĞİL: bileği zaten bu sınıf kilitliyor, kilitli eli okumak ölçüyü bir kare içinde kendi çıktısına kilitlerdi. Değer execution order yüzünden **bir kare bayattır** ve bu bilinçlidir — delta fiziksel olarak sabit bir ofsettir, bir karelik gecikmesi görünmez; ölçümü öne almak elin bir kare gerideki silaha sarılması pahasına olurdu. ⚠️ **Boştaki el de bu sınıfın kilidindedir ve kuralı silahlı elin TERSİDİR:** silah yokken (ya da kavrama yakalanmamışsa) el izlemeye BIRAKILMAZ, `ApplyIdle` gevşek duruşu yazar — bilek serbest, **parmaklar kilitli**. Gerekçe §6.9 ile aynı: boş elin duruşu oyuncunun ekranında da uzak ekranlarda da tek bir tanımdan gelmeli (`HandPoseProfile.Idle`), yoksa oyuncu kendi elini kıvrılırken görür, başkaları sabit görür. İki dalın zıt olması, silahlı dalda `Free`'nin **açıkça** yazılmasını zorunlu kılar. ⚠️ Idle duruşu bir **asset değil ÖRNEKtir**: kumanda kipinde hiçbir tuşa basılmıyorken üretilen poz zaten "yumruk değil, hafif açık" eldir ve bir kez örneklenip sonsuza dek uygulanır — tetik/grip basılıyken örnek alınmaz (yarı yumruk bir idle kalıcı olurdu). Elle yazılmış bir idle donanımın ürettiğinden kaçınılmaz olarak saparadı ve fark tam da oyuncunun kendi eline baktığı yerde görünürdü |
...
| `Combat/HandPoseProfile` | Bir elin parmak duruşu: **beş sayı**, parmak başına kapanma oranı (`0` açık, `1` kapalı) + `Idle`/`DefaultGrip` hazır değerleri. ⚠️ **Var olma sebebi SENTEZDİR:** parmaklar telde gitmiyor (§6.9), yani uzak avatarın parmakları üretilmek zorunda — eşya tutan elde `DefaultGrip`, boş elde `RemoteAvatar.idleHandPose` (boşsa `Idle`). ⚠️ **Eşya başına yazılan bir profil YOKTUR ve eklenmez:** "bu silah nasıl sarılır" sorusunun ayarlanabilir bir cevabı olsaydı her silahta ikinci bir elle ayar düğmesi doğardı; yerel elde parmaklar zaten izlemeden geliyor. ⚠️ Quaternion DEĞİL oran tutulur, çünkü duruş iki ayrı iskelete uygulanabilmeli ve iki iskeletin kemik eksenleri aynı değil — ham rotasyon taşımak §7'deki "izleme uzayından gelen rotasyon humanoid kemiğe doğrudan yazılmaz" tuzağının parmak ölçeğinde tekrarı olurdu. ⚠️ `IsEmpty` (tümü sıfır) **yazılmamış** sayılır: değeri hiç girilmemiş bir alan elin tahta gibi düz kalmasına değil makul bir kavramaya düşsün |
...
geometrisini üreten araçlar (kavrama artık burada değil: ölçüm gözlükle yapılır —
`Combat/WeaponGripCalibration`):
...
| Pencerede | Ne yapar |
|---|---|
| **Rol** (Player / Admin / Silah) | `AppSession.Role`'ü Boot koşmadan önce yazar. **Kısayol `Ctrl+Alt+R`** üç rolü sırayla döndürür (player → admin → weapon) ve pencere kapalıyken de çalışır (sahne görünümünde bildirim + konsol satırı) |
| **Rol: Silah** (+ `WD_*` seçicisi) | Silah kavraması yakalama kipi: Play doğrudan `_Shared/Scenes/WeaponCalibration.unity`'i açar (Boot/Lobby akışı hiç koşmaz), **sunucuya bağlanılmaz ve XR bırakılmaz**; hedef, sandbox ve "Başlangıç" seçimi **yok sayılır** (pencerede sönük çizilir). Seçilen silah `EditorPrefs`'te kişisel kalır. ⚠️ Sahne **Build Settings'e KONMAZ**: yakalama `AssetDatabase` ile diske yazıldığı için rol yalnız editörde anlamlıdır — bu yüzden `AppBoot` o sahneyi yükleyemez ve o rolde Lobby'ye gitmeyip hata basar. Akış: `Docs/Gelistirici/Yemek-Kitabi.md` |
...
97. **Kavrama kayıtları DÜNYA metresidir ve ÖLÇEKSİZDİR; eşyanın yerel konum uzayı değildir —
    ölçekli bir kökte ikisi aynı sayı değildir.** Yakalama da geri bileşim de bilerek
    ...
    `ItemGripAuthority`): biri ölçekli bileşim kullanırsa kavrama o yolda sessizce kayar.
...
120. **`SyntheticHand.OverrideAllJoints` TEK BAŞINA hiçbir şey yapmaz — parmakları büken şey
    ...
    ikiliyi kullanan yer boş elin idle duruşudur (`HandGripPoser.ApplyIdle`).
...
121. **Bir araç "boş iskelet" üretiyorsa, o iskeleti okuyan taraf "üretildi" ile "dolduruldu"yu
    ...
    (her kayıt doğduğu anda doludur — `ItemGripCapture.captured` bunun için vardır), (b) tüketicinin
...
140. **Bir şeyi ÖLÇEN araç ile onu KULLANAN oyun, ölçüyü AYNI KAYNAKTAN okumak zorundadır — iki
    okuma yolu iki ucun sessizce sapması demektir.** Kavrama, elin (ISDK bileğinin) silaha göre
    pozudur ve oyunda bileği `HandGripPoser` okuyor (`TryGetTrackedWrist`: sentetik elin
    KAYNAĞINDAN, izleme→dünya çevirisiyle). Yakalama kendi bileğini başka bir yerden okusaydı —
    sentetik elin kendisinden, anchor'dan türetilmiş bir tahminden, ISDK'nın başka bir poz
    alanından — iki uç birkaç santim ayrışır ve kavrama oyunda **tam o kadar kaymış** uygulanır.
    Hata sessizdir: araçta her şey doğru görünür, belirti yalnız gözlükte "silah elimin biraz
    yanında" olarak çıkar ve sayı elle yazılmış gibi okunur. Kural: **ölçüyü veren tek bir kapı
    olur** ve yakalama da tüketici de o kapıdan geçer.
    ⚠️ Aynı hikâyenin ikinci yarısı **çerçevenin seçimidir**: ölçülen ile yazılan aynı uzayda
    olmalı; arada tahmin edilmiş bir dönüş varsa (ör. bilek → kumanda anchor'ı çevirisi,
    `HandGripConvention`) o dönüşüm er geç veriye kaçar ve silah oyunda tahminin hatası kadar dönük
    çıkar. Bu yüzden kayıt **ham bilek pozudur** ve silahın rotasyonu zincirin tamamen dışındadır
    (kimlik).
    ⚠️ Üçüncü yarısı **referansın seçimidir**: sabit tutulan şey ölçünün paydası olmalı. Kavramada
    ölçü "el SİLAHA göre nerede" olduğu için sabit duran silahtır ve hareket eden eldir — silah
    ölçüm sırasında kafayı izleseydi ölçüm kendi hedefinin peşinden koşardı. Hem referansı hem
    ölçüyü aynı anda oynatan bir araç, kullanıcıya "neyi düzelttiğini" söyleyemez.
...
164. **Aynı karede `SetActive(true)` yapılacak bir objenin bileşenleri `Destroy` ile DEĞİL
    ...
    koşturur. Kavrama kalibrasyonunda bunun karşılığı, "ölçü hedefi" olarak örneklenen silahın
    kavrama/ateş/ses bileşenlerinin bir kez uyanması, yani hedefin kavranabilir hâle gelmesi ve ses
    çıkarmasıdır. Bu yüzden örnek **pasif bir kökün altında** doğar, bileşenler `DestroyImmediate`
...
165. **Bir "askıya al" bayrağı, davranışın TAMAMINI değil yalnız ÇIKTISINI durdurmalıdır.**
    `HandGripPoser.Suspended` sentetik ele poz UYGULAMASINI kapatır (bilek ve parmak kilidi çözülür,
    el ham izlemeye döner) ama el çözümü ve anchor→bilek ölçümü **sürer**. Ölçümü de durduran bir
    askı, kalibrasyonun tam da ihtiyaç duyduğu şeyi (izlenen bilek) ortadan kaldırırdı: araç eli hiç
    bulamaz ve belirti "el izlenmiyor" olarak okunur — oysa el izleniyordur, ölçen kapı kapalıdır.
    Genel kural: askıya alınan şeyin **sınırı yazılı olmalı**, yoksa bir sonraki tüketici bayrağın
    kapattığı şeyi tahmin eder.
166. **Aynı sınıfın iki dalı zıt kurallar işletiyorsa, ikinci dal ilkinin kalıntısını AÇIKÇA
    temizlemek zorundadır.** `HandGripPoser` boş elde parmakları KİLİTLER (idle duruşu tek tanımdan
    gelsin), silah tutan elde ise parmakları izlemeye BIRAKIR. İkinci dal beş parmağa açıkça
    `JointFreedom.Free` yazmazsa, elde silah belirdiği anda parmaklar idle'dan devraldıkları kilitle
    **donar** ve oyuncu tetiği çektiğini elinde göremez. Hata sessizdir: silahın kavraması doğru,
    bilek doğru yerde, yalnız parmaklar ölüdür. Genel kural: bir durumdan ötekine geçerken
    "yazmadığım şey eski hâlinde kalır" — serbestlik/override gibi **yapışkan** ayarlarda geçiş her
    iki yönde de açıkça yazılır.
...
`Ctrl+Alt+R`; **gözlükle kavrama yakalama** = rol *Silah* → `WeaponCalibration` sahnesi) · rolden bağımsız adres zinciri + `ConnectionOverlay` bağlantı hata ekranı ·
