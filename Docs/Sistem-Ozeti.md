...
| `Combat/ItemGripPose` | **Bir kavrama kaydı** (serialize edilen struct): `authored` + kumanda anchor'ının eşyaya göre yerel **konumu** + o silaha özel **riglenmiş parmak duruşu** (`fingerJoints` — `HandJointRotation` dizisi; boş olabilir, o zaman el boşta duruşunda kalır ve `HasFingers` bunu söyler). ⚠️ **DÖNÜŞ YOKTUR ve eklenmez** — kayıt yalnız "kumanda eşyanın NERESİNDE durur" der; eşyayı hiçbir elin dönüşü çevirmez, tek elde dönüş ana kumandanınkidir ve iki elli tutuşta ekseni ikinci elin avuç KONUMU nişanlar (`ItemGripSolver`). ⚠️ **Konum METREdir ve eşyanın görsel ölçeğiyle BÜYÜTÜLMEZ:** geri bileşim her zaman `item.position + item.rotation * position` ile yazılır, `Transform.TransformPoint` ile DEĞİL — `WPN_*` kökleri 0.8 ölçekli, `TransformPoint` aynı ölçüyü ikinci kez uygular ve el silahın yanında yüzer. Stüdyo da aynı simetrik yolla yazar, iki uç tek sözleşmede kalsın. ⚠️ **Kayıt gözlükle yakalanmaz, stüdyoda yazılır:** yakalanan kayıt tek karelik bir izleme örneğinin gürültüsünü taşır, oysa ölçülen şey kabzanın neresinin tutulduğudur ve o kabzanın sabit bir yeridir |
