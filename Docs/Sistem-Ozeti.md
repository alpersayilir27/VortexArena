...
| `NetIdentity` / `NetSpawnCatalog` | Sahne objesi kimliği (`sceneId`; aralığı `NET_ID_SCENE_MIN..NET_ID_SCENE_MAX`, sahne kaydında `SceneIdGuard` bake'ler ve zorlar) ve id→prefab kataloğu. ⚠️ Oyuncu senkronu bunlardan geçmez, `playerId` ile gider. Katalogun tüketicisi **dinamik obje doğuşudur** (`NetObjectSpawner`): `kind` → prefab; kaydı olmayan `kind` gelirse obje doğmaz ve tür başına tek satır uyarı basılır. Katalog `Resources` altındadır (`_Shared/Data/Resources/NetSpawnCatalog.asset`) — hiçbir sahne onu referanslamaz, taşınırsa dinamik doğuş sessizce durur |
