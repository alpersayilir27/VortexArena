...
| `AdminSession` | **YEREL** seçimler (kamera kipi, seçili oyuncu, açık panel) + görünüm tercihleri (`PlayerPrefs`'te kalıcı, admin PC'sine özel — halkalar, ad etiketleri, kamera hızı, **çatı kipi**, **ihlal sesi**, **pencere kipi**). **`FullScreen`** tam ekran ↔ pencereli tercihidir ve **`Screen`'e yazan TEK kapıdır**: setter tercihi kaydeder, `ApplyScreenMode()` ile pencereye uygular ve `Changed` yayar — F11 de tercihler panelindeki düğme de buradan geçer, ikinci bir yerden `Screen.fullScreenMode` yazılırsa tercih ile pencerenin gerçek hâli sessizce ayrışır. Tam ekran `FullScreenWindow`'dur (kenarlıksız), `ExclusiveFullScreen` DEĞİL: operatör aynı PC'de launcher/sunucu penceresine geçip duruyor ve çözünürlük değiştiren kip alt-tab'da admin'i saniyelerce kaybettirir. ⚠️ Varsayılan sabit bir değer değil **pencerenin o anki hâlidir** (`Screen.fullScreen`) — hiç seçim yapılmamış bir kurulumda tercih, build'in açılış kipini ezmemeli. Tek doğruluk noktası; `Changed` ile HUD/kamera/işaretçiler senkron kalır. `RoofAlphaNow()` tercih + kamera kipinden çatı alfasını türetir. **`ViolationSound`** ihlal uyarı sesinin kapısıdır ve **varsayılan açıktır** — operatör ekrana bakmıyor olabilir, sesin tek işi ona ulaşmaktır. ⚠️ **Bir EKRAN tercihidir, `AdminSelection`'a (yani `admin_state`'e) GİRMEZ:** iki operatörün hoparlörünü birbirine bağlamak yönetimi kolaylaştırmaz — biri sesi kapatınca diğerininki çalmaya devam etmeli |
...
**Ön kabza kapısı ve göstergesi de bu sınıftadır**, ayrı bir bileşen YOKTUR:
`IsHandOnSecondaryGrip(hand)` = boş elin **avucu** (`WeaponGranter.TryResolvePalm`) ön kabza
noktasının `secondaryGripRadius`'u içinde mi — granter ikinci eli buna göre bağlar, gösterge de aynı
ölçüyle yeşile döner (iki ölçü olsaydı "şimdi bas" diye büyütülen gösterge sessizce reddedilebilirdi).
...
bağlanmamış** silahta koşar: boş el 0.30 m'ye girince katalogdaki gösterge prefabı
(`WeaponCatalog.secondaryGripIndicatorPrefab`, tüm silahlar aynı sanatı paylaşır) ön kabza noktasına
konur (mavi, soluk), kabul yarıçapına girince yeşil + %35 büyük; ikinci el bağlanınca kaybolur.
Gösterge kameraya çevrilir (bir noktayı işaretliyor, kenarından bakılan halka çizgiye dönerdi),
ölçeği dünya ölçüsüdür (WPN kökleri 0.8, yerel ölçek düz yazılsa silahtan silaha farklı büyürdü),
prefabtan collider/Rigidbody sökülür (ışına ve kavramaya takılmasın). Prefab yoksa bir kez uyarır ve
çizmez — kapı yine çalışır. Ana kabzanın göstergesi YOKTUR (silah ana ele verilerek geliyor).
...
**ön kabza göstergesi prefabı** (`secondaryGripIndicatorPrefab`: sanat buradadır, `Weapon` yalnız
yerini/ölçeğini/rengini sürer — `LineRenderer` varsa rengi ona, yoksa ilk Renderer'ın materyaline;
varsayılan halkayı silah kiti koşusu (`Configure All Build Elements`) yalnız yoksa üretir, alan
yalnız boşsa bağlanır);
...
| `Combat/ItemDefinition` | Elde tutulabilen her şeyin (silah, ileride bomba) **dar** tabanı: `netItemId` (telde giden kimlik, §6.6), prefab, `holdMode`, kanonik kavrama pozları, tracer görünümü. Davranış alanı (hasar/şarjör/fitil) **girmez** — `RemoteAvatar` eşyayı ne YAPTIĞINI bilmeden çizer; Net katmanının "oyun bilgisi içermez" ilkesinin sunumdaki karşılığı. Kavrama **dört `ItemGripPose` kaydıdır** (`primaryGripRight/Left`, `secondaryGripRight/Left`; her biri `authored` + konum + euler + parmak preset'i) ve ⚠️ **dördü de AYNI uzaydadır: elin (ISDK bileğinin) EŞYAYA göre yerel pozu, METRE ve ölçeksiz.** Ters uzaylı bir çift tutmak (biri "el → eşya", öteki "eşya → el") iki alandan birinin er geç ters yazılması demekti. ⚠️ **Kayıt EL BAŞINADIR:** kabza simetrik olmadığı için iki elin bileği eşyanın farklı yerlerine düşer, tek kayıt tutup aynalamak sol eli silahın içine sokardı; okuma yolu (`GetGrip`) eksik eli öteki elin kaydına düşürür, yani `HasGrip` bir el eksik diye düşmez. ⚠️ **Kaydın DÖNÜŞÜ de anlamlıdır:** ana kabzada eşyayı döndürür (`PrimaryGripRotation`/`PrimaryGripPosition` = eşyanın BİLEĞE göre pozu, `InverseLocalPose`) — el nasıl tutuyorsa silah öyle durur; ön kabzada ise bileği oturtur (sentetik el kayda tam kilitlenir). ⚠️ **Ayrı bir "yazıldı" bayrağı ZORUNLU** (`authored`): sıfır poz geçerli bir kavramadır (bileği tam eşyanın orijininde), "hepsi sıfır = yazılmamış" kestirmesi sessizce yanlış olurdu. Parmak duruşu slot başına **preset**tir (`GripPreset(kind, rightHand)` → `HandGripPreset`), serbest eklem verisi değil. Yazma yolu yalnız editördedir (`EditorSetGrip`/`EditorClearGrip`; çağıran `GripPoseStudio`). Ana noktanın eşyaya göre yeri (`PrimaryGripPointOnItem`) **kaydın kendisinden türetilir**, ayrı bir alan olarak tutulmaz — aynı nokta iki yerde yaşasa biri güncellenip diğeri unutulurdu. `secondaryGripRadius` duruşun değil **ön kabza kapısının** ölçüsüdür (`Weapon.IsHandOnSecondaryGrip`; gösterge de aynı sayıyla yeşile döner) ve Inspector'dan girilir; ana kabza için yarıçap YOKTUR (silah ana ele verilerek gelir, okuyanı olmayan ölçü bayatlar) |
...
| `Combat/ItemGripPose` | **Bir kavrama kaydı** (serialize edilen struct): `authored` + bileğin eşyaya göre yerel konumu/euler'i + parmak `preset`'i. `LocalPose` bileğin eşyaya göre pozunu, `InverseLocalPose` aynı ölçünün ters yönünü (eşyanın BİLEĞE göre pozu) verir — ana el ikincisini kullanır. ⚠️ **Konum METREdir ve eşyanın görsel ölçeğiyle BÜYÜTÜLMEZ:** geri bileşim her zaman `item.position + item.rotation * position` ile yazılır, `Transform.TransformPoint` ile DEĞİL — `WPN_*` kökleri 0.8 ölçekli, `TransformPoint` aynı ölçüyü ikinci kez uygular ve el silahın yanında yüzer. Stüdyo da aynı simetrik yolla yazar, iki uç tek sözleşmede kalsın. ⚠️ **Kayıt gözlükle yakalanmaz, stüdyoda yazılır:** yakalanan kayıt kaçınılmaz olarak o anki bilek eğikliğini içerir ve o eğiklik artık eşyanın kendi dönüşünü sürdüğü için doğrudan namluya taşınırdı |
| `Combat/HandGripPreset` | Parmak duruşunun **adı**: `Idle` (hafif açık — boş elin duruşu) · `Firing` (işaret tetikte, kalan dördü sarar) · `Grip` (beş parmak da kapanır). Üçü de **sabit** duruştur, hiçbiri donanımdan sürülmez. Sayı taşımaz — oranların yeri `HandPoseProfile`, dağıtım kapısı `HandGripPresets`. ⚠️ **Serialize edilir** (`ItemGripPose.preset` asset'lerde yaşıyor) → yeni değer **SONA** eklenir, yoksa yazılmış bütün kavramaların duruşu kayar |
| `Combat/HandGripPresets` | Preset'lerin **tek dağıtım kapısı** (saf statik): `Profile(preset)` beş kapanma oranını (`HandPoseProfile`), `JointRotations(preset, rightHand)` ISDK sentetik eline yazılacak yerel eklem dönüşlerini, `DefaultFor(kind)` slot varsayılanını (ön kabza → `Grip`, aksi `Firing`), `TransitionSeconds` + `Ease(progress)` preset'ler arası geçişin süresini/eğrisini verir (yerel el ve uzak avatar aynı sayıyı okur — aynı kavrama iki ekranda farklı hızda kapanmasın; süre kısadır çünkü silah ele anında geliyor, parmaklar geç kalırsa el bir an silahın içinden geçer). ⚠️ **Parmaklar HİÇBİR ZAMAN donanımdan sürülmez** — kumandanın tetiği/kabzası ya da el izlemesi bir parmağı kıpırdatmaz; bir elin duruşu her karede yalnız bir preset'tir ve serbest bırakılan parmak (`JointFreedom.Free`) diye bir kavram bu kapıda yoktur, geri gelmez: tek bir parmağı bile donanıma bırakmak, stüdyoda görülen el ile oyunda görülen elin o parmakta ayrışması demektir. **Neden tek kapı:** aynı üç duruş üç ayrı iskelette çiziliyor (yerel sentetik el, stüdyodaki hayalet el, uzak avatarın Mixamo eli) — stüdyoda görülen el ile oyunda görülen elin aynı olmasının şartı bu. ⚠️ **Eklem dönüşleri SABİT YAZILMAZ, ISDK'nın kendi varsayılan iskeletinden ÖLÇÜLÜR** (`HandSkeleton.Default*Skeleton`): SDK iskeleti değiştirdiğinde burada tek satır değişmesin diye (`HandFingerRig` ile aynı gerekçe). ⚠️ **Avuç (volar) yönü de VERİDEN çıkarılır** — gevşek iskeletin parmakları zaten avuca doğru kıvrık, yani "avuç hangi tarafta" sorusunun cevabı iskeletin kendisindedir; sol/sağ çapraz çarpım sırasına dayanan bir sözleşme işaret hatasına açıktır ve belirtisi "parmaklar el sırtına doğru kırılıyor" olurdu. Menteşe ekseni kemik ile volar yönün çapraz çarpımıdır, işareti küçük bir deneme dönüşüyle **öz-denetlenir**. Kapanma açıları parmak/başparmak için ayrıdır (başparmak anatomik olarak daha az kıvrılır, kabzayı sarmak yerine üstüne yatar); metakarpallar sürülmez. Dönüş dizisi önbelleklidir ve **paylaşılır** (çağıran değiştirmez — geçişin ara dizisi çağıranın kendi kopyasıdır) |
| `Combat/GripHandAuthoring` | Stüdyonun sahneye koyduğu **tek elin** kimliği ve ayar yüzeyi: hangi kavrama noktası, hangi el, hangi parmak preset'i. Preset yazıldığı anda kemiklere uygulanır (tezgâhta "uygula" adımı yok). ⚠️ **Bilerek RUNTIME asmdef'indedir** ve dosyanın tamamı `#if UNITY_EDITOR` sarmalındadır: Unity editör derlemesinde tanımlı bir `MonoBehaviour`'ı GameObject'e eklemeyi reddeder ("it is an editor script") ve `AddComponent` sessizce `null` döner. Build güvenliği iki yerden gelir — sarmal tipi oyunun derlemesine sokmaz, objeler de `HideFlags.DontSave` olduğu için sahneye/prefaba yazılmaz. ⚠️ **Objenin transformu ISDK BİLEK çerçevesidir** ve başka hiçbir şeye çevrilmez; anchor (kumanda) uzayına köprü runtime'ın işidir (`ItemGripAuthority`) — authoring döngüsüne ölçülmemiş bir düzeltme sokmak, kullanıcının gözüyle "düzgün" gördüğü eli o düzeltmenin hatası kadar yanlış yere koyardı. ⚠️ **Parmak slider'ı YOKTUR:** tezgâhta ayarlanan ama kayda giremeyen bir duruş, oyunda hiç görülmeyecek bir ince ayar olurdu |
| `Combat/ItemGripAuthority` | "Bu eşya bu elde nasıl durur" sorusunun **etkin** cevabı (saf statik) — **iki uzay arasındaki köprü**: kayıt BİLEK uzayındadır (`ItemGripPose`), çözücü (`ItemGripSolver`) ve tel (§6.6) ise elin ANCHOR pozunu bilir. `TryResolvePrimaryGrip(definition, rightHand, out pos, out rot)` çıktısı **eşyanın ANCHOR'a göre pozudur** ve `delta ∘ InverseLocalPose` ile kurulur — yani **dönüş de kayıttan gelir**: eşya ele göre durur (`item = bilek ∘ Inverse(kayıt)`), el nasıl tutuyorsa silah öyle. ⚠️ Eşyanın kendi transformu OKUNMAZ (parametre olarak da alınmaz): ölçünün iki ucu da yereldir, eşya nerede durursa dursun sonuç aynıdır — transformu işin içine sokmak çağıranı "silahı taşımadan önce mi sonra mı sorayım" sorusuyla baş başa bırakırdı. **Delta üç basamakta çözülür** (`ResolveAnchorToWrist`): canlı ölçüm (`HandGripPoser.TryGetAnchorToWrist`) → **ölçülmüş sabit** (`HandGripConvention.AnchorToWrist`) → kimlik; sıra iki uçta da aynıdır ve delta kumanda sürümlü el pozlarından türediği için deterministiktir (aynı kumanda, aynı SDK, aynı sonuç). ⚠️ **Rig'i olmayan izleyici (admin gözlemci) yalnız sabiti kullanabilir** — sabit yazılmamışsa uzak silahlar deltanın dönüşü kadar yanlış çizilir. `WristToAnchor(rightHand, wristWorld)` ters yöndür ve ⚠️ **uzak avatarın el hedefleri ile ön kabza yapıştırması bundan geçmek ZORUNDA:** `RemoteHandPoser` aldığı pozu anchor çerçevesi sayar, kayıt ise bilek uzayındadır — çeviri atlanırsa uzak el delta kadar kaymış ve dönmüş çizilir. ⚠️ **Sol ve sağ AYRI çözülür** — kayıtlar el başınadır, kabza simetrik olmadığı için iki elin bileği eşyanın farklı yerlerine düşer. ⚠️ **Çözülemediğinde `false` döner ve çağıran `ItemDefinition.PrimaryGripPosition(bool)`'a düşer**: kavraması yazılmamış silah ve delta ölçülmeden önceki ilk kare o yoldan geçer — fallback bir hata değil normal bir yoldur ve **aynı kayıttan** türer, yani iki yol arasında sapma olamaz. ⚠️ **Sıra her çağıranda AYNI olmak zorunda** (`Weapon.ApplyCanonicalGrip`, `WeaponGranter.SolveInitialGrip`, `RemoteAvatar.ApplyGrip`/`TryResolveGripPalm`): biri canlı çözüme, öteki fallback'e bakarsa silah verildiği karede bir duruşta belirip sonraki karede ötekine zıplar — ya da el silahın yanında yüzer. Ters yön (`GripPointOnItem`) de buradan alınır. ⚠️ **Kayıt METRE ve ölçeksizdir**, geri bileşim daima `item.position + item.rotation * p`'dir — `TransformPoint` DEĞİL (§7), yani ölçek tuzağı burada hiç doğmaz. ⚠️ **Protokolde karşılığı YOKTUR, tel formatı değişmez** (§6.6): duruş yine telde gitmiyor, iki uç da aynı `WD_*`'ı okuyup aynı deltayı çözüyor |
| `Combat/HandGripPoser` | Elin duruşunu ISDK'nın **sentetik eline** yazar — oyuncunun gözlükte gördüğü el budur. **Üç durum, üç davranış:** *boş el* → bilek serbest, parmaklar `Idle`; *ana el* → bilek **SERBEST** (el izlemeden/kumandadan gelir, silah ona uyar — eşyayı ana kavrama kaydı döndürüyor), parmaklar slotun preset'i; *ön kabza* → bilek `SyntheticHand.LockWristPose(..., WristLockMode.Full, worldPose: true)` ile **TAM** kilitlenir (konum + dönüş), el eşyaya yapışır. ⚠️ **İkisini aynı kurala bağlamak ya silahı elden koparır ya ikinci eli havada bırakır:** ana elde bileği kilitlemek eli kumandadan koparır ve aynı şeyi iki yazar sürer. ⚠️ Kilit pozu **elle bileşimle** kurulur (`item.position + item.rotation * grip.position`), `TransformPoint` ile DEĞİL: kayıt ölçeksiz metredir, ölçekli bileşim bileği silahtan 1/0.8 kadar uzağa koyar ve el silahın yanında yüzer. ⚠️ Poz `worldPose: true` ile verilir — çeviriyi `LockWristPose` yapar, doğrudan `LockWristPosition` çağırmak izleme→dünya çevirisini atlar ve el rig'in dünyadaki yerine göre sessizce kayar. **Parmaklar üç durumda da bu sınıfın yazdığıdır** (`OverrideAllJoints` + `SetFingerFreedom`, dizi `HandGripPresets`'ten) ve ⚠️ **HİÇBİR durumda donanımdan sürülmez** — ne kumandanın tetiği/kabzası ne el izlemesi bir parmağı kıpırdatır: beş parmak her karede `JointFreedom.Locked` yazılır (kısaltılamaz — seviye sentetik elde kalıcıdır ve başka bir bileşen onu değiştirebilir; kilit bir kare yazılmazsa parmaklar donanıma döner ve tetik parmağı kumandayla kıpırdamaya başlar; değişmeyen seviyeyi yeniden yazmak ISDK'da ucuzdur). **Preset geçişi burada yumuşatılır** (`HandState`): hedef değişince o anki GÖSTERİLEN dizi başlangıç alınır ve `HandGripPresets.TransitionSeconds` boyunca eklem eklem slerp'lenir — silahı alınca el `Idle`'dan kavrama duruşuna kapanır, bırakınca geri açılır; ISDK'nın kendi lock eğrisi bunu yapmaz (yalnız serbest↔kilitli geçişini yumuşatır, kilitliyken hedef değişimi anında uygulanır). Başlangıç noktası önceki preset'in dizisi değil gösterilen dizidir: geçişin ortasında yeni hedef gelirse el zıplamadan yön değiştirir. ⚠️ **Boş elin parmakları donanımdan ÖRNEKLENMEZ:** üç preset de tek kaynaktan gelmek zorunda, yoksa stüdyoda yazılan duruş ile oyunda görülen el iki ayrı şey olur. **Kendini önyükleyen kalıcı tekil** (`WeaponGranter` kalıbı) — sahneye konmaz, yeni arenaya kurulum adımı doğurmaz; rig yoksa (admin gözlemci, sahne henüz yüklenmedi) sessizce hiçbir şey yapmaz. ⚠️ **Silahın pozuna DOKUNMAZ**: silahın dünya pozunun tek yazarı `Weapon.ApplyCanonicalGrip` + `ItemGripSolver`'dır; iki yazar aynı silahı kendi ekranında başka, karşı ekranda başka gösterirdi. ⚠️ **Yalnız YEREL oyuncuda koşar** — uzak avatarın eli ağdan gelen iskeletten çizilir, bu sınıfın ağ tarafında hiçbir işi yoktur ve **protokolde karşılığı yoktur**. ⚠️ **Ön kabzanın bilek kilidi KOŞULSUZDUR**, mesafe/açı kapısı yoktur ve eklenmez: bedeli fiziksel kumanda ön kabzadan uzaklaşınca elin kolla arasının görsel olarak gerilmesidir, kazancı oyuncu grip tuşunu bırakmadıkça elin silahtan kopmamasıdır — mesafeye bakan bir kapı eli oyuncu hiçbir şey yapmadan bırakır ve "iki elle tuttum ama ikinci el havada" hissi üretir. Bu bir üründür, eksik değil. `[DefaultExecutionOrder(100)]`: `Weapon.LateUpdate` silahın pozunu yazıyor, bilek ona kilitleniyor — daha erken koşan bir kilit bir kare gerideki silaha sarılır ve hızlı harekette titrer. Sentetik el **adla** süzülür (`SyntheticHandData`): rig'in altında mesafeli kavramanın hayalet elleri de aynı tipte, filtresiz arama oyuncunun elini bırakıp odanın öbür ucundaki hayaleti sarardı. Ön kabza kilidini bırakmak yalnız **kilitli → serbest geçişinde** yapılır (her karede koşulsuz `FreeWrist` ISDK'nın kendi kilitlerini de iptal ederdi; `FreeAllJoints` çağrılsaydı el bir kare izlemeye dönüp titrerdi). **Kavraması yazılmamış silahta el `Idle`'a düşer** + oturum başına tek uyarı. ⚠️ **İkinci işi ölçüdür: anchor → izlenen bilek deltasını her karede, el başına ölçer** (`TryGetAnchorToWrist`) — silahın duruşunu çözen taraf (`ItemGripAuthority`) elin ANCHOR pozunu bilir, kayıt ise bileği tarif eder; ikisi arasındaki köprü bu deltadır ve onsuz kavrama ölçülmüş sabite (`HandGripConvention`) düşer. **Ölçü kararlı olunca el başına BİR KEZ loglanır** (30 kare boyunca 2 mm / 0.5° altında oynama) — o satır `HandGripConvention`'a yapıştırılacak sayıdır; ilk kareler izleme ısınırken gürültülüdür, tek kareye bakıp loglamak yanlış sabit verirdi. Ölçü sentetik elin **KAYNAĞINDAN** alınır (izleme uzayı → dünya çevirisi `ITrackingToWorldTransformer` ile) ve **pozu uygulamadan ÖNCE**: kilitli eli okumak, silahın nerede olduğunu silahın kendisine sormak olurdu. Değer execution order yüzünden **bir kare bayattır** ve bu bilinçlidir — delta fiziksel olarak sabit bir ofsettir, bir karelik gecikmesi görünmez; ölçümü öne almak elin bir kare gerideki silaha sarılması pahasına olurdu |
...
| `Combat/HandPoseProfile` | Bir elin parmak duruşu: **beş sayı**, parmak başına kapanma oranı (`0` açık, `1` kapalı) + üç preset'in hazır değerleri (`Idle` · `Firing` · `Grip`). **Preset'lerin sayı tarafı burasıdır**, dağıtım kapısı `HandGripPresets` — aynı beş oran yerel sentetik eli, stüdyodaki hayalet eli ve uzak avatarın Mixamo elini sürer. ⚠️ **Var olma sebebi SENTEZDİR:** parmaklar telde gitmiyor (§6.9), yani uzak avatarın parmakları üretilmek zorunda — eşya tutan elde slotun preset'i, boş elde `RemoteAvatar.idleHandPose` (boşsa `Idle`). ⚠️ **Eşya başına serbest parmak verisi YOKTUR ve eklenmez:** duruş bir preset SEÇİMİdir; parmak başına ayar olsaydı her silahta ikinci bir elle ayar yüzeyi doğardı ve stüdyoda görülen el ile oyundaki el ayrışırdı. ⚠️ Quaternion DEĞİL oran tutulur, çünkü duruş iki ayrı iskelete uygulanabilmeli ve iki iskeletin kemik eksenleri aynı değil — ham rotasyon taşımak §7'deki "izleme uzayından gelen rotasyon humanoid kemiğe doğrudan yazılmaz" tuzağının parmak ölçeğinde tekrarı olurdu. ⚠️ `IsEmpty` (tümü sıfır) **yazılmamış** sayılır: değeri hiç girilmemiş bir alan elin tahta gibi düz kalmasına değil makul bir kavramaya düşsün. `Lerp(a, b, t)` + `Approximately` uzak avatardaki preset geçişinin adımıdır (süre/eğri `HandGripPresets`'ten) |
...
`MapDefinition.ambienceClip`'te durur; tekil, aktif sahnenin adıyla `GameCatalog.FindMap` yapıp
...
zaten üretilen `MapDefinition`'a bir klip sürüklemektir. Sahne geçişi iki `AudioSource` arasında
...
yerel bir zaman çıpasına çevirir ve klibi `(geçen süre) mod (klip uzunluğu)` noktasından açar.
...
duyardı. ⚠️ **Maç sonucu duyurusu oyuncuya özel DEĞİLDİR**: ses dinleyene göre değil MAÇA göre
...
geometrisini üreten ve kavrama pozunu yazan araçlar:

| Sınıf | Görevi |
|---|---|
| `GripPoseStudio` | `Kavrama Pozu Stüdyosu` — silahın elde nasıl duracağının yazıldığı tezgâh, **gözlük takmadan**. Akış prefab kipindedir: `WPN_*` prefabını çift tıkla aç → pencere stage'i kendiliğinden tanır → *Ana/Ön Kabza Ellerini Oluştur* → ISDK hayalet elleri Scene View'da beliriyor → elleri kabzalara sürükle/çevir, parmak duruşunu elin Inspector'ından preset olarak seç → gerekirse *Karşı Ele Aynala* → **Kaydet**. Kaydet **yaşayan her el için** `ItemDefinition.EditorSetGrip`'i çağırır (Undo'lu, `WD_*.asset`'e). ⚠️ **Kullanıcının sürüklediği el kökü ISDK BİLEK çerçevesidir** ve başka hiçbir şeye çevrilmez; anchor köprüsü runtime'ın işidir (`ItemGripAuthority`) — ölçülmemiş bir ergonomi düzeltmesi authoring döngüsüne girdiği anda, gözle "düzgün" görülen eli o tahminin hatası kadar yanlış yere koyar (belirtisi: silah oyunda dönük). ⚠️ **Ana kabzada tezgâhtaki el SİLAHI DÖNDÜRÜR** (`item = bilek ∘ Inverse(kayıt)`); ön kabzada tersi geçerlidir, el silaha yapışır. ⚠️ **Eller prefabın İÇİNE girmez:** her el, prefab stage sahnesinin ayrı bir KÖK objesidir (`[VA El_*]`, `HideFlags.DontSave`, ölçek 1) — prefab kipinde diske yalnız `prefabContentsRoot` altındaki ağaç yazılır, el oraya asılsaydı silahın içine bir el modeli girer ve arenada havada el görünürdü. ⚠️ **Prefab içeriğine HİÇBİR ŞEY yazılmaz** (poz düğümü, el rig'i, işaretçi): kaydın tek yeri `WD_*.asset`'tir. Başlangıç pozu **kayıt varsa oradan** gelir (oluştur → dokunma → Kaydet aynı değeri yazar), yoksa kabza parçasının bounds merkezinden (ad anahtarları, spesifikten genele sıralı), o da yoksa kökün biraz üstünden. Aynalama eşya uzayında YZ düzlemine göredir ve yalnız **başlangıçtır** — kabza simetrik olmadığı için son söz değildir. Eller SAHNEDE yaşar ve adlarıyla bulunur (domain reload pencereyi sıfırlayabilir); temizlik kancaları pencereden bağımsız kurulur: stage kapanınca / Play'e girerken / sahne açılınca eller silinir. Play kipinde yazma yapılmaz. ⚠️ **Dialog YOK** (`WeaponKitBuilder` ile aynı gerekçe: modal dialog Unity ana thread'ini kilitler, CLI'dan çalıştırıldığında komut timeout verir) — sonuç konsola yazılır. ⚠️ Hayalet el sağlayıcısının yolu sabit tutulur, ISDK'nın `HandGhostProviderUtils`'ü KULLANILMAZ: o sınıf ISDK'nın editör asmdef'indedir ve ona referans vermek bu aracı paket yükseltmelerinde kırılan bir bağa sokardı; OpenXR/OVR dalı iskeletin kendi verisinden seçilir |
| `GripHandAuthoringEditor` | Tezgâhtaki **tek elin** Inspector'ı: parmak preset'i açılır kutusu + *Karşı Ele Aynala* / *Bu Eli Kaldır* / *Kaydet (tüm eller)*. Preset değişimi Scene View'ı tazeler — `GripHandAuthoring` runtime asmdef'inde olduğu için `SceneView.RepaintAll` oradan çağrılamaz, tazeleme bu tarafta durur |
...
| **Yeni silah** | `WeaponKitBuilder` tablosuna satır ekle (istatistik + ses profili + pack modeli = köken kaydı) → `Tools > VortexArena > Build > Configure All Build Elements` (Hepsini Yapılandır / Yalnız Senkronize Et — silah kiti her eşitlemede koşar) → `WD_*.asset` üretir, **mevcut** `WPN_*.prefab`'ı yerinde günceller (ses + namlu alevi/dumanı + kovan kiti dahil), `WeaponCatalog`'u tazeler → **kavramayı yaz** (`Kavrama Pozu Stüdyosu`, prefab kipinde; yazılmadan el idle'da kalır ve silah tanımın ham ölçüsüyle durur) → gerekiyorsa `ModeDefinition.loadout` + sahneye yerleştir. **Export GEREKMEZ** (sunucuda silah tablosu yok). ⚠️ Araç **mevcut prefabların `Muzzle`/`Model` yerleşimine DOKUNMAZ**, yalnız definition bağlarını + ses/VFX/kovan kitini tazeler — VR'da elle ayarlanmış tutuş/namlu konumu tekrar çalıştırmakla bozulmaz. Paylaşılan şablon yoktur: sıfırdan farklı gövde için mevcut bir `WPN_*` prefabını kopyalayıp `Model` altındaki pack prefabını ve `definition`'ı değiştir, sonra eşitlemeyi bir kez daha çalıştır. ⚠️ **Ses klipleri yalnız alan BOŞSA yazılır** (elle sürüklenen klip korunsun diye): mevcut bir silahın sesini tablodan değiştiriyorsan önce `WD_*.asset`'teki klip alanlarını boşalt, yoksa değişiklik sessizce hiç inmez (Tuzaklar: "elle atanmışsa ezme"). Diğer alanlar (hasar/rpm/menzil/saçılım/kimlik) her koşuda ezilir |
...
58. **İzleme/ağ uzayından gelen bir rotasyon humanoid kemiğe DOĞRUDAN yazılmaz — iki iskeletin
    ...
    **Karakter değişirse kafa da aynı sebeple kırılır.** Meta'nın kendi body tracking'inde sorun
    ...
    `ItemDefinition` kavramaları bilek uzayındadır ve anchor'a `ItemGripAuthority`'nin deltasıyla
    çevrilir). Yan kanıt: uzak **silah** zaten doğru
...
120. **`SyntheticHand.OverrideAllJoints` TEK BAŞINA hiçbir şey yapmaz — parmakları büken şey
    ...
    ikisini birlikte yazan tek yer preset uygulamasıdır (`HandGripPoser.ApplyFingers`).
...
121. **Bir araç "boş iskelet" üretiyorsa, o iskeleti okuyan taraf "üretildi" ile "dolduruldu"yu
    ...
    ZORUNLU, çünkü sıfır poz geçerli bir kavramadır ve "hepsi sıfır = yazılmamış" kestirmesi sessizce
...
140. **Bir pozu YAZAN araç ile onu KULLANAN oyun aynı çerçevede konuşmalı; arada TAHMİN edilmiş bir
    dönüş varsa o tahmin er geç veriye kaçar.** Kavrama kaydı, elin (ISDK **bileğinin**) eşyaya
    göre pozudur ve stüdyoda tam o çerçevede yazılır; tüketici tarafta çözücü ile tel ise elin
    **anchor** pozunu bilir. İki uzayı birleştiren şey ölçülmüş bir deltadır
    (`ItemGripAuthority.ResolveAnchorToWrist`: canlı ölçüm → ölçülmüş sabit → kimlik) ve delta
    kumanda sürümlü el pozlarından türediği için deterministiktir. Authoring döngüsüne tahmini bir
    ergonomi düzeltmesi sokulsaydı (bilek → anchor çevirisini `HandGripConvention`'ın tahmin edilmiş
    sabitleriyle yapmak gibi), kullanıcının gözüyle "düzgün" gördüğü el oyunda tam o tahminin hatası
    kadar yanlış yere düşerdi — belirtisi sessizdir: araçta her şey doğru görünür, gözlükte silah
    dönük durur ve sayı elle yazılmış gibi okunur. Kural: **çeviri authoring'de değil runtime'da,
    ölçüyle yapılır.**
    ⚠️ İkinci yarısı **sabitin nereden geldiğidir**: canlı ölçümü olmayan uç (rig'siz admin
    gözlemci, ilk kareler) ölçülmüş sabite düşer ve o sabit TAHMİN EDİLMEZ — editör Play'inde
    kararlı ölçüm el başına bir kez loglanır ve `HandGripConvention`'a yapıştırılır. Sabit boş
    (kimlik) bırakılırsa hata yalnız o uçta görünür: admin uzak silahları deltanın dönüşü kadar
    yanlış çizer, oyuncuların ekranında hiçbir iz yoktur.
    ⚠️ Üçüncü yarısı **yönün seçimidir**: "silah ele göre durur" (`item = bilek ∘ Inverse(kayıt)`),
    "el silaha göre" değil. Ana elin bileğini ayrıca kayda kilitlemek eli kumandadan koparır
    (oyuncu kumandayı dosdoğru tutarken elini yamuk görür) ve aynı şeyi iki yazar sürer. Bileğin
    kilitlendiği tek yer ön kabzadır — orada soru zaten terstir: el eşyaya yapışmalıdır.
...
161. **Elin nerede olduğunu EŞYANIN KÖKÜNDEN okuma — kök avuçta durmaz, eşyanın açısıyla süzülür.**
    Kanonik kavrama eşyayı `kök = avuç + avuçRotasyonu × gripPosition` ile yerleştirir; o ofset
...
165. **Prefab kipinde çalışan bir authoring aracının yardımcı objeleri prefabın İÇİNE asılmaz —
    ...
    **temizliğin sahipliğidir**: objeler pencerede değil sahnede yaşadığı için (domain reload
    ...
    `Build Weapon Prefabs` prefaba sızmış el köklerini siler.
166. **Sentetik elde parmak SERBESTLİĞİ yapışkandır — kilit her karede yazılır, "değişince
    ...
    yüzden bizim tarafımızda (`HandState`, `HandGripPresets.TransitionSeconds`), sentetik ele her
    karede ara dizi yazılarak yapılır.
...
169. **Aynı duruş birden çok iskelette çiziliyorsa "ne kadar kapanacağı" TEK kaynaktan gelir, eklem
    dönüşü ise her iskeletin KENDİ verisinden ÖLÇÜLÜR.** Üç el birden aynı üç duruşu çiziyor (yerel
    sentetik el, stüdyodaki hayalet el, uzak avatarın Mixamo eli) ve üçünün kemik eksenleri farklı;
    oranları tek yerde tutmak (`HandPoseProfile` + `HandGripPresets`) tezgâhta görülen elin oyunda
    birebir tekrarlanmasının şartıdır — ikinci bir tablo açıldığı gün "stüdyoda güzeldi, gözlükte
    başka" başlar. Ölçümün iki ayrıntısı pahalıdır: (1) eklem açıları SDK'nın kendi varsayılan
...
sıfırlanır) + **ön kabza göstergesi** (tutulan çift elli silahta boş el ön kabzaya yaklaşınca
gösterge belirir, kabul yarıçapında grip'e basılınca ikinci el ön kabzaya bağlanır) + **UDP
