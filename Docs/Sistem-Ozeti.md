...
| Sınıf | Görevi |
|---|---|
| `ItemHandGripBake` | El düğümü ↔ kavrama alanları dönüşümünün **tek** uygulaması: `FromWrist` bake yönü (bileğin dünya pozu → tanım alanları), `ToWristLocal` ters yön (*El Ekle* tohumlaması — mevcut altı silahın kavraması sıfırdan yazılmasın diye el doğrudan bugünkü doğru yerinde belirir). ⚠️ **Var olma sebebi ASİMETRİ:** `primaryGrip` "el → eşya", `secondaryGrip` "eşya → el" yönündedir — ana elde ters bileşim gerekir, ikincilde gerekmez. İki ayrı yerde elle yazılsaydı biri er geç ters yazılır ve belirtisi "silah elde ters duruyor" olurdu. ⚠️ Ölçek dönüşümü kullanılmaz (elle bileşim): kavrama ofsetleri METRE cinsindendir (§7 tuzak 97) |
| `GripPoseStudio` | `Kavrama Pozu Stüdyosu`: silahın elde nasıl duracağını **gözlük takmadan** ayarlama aracı. **El Ekle** silahın altına ISDK'nın gerçek el modelini kalıcı olarak koyar (`Hands/Hand_<Kind>`, mevcut kavrama değerinden konumlanır, parmaklar varsa mevcut pozdan başlar); eli kabzaya oturtup parmakları büktükten sonra **Bake** dört işi birden yapar — bilek → `WD_*.asset` kavrama alanları, parmaklar → `GripPoses/Pose_<Kind>_R`, sol ele **ayna** (`HandGrabUtils.MirrorHandGrabPose`, ayna matematiği elle yazılmaz), sonra el modelini **gizler**. Model SİLİNMEZ: *Göster* ile geri açılıp yeniden bake edilir. Scene view'da avuç → kabza ve işaret parmağı → tetik mesafesi cm olarak çizilir (hedef kavrama noktası DEĞİL silahın gerçek geometrisidir — bileğe olan mesafe tanımı gereği sabittir ve hiçbir soruyu cevaplamaz). **Var olma sebebi geri besleme süresidir:** kavrama ofseti tek doğru değeri olan bir sayı değil, "avuç kabzaya değiyor mu / işaret parmağı tetiğe ulaşıyor mu" sorusunun cevabıdır ve o soruyu APK build'i + gözlük turuyla sormak her denemeyi dakikalara çıkarır. ⚠️ **Yazan tek düğme Bake'tir**; "Bake çıktısı" bölümü salt okunurdur ve poz düğümüne yönlendiren bir düğme YOKTUR — bir `HandGrabPose` seçildiğinde ISDK'nın kendi inspector'ı düğümün altına geçici bir el örnekler (`HideAndDontSave`), sahnede ikinci bir el belirir ve hangisinin kaynak olduğu belirsizleşir. ⚠️ Eklem eşlemesi **ada göre değil KİMLİĞE göre** yapılır (`HandJointMap.id`): ISDK'nın OpenXR ve OVR dallarında kemik adları farklı, kimlikler aynıdır. Dal tahmin edilmez **ölçülür** (`#if ISDK_OPENXR_HAND` bizim derlememizde her zaman false olurdu). Modal dialog yok, sonuç `Debug.Log` |
...
97. **Kavrama ofsetleri DÜNYA metresidir; işaretçinin YEREL konumu değildir — ölçekli bir kökte
    ...
    bir noktadan kayar. Doğru yol tek: değeri **araçla** yaz (`Kavrama Pozu Stüdyosu` → Bake —
    dünya farkını kendisi alır) ve gözle doğrula: **camgöbeği tel küre (SO'nun dediği) el modelinin
    bileğiyle ÇAKIŞMALIDIR.** Çakışmıyorsa değer elle yazılmış ya da bake'e basılmamış demektir.
