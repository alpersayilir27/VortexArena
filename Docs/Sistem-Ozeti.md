...
skeleton   (sunucu RX) = SKELETON_RATE_HZ × (14 + B) × N            ← B = SkeletonWire.BLOB_BYTES = 8 + 4 × eklem
...
(`SkeletonWire`, ArenaNet-Protokol §6.9), boyu `8 + 4 × eklem sayısı`dır — bugünkü listeyle 112 B,
yani `0x07` karesi 126 B ve `0x08` girdisi 123 B. Sunucu konsolundaki `[state]` satırı batch'lenmiş
...
| `0x07` iskelet (126 B, 12 Hz × 10) | ↑ | 0,15 Mbps | 120 | 120 |
| `0x08` iskelet batch (10 × 123 B → tikte hedef başına 2 datagram: 9 + 1 girdi) | ↓ | 1,37 Mbps | 264 | 264 |
...
harita geçişindeki sessiz geri yükleme de ölçtüğü için oturum içindeki kayma da satıra düşer. ⚠️ Ölçü **tracking-yerel** alınır — arena hizalamasından SONRAKİ dünya
...
| `BurgerDispenser` | Elin soketin içinde kavramaya basması `take` olayını yollar. ⚠️ Tür `grab:"none"` ve objede `NetObjectGrabBridge` **yoktur** — dağıtıcının kendisi hiç alınmaz, yalnız olay üretir; malzeme **sunucuda, doğrudan elde** doğar (`object_spawn` + `owner` + `Held`), yerelde hiçbir şey üretilmez. ⚠️ Basış kenarı **her karede el başına** izlenir, sokete girişte örneklenmez: zaten sıkılı bir el hacme sürüklenerek malzeme alırdı |
