...
| `AdminSession` | **YEREL** seçimler (kamera kipi, seçili oyuncu, açık panel) + görünüm tercihleri (`PlayerPrefs`'te kalıcı, admin PC'sine özel — halkalar, ad etiketleri, kamera hızı, **çatı kipi**, **ihlal sesi**). Tek doğruluk noktası; `Changed` ile HUD/kamera/işaretçiler senkron kalır. `RoofAlphaNow()` tercih + kamera kipinden çatı alfasını türetir. **`ViolationSound`** ihlal uyarı sesinin kapısıdır ve **varsayılan açıktır** — operatör ekrana bakmıyor olabilir, sesin tek işi ona ulaşmaktır. ⚠️ **Bir EKRAN tercihidir, `AdminSelection`'a (yani `admin_state`'e) GİRMEZ:** iki operatörün hoparlörünü birbirine bağlamak yönetimi kolaylaştırmaz — biri sesi kapatınca diğerininki çalmaya devam etmeli |
...
içindeyse ekran aynı karede, **rampasız**, tam siyah olur. ⚠️ **Kapı `IsViolating` DEĞİLDİR ve oraya
bağlanmaz:** ceza eşiği (nokta sayısı + 0.15 sn) bilerek toleranslıdır ve aynı toleransı görüşe
...
haritadan ve moddan **bağımsız** duyuru sesleri (rakip elendi · öldün · canlandın · maç başladı ·
