...
kazandın/kaybettin · geri sayım). Hangi ağ olayının hangi sesi tetiklediği `GameAudio`'da, klipler
...
yazmaz. ⚠️ Oyuncuya özel sesler (öldürme/ölüm/kazanma) yerel oyuncu kimliği çözülemiyorsa —
admin gözlemci, henüz bağlanmamış istemci — atlanır; yoksa operatör her maç sonunda "kaybettin"
duyardı. ⚠️ `match_state` tekrar tekrar geldiği için maç başlangıcı yalnız **faz geçişinde** çalar,
