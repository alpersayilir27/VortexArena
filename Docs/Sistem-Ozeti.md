...
⚠️ **Sayı tek başına iki arızayı ayırmaz:** "kumandanın ucu yakalamada yüksekteydi" (hizalama kaymıştır, oyuncunun ayakları zemine batar) ile "başlığın zemin tahmini kaymış" (hizalama doğrudur, sapma zararsızdır) aynı değeri üretir. Ayrımı gözlük logu yapar: `[Kalibre] zemin ölçümü: kaynak=… nokta=… kafa=… sanalZemin=… köken=…` satırı ve onu izleyen `hizalama sonrası kafa yüksekliği` satırı. Okuma kuralı: `nokta ≈ 0,6` iken `kafa ≈ 1,0–1,4` ise oyuncu eğilmiş ama uç havada kalmıştır (ayaklar batacak); `kafa ≈ ayakta boy + 0,6` ise kayan şey başlığın zemin tahminidir ve hizalama doğrudur. Hizalama sonrası kafa yüksekliği oyuncunun **gerçek göz hizasına** eşit olmak zorundadır — değilse hizalama yanlıştır, sapmanın büyüklüğü değil.
...
| `BurgerCarrier` | Elde tutulan bir taşıyıcının üstünde (spatulanın bıçağı, servis tahtasının yüzü) malzeme taşır; `NO_spatula` ve `NO_board` prefablarında bulunur ve `BurgerServingBoard` ile aynı hacmi (`Stack`) paylaşır. ⚠️ **Taşıma ilişkisi telde YOKTUR, türetilir:** malzeme sıradan `object_grab` ile taşıyıcıyı tutan elin malı olur, yani telde yalnız `owner` + `held` + `heldRight` durur; her istemci aynı kuralı uygular — *taşıyıcıyla aynı elde tutulan malzeme, o taşıyıcının ankorunda durur*. Ayrı bir alan ya da mesaj eklenmez. ⚠️ **Yığın sırası dinlenme pozunun YÜKSEKLİĞİNDEN okunur**, geliş sırasından değil: geliş sırası maçın ortasında bağlanan başlıkta yoktur (`world_state` sırasızdır) — hamburger o başlıkta karışır ve tarif alttan üste okunduğu için yanlış servis olurdu. Taşınan malzeme **poz paketi göndermez** (`held` olduğu için `0x09` kapısı zaten kapalı): pozu taşıyıcıdan, taşıyıcınınki elden gelir. **Yalnız sahibi talep eder ve bırakır;** diğer başlıklar aynı durumdan aynı yerleşimi türetir. Yük, taşıyıcı `spillAngle` derecesinden fazla yatınca ya da taşıyıcı elden bırakılınca `object_release` ile dökülür — oradan sonrası sıradan uçuş penceresidir (fizik + poz akışı + `object_rest`), yani ızgara ve tahta kancaları değişmeden çalışır. Kapasite alandan gelir: spatula tek malzeme, servis tahtası bir hamburger kadar. ⚠️ **İki şerit vardır:** malzeme türleri ankorda **yığılır**; malzeme olmayan tutulabilir nesneler (bıçak, spatula, bütün ekmek) taşıyıcı kaldırılırken üstünde duruyorsa **gevşek** biner (kapasitesi `looseCapacity`, `0` = hiç binmez ve **varsayılan budur** — yalnız servis tahtası açar; spatulada açık olsaydı hedeflenen köftenin yanındaki bıçağı, hatta tahtanın kendisini kaldırırdı). Gevşek yükün yerleşimi iki **dinlenme pozunun** farkıdır — nesnenin taşıyıcı üstündeki dinlenme pozu ile taşıyıcının tezgâhtaki dinlenme pozu; sunucunun hiç hareket ettirmediği bir sahne nesnesi için `NetObject.ScenePosition`/`SceneRotation` kullanılır — yani ofseti her başlık aynı türetir ve telde yeni alan yoktur. ⚠️ **Gevşek yük yalnız kaldırışın ilk anlarında alınır:** sonrasında taşıyıcı dinlenme pozundan uzaklaşmıştır ve aynı fark nesneyi havada oturturdu. ⚠️ **Başka bir taşıyıcının üstünde giden taşıyıcı UYUR** (tahtadaki spatula): talep etmez, kendi yükünü oturtmaz — aynı elin yükünü iki taşıyıcı yazsaydı tek transforma iki yazıcı düşerdi. ⚠️ **Hacme giren her serbest malzeme alınmaz:** aday dinlenmiş olmalı (`awake` değil — uçan malzeme kapılmaz) ve merkezi yığının tepesinden prefabdaki temas bandı içinde durmalı (`0` = bant yok, hacmin tamamı). Tahtanın hacmi bir hamburger boyu yüksek olduğundan bant olmadan boş tahtayla ızgaranın üstünden geçmek köfteleri süpürür; spatulanın tek slotu bant istemez. ⚠️ **Taşıma hacmi TETİK collider'dır** ve taşıyan yüzeyin ölçüsünde durur: tetik değilse propun kendi `Rigidbody`'sinin bileşik çarpışmasına katılır (prop görünmeyen bir kutuya çarpar, yerde yanlış durur), yüzeyinden büyükse malzeme yüzeyin epey yukarısındayken kabul edilir ve havada asılı görünür — ikisi de hata vermez |
| `BurgerServingBoard` | Servis tahtası: slotta dururken üstündeki yığını `serve` ile bekleyen müşteriye bildirir (yığın **dünya Y'sine göre alttan üste** sıralanır — sunucunun karşılaştırdığı sıra budur). **İki an tetikler ve ikisinde de hamburger KAPALI olmalıdır** (en üstte `bun_top`): park etmiş bir tahtaya üst ekmek konması, ya da bitmiş bir tahtanın slota bırakılması. ⚠️ Birincisi olmadan döngü kapanmaz — slotta duran tahta bir daha "dinlenmeye" geçmez, yani müşterinin önünde kurulan hamburger asla teslim edilemezdi; kapalılık şartı da her malzemede yeniden reddedilmeyi önler. ⚠️ **Reddin kendi mesajı yoktur:** doğru servis `object_state` üretir, yanlış tarif `serve` olayının kendisi olarak geri gelir — yani gelen `serve` **reddin ta kendisidir**. Red gelince **sebebi yerelde teşhis eder** (olaydaki yığın + müşterinin tarifi → alt/üst ekmek yeri, köfte aşaması, eksik/fazla tür) ve müşterinin balonuna yazdırır. ⚠️ Teşhis sunucunun servis kuralının **aynasıdır**; kural değişince burası da değişir, yoksa balon yalan söyler. Kabul edilen servis yalnız servis edende kısa haptik verir |
| `BurgerCounterSlot` | Bir banko slotu: müşterinin bekleyeceği yer + tahtanın bırakılacağı hacim. Telde giden şey slotun **numarasıdır** (müşterinin `s` alanında), konumu değil. ⚠️ Collider'ı olmayan slot hiçbir tahtayı sahiplenmez (sessizce hepsini yutmasın diye) |
