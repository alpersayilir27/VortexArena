...
| Sınıf | Görevi |
|---|---|
| `ItemHandGripBake` | **Avuç** düğümü ↔ kavrama alanları dönüşümünün **tek** uygulaması: `FromWrist` kayıt yönü (avucun dünya pozu → tanım alanları), `ToWristLocal` ters yön (tezgâh kurulumu). İkisinin birbirinin tersi olması "tezgâhı aç → dokunma → kaydet değeri değiştirmez" kimliğinin dayanağıdır. ⚠️ **Referans AVUÇTUR, el modelinin bileği DEĞİL** — o çeviriyi `GripPoseStudio` el modelini kurarken `HandGripConvention.Correction` ile bir kez yapar; buraya bilek verilirse aradaki sabit dönüş sessizce tanıma yazılır (§7 tuzak "iki el çerçevesi"). ⚠️ **Var olma sebebi ASİMETRİ:** `primaryGrip` "el → eşya", `secondaryGrip` "eşya → el" yönündedir — ana elde ters bileşim gerekir, ikincilde gerekmez. İki ayrı yerde elle yazılsaydı biri er geç ters yazılır ve belirtisi "silah elde ters duruyor" olurdu. ⚠️ Ölçek dönüşümü kullanılmaz (elle bileşim): kavrama ofsetleri METRE cinsindendir (§7 tuzak 97) |
| `GripPoseStudio` | `Kavrama Pozu Stüdyosu` (kavrama tezgâhı): silahın elde nasıl duracağını **gözlük takmadan** ayarlama aracı. **Tezgâhı Aç** açık sahneye `[VA Kavrama Tezgahı]` kurar — dünya orijininde **dönüşsüz ve tıklanamaz** bir silah kopyası, kabzalarında ayrı ayrı taşınabilen `El_Primary` / `El_Secondary` düğümleri (her biri = `ItemGripSolver`'ın aldığı avuç pozu, altında ISDK'nın el modeli). ⚠️ **Sabit olan SİLAHTIR, taşınan eller** ve bu yön bilinçlidir: kaydedilen ölçü "silah ele göre nerede" olduğu için silah orijindeyken elin transformu doğrudan o ölçüdür; ayrıca her silahın kabzası farklı açıdan tutulur ve kabza–tetik mesafesi aynı değildir, iki elin bağımsız ayarlanabilmesi gerekir. ⚠️ **El modeli HAM konmaz:** kök transformu ISDK **bilek** çerçevesinde, kavrama alanları ise **kumanda anchor'ı** çerçevesindedir; ham bir eli gözle kabzaya oturtmak aradaki sabit dönüşü tanıma yazar (§7 tuzak "gözle düzeltilen şey"). Modelin yerel dönüşü `HandGripConvention.Correction` ile **kendi bind pozundan ölçülür** (sabit yazılmaz), yerel konumu sıfırdır (`PalmOffset` avuç pozuna zaten dahil — iki kez sayılmaz) ve yerel ölçeği 1'dir (silah 0.8; el küçülseydi "avuç kabzayı sarıyor mu" sorusu %25 yanlış orandan cevaplanırdı). Eller `ItemHandGripBake.ToWristLocal` ile, yani kaydın **birebir tersiyle** yerleşir → "aç, dokunma, kaydet" değeri değiştirmez. **Kaydet** üç iş yapar: ellerin silaha göre transformu → `WD_*.asset` kavrama alanları, bilek+parmaklar → `GripPoses/Pose_<Kind>_R`, sol ele **ayna** (`HandGrabUtils.MirrorHandGrabPose`, ayna matematiği elle yazılmaz); ayrıca prefabta kalmış eski `Hands/Hand_*` rig'ini siler. Prefab **headless** yazılır (`PrefabUtility.LoadPrefabContents`), poz düğümünün yerel konumu `InverseTransformPoint` ile — bilerek **ölçekli**, çünkü ISDK'nın `HandGrabPose.RelativePose`'u da öyle. Scene view'da el çerçeveleri + avuç → kabza ve işaret parmağı → tetik mesafesi cm olarak çizilir (hedef kavrama noktası DEĞİL silahın gerçek geometrisidir — bileğe olan mesafe tanımı gereği sabittir ve hiçbir soruyu cevaplamaz); pencere kaydedilecek sayıları canlı, **salt okunur** gösterir. **Var olma sebebi geri besleme süresidir:** kavrama ofseti tek doğru değeri olan bir sayı değil, "avuç kabzaya değiyor mu / işaret parmağı tetiğe ulaşıyor mu" sorusunun cevabıdır ve o soruyu APK build'i + gözlük turuyla sormak her denemeyi dakikalara çıkarır. ⚠️ Tezgâh `HideFlags.DontSave` ile kurulur (sahneye girmez) ve Play'e girerken / sahne değişince kendini siler. ⚠️ Eklem eşlemesi **ada göre değil KİMLİĞE göre** yapılır (`HandJointMap.id`): ISDK'nın OpenXR ve OVR dallarında kemik adları farklı, kimlikler aynıdır. Dal tahmin edilmez **ölçülür** (`#if ISDK_OPENXR_HAND` bizim derlememizde her zaman false olurdu). Modal dialog yok, sonuç `Debug.Log` |
...
97. **Kavrama ofsetleri DÜNYA metresidir; işaretçinin YEREL konumu değildir — ölçekli bir kökte
    ...
    bir noktadan kayar. Doğru yol tek: değeri **araçla** yaz (`Kavrama Pozu Stüdyosu` → Tezgâhı Aç →
    Kaydet — dünya farkını kendisi alır) ve gözle doğrula: **camgöbeği tel küre (SO'nun dediği)
    tezgâhtaki elin bileğiyle ÇAKIŞMALIDIR.** Çakışmıyorsa değer elle yazılmış ya da Kaydet'e
    basılmamış demektir.
    ⚠️ Tek istisna ISDK'nın **kendi** verisidir: `HandGrabPose.RelativePose` `PoseUtils.DeltaScaled`
    ile, yani eşyanın ölçeğine **bölünerek** üretiliyor — onu geri bileştiren taraf da
    (`HandGripPoser.Apply`, `GripPoseStudio`'nun poz yazımı) ölçekli bileşim kullanır. Kural bizim
    tanım alanlarımız içindir; ISDK'nın sözleşmesini ölçeksiz okumak eli eşyadan `1/ölçek` kadar
    uzağa yapıştırır.
...
121. **Bir araç "boş iskelet" üretiyorsa, o iskeleti okuyan taraf "üretildi" ile "dolduruldu"yu
    ...
    savunma birden konur ve **ikisi de kalır**: (a) araç artık boş düğüm HİÇ açmaz — düğümleri
    yalnız Bake üretir, yani her düğüm doğduğu anda doludur; (b) tüketicinin girişinde bir kapı
...
140. **Bir ayarlama aracında GÖZLE DÜZELTİLEN şey, oyunun okuduğuyla AYNI ÇERÇEVEDE olmak
    ...
    var: ISDK el modelinin kök transformu **bilek** çerçevesindedir, `ItemGripSolver`'ın okuduğu
    kavrama alanları ise **kumanda anchor'ı** çerçevesinde tanımlıdır (`HandGripPivot`) ve ikisi
    arasında sabit bir dönüş bulunur. Araç eli silahın üstüne koyup "eli kabzaya oturt" dediğinde,
    doğru bir tanımdan tohumlanan el bile **yanlış yönde** beliriyordu; kullanıcı onu gözle
    düzeltince o sabit dönüş tanıma yazılıyor ve silah oyunda o kadar dönük çıkıyordu. Hata sessiz:
    araçta her şey doğru görünür, belirti yalnız gözlükte ortaya çıkar ve sayı elle yazılmış gibi
    okunur. Çözüm **çeviriyi kullanıcıdan almak**tır: araç el modelini ham koymaz, aradaki dönüşü
    (`HandGripConvention.Correction`, modelin kendi bind pozundan ölçülür) bir kez uygular ve
    kullanıcıya **zaten anchor çerçevesinde olan** bir düğüm sürükletir. Böylece dokunulan transform
    ile yazılan sayı aynı uzayda olur ve araya bir çevirinin sızması yapısal olarak imkânsızlaşır.
    Genel kural: **bir düzenleyicide düzeltilen şeyle yazılan şey aynı uzayda değilse, arada duran
    dönüşüm er geç veriye kaçar** — dönüşümü aracın içine koy, kullanıcının gözüne bırakma. İkinci
    güvence de ucuzdur: "aç, hiç dokunma, kaydet" değeri değiştirmemelidir; bu kimlik tutuyorsa
    kurulum ile kayıt gerçekten birbirinin tersidir.
