...
`IsHandOnSecondaryGrip(hand)` = boş elin **bileği** (`TryResolveWrist`: kumanda anchor'ı ∘
anchor→bilek deltası, `ItemGripAuthority.ResolveAnchorToWrist`) ön kabza kaydının etrafındaki
`secondaryGripRadius` küresinin içinde mi — granter ikinci eli buna göre bağlar, soket de aynı ölçüyle
"içeride" alfasına geçer. ⚠️ **Ölçülen nokta BİLEKTİR, kumanda anchor'ı değil:** kayıt bileğin eşyaya
göre pozudur (stüdyoda ISDK bilek çerçevesiyle yazılır); anchor'la ölçmek elin tam yazıldığı yerinde
...
bağlanmamış** silahta koşar: bilek 0.30 m'ye girince katalogdaki soket prefabı
(`WeaponCatalog.secondaryGripIndicatorPrefab`, tüm silahlar aynı sanatı paylaşır) ön kabza kaydına
merkezlenir (açık mavi, %70 saydam), bilek kürenin içine girince biraz dolgunlaşır; ikinci el
...
sıfırlanır) + **ön kabza soketi** (tutulan çift elli silahta boş elin bileği ön kabzaya yaklaşınca
yarı saydam açık mavi soket küresi belirir; bilek kürenin içindeyken grip'e basılınca ikinci el ön
