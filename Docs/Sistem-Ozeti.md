...
kafa kırmızı · karın turuncu · bacak sarı · gövde yeşil) — seçili olmaları gerekmez, gerekçe
`GripSocketMarker`'daki ile aynı. Elle ayar bu gizmo'ya bakılarak yapılır.
...
| `GripPoseStudio` | `Kavrama Pozu Stüdyosu`: silahın elde nasıl duracağını **gözlük takmadan** Scene view'da görme aracı — seçili silahın kavrama soketlerine ISDK'nın hayalet elini oturtur (`GhostProvider.asset`), işaretçi sürüklenirken **aynı karede** takip ettirir ve baş parmak ucu → soket, avuç → kabza, soket ekseni mesafelerini cm olarak çizer. **Var olma sebebi geri besleme süresidir:** kavrama ofseti tek doğru değeri olan bir sayı değil, "avuç kabzaya değiyor mu / baş parmak tetiğe ulaşıyor mu" sorusunun cevabıdır ve o soruyu APK build'i + gözlük turuyla sormak her denemeyi dakikalara çıkarır. Poz düğümü üretir (`ItemGripPoses` adlandırması), gerekirse tüm silahlara toplu açar, parmak düzenlemesini ISDK'nın kendi `HandGrabPose` editörüne devreder ve pozu karşı ele **aynalar** (ayna en iyi tahmindir, karşı elin bileği sahnede elle düzeltilir). ⚠️ **Hiçbir şey YAZMAZ** — ne `WD_*.asset`'e ne silahın duruşuna: kavrama duruşunun tek yazma yolu `Write Grip Sockets To Definition`'dır, ikinci bir yol aynı ters/düz bileşim matematiğini ikinci kez uygulardı. ⚠️ Bilek kaynağı seçilir: **işaretçi** = yazılmayı bekleyen, **tanım** = oyunun kullandığı (sarı/camgöbeği gizmo ayrımının aynısı). ⚠️ Hayalet eller `HideAndDontSave`'dir ve pencere kapanınca yok edilir — sahneye kaydedilseler her silahın yanında oyunda da çizilen bir el kalırdı; domain reload'dan sağ çıkan yetimler ad önekiyle süpürülür. ⚠️ Kitin silahın orijininde açtığı düğüm, düğme ile üretim/aynalama sırasında **sokete taşınır**: orada kalan düğüm geçersiz sayıldığı için (§7) poz hiç devreye girmezdi. Modal dialog yok (CLI'dan çalıştırıldığında ana thread'i kilitler), sonuç `Debug.Log` |
...
111. **Kumanda anchor'ı oyuncunun ELİ DEĞİLDİR — kavramayı ona göre hizalamak matematiksel olarak
    ...
    birkaç santimlik sabit bir fark vardır. Kanonik kavrama anchor'a hizalarken `GripSocket_Primary`
...
121. **Yerleştirilmemiş bir kavrama pozu düğümü geçerli sayılmaz — sayılırsa aracı çalıştırmak,
    işin geri kalanı yapılmadan önce mevcut davranışı BOZAR.** Silah kiti poz düğümlerini her
    `WPN_*` altında **silahın orijininde, sıfır transformla** açar (parmak pozunu insan sonradan
    yazar). O hâldeki bir düğümü "poz var" sayan bir tüketici bileği kavrama noktasına değil
    silahın orijinine kilitler ve eli nötr bind duruşunda dondurur: kullanıcı yalnız kiti tazelemek
    için aracı çalıştırdığında bütün silahlarda el bir anda bozulur, üstelik sebep az önce
    çalıştırılan araçta değil çok daha eski bir kapıda aranır. Kapı bu yüzden tüketicinin girişinde
    durur (`ItemGripPoses.IsUnplaced`) ve ölçüsünü **tüketicinin gerçekten kullandığı büyüklükten**
    alır (`RelativePose`), düğümün ham yerel transformundan değil — araya bir ara düğüm girdiği için
    ikisi ayrışabilir ve kapı o zaman yanlış soruyu cevaplardı. Anlamı dar ve tek: "bu düğüm henüz
    kabzaya taşınmadı"; kullanıcı düğümü yerine sürüklediği an poz kendiliğinden devreye girer,
    ayrıca bir onay adımı gerekmez. Genel kural: **bir araç boş iskelet üretiyorsa, o iskeleti
    okuyan taraf "üretildi" ile "dolduruldu"yu ayırt edebilmelidir.**
