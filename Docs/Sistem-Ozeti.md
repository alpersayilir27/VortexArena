...
çarpar. Dosya olmasının kazancı: ölçüyü sahadan
alan kişi Unity açmadan güncelleyebilir),
...
`ArenaDimensionMesh` + `DimensionPolygon` (ölçü maketinin işaretçileri; kökte mekan adı + kaynak
`TextAsset` + geri yazma taşıyıcıları, çokgenlerde yalnız `Kind { Plane, Column }`. ⚠️ Nokta/ad/
yükseklik işaretçide TUTULMAZ — kaynakları sırasıyla mesh, `GameObject` adı ve mesh'in Y aralığı;
kopyalamak sahnede düzenlenen değerden sapan ikinci bir kaynak üretirdi. Maketin kökü `EditorOnly`
etiketlidir, build'e girmez),
...
| `Combat/WeaponDissolve` | *(her `WPN_*` kökünde; `WeaponKitBuilder` takar ve `DissolveEffect.mat`'i bağlar)* Silah ele geldiğinde **çözülerek belirir**: model geçici olarak dissolve materyaline çevrilir, `_Dissolve` 1→0 sürülür, sonra özgün materyaller geri konur. Kapı **`Weapon.HeldChanged`**'dir, çağrı noktaları değil — üç tutma yolu da (rastgele verilen silah, çerçeve klonu, ISDK kavraması) tek yerden karşılansın, yeni bir yol açıldığında sessizce unutulmasın (`WeaponFrame` aynı olayı aynı sebeple dinliyor). Silahın **kendi albedosu** (`_BaseMap`/`_BaseColor`) özgün materyalden okunup `MaterialPropertyBlock` ile taşınır: dissolve materyali TEK bir asset ve hangi silaha takıldığını bilmiyor, taşınmasaydı silah düz renkli bir siluet olarak çözülürdü. Çözülme deseni **modelin yerel uzayından** üretilir, UV'den değil — silah dokusu bir renk paleti atlası (her yüzeyin UV'si tek bir karenin içinde), UV'den okunsaydı desen sabit çıkar ve silah desensiz, topluca kaybolurdu. Aynı sebeple `_NoiseScale` **metre başına periyottur**: silah ~0.7 m olduğu için ince parçalanma 60-150 arası ister, düşük değerler avuç içi kadar iri leke verir. Desen saf gürültü değil, bir **eksen gradyanıyla harmanlanır** (`_DissolveAxis` · `_DirectionStrength`) — silah bir uçtan diğerine doğru erir; saf gürültüde her yer aynı anda çözüldüğü için göz bir yön göremiyor ve efekt "titreme" gibi duruyordu. Vektörün UZUNLUĞU aynı zamanda ölçektir (ayrı bir scale property'si yok — ikisi tek yerden ayarlansın). **İki materyal seçeneği var:** `DissolveEffect` (Simple Noise — yumuşak, bulanık lekeler) ve `VoronoiDissolve` (Voronoi — hücresel, "parçalara ayrılıyor" görüntüsü); ikisi de aynı property setini konuşur, yani bileşende yalnız materyal alanı değişir. ⚠️ Süre/desen ayarlarının **tek doğruluk kaynağı `WeaponKitBuilder`'dır** — araç her koşuda prefaba geri yazar; prefabdaki değer denemeliktir. Materyal `.sharedMaterials` ile takılır (`.materials` her renderer için toplanmayan bir kopya üretirdi). ⚠️ `OnDisable` materyalleri geri koyar: obje kapanınca coroutine ölüyor — geri konmasaydı silah bir dahaki çağrılışında yarı çözülmüş belirir, üstelik property block'lu renderer SRP Batcher dışında kalmaya devam ederdi. Bırakışta kaybolmayı `WeaponDissolveGhost` devralır; silahın kendisi hemen özgün materyaline döner. ⚠️ Kopya bırakma olayının **iki yolundan** da üretilir (`HeldChanged(false)` ve doğrudan kapanma) — bileşenlerin `OnDisable` sırası garanti olmadığı için olay bazı yollarda bize hiç ulaşmadan silah kapanabiliyor |
| `Combat/WeaponDissolveGhost` | Elden çıkan silahın **yerinde kalan kopyası**: yalnız mesh + dissolve materyali taşır, `_Dissolve` 0→1 sürülür ve kendini yok eder. `WeaponDissolve` üretir, prefabda durmaz. **Neden ayrı obje:** silah bırakıldığı anda yok ediliyor/gizleniyor, yani kaybolma geçişinin silahın KENDİ üstünde koşacak karesi yok. **Neden `WeaponGranter` değiştirilmedi:** alternatif, yok etmeyi/gizlemeyi efekt süresi kadar ertelemekti — ama o kapıdan "silah elde mi" sorusunun TÜM cevapları geçiyor (ölüm, harita değişimi, kural değişimi, tur başı dolum) ve her birine gecikme sokmak görsel bir efekt için maç kurallarını kırılganlaştırırdı. ⚠️ Kopya **hareket etmez** (bırakıldığı dünya pozunda çözülür): eli takip etseydi "silah hâlâ elimde" derdi. Yalnız `MeshFilter` taşıyan parçalar kopyalanır — deri sarılı bir model gelirse o parça kopyada çizilmez |
...
| `DimensionMeshBuilder` | `JSON'dan DimensionMesh Üret`: boyut dosyasından **ölçü maketi** üretir — tek `Plane` (ProBuilder çokgeni, extrude 0) + kolon başına bir prizma (pivotu ayak izinin ağırlık merkezinde, sürüklemek doğal olsun diye). ⚠️ **Duvar ÜRETMEZ ve maket oynanan geometri değildir**: kök `EditorOnly` etiketlenir, build'e girmez; arena sanatı maketin üstüne kurulur. Kök = sahnedeki `ArenaBoundary`'nin altı, yerel dönüşümü sıfırlanmış (plan koordinatları onun yerel XZ'sinde; başka bir ebeveyn planı sessizce kaydırır) — muhafaza yoksa sahne köküne düşer ve uyarı basar. **İdempotent**: aynı mekanın maketi varsa silinip yeniden kurulur. Üretimden önce halkayı `Polygon2D.IsSelfIntersecting` ile denetler |
| `DimensionMeshReader` | `DimensionMesh'i JSON'a Çevir`: maketi okuyup **kaynak dosyanın üstüne** yazar (hedef sorulmaz, maketin işaretçisinden gelir). Ayak izi çıkarımı: yatay yüzler (`\|normal.y\| > 0.9`) Y seviyesine göre gruplanır, **en alt** grup alınır (prizmada alt yüz kazanır), kenarlar XZ'ye izdüşürülüp kaynaştırılır, **yalnız bir kez geçen** kenar sınır sayılır, halka yürünür ve doğrusal ara köşeler ayıklanır. Noktalar dünya üstünden kök uzayına çevrilir — kolonu sürüklemek/döndürmek doğru yazılır. ⚠️ Kenarlar köşe **indeksiyle değil konumla** anahtarlanır: ProBuilder sert normaller için köşeleri yüz başına ayırıyor, indeksle bakan tespit tüm mesh'i sınır sanar. Yazmadan önce çıktı geri ayrıştırılır; doğrulanamazsa dosyaya **dokunulmaz** |
| `TemplateBasicsLoader` | `Template Temellerini Yükle`: aktif sahneye altyapıyı **prefab örneği** olarak koyar (`VA_ArenaRoot`, `VA_CameraRig`, `VA_PoseSync`, `VA_CalibrationManager`; seçime bağlı `VA_ModeHud` · taban bölgeleri · `SpawnPoint`), `ArenaCalibrator`'ın sahneye bakan alanlarını bağlar, `ArenaBoundary`'nin rig'e bakan alanlarını (`head`/`fadeRenderer`/`warningText`) `VA_CameraRig` içinden bağlar ve mekanın boyut dosyasını `ArenaBoundary.dimensionsJson`'a takar. Taban bölgelerini takım malzemesiyle boyar (tek `VA_BaseZone` prefabı iki takıma da hizmet ediyor; şerit rengini çalışma anında kimse yazmıyor). **İdempotent** — var olan örneği asset yoluyla tanır ve atlar; dolu bir alanın üstüne YAZMAZ |
| `BuildElementsConfigurator` | `Configure All Build Elements`: `MapDefinition` yazar/günceller, `GameCatalog.maps`'e ve haritayı destekleyen her modun **DOLU** `maps` listesine ekler (boş liste "kısıtsız" demek, dokunulmaz), Build Settings'e koyar, `ServerConfigExporter.Export(false)` çağırır ve bir **sağlık raporu** basar (`SpawnPoint` sayısı, `dimensionsJson` dolu mu, maket `EditorOnly` ve doğru ebeveyn altında mı, sahnede `Wall_*` kalıntısı var mı). Bu adımların biri atlanınca hata sessizdir — tek düğmede toplanmasının sebebi budur |
...
2. Arena çerçevesini kur: alana hizalı bir objeye **`ArenaBoundary`**
   ...
   **Ölçüyü mekanın boyut dosyasına yaz** (`Venues/<İşletme>/Data/<İşletme>_dimensions.json`) ve
   ...
   `(0,0)` olur (plan sıfırının arena origin'i olması gerekmez). Elle konmuş kolon/kasa varsa
   üstlerine `ArenaObstacle` ekle.
...
5–6. **`Tools > VortexArena > Configure All Build Elements`** — `MapDefinition` (sceneName +
...
75. **İzlemeden gelen "kafa" GÖZÜN pozudur; humanoid kafa KEMİĞİ oraya oturtulmaz.**
...
76. **Meta'nın `IKUtilities.SolveCCDIK`'i ÖLÇEĞİ görmezden gelir — hedef ona ham verilmez.**
...
77. **Bir eklemde biriken burulma mesh'i çökertir — bilek roll'ü önkolla PAYLAŞILIR.** Ele mutlak
...
78. **URP overlay kamerasının near-clip'i XR'da SESSİZCE yok sayılır.** "Gövdeyi ayrı bir katmana
    ...
    maddede 75'teki ofset eksikliğiydi. ⚠️ `LocalBody` katmanı `TagManager`'da bırakıldı (artık
