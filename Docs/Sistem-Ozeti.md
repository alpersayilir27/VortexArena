...
düşürmez, hedef başlığa `clear_calibration` **iletir** (`identify` deseni) ve başlık hizalamayı ve
...
| `AdminStatsPanel` | Takım toplamları + **oyuncu satırı listesi** (`AdminStatsRow`, `ScrollRect` içinde) + maç bilgisi + altta toplu düğme şeridi (kalibrasyonun üç komutu — kayıttan yeniden yükleme, hizalamayı geçersiz kılma, cihaz kaydını silme — ve **TÜMÜNÜ ÖLÇEKLENDİR**) + reddedilen komutun gerekçesini yazan hata penceresi (birkaç saniyede kendi kapanır; ⚠️ arkasına **scrim koyulmaz** — panel bir iş listesidir, tek bir hata operatörü geri kalanından koparmamalı). Liste **kaplı DEĞİLDİR**: bağlı olan herkes çizilir, satır yüksekliği prefabtan okunur (sanatçı satırı büyütünce yerleşim kendiliğinden uyar), satır arası `_rowGap` alanıdır. **Metin kolonu YOKTUR** — hücre okunur ama tıklanmaz, oysa operatörün bu paneldeki işi oyuncu başına EYLEMDİR; sayılar satırın kendi hücrelerinde ve ayrıntı şeridindedir (`AdminStatsRow`). **FFA'da liste skora göre azalan sıralanır**, başlık lideri yazar. ⚠️ **Alt şeritte kalibrasyonun ZIT komutları yan yana durur ve ayrımı tek bir metin taşımaz:** `TÜMÜNÜ KALİBRE ET` gözlükte KAYITLI çapadan **yeniden yükletir** (`reload_calibration`; geri alınabilir, onay istemez, kimseyi savaş dışı bırakmaz), *hizalamayı geçersiz kılma* **herkesi kalibresiz bırakır** ama gözlükteki kaydı korur (`clear_calibration{playerId:0, keepSaved:true}` — yani yeniden yükleme düğmesi ondan sonra da çalışır), *cihaz kaydını silme* ise kayıtlı çapayı da yok eder (`keepSaved:false`; sonrasında tek yol oyuncunun elle A/B sekansıdır). Yıkıcı iki komutun ayrımını iki şey birden anlatır: **görünür** (kırmızı yazı + küçük düğme) ve **sürtünme** (iki adımlı onay — bir kez basınca düğme onay metnine döner, ~3 sn içinde ikinci basış komutu gönderir, basılmazsa kendiliğinden vazgeçer). Yalnız birine yaslanmak yetmez: renk okunmadan tıklanır, onay ise komutlar aynı ağırlıkta göründüğünde operatörü yanlış düğmede iki kez bastırır. Onay gerekçesi `AdminPlayerRow`'un `KAL`/`AT` düğmeleriyle aynıdır ve ölçeği yüzünden daha ağırdır — geçersiz kılma sahadaki **herkesi** ateş edemez/vurulamaz/canlanamaz hâle getirir. Oyuncu başına sıfırlama yine `AdminPlayerRow`'un `KAL` düğmesindedir (§3.11) ve o **yumuşak kiptir**; sert kip yalnız topludur, çünkü cihaz kaydını yok etmek mekan seviyesinde bir bakım işidir (zemin bantları taşındığında yapılır). Protokolde olmayan metrik (hasar/isabet oranı) **gösterilmez**; jitter/kayıp protokolde VAR ama panelde bilinçli olarak yok (operatörün eyleme çevirebileceği sayı ping'dir). ⚠️ **Panelin KÖK objesi ETKİN kalır** (tercihler paneliyle aynı sözleşme) — gizlenen yalnız `_root` kartıdır, kök kapatılırsa panel hiçbir tuşla açılmaz (Tuzaklar) |
...
| `Combat/ItemGripPose` | **Bir kavrama kaydı** (serialize edilen struct): `authored` + kumanda anchor'ının eşyaya göre yerel **konumu** + parmak `preset`'i. ⚠️ **DÖNÜŞ YOKTUR ve eklenmez** — kayıt yalnız "kumanda eşyanın NERESİNDE durur" der; eşyayı hiçbir elin dönüşü çevirmez, tek elde dönüş ana kumandanınkidir ve iki elli tutuşta ekseni ikinci elin avuç KONUMU nişanlar (`ItemGripSolver`). ⚠️ **Konum METREdir ve eşyanın görsel ölçeğiyle BÜYÜTÜLMEZ:** geri bileşim her zaman `item.position + item.rotation * position` ile yazılır, `Transform.TransformPoint` ile DEĞİL — `WPN_*` kökleri 0.8 ölçekli, `TransformPoint` aynı ölçüyü ikinci kez uygular ve el silahın yanında yüzer. Stüdyo da aynı simetrik yolla yazar, iki uç tek sözleşmede kalsın. ⚠️ **Kayıt gözlükle yakalanmaz, stüdyoda yazılır:** yakalanan kayıt tek karelik bir izleme örneğinin gürültüsünü taşır, oysa ölçülen şey kabzanın neresinin tutulduğudur ve o kabzanın sabit bir yeridir |
...
| **Ortam sesi (ambiyans/müzik)** | Haritanın `MapDefinition`'ındaki `ambienceClip` alanına bir klip sürüklemekle biter — `SceneAmbience` gerisini yapar (§4). ⚠️ Sahneye ses objesi konmaz, klip ikinci bir yere yazılmaz; klipler `Assets/Audio/Ambience/` altında ve **`Streaming`** import'ludur. Haritadan bağımsız duyurular `GameSoundBank`'e, moda/haritaya göre değişenler `ModeAudioRegistry`'ye girer (§4) |
...
112. **İkinci el silahı NİŞANLAR ama TAŞIMAZ — ve silahı hiçbir elin DÖNÜŞÜ çevirmez.** Eşyanın
    ...
    sokete yapışması ve saçılım/geri tepme çarpanı.
...
140. **Bir pozu YAZAN araç ile onu KULLANAN oyun aynı çerçevede konuşmalı; arada TAHMİN edilmiş bir
    ...
    kumandaya asılır (dönüşünün tabanı kumandanınki, yeri kayıttan). Ana elin bileğini ayrıca kayda kilitlemek eli kumandadan koparır
