...
| `Combat/ItemDefinition` | Elde tutulabilen her şeyin (silah, ileride bomba) **dar** tabanı: `netItemId` (telde giden kimlik, §6.6), prefab, `holdMode`, kanonik kavrama pozları, tracer görünümü. Davranış alanı (hasar/şarjör/fitil) **girmez** — `RemoteAvatar` eşyayı ne YAPTIĞINI bilmeden çizer; Net katmanının "oyun bilgisi içermez" ilkesinin sunumdaki karşılığı. Kavrama **dört `ItemGripPose` kaydıdır** (`primaryGripRight/Left`, `secondaryGripRight/Left`; her biri `authored` + konum + parmak preset'i) ve ⚠️ **dördü de AYNI uzaydadır: elin KUMANDA ANCHOR'ININ EŞYAYA göre yerel KONUMU, METRE ve ölçeksiz.** Ters uzaylı bir çift tutmak (biri "el → eşya", öteki "eşya → el") iki alandan birinin er geç ters yazılması demekti. ⚠️ **Kayıt EL BAŞINADIR:** kabza simetrik olmadığı için iki elin kumandası eşyanın farklı yerlerine düşer, tek kayıt tutup aynalamak sol eli silahın içine sokardı; okuma yolu (`GetGrip`) eksik eli öteki elin kaydına düşürür, yani `HasGrip` bir el eksik diye düşmez. ⚠️ **Kayıt DÖNÜŞ TAŞIMAZ:** eşyanın dönüşü her zaman ana kumandanınkidir, kayıt yalnız "kumanda eşyanın NERESİNDE durur" der (`PrimaryGripPosition` = kaydın tersi, yani kumandayı o noktaya oturtan geri kaydırma); ön kabza kaydı da ikinci elin GÖRSELİNİN yapışacağı yeri söyler. Kayıt bir dönüş taşısaydı stüdyoda kökü çeviren (ya da yanlış eksende çizilen hayalet eli düzeltmeye çalışan) herkes oyunda silahı kumandadan saptırırdı. ⚠️ **Ayrı bir "yazıldı" bayrağı ZORUNLU** (`authored`): sıfır poz geçerli bir kavramadır (anchor'ı tam eşyanın orijininde), "hepsi sıfır = yazılmamış" kestirmesi sessizce yanlış olurdu. Parmak duruşu slot başına **preset**tir (`GripPreset(kind, rightHand)` → `HandGripPreset`), serbest eklem verisi değil. Yazma yolu yalnız editördedir (`EditorSetGrip`/`EditorClearGrip`; çağıran `GripPoseStudio`). Ana noktanın eşyaya göre yeri (`PrimaryGripPointOnItem`) **kaydın kendisinden türetilir**, ayrı bir alan olarak tutulmaz — aynı nokta iki yerde yaşasa biri güncellenip diğeri unutulurdu. `secondaryGripRadius` duruşun değil **ön kabza soketinin** yarıçapıdır (`Weapon.IsHandOnSecondaryGrip`: kumanda anchor'ı bu kürenin içindeyse grip ikinci eli bağlar; oyuncunun gördüğü küre de tam bu yarıçapla çizilir — varsayılan 0.10 = 20 cm çap) ve Inspector'dan girilir; ana kabza için yarıçap YOKTUR (silah ana ele verilerek gelir, okuyanı olmayan ölçü bayatlar) |
| `Combat/ItemGripSolver` | Kanonik kavramanın **matematiği** — saf statik, sahne bağımlılığı yok. Girdi: eşya tanımı + **ana** elin avuç (kumanda anchor) pozu (`HandGripPivot`); çıktı eşyanın dünya pozu. Denklem tek satırdır: `itemRot = palm.rot`, `itemPos = palm.pos + palm.rot * (−kayıt.position)` — kayıt kumandanın eşyaya göre yerel konumu olduğu için eşya, kumandayı o noktaya oturtacak biçimde geri kaydırılır (`Solve(def, primaryRight, in palm, out pos, out rot)` → `PrimaryGripPosition(bool)`); yazılmamış kayıtta ofset sıfırdır, eşya kumandanın tam üstünde durur. ⚠️ **Eşyanın DÖNÜŞÜ HER ZAMAN ana kumandanın dönüşüdür — başka hiçbir şey onu döndürmez:** ne kayıt (yalnız KONUM taşır), ne ikinci elin yeri, ne bileğin duruşu. Silahın eksenini ikinci elin avucuna çeviren bir **"iki elli nişan" YOKTUR ve eklenmez**: olsaydı ikinci el ön kabzayı tuttuğu anda silah kumandadan sapar ve oyuncu bunu "el konumuna göre silah bozuk geliyor" olarak yaşardı. İkinci elin görünür sonucu yalnız ikidir — sentetik elin görseli ön kabzaya yapışır (`HandGripPoser`) ve saçılım/geri tepme düşer (`Weapon`). ⚠️ **Yerel ve uzak uç AYNI fonksiyonu koşar** — duruş telde gitmediği için (§6.6) ikinci bir uygulama aynı silahı iki ekranda iki ayrı duruşta çizerdi. ⚠️ Sahne/bileşen bağımlılığı **girmez**: saf kalması iki uçta da koşabilmesinin ön koşuludur |
...
| `Combat/ItemGripPose` | **Bir kavrama kaydı** (serialize edilen struct): `authored` + kumanda anchor'ının eşyaya göre yerel **konumu** + parmak `preset`'i. ⚠️ **DÖNÜŞ YOKTUR ve eklenmez** — eşyanın dönüşü her zaman ana kumandanınkidir (`ItemGripSolver`), kayıt yalnız "kumanda eşyanın NERESİNDE durur" der. ⚠️ **Konum METREdir ve eşyanın görsel ölçeğiyle BÜYÜTÜLMEZ:** geri bileşim her zaman `item.position + item.rotation * position` ile yazılır, `Transform.TransformPoint` ile DEĞİL — `WPN_*` kökleri 0.8 ölçekli, `TransformPoint` aynı ölçüyü ikinci kez uygular ve el silahın yanında yüzer. Stüdyo da aynı simetrik yolla yazar, iki uç tek sözleşmede kalsın. ⚠️ **Kayıt gözlükle yakalanmaz, stüdyoda yazılır:** yakalanan kayıt kaçınılmaz olarak o anki kumanda eğikliğini içerir ve o eğiklik eşyanın kendi dönüşünü sürdüğü için doğrudan namluya taşınırdı |
...
140. **Bir pozu YAZAN araç ile onu KULLANAN oyun aynı çerçevede konuşmalı; arada TAHMİN edilmiş bir
    ...
    **kimlik (döndürülmemiş) kayıt = kumandayla hizalı silah**, yani yazılmamış kavrama da
    kestirilebilir — kayıt bilek uzayında tutulsaydı "hiç döndürmedim" bile anchor→bilek deltası
    kadar, yani onlarca derece dönük bir silah üretirdi. Kural: **authoring çerçevesi tüketicinin
    çerçevesiyle AYNI seçilir**, aradaki fark ölçülüp kapatılmaz.
    ⚠️ İkinci yarısı **deltanın kapsamıdır**: anchor→bilek deltası
    (`ItemGripAuthority.ResolveAnchorToWrist`: canlı ölçüm → ölçülmüş sabit → kimlik) yalnız GÖRSEL
    elin işidir — ön kabzayı saran sentetik el ve stüdyodaki hayalet el. Sabit TAHMİN EDİLMEZ:
    editör Play'inde kararlı ölçüm el başına bir kez loglanır ve `HandGripConvention`'a
    yapıştırılır. Boş (kimlik) sabit silahın yönünü etkilemez, yalnız o iki eli kumandanın tam
    üstüne çizer — yani bedeli stüdyoda elin silahı nasıl sardığını yanlış görmektir.
    ⚠️ Üçüncü yarısı **yönün seçimidir**: "silah ele göre durur" (`item = anchor ∘ Inverse(kayıt)`),
