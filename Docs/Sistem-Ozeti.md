...
140. **Bir ayarlama aracında GÖZLE DÜZELTİLEN şey, oyunun okuduğuyla AYNI ÇERÇEVEDE olmak
    ...
    **çeviriyi hiç yaptırmamaktır**: kullanıcının sürüklediği el kökü bilek çerçevesidir, kaydedilen
    poz düğümü de bilek çerçevesindedir ve oyun tarafı o düğümü **canlı ölçülen** bileğe hizalar
    (`ItemGripAuthority` + `HandGripPoser.TryGetAnchorToWrist`) — yani dokunulan transform ile
    okunan veri aynı uzaydadır, arada tahmin edilmiş hiçbir sabit yoktur. Anchor'a çeviri yalnız
    fallback alanlarının **yazımında**, tek yönde kalır. Genel kural: **bir düzenleyicide düzeltilen
