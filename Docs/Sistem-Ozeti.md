...
- **Oyun tipi tele HİÇ girmez** — admin tarafında tümüyle yerel bir süzgeçtir; `set_selection` ve
  ...
  oyun tipi varken** çizilir: tek tip varken panel tek katmanlı kalır.
...
şeydir** (kaynağın `localScale`'i = `BodyScale`, `ArenaNetCharacterBehaviour.ApplyArenaRoot` yazar) ve ayrıca çarpılır — ikisi karıştırılmaz. Ağ yolu, MSDK retarget config'i ve
...
| `World/WorldObjectTable` | Ağ nesnelerinin sunucu-otoriter defteri: `netId → {kind, hp, maxHp, flags, owner, stage, dinlenme pozu, dinamik mi}`; hasar, kırılma, **sahiplik** ve tur/sahne sıfırlaması. Sahiplik üç adımda yürür: kavrama (boştaysa sahip yazılır, `Held` +gerekirse `HeldRight` kalkar), elden çıkış (`Held` düşer, `Awake` kalkar, ⚠️ **sahiplik KORUNUR** — uçuş penceresi tam olarak budur), durma (`Awake` düşer, `owner = 0`, bildirilen poz dinlenme pozu olur). Sahibi kopan ya da ölen oyuncunun objelerini serbest bırakır; bu kapı olmadan bir kopma objeyi tur sonuna kadar kilitler. Dinlenme pozunu **çağıran çözer** (`MatchDirector`): **ölümde ve bağlantı kopmasında** son bilinen pozun tam altındaki zemin (`OBJECT_DROP_CLEARANCE`) — elde tutulan obje poz akıtmadığı için son bilinen poz orada **sahibin o eli**dir. Kopuş `left`'i beklemez: bekleseydi obje `RECONNECT_GRACE` boyunca havada asılı kalırdı. **Dinamik kimliği de burası tahsis eder** (`NET_ID_DYNAMIC_*`): tahsis tur içinde **monotondur** — despawn edilen kimlik tur bitmeden geri verilmez, yoksa yolda olan bir poz paketi ya da olay kimliği devralan yeni objeye yazar ve hiçbir yerde hata görünmez. ⚠️ **Kendi kilidi YOKTUR, `MatchDirector`'ın `_gate`'i altında yaşar** — ikinci kilit deadlock adayıdır; bu yüzden metotları `…Locked` adlanır ve kilidi tutmak çağıranın sözleşmesidir |
| `Modes/IGameMode` + `TdmMode` + `FfaMode` | Mod kuralları: skor, kazanma koşulu, tur süresi. Yeni kancalar **varsayılan gövdeyle** eklenir (default interface method) → mevcut modların hiçbiri değişmez; **tüketicisi olmayan kanca EKLENMEZ**. `FfaMode` yüzeyin ilk tüketicisidir: takımsız + bireysel skor + sabit durma canlanması, `MatchDirector`'a tek satır kayıt dışında hiçbir dokunuş yok. `OnRoundStart` ikinci örnektir: Live'a HER girişte çağrılır, tur kavramı olmayan modlar hiç yazmaz. `OnMatchEnd` de aynı düzendedir: `finished`'a girişte **maç başına bir kez**, kilit dışında ve bir sonraki tick'te çağrılır (`EnterEndLocked` bayrağı kurar, `TickAsync` tüketir; yeni maç, lobi ya da sahneleme bayrağı siler — eski modun `netId`'leri yeni sahnenin objelerine denk gelebilir). `finished`'da `OnTick` çağrılmadığı için sahada kalan nesneyi toplamanın tek yeri burasıdır. ⚠️ **`OnTick` yalnız Live'da değil, geri sayımda ve mod duraklatmasında da çağrılır** (tur tabanlı mod geri sayımı iptal edebilmek için yoklar) — yalnız Live'da iş yapması gereken mod fazı kendisi kontrol eder. `OnObjectEvent` üçüncüsüdür: ⚠️ **dönüş değeri TEK bir soruyu cevaplar — olay relay edilecek mi?** `true` = "işledim, relay etme" (gerçeği, yaptığın yazmaların yayınladığı `object_state` duyurur), `false` = kozmetik (aynı `object_event` herkese relay edilir). İşlenmiş bir olayı relay etmek aynı gerçeği iki kez duyurur ve istemcide sunum iki kez oynar. Mod tabloya hiç dokunmaz, yazma yolu direktörün API'sidir |
