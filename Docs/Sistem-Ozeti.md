...
- **Sahip koparsa ya da ölürse sunucu serbest bırakır** — obje son bilinen pozunda kalır. Bu kapı
...
| `World/WorldObjectTable` | Ağ nesnelerinin sunucu-otoriter defteri: `netId → {kind, hp, maxHp, flags, owner, stage, dinlenme pozu, dinamik mi}`; hasar, kırılma, **sahiplik** ve tur/sahne sıfırlaması. Sahiplik üç adımda yürür: kavrama (boştaysa sahip yazılır, `Held` +gerekirse `HeldRight` kalkar), elden çıkış (`Held` düşer, `Awake` kalkar, ⚠️ **sahiplik KORUNUR** — uçuş penceresi tam olarak budur), durma (`Awake` düşer, `owner = 0`, bildirilen poz dinlenme pozu olur). Sahibi kopan ya da ölen oyuncunun objelerini **son bilinen pozunda** serbest bırakır; bu kapı olmadan bir kopma objeyi tur sonuna kadar kilitler. **Dinamik kimliği de burası tahsis eder** (`NET_ID_DYNAMIC_*`): tahsis tur içinde **monotondur** — despawn edilen kimlik tur bitmeden geri verilmez, yoksa yolda olan bir poz paketi ya da olay kimliği devralan yeni objeye yazar ve hiçbir yerde hata görünmez. ⚠️ **Kendi kilidi YOKTUR, `MatchDirector`'ın `_gate`'i altında yaşar** — ikinci kilit deadlock adayıdır; bu yüzden metotları `…Locked` adlanır ve kilidi tutmak çağıranın sözleşmesidir |
...
214. **Servis tahtasını yuvasına bağlayan tek şey KONUMUDUR.** `BurgerServingBoard.ResolveSlot()` bir
    ...
    ama **hiç servis yapmaz**: müşteri sabrı dolana kadar bekler ve sebebi hiçbir yerde yazmaz.
