...
varsayılandır** — operatör raundu kısaltıp uzatabilir.
...
motor yeniden kurulunca çalan tüm kaynaklar susar ve ortam sesi kendiliğinden geri gelmez — klip
elle sürdürülüp ortak faza (`SeekToEpoch`) geri oturtulur, yani geç katılan başlık gibi atlayarak
devam eder, baştan başlamaz (Tuzaklar: "ses cihazı değişince çalan her `AudioSource` durur").
...
112. **İkinci el silahı ne KAYDIRIR ne DÖNDÜRÜR — ön kabzanın görünür karşılığı yalnız ELİN
    GÖRSELİDİR.** Silahın dönüşü her zaman ana kumandanın dönüşüdür (`ItemGripSolver`); eşyanın
    eksenini ikinci elin avucuna çeviren bir "iki elli nişan" YOKTUR ve eklenmez: olsaydı ikinci el
    kabzayı tuttuğu anda silah kumandadan sapar ve oyuncu bunu "el konumuna göre silah bozuk
    geliyor" olarak yaşardı — üstelik teşhis kavrama kaydında aranır, oysa kayıt doğrudur. Ana
    kavrama noktasını iki elin ortasına almak ya da konumu harmanlamak da ana elin silahı bıraktığı
    izlenimini verir; gerçek silahta da ağırlığı tetik eli taşır. Ön kabza bağının tüm karşılığı
    ikidir: saçılım/geri tepme çarpanı ve sentetik elin sokete yapışması.
...
117. **`Quaternion.ToAngleAxis` 180°'yi AŞAN bir açı ve işareti dönmüş bir eksen döndürebilir —
    ...
    ⚠️ Kavramada bunun bir tüketicisi YOKTUR ve doğmaz: silahın dönüşü her zaman ana kumandanın
    dönüşüdür, iki vektörü birbirine çeviren bir kavrama çözümü yazılmaz.
...
140. **Bir pozu YAZAN araç ile onu KULLANAN oyun aynı çerçevede konuşmalı; arada TAHMİN edilmiş bir
    ...
    kayıt **DÖNÜŞ TAŞIMADIĞI** için silah her zaman kumandayla hizalıdır ve yazılmamış kavrama da
    ...
    kumandaya asılır (dönüşü kumandanınki, yeri kayıttan). Ana elin bileğini ayrıca kayda kilitlemek eli kumandadan koparır
...
169. **Taban şeridinin görünüşü PREFABA yazılmaz — `VA_BaseZone` tek prefabtır, takımı ayıran şey
    ...
    alanda durur (`VortexArena/BaseZoneMetallicMirror` → `_MirrorColor`).
...
172. **Ses cihazı değişince çalan her `AudioSource` DURUR ve kendiliğinden geri gelmez.** Unity
    ...
    `AudioSettings.OnAudioConfigurationChanged`'de klibi elle sürdürmesidir (`SceneAmbience` bunu
    yapar ve ortak faza geri oturur). Sürekli çalan yeni bir kaynak eklenirse aynı kapı ona da
    gerekir; tek atımlık duyuru sesleri bu listede değildir (bir sonraki tetikte zaten yeniden
    çalarlar).
