...
192. **İç engel cezası `weaponSource:"none"` modlarda KAPALIDIR.** O modlarda canlanma yoktur
    ...
    yalnız can gitmez.
193. **`Rigidbody.detectCollisions` `isKinematic`'ten BAĞIMSIZDIR — gövdeyi dinamiğe döndürmek
    kapatılmış çarpışma algılamasını geri açmaz.** Bir eşyayı taşınırken atıl hâle getiren yol
    (`WristHolster.Deactivate`, `WeaponGranter`) üç bayrağı birden kapatır; uçuşa devreden yol
    hepsini geri açmak zorundadır. Biri eksik kalırsa nesne **hata vermeden** bütün geometriden
    geçer: collider'ların açık olması hiçbir şey değiştirmez, temas hiç üretilmez. Arıza yalnız
    **atıcının kendi kopyasında** görünür — uzak kopyalar prefabtan taze örneklendiği için
    doğru davranır, yani "iki başlıkta aynı görünüyor mu" kontrolü yapılmadan fark edilmez.
    Sözleşme: atılabilirde gövdenin tek sahibi `Throwable.Arm`'dır ve devralmayı eksiksiz yapar.
