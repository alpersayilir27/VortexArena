...
| `Combat/HandGripPreset` | Parmak duruşunun **adı**: `Idle` (hafif açık) · `Firing` (işaret tetikte serbest, kalan dördü sarar) · `Grip` (beş parmak da kapanır). Sayı taşımaz — oranların yeri `HandPoseProfile`, dağıtım kapısı `HandGripPresets`. ⚠️ **Serialize edilir** (`ItemGripPose.preset` asset'lerde yaşıyor) → yeni değer **SONA** eklenir, yoksa yazılmış bütün kavramaların duruşu kayar |
| `Combat/HandGripPresets` | Preset'lerin **tek dağıtım kapısı** (saf statik): `Profile(preset)` beş kapanma oranını (`HandPoseProfile`), `Freedom(preset)` parmak serbestlik dizisini, `JointRotations(preset, rightHand)` ISDK sentetik eline yazılacak yerel eklem dönüşlerini, `DefaultFor(kind)` slot varsayılanını (ön kabza → `Grip`, aksi `Firing`) verir. **Neden tek kapı:** aynı üç duruş üç ayrı iskelette çiziliyor (yerel sentetik el, stüdyodaki hayalet el, uzak avatarın Mixamo eli) — stüdyoda görülen el ile oyunda görülen elin aynı olmasının şartı bu. ⚠️ **Eklem dönüşleri SABİT YAZILMAZ, ISDK'nın kendi varsayılan iskeletinden ÖLÇÜLÜR** (`HandSkeleton.Default*Skeleton`): SDK iskeleti değiştirdiğinde burada tek satır değişmesin diye (`HandFingerRig` ile aynı gerekçe). ⚠️ **Avuç (volar) yönü de VERİDEN çıkarılır** — gevşek iskeletin parmakları zaten avuca doğru kıvrık, yani "avuç hangi tarafta" sorusunun cevabı iskeletin kendisindedir; sol/sağ çapraz çarpım sırasına dayanan bir sözleşme işaret hatasına açıktır ve belirtisi "parmaklar el sırtına doğru kırılıyor" olurdu. Menteşe ekseni kemik ile volar yönün çapraz çarpımıdır, işareti küçük bir deneme dönüşüyle **öz-denetlenir**. Kapanma açıları parmak/başparmak için ayrıdır (başparmak anatomik olarak daha az kıvrılır, kabzayı sarmak yerine üstüne yatar); metakarpallar sürülmez. ⚠️ **`Firing`'de işaret parmağı `JointFreedom.Free` kalır** — tetik parmağının kıvrımı yazılmış bir duruş değil kumandanın analog girdisidir, kilitlenirse oyuncu ateş ederken parmağının kıpırdamadığını görür. Dönüş dizisi önbelleklidir ve **paylaşılır** (çağıran değiştirmez), serbestlik dizisi her çağrıda yenidir (sentetik elde kalıcı olduğu için paylaşılan bir dizi öteki elin duruşunu sessizce bozardı) |
...
120. **`SyntheticHand.OverrideAllJoints` TEK BAŞINA hiçbir şey yapmaz — parmakları büken şey
    ...
    ikisini birlikte yazan tek yer preset uygulamasıdır (`HandGripPoser.ApplyPreset`).
...
166. **Sentetik elde parmak SERBESTLİĞİ yapışkandır — her karede yazılır, "değişince yazalım"
    denmez.** `JointFreedom` sentetik elde kalıcıdır: bir duruştan ötekine geçerken `Free` olması
    gereken parmağa açıkça yazılmazsa el, önceki duruştan devraldığı kilitle **donar**. Bugünkü
    karşılığı tetik parmağıdır (`Firing` preset'inde `Free`, çünkü kıvrımı yazılmış bir duruş değil
    kumandanın analog girdisidir): kısa devre yapıldığında belirti "ateş ediyorum, tetik parmağım
    kıpırdamıyor" olur ve kavramanın kendisinde hata aranır. Genel kural: bir durumdan ötekine
    geçerken **"yazmadığım şey eski hâlinde kalır"** — serbestlik/override gibi yapışkan ayarlarda
    geçiş her iki yönde de açıkça yazılır.
