  ...
  Assets\
    _Shared\                 ← "ikinci bir mod/arena bunu aynen kullanır mı?" → EVET olan her şey
      ...
      Net\Scripts\           VortexArena.Net         (bağlantı/keşif/senkron; oyun bilgisi YOK)
        Editor\              VortexArena.Net.Editor  (Network Parent, sceneId bekçisi + toplu onarım)
      App\Scripts\           VortexArena.App         (Boot yönlendirme, Lobi, UiKit, köprüler)
        ...
        Survey\              (aynı asmdef) sahada kumandayla mekan ölçümü (§3.11). ⚠️ ProBuilder
                             runtime'ı build'e YALNIZ buradan girer — asmdef'in
                             Unity.ProBuilder referansı bu klasör içindir, Core'a eklenmez
      ...
      Arsenal\ FX\ Environments\ Scenes\             (kod-dışı ortak içerik + Boot/Lobby +
                             VenueSurvey — ölçüm sahnesi BOŞTUR: rig, ışık, zemin; içindeki her
                             şeyi kod kurar. Mekana ait değildir, Build Settings'te elle durur)
    ...
    Modes\Tournament\        mod kutusu: {Scripts → VortexArena.Modes.Tournament, Data, UI}
    Modes\Burger\            mod kutusu: {Scripts → VortexArena.Modes.Burger, Data, UI, Prefabs}
                             — Çocuk Oyunları ailesi; silahsız kooperatif, eşyaları ağ nesnesi
    Modes\Mole\              mod kutusu: {Scripts → VortexArena.Modes.Mole, Data, UI, Prefabs}
                             — Çocuk Oyunları ailesi; silahsız ama TAKIMLI ve kazananı olan,
                             deliği ağ nesnesi, balyozu netItemId'li eşya
    Modes\Lobby\             yalnız Data\ — lobinin ModeDefinition'ı; kodu yok (lobi bir FAZ değil
                             bir TÜR, §3.8.1)
...
```
        ...
        VortexArena.Core          oyun kodu (arena, savaş)     │ ile derlenir
           ▲          ▲                                       │
  VortexArena.App   VortexArena.Modes.* (mod kutuları)     ──┘
```
...
- **Kat geçişi bu çerçeveyi bozmaz:** çok katlı arenada rig kökü dikey sanal ofsetle taşınır (§3.13),
  telde giden Y mutlak kalır.
...
Sunucu → 15 sn status gelmezse reconnecting + bağlantıyı kapat + lobby_state yayınla
       → 45 sn daha dönmezse left (maç katılımcısıysa) ya da kayıt silinir
```
...
PlayerPoseTracker (kafa + 2 el, dünya→arena) + HeldItems (elde ne var)
        │ IPoseSource
        ▼
UdpStateChannel ──0x01 PoseUpdate (41 B)──► StateHost ──0x02 Snapshot (≤550 B)──► TÜM kayıtlı endpoint'ler
...
⚠️ **Elde bir AĞ NESNESİ varsa bayt `0` kalır** (§3.6b): o objeyi uzak taraf `object_state`'ten gelen
**kendi örneğiyle** çizer, katalogdan kopya üretmez. Baytı da doldurmak aynı elde iki obje demektir
ve ikisi gecikmede ayrışır. Parmaklar yine kavrama kaydından gelir — yolu bayt değil, alıcının
elindeki objenin kendi örneğidir.
...
- **Eşzamanlı oyuncu/admin sınırı YOKTUR** (kota ileride lisanslamayla gelecek). Tek tavan
  ...
  (`SNAPSHOT_MAX_ENTRIES_PER_PACKET = 16` → 550 B); **istemcide birleştirme yoktur ve
...
Weapon.Fire() / balta savurma / ok isabeti / bomba patlaması
   ...
   └─ hit_report (WS) → sunucu 5 tutarlılık kontrolü yapar:
        ...
        hedef başkası + takım arkadaşı DEĞİL mi? · damage sonlu ve pozitif mi?
                            ↓ geçerse
        hp -= damage → health_update (kurban + adminler) → hp ≤ 0 ise kill_event + IGameMode.OnKill
                                                          + respawn
```
...
**Can kendiliğinden dolar** (`MatchDirector.TickRegenLocked`, maç tick'inde): son hasarın üzerinden
`PLAYER_REGEN_DELAY_SECONDS` geçen oyuncunun canı **saniyede** `PLAYER_REGEN_PER_SECOND` artarak tam
cana yürür, her adım `health_update{attackerId:0}` olarak kurbana + adminlere gider. Kapıları
hasarın kapılarıdır: faz `playing` · canlı · **kalibre** (kalibresiz oyuncu hasar almadığı gibi
yarasını da kapatmaz — yoksa kalibrasyonu düşürmek bedava tam can olurdu).
⚠️ **Saati canı düşüren HER yol kurar** (`DelayRegenLocked`): vuruş hattı da, engel erimesi de. Bir
yol unutulursa hata sessizdir — yara açılırken kapanmaya başlar, engel cezası da öldürmez olur.
⚠️ **Mod ayarı DEĞİLDİR** ve `ModeRules`'a girmez: silahsız modda can hiç düşmediği için
yenilenecek bir şey yoktur ("hasar kapalı" anahtarının eklenmeme gerekçesinin aynısı).
İstemcide de iş yoktur — `DamageVignette` yalnız DÜŞÜŞTE yanar, `DamageDirectionIndicator`
`attackerId:0`'ı zaten eler, HUD barı `PlayerCombatState.Hp`'yi çizmeye devam eder.
...
⚠️ **Dost ateşiyle gelen takımdaş öldürme Counter-Strike kuralıyla işler:** öldürücü vuruş takım
arkadaşınaysa `IGameMode.OnKill` hiç çağrılmaz (ne takım skoru ne bireysel ödül), öldürene ise
`kills` −1 ve `score` −1 yazılır (eksiye düşebilir), ölene `deaths` +1; kill feed satırı normal
işler. Ceza `KillPlayerLocked`'ta sayaçların yanında durur — sayaç düzeltmesidir, ödül değil. Ödül
kapısı modun içine değil **çağrı yerine** konur: ödülün tek yazarı `OnKill` olduğu için kural tek
noktada durur ve her yeni mod ona kendiliğinden uyar. Kendini öldürmede ceza yoktur.

### 3.6b Ağ nesneleri — oyuncu olmayan varlıkların senkronu

Oyuncu olmayan her ağ varlığı (kırılabilir örtü, hedef tahtası, ileride taşınan malzeme) **tek
modelle** taşınır. Otorite bölünmesi §3.2'nin aynısıdır: **defteri sunucu tutar, sunumu istemci
yapar.**

```
Sahne kaydı:  NetObject + NetIdentity  →  Data/<Sahne>_objects.json  →  export  →  maps.json
                                                                                    │
Sahne sahnelenir (load_match / lobiye dönüş)                                        │
   └─ sunucu WorldObjectTable'ı haritanın objects[] listesinden SIFIRDAN kurar ◄─────┘
        └─ world_state (herkese) → NetObjectSync → NetObject.StateChanged → sunum
Vuruş:  istemci hit_report{targetNetId} → sunucu hp düşürür, hp==0 ise Broken biti
        └─ object_state (HERKESE) → aynı yol
```

- **Kimlik sahnede bake'lidir** (`NetIdentity.sceneId`), **tür içerikte** (`NetObjectKind`).
  İkisinin ayrı olmasının sebebi sahiplik: aynı tür on arenada geçer ve canı tek yerden değişir,
  ama hangi kimlikte hangi tür olduğu o sahnenin kendi bilgisidir. Sunucu hiçbir sahneyi açmaz;
  ikisini de `maps.json`'dan okur.
- ⚠️ **Obje durumunu istemci YAZMAZ.** Vuran taraf yalnız rapor eder; kırılma kararı sunucudadır.
  "Ben vurdum, ben kırayım" kısayolu iki başlıkta farklı siper üretir — ve fark tam da oyuncunun
  arkasına saklandığı şeyde ortaya çıkar.
- ⚠️ **`object_state` broadcast'tir**, `health_update` ise değildir (§3.12): bir oyuncunun canı
  yalnız onu ve operatörü ilgilendirir, kırılan örtü **herkesin siperidir**. Fan-out da oyuncu
  sayısıyla değil objeye vurulmasıyla ölçeklenir.
- **Kapıları oyuncununkinden farklıdır:** faz kapısı `playing` **veya** `fireWhilePaused` (lobide
  hedef tahtası vurulur), doğma koruması ve dost ateşi kapıları hiç yoktur, ve **skor/kill
  yazılmaz** — obje kırmak bir oyun olayı değil bir dünya durumudur. Tam liste
  `Docs/ArenaNet-Protokol.md` §10.10.
- ⚠️ **Anlık görüntü efektsizdir.** `world_state` sahne yüklemesinde, geç katılanda ve tur başında
  gelir; sunum onu **sessizce** uygular. Kırılma efekti/sesi yalnız `object_state` ile oynar —
  yoksa geç katılan oyuncu katılmadan önce olmuş patlamayı görürdü. Kaynağı `NetStateOrigin`
  taşır (`Live` / `Snapshot`).
- **Patlama da ağ nesnesine hasar verir:** alan etkisi yarıçaptaki her ağ nesnesi için ayrı
  `hit_report{targetNetId}` üretir; oyuncu ile obje arasında ayrı bir yol yoktur.
- **Sıfırlama sahne sahnelemesine bağlıdır**, maça değil: aynı haritada ikinci maç da sıfırlar
  (istemci sahneyi yeniden yüklediği için objeleri zaten sağlam çizer). Tur tabanlı modlar tur
  başında `MatchDirector.TryResetObjectsForMode()` ile aynı sıfırlamayı tetikler.
**Otorite dört satırdır** (tam tablo `Docs/ArenaNet-Protokol.md` §10.10):

| Ne | Kim yazar |
|---|---|
| Varlık, `hp`, `flags`, `owner`, `stage` | yalnız sunucu (`WorldObjectTable`) |
| Hareket hâlindeki poz | **sahip istemci** (UDP `0x09` → `0x05`'in obje bölümü) |
| Dinlenme pozu | sunucu, ama **sahibin bildirdiği değerle** (`object_rest`) |
| Sunum (efekt, ses, collider, materyal) | her istemci kendi başına |

Poz satırı otoritenin istisnası değil **oyuncu modelinin aynısıdır**: sunucu metre bilmez, objenin
duvarın içinde olup olmadığını yargılayacak geometrisi yoktur — doğruladığı şey poz değil
**sahipliktir**.

```
Kavrama:  grip basılır → obje YERELDE hemen ele gelir (iyimser) + object_grab
             └─ object_state{owner} (herkese) → sahip ben değilsem yerel kavrama GERİ ALINIR
Bırakma:  object_release → Held düşer, Awake kalkar, SAHİPLİK SÜRER  ← uçuş penceresi
             └─ sahip 0x09 ile OBJECT_POSE_RATE_HZ poz akıtır → uzak uçta interpolasyon
Yakalama: uçuştaki objeye (sahipli, Held değil) bir el değince YERELDE alır + object_grab
             └─ object_state{owner=yakalayan, Held} → atanın akışı kapıdan düşer, obje o ele oturur
Durma:    hız < OBJECT_REST_SPEED, OBJECT_REST_SECONDS boyunca → object_rest
             └─ Awake düşer, owner = 0, bildirilen poz DİNLENME pozu olur
```

- **Kavrama iyimserdir ve reddi ayrı bir mesaj taşımaz:** `object_grab`'in cevabı yayınlanan
  `object_state.owner`'dır. ⚠️ Bu yüzden `NetObject.OwnerChanged`, `StateChanged`'den **ÖNCE**
  tetiklenir — geri alma, sunum o duruma tepki vermeden yapılmalıdır.
- **Elden çalma yoktur, havadan yakalama vardır:** bir **elde** olan objeye gelen ikinci
  `object_grab` sessizce düşer; uçuştaki obje (sahipli ama `Held` değil) kimin eline girerse onun
  olur, atanın kendisi dahil. Teklik ayrı bir kilit değil, **sahipliğin kendisidir** — "aynı anda tek
  kişi tutsun" diye ikinci bir mekanizma yazılmaz.
- **İyimser kavrama süreyle bekler:** `OBJECT_GRAB_CONFIRM_SECONDS` içinde sahip + `Held` gösteren
  bir `object_state` gelmezse köprü yerel kavramayı geri alır — sessiz red aksi hâlde eli, objeyi hiç
  tutmadığı hâlde dolu bırakırdı. O pencerede `NetObjectBody`/`NetObjectPoseSender` yerel eldeki
  objeye dokunmaz (`HeldItems.Holds`): akan poz elle yarışmaz, elde duran obje "durdum" diye kendini
  serbest bırakmaz.
- ⚠️ **Uçuş penceresi bilinçlidir.** Sahiplik iki adımda biter (elden çıkış / durma) çünkü iki anın
  iki ayrı gerçeği var: tek mesajla tel uçuş boyunca "obje elde" derdi (o sırada bağlanan oyuncu
  havada uçan objeyi bir elin ucunda görürdü), tek mesajla objenin nereye düştüğü hiç kaydedilmezdi.
  **Durmayı istemci ölçer** — sunucunun fiziği yoktur, "poz akışı kesildi" ölçüsü birkaç kayıp
  pakette objeyi havada dondururdu.
- ⚠️ **Tutulan obje poz paketi ÜRETMEZ** ve eşya baytını da doldurmaz (§3.5): sahibin eli zaten
  `0x01` ile akıyor ve obje o ele **kanonik kavrama poziyle** bağlı; ikinci bir kanal aynı gerçeği
  taşıyıp gecikmede ayrışırdı (belirti: "bıçak elin yanında uçuyor").
- **Sahip koparsa ya da ölürse sunucu serbest bırakır** — obje son bilinen pozunun altındaki
  **zemine** iner (kopuşta `left` beklenmeden; sahipsiz obje kinematiktir, el hizasında bırakılsa
  havada asılı kalırdı). Bu kapı
  olmadan bir oyuncunun kopması objeyi tur sıfırlamasına kadar **kilitler**: kimse alamaz, kimse
  göremez.
- **Obje olayının sonucu ikisinden biridir, ikisi birden değil:** durum değiştiyse sunucu
  `hp`/`flags`/`stage`/`s`'i yazar ve `object_state` yayınlar, kozmetikse aynı `object_event` herkese
  relay edilir. Aynı gerçeği iki mesajla duyurmak istemcide iki kez oynayan bir sunum üretir. Olayı
  yorumlayan **moddur** (`IGameMode.OnObjectEvent`); adı sunucuda türün izinli listesine karşı
  doğrulanır, yani telde serbest metin var, sunucuda **yok**.
- ⚠️ **Duyuran, yazandır.** Durumu değiştiren her yol (hasar, `SetObjectStage`/`SetObjectFlags`/
  `SetObjectPayload`, `SpawnObject`/`DespawnObject`) sonucu **kendi** yayınlar; olay kancasının
  dönüşü yalnız "relay edilsin mi" sorusunu cevaplar. Sebebi bir olayın **birden çok objeyi**
  değiştirebilmesidir — "olayın objesini yayınla" kısayolu yalnız birini duyurur, diğerleri iki
  başlıkta farklı görünürdü.
- **Örnek verisi `s`'te taşınır:** `stage` sayıdır, bir örneğin kendine ait metni (bir müşterinin
  siparişi) oraya sığmaz. `modeState` ile aynı sözleşme — modun yazıp modun okuduğu serbest metin,
  çekirdek yorumlamaz. ⚠️ Olayda değil obje durumunda durur, çünkü `world_state` onu taşır: geç
  katılan başlık bekleyen müşteriyi siparişiyle kurmak zorundadır.
- **Dinamik doğuşun iki kaynağı vardır ve üçüncüsü yoktur:** modun kendisi (`SpawnObject`) ve türün
  kuralı. ⚠️ **İstemci spawn isteyemez** — isteyebilse "sunucu icat etmez" kuralı anlamını yitirir.
  Kimlik `NET_ID_DYNAMIC_MIN..MAX` aralığından gelir (sahne aralığının üstündeki yarı, ikisi asla
  çakışmaz); istemci **dinamik aralıktaki** bilinmeyen kimliği doğuş sayar, **sahne aralığındaki**
  bilinmeyen kimlik ise "export ile sahne kaymış" logudur. Despawn edilen kimlik tur bitmeden geri
  verilmez: yolda olan bir poz paketi ya da olay onu devralan yeni objeye yazardı.
- **Geri düşüş yolunda obje pozu DÜŞER** (`0x02`/`0x04`'te obje bölümü yoktur), yani 17+ oyunculu
  parçalanmış snapshot'ta kaybolan şey objenin son pozu değil **hareketinin akıcılığıdır** —
  dinlenme pozu güvenilir WS kanalından gelir.

### 3.7 Free-roam canlanma (respawn) — ürünün en özel kuralı

Fiziksel oyuncu **yatayda ışınlanamaz** (çok katlı arenada kat geçişi yalnız dikey sanal ofsettir,
§3.13). Bu yüzden respawn bir **konum değil, DURUM değişimidir**:
...
⚠️ **Çok katlı arenada ölüm oyuncuyu tabanının katına indirir** (§3.13): ölünce ~1 sn'lik bir
karartma gelir ve karanlıkta oyuncu, açık taban bölgelerinin en alçağının katına iner (taban yoksa
kat 0'a) — hedef hiçbir zaman bulunduğu kattan **yukarı** değildir. Fiziksel konumu değişmez, yalnız
dikey ofseti değişir — üst katta ölen oyuncu aksi hâlde `base` şartını hiç sağlayamazdı. Zaten o
kattaysa hiçbir şey olmaz.
...
⚠️ **Ölü oyuncunun elleri BOŞTUR:** silah verilmez (ne rastgele dağıtım ne çerçeve klonu; çerçeveden
seçim de kapalı) ve bilekte eşya durmaz — süren kılıf dolumu ölü beklerken tamamlanmaz. Canlanınca
seçim korunmuş hâlde döner (silah `RefillFull` ile dolu) ve bilek eşyası geri gelir. Kabul edilmiş
bedel: canlanması olmayan tur tabanlı modda hayalet tur kapanana kadar eli boş bekler, sonra tezgâha
yeniden yürür. **Tetik ve hasar kapıları bu kuralın dışındadır ve değişmez** (§10.3, `CanFire`).
...
⚠️ **Sonucunu operatöre bırakan modda o emniyet TÜMÜYLE kapalıdır** (`IGameMode.HoldsResultForOperator`;
bugün yalnız `tournament`). Sayaç hiç işlemez — ekran yukarıdaki komutlardan biri gelene kadar durur.
Bayrak modda çünkü karar modundur: sonuç tablosunun maçın **ürünü** olup olmadığını çekirdek bilemez,
ve mod başına bir `if` yazmak çekirdeğe mod adı gömerdi.
...
`TournamentMode` içinde yaşar; çekirdek yalnız birkaç yetenek sunar ve hiçbirini yorumlamaz:
...
| `ConsumeModeContinue()` | Operatörün bekleyen `mode_continue`'unu okur **ve siler**; çekirdek komuta kendisi davranmaz, yalnız moda taşır |

Akış: tur `playing`'de koşar → mod turu bitirir (eleme ya da süre) → maç bitmediyse
`paused`/`mode` + `modeState:"roundend:red:3"` **+ kadronun tamamı anında tam cana çekilir** →
**operatör sonucu okur ve `mode_continue` yollar** → `modeState:"regroup:2/6"` → herkes
...
**Tur biter bitmez akış PARK EDER — ilerleten tek şey operatördür.** Mod `RoundStage.Review`'a girer,
`modeState` biten turun `roundend:<kazanan>:<n>`i olarak asılı kalır ve mod kendiliğinden toplanmaya
GEÇMEZ; kapı `mode_continue`'dur (§3.8 admin komutları) ve admin istatistik panelini kapatmak onu
gönderir. Zaman aşımı yoktur, olamaz da: tur sonu hakemin karar anıdır — skoru ve oyuncu tablosunu
okur, gerekirse müdahale eder (ad/takım düzeltmesi, kalibrasyon, `kick`). Sayaçla ya da "herkes
tabanına döndü" ile açılan bir kapı tabloyu tam okunurken elinden alırdı.

⚠️ **İnceleme salt okuma basamağıdır: durum OKUR, yazmaz.** Puan, galibiyet limiti, tur tavanı ve maç
saati kararlarının tamamı turu kapatan `EndRound`'da, incelemeye girmeden ÖNCE verilir. Aksi hâlde
maçın sonucu bir düğmeye ne zaman basıldığına bağlı olurdu. Aynı sebeple `_roundLive` inceleme
boyunca `false` kalır — eleme taraması kapalı olduğu için operatör okurken boşalan bir arena ikinci
bir tur kapatamaz.

⚠️ **Bekleyen `mode_continue` bayrağı TÜKETİLİR ve mod her tikte tüketir**, yalnız incelemede davranır.
Basış tek bir olaydır; ayakta kalan bir bayrak (örneğin toplanmada gelmiş bir basış) bir sonraki
incelemeyi kimse düğmeye dokunmadan açardı.
...
`TAKIM ARKADAŞLARINIZ BASE'DE BEKLENİYOR` · `RAKİP BASE'DE BEKLENİYOR`, her birinin sonunda kapının
kendi sayacı (`· TOPLANMA h/t`, `regroup:` durumundan okunur). Sıra bilinçlidir: oyuncunun
KENDİ borcu önce gelir — kendisi dışarıdayken başkalarını sayan bir metin onu yürümek yerine
etrafına bakmaya iter. ⚠️ Roster herkesi içeride gösterirken kapı hâlâ açılmamışsa başlık yalnız
`TOPLANMA h/t` olur (yeniden bağlanan ya da bayrağı sunucuya ulaşmamış oyuncu roster sayımında
görünmez): sayı, beklemenin oyuncunun üstünde olmadığını ve kimin eksik olduğunu operatörün
sunucu penceresinden bulacağını söyler. Aynı ögeyi tur geri sayımı da kullanır, bu yüzden başlık
kısadır.
...
gönderici doğurmaya değmez. İstemci onu **mandallar**: değer turu kapatan yayınla gelir ve üstüne
yazan tek şey, inceleme bittiğinde açılan `regroup:…`dır; yoklayan bir istemci geçişi kaçırır.
Turun **numarası** da
tokenin parçasıdır — onsuz aynı takımın kazandığı iki tur birebir aynı string olur ve mandal
ikincisini "zaten gösterdim" diye yutar. Maçı **bitiren** tur da yayınlar (turu kapatan aynı tikte,
incelemeye girilmeden) — aksi hâlde maçı belirleyen tur, oyuncunun kazandığı söylenmeyen tek tur
olurdu; hemen ardından faz `finished`'a gider ve sonucu maç sonu ekranı taşır.

⚠️ **Tur sonucu ile toplanma başlığı AYNI saniyede başlar, bu yüzden ayrı yerlerdedir.** Sonuç
(`RoundResultBanner`) can şeridinin altında durur — **operatör incelemesi boyunca sayaçsız**
(`Show(..., sticky:true)`), çünkü o beklemenin uzunluğu bilinmez ve bir okuma süresi ya onu keser ya
tahmin olur; toplanma açılınca (`roundend:` gidince) iner. Başlık (`SetCenterNotice`) ekranın
...
| `Scoring` | `Team` / `Player` / `PlayerAndShared` | `Team` | Skor `match_state.scoreRed/Blue`'ya mı `PlayerInfo.score`'a mı, yoksa **ikisine birden** mi yazılır (kooperatif — aşağı bak) |
...
- **`Weapons = None` (`weaponSource:"none"`) = silahsız tur.** Tümüyle istemci sunumudur: sahnedeki
  çerçeveler gizlenir (`WeaponGranter.HidesSceneWeapons`), hiçbir grant koşmaz ve ateş kapısı
  kapalıdır (`ArenaCombat.CanFire`) — **atılabilir eşya dahil**, o da aynı kapıyı okur.
  ⚠️ "Bu modda silah var mı" sorusunun tek cevabı **kuraldır** (`ModeRuntime.IsWeaponless`),
  `modeId` değil: `FireWhilePaused` ile aynı gerekçe — mod adına bakan bir zincir her yeni modda
  bir daha yazılır ve biri unutulur.
- **`Scoring = PlayerAndShared` (`scoring:"shared"`) = kooperatif skor.** Herkesin katkısı
  `PlayerInfo.score`'a, ortak toplam `match_state.scoreRed`'e yazılır; `scoreBlue` **daima `0`**
  ve **kazanan yoktur**. ⚠️ **Bu değerin YAZAN yolu bilerek yoktur:** ortak skoru artıran bir
  sunucu metodu da, HUD tabanında hazır bir ortak skor satırı da bulunmaz — tüketicisi olmayan kod
  ölü koddur ve ilk kooperatif mod gelene kadar sessizce sapar. Bugün var olan: kural değeri, telde
  string karşılığı ve **okuma** tarafı — maç sonu kartı kooperatif skorda "kendi katkın + toplam"
  gösterir (`MatchResultOverlay.ScoreLine`, takım dalından ÖNCE: `scoreBlue` sıfır olduğu için
  takım satırı "KIRMIZI n — 0 MAVİ" derdi).
...
### 3.10 Oyun tipi → tur tipi — seçimin iki katmanı

Operatörün seçimi **iki katmanlıdır**: önce **oyun tipi** (`gameType`) — hangi oyun ailesi
oynanacak (`QuickBattle` = Hızlı Savaş, `Kids` = Çocuk Oyunları) —, sonra o ailenin içindeki
**tur tipi**, yani `modeId` (`tdm` / `ffa` / `tournament`). Oyun tipi haritaları da gruplar: bir
harita tek bir aileye aittir.

- **Kaynak istemci kataloğudur.** `GameType` enum'u `VortexArena.Core`'dadır ve hem
  `MapDefinition.gameType` hem `ModeDefinition.gameType` alanı onu serialize eder; ikisinin de
  varsayılanı `QuickBattle`'dır, yani mevcut asset'ler dokunulmadan doğru değere düşer. Telde
  karşılığı `GameTypeIds.ToWire` ile üretilen `"quickbattle"` / `"kids"` string'idir.
- **Katalog sorguları:** `GameCatalog.MapsForGameType(GameType)` operatörün ilk seçim listesini,
  `GameCatalog.ModesForMap(MapDefinition)` ise o haritada başlatılabilecek tur tiplerini verir
  (lobi profilini eler, oyun tipini ve haritanın `supportedModeIds`'ini birlikte süzer).
- **Sunucu bunu `maps.json`'dan görür:** harita satırındaki `gameType` (`MapTable.MapEntry`; boş
  gelen eski export `"quickbattle"` sayılır) ve modun kendi tipi (`IGameMode.GameType`, varsayılan
  gövde `"quickbattle"`). `start_match`'in **üçüncü kapısı** ikisinin eşitliğidir — uyuşmazsa komut
  reddedilir ve konsola sebep yazılır (§10.1). Sahadaki belirtisi "maç başlamıyor"dur.
- ⚠️ **Tur tipinin telde adı `modeId` OLARAK KALIR.** `set_selection` / `start_match` /
  `load_match` alanı yeniden adlandırılmaz: kazancı sıfırdır, bedeli tüm başlıklara yeni bir APK
  turudur (karışık sürüm desteklenmiyor).
- **Oyun tipi tele HİÇ girmez** — admin tarafında tümüyle yerel bir süzgeçtir; `set_selection` ve
  `start_match` yine yalnız `modeId + sceneName` taşır. Seçici de **yalnız katalogda birden fazla
  oyun tipi varken** çizilir: tek tip varken panel tek katmanlı kalır. ⚠️ **Oyun tipi değişimi mod
  değişimi gibi YAYINLANIR:** yeni tipin ilk modu + o mekandaki ilk haritası `set_selection` ile
  gider ve harita herkese yüklenir. Yalnız yerel kalsaydı bir sonraki `admin_state`'te ortak mod
  hâlâ eski tipe ait olduğu için seçici eski tipe geri kayardı.
...
**Zemin sapması bir bakım sinyalidir:** başlık, izleme uzayının kendi zemin tahmini ile fiziksel
zemindeki referans arasındaki farkı ölçüp `set_calibration.floorOffset` ile bildirir — referans elle
kalibrasyonda kumandanın yakaladığı nokta, çapadan geri yüklemede çapanın kendisidir; eşiği aşan farkta sunucu adminlere duyuru basar ve satırın kalibre etiketi turuncu
...
**Mekan ölçümü — aynı kumanda, ayrı iş.** Kalibrasyon *gözlüğü mekana* hizalar; ölçüm ise *mekanın
kendisini* çıkarır. Maç yokken sağ kumandada A+B üç saniye tutulunca oyuncunun cihazında **yerel**
bir ölçüm sahnesi açılır (`AppSession.SceneVenueSurvey`); oyuncu sırayla iki kalibrasyon noktasını,
duvar köşelerini ve kolonları gezip her noktada B'yi üç saniye basılı tutar, A+B ile bitirir. Sonuç
`venue_survey` ile sunucuya gider, sunucu exe'sinin yanına `<Mekan>_dimensions.json` yazar; bir kopya
gözlükte de kalır (sunucu bağlı değilken tek çıktı odur). Akışın tamamı ve doğrulama sabitleri:
`Docs/ArenaNet-Protokol.md` §10.11.

⚠️ **Çıktı eski dosyanın çerçevesinde yazılır.** Ölçüme girilen sahnenin planında kalibrasyon
noktaları varsa yeni A/B onların üstüne döndürülür; yoksa A orijine oturur ve A→B ekseni +Z olur.
Gerekçe: zemindeki bant sahada fizikseldir ve yerinden oynamaz — düzeltilen şey duvarların banda göre
konumudur, mevcut sahne sanatı da o çerçeveye kuruludur. Çerçeveyi her ölçümde sıfırlamak ölçüyü
düzeltirken bütün sahneyi kaydırırdı.

⚠️ **Ölçüm sunucuda bir durum AÇMAZ:** tek yönlü bir yükleme ve tek satırlık bir cevaptır, faz/oturum
yoktur. Sahne de yönlendirilmiş değildir — `LastMatchScene`/`LastModeId`'ye dokunulmaz, bitince
istemci sunucunun açık sahnesine döner. Koşan maç varken jest hiç çalışmaz (kapı istemcidedir).
...
(`TickIntervalMs`) faz/süre/zorla canlanma çözünürlüğüdür ve ağa **kendi kadansında** hiçbir şey
yazmaz: canı sürekli değiştiren iki işi de (engel erimesi, can yenilenmesi) kendi kısmasıyla duyurur
— 4 Hz ve saniyede bir adım.

**Hesap formülleri** (`N` = pozlu oyuncu, `A` = admin, hedef sayısı `N+A`, `E` = saniyedeki olay,
`O` = tikte akan obje pozu; snapshot her hedefe
**ayrı unicast** gider, multicast yoktur):

```
snapshot   (sunucu TX) = SNAPSHOT_RATE_HZ × (6 + 34×N) × (N+A)     ← N² büyür
                         olay/obje varken tek pakette birleşir (0x05, §6.8): (8 + 34×N + 9×E/20 + 12×O) × (N+A)
pose       (sunucu RX) = POSE_RATE_HZ × 41 × N
skeleton   (sunucu RX) = SKELETON_RATE_HZ × (14 + B) × N            ← B = SkeletonWire.BLOB_BYTES = 10 + 4 × eklem
           (sunucu TX) = SKELETON_RATE_HZ × ⌈(11+B)×N / 1200⌉ datagram × (N+A)   ← N² büyür
health_update (sunucu TX, TCP) = (isabet/sn + yaralı oyuncu) × (1 + admin sayısı) × ~140 B ← N ile büyümez
                                 yaralı oyuncu = saniyede bir adım (can yenilenmesi)
...
`⌈(11+B)×N / 1200⌉` ile böler. `B` **sabittir ve hesaplanır**: blob'u VortexArena yazar
(`SkeletonWire`, ArenaNet-Protokol §6.9), boyu `10 + 4 × eklem sayısı`dır — bugünkü listeyle 114 B
(10 B başlık + 26 × 4 B), yani `0x07` karesi 128 B ve `0x08` girdisi 125 B. Datagrama
`6 + 9 × 125 = 1131 B ≤ 1200` ile 9 girdi sığar; uzak gövde başına indiş 125 × 12 = 1500 B/s,
kendi gövdesinin çıkışı 128 × 12 = 1536 B/s. Sunucu konsolundaki `[state]` satırı batch'lenmiş
çıkışı doğrudan ölçer (`iskelet … p/s … kB/s`). Kanal ayrıca poz kanalından **daha düşük hızda** akar;
oradaki sayı bir akıcılık ayarı değildir, alıcı kök ile kemikleri aynı `INTERP_DELAY_MS` tamponunda
interpole ediyor.
...
|---|---|---|---|---|
| `0x05` snapshot+olay birleşik (393 B/tik/hedef, obje yok) | ↓ | 0,74 Mbps | 220 | 440 |
| WS `health_update` + `kill_event` | ↓ | 0,06 Mbps | 50 | 275 |
| WS `match_state` (1 Hz) | ↓ | 0,02 Mbps | 11 | 11 |
| `0x06` RTT echo | ↓ | ~0 | 11 | — |
| WS `net_stats` (1 Hz, admin) | ↓ | ~0 | 1 | — |
| `0x01` poz | ↑ | 0,11 Mbps | 200 | 200 |
| `0x03` atış | ↑ | 0,03 Mbps | 100 | 100 |
| WS `hit_report` | ↑ | 0,03 Mbps | 25 | 25 |
| `0x06` RTT yoklaması | ↑ | ~0 | 11 | — |
| `0x07` iskelet (128 B, 12 Hz × 10) | ↑ | 0,15 Mbps | 120 | 120 |
| `0x08` iskelet batch (10 × 125 B → tikte hedef başına 2 datagram: 9 + 1 girdi) | ↓ | 1,39 Mbps | 264 | 264 |
| **Toplam** | | **~2,5 Mbps** | **~1.010** | **~1.430** |

Sakin durumda (lobi, atış yok) `0x02` + poz + iskelet + yoklama kalır: **~2,3 Mbps, ~830 paket/sn**.

**1 Gbps'lik bir AP'ye göre bant %0,25'tir.** Buna karşılık ~1.100 çerçeve/sn (TCP ACK'leri dahil),
802.11ax'te küçük unicast çerçeve başına ~120–200 µs havayı tuttuğu için **tek radyoda ~%13–22
airtime** demektir. Yani darboğaz bant değil airtime; konforlu üst sınır kabaca
**2.500–3.500 çerçeve/sn**, bugünün ~2–3 katı. (AP'nin DL/UL OFDMA'sı
...
Karşılaştırma için: 60 Hz snapshot 10 oyuncuda 2,0 Mbps / 660 paket-sn; 20 oyuncu bugünkü formatta
2,5 Mbps / 840 paket-sn; 32 oyuncu 6,1 Mbps / 1.320 paket-sn (son ikisinde 16 girdi sınırı
aşıldığı için tik başına iki datagram **ve birleştirme devre dışı**). **Hiçbiri bant sorunu değil, hepsi airtime
sorunu.**

### 3.13 Kat modeli — dikey sanal ofset

Bazı arenalarda birden çok kat vardır. **Kat, oyuncunun rig kökünün dikey sanal ofsetidir:** oyuncu
fiziksel olarak hiçbir yere gitmez, salonda yürüdüğü yer aynı kalır; değişen tek şey rig kökünün
Y'sidir (`ArenaCalibrator.SetFloorLift(o katın zemin yüksekliği)`). Arena uzayı = dünya uzayı kalır
(§3.3) ve poz teli **mutlak Y** taşımaya devam eder: tel biçimi değişmez, alıcı kat hesabı yapmak
zorunda değildir. **Yatay ışınlanma yine YOKTUR** — tek istisna dikey ofsettir ve onu uygulayan tek
yer `ArenaCalibrator`'dır (§7, "Rig'i/kamerayı asla taşıma").

⚠️ Ofset her hizalama yolunun **kuyruğunda** yeniden uygulanır (elle hizalama · kayıtlı çapadan geri
yükleme · izleme gelince ön-hizalama); çapa kaydına yazılan zemin `VirtualFloorY + FloorLiftMeters`,
ve `Awake` statik ofseti sıfırlar — yoksa yeni sahnenin rig'i bir önceki arenanın katı kadar havada
başlar.

**Haritanın kat kuralları:** zemin kat (kat 0, dünya y = 0) **her arenada vardır**; üst katlar
isteğe bağlıdır ve zeminin **altında kat YOKTUR** (bodrum diye bir seviye tanımlanmaz). Her takımın
taban şeridi zemin kattadır — üst kata taban konmaz, çünkü ölüm oyuncuyu tabanının katına indirir ve
yukarı taşımaz.

**Kat seviyeleri sahnedeki portallardan türer, hiçbir yere elle yazılmaz** (`ArenaFloors`): kat 0 =
dünya y 0; kökü kat `k` zeminine oturan (±`LevelToleranceMeters`) ve `upperHeight` = h olan bir
`FloorPortal`, kat `k+1`'i `kat k + h` diye tanımlar. Gerekçe `BaseZone`'un ölçüsünü şeritten
almasıyla aynıdır: sayı listesi mesh'ten sessizce sapar ve oyuncu **altında zemin olmayan** bir
yüksekliğe kaldırılır. Aynı kat çifti için çelişen ölçü uyarı basar (ilk değer geçerli), hiçbir kat
zeminine oturmayan portal listeye girmez ve kendini kapatır; **diskleri aynı X/Z'ye düşen iki portal
uyarı basar** (bir portaldan gelen oyuncu doğrudan diğerinin dolumuna girer). Liste portal açılıp kapandıkça ve sahne
yüklenip boşaldıkça tembel yeniden kurulur; kat sayısı bir arenada birden büyükse konsola
`[ArenaFloors] n kat: 0.00 m / 3.00 m` satırı düşer.

**Geçişin kararı istemcinindir** (`FloorState`, kendini önyükleyen kalıcı tekil — sahneye konmaz):
portal ya da ölüm kat değiştirir, ofset hemen uygulanır ve `set_floor` gönderilir. Sunucu **yalnız
defter tutar** (aralık kapısı + roster'da yayma) ve katı `hello`, `load_match` ile lobiye dönüşte
`0`'a çeker; istemci **kendi roster satırından** uzlaşır, `FLOOR_CONFIRM_SECONDS` içinde yankı
gelmezse yerel katı geri alır. Ağ davranışının tek doğruluk kaynağı
`Docs/ArenaNet-Protokol.md` §10.6 "Kat modeli"dir.

`MoveWithFade` geçişin **olup olmayacağını döndürür** (`false` = geçersiz kat ya da zaten o kat) —
çağıran, ses çalmadan ve kendi durumunu harcamadan önce buna bakar. Bir geçiş sürerken gelen ikinci
istek düşürülmez: karartmanın **tepesine (tam siyah) ulaşılmadıysa hedef güncellenir**, ulaşıldıysa
**kuyruğa alınır** ve süren geçiş biter bitmez uygulanır — aksi hâlde portalın üstüne düşen bir ölüm
isteği sessizce kaybolur ve oyuncu yanlış katta canlanırdı. **Sunucudan gelen kat da karartmayla
uygulanır** (roster'dan okunan kat ve yankı gelmeyince yapılan geri alma dahil) ve ağa geri
gönderilmez. Sunucudan gelen kat arenanın kat sayısına karşı denetlenir; aralık dışıysa **kat 0'a
düşürülür ve loglanır** — sunucu defter tutar, geometriyi bilmez. Kat 0'a dönülen bir sahne
yüklemesi süren karartmayı durdurur ve roster aynasını temizler.

**Portal** (`FloorPortal` + `VA_FloorPortal`): kafa kendi katındaki çemberin merkezine XZ'de
girip **2 sn** kalınca kısa bir karartmayla diğer kata geçilir; dolum çemberi büyür, HUD'da kalan
saniye yazar. Aynı anda tek oyuncu geçer — iki kattaki çemberlerin içindeki kafalar arasında en düşük
`playerId` sahiptir, diğeri kırmızı çember + "Portal meşgul" görür. Ölü oyuncu da kullanabilir;
kalibre olmayan oyuncu kullanamaz (ofset hizalanmamış bir rig'i anlamsız bir yüksekliğe taşır).
⚠️ **Herhangi bir kat değişimi portalı KURULMAMIŞ yapar** (kendi geçişi, başka bir portal, ölüm
inişi, sunucunun sıfırlaması): dolum sıfırlanır ve portal ancak kafa diskten çıkıp yeniden girince
kurulur — yoksa yeni katta zaten diskin içinde duran oyuncu anında geri ışınlanır. Geçiş
gerçekleşmediyse (`MoveWithFade` `false`) ışınlanma sesi çalmaz ve portal harcanmaz. ⚠️ **İki
portalın diskleri aynı X/Z'de olmaz** — biri diğerinin dolumunun içine açılır (`ArenaFloors` uyarır).

**Ölüm oyuncuyu kendi tabanının katına indirir** (`FloorState` → `PlayerCombatState.TryGetOpenBaseFloor`,
açık taban bölgelerinin en alçağının katı; taban yoksa kat 0) — ~1 sn'lik karartmanın karanlık
anında. Hedef **hiçbir zaman bulunduğu kattan yukarı değildir** (`min(taban katı, bulunduğu kat)`):
ölüm bir cezadır, oyuncuyu yukarı taşıyan bir asansör değil. Gerekçe §3.7'dedir: tabanına yürüyerek
canlanan oyuncunun tabanı başka kattaysa şartı hiç sağlayamazdı.

**Farklı kattaki oyuncu iki kez çizilir ve ikisi aynı şey değildir.** Gerçek gövde **kendi katında,
normal ve vurulabilir** kalır (balkondan görünen rakip vurulabilmelidir); ek olarak izleyenin katına
izdüşürülmüş bir **silüet** çizilir (`RemoteAvatar`: aynı kemikleri paylaşan ikinci bir
`SkinnedMeshRenderer` kabuğu, `AvatarGhost` materyali, `_FloorShift` = iki kat yüksekliğinin farkı;
collider yok, gölge yok, nabız yok) ve ad etiketine " (ÜST KAT)" / " (ALT KAT)" eki gelir. Silüet
yalnız bir sunumdur: vuruş hattı onu tanımaz. **Görünümü ölü/kalibresiz hayaletten bilerek
farklıdır:** silüet **düz**, kenar parlaması (rim) olmadan, **alfa 0,2** takım tonunda çizilir;
hayalet kenar parlamalı ve alfa 0,28'dir — silüet CANLI bir rakiptir, ceset değildir ve ikisi tek
bakışta ayrılmalıdır. **Admin gözlemcide silüet ve kat etiketi soneki hiç çizilmez**
(`FloorState.ViewerHasFloor` `false`): gözlemci bir katta durmaz, izdüşürülecek bir "izleyen katı"
yoktur. ⚠️ `ProximityWarning` bilinçle Y'yi yok sayar — iki oyuncu farklı katlarda da olsa
**fiziksel olarak** aynı noktada duruyor olabilir.

**Uzak gövdenin kökü iskelet akışından gelir** (`ArenaNetCharacterBehaviour`): iskelet karesi (§6.9)
varken kökün y'si **doğrudan telin `dy`'sidir** — `dy` kökün MUTLAK arena yüksekliğidir (kafa
izdüşümünün y'si 0). ⚠️ **Alıcıda `dy`'ye kat yüksekliği EKLENMEZ:** eklenirse üst kattaki gövde bir
kat daha yukarıda çizilir. Kat defteri (`ArenaFloors.HeightOf(FloorState.PlayerFloor(id))`) yalnız
**iskelet akışı yokken** kullanılan yedek yolda kafa izdüşümünün zeminini seçer (poz kafası o
zeminin altındaysa `FloorAt`'a düşer). Gönderici T-poz kökünü `FloorState.LiftMeters`'a yazar ve SDK kare kabul kapısı
`arenaRoot.y − LiftMeters` karşılaştırır — ofset karşılaştırmadan düşülmezse üst kattaki oyuncunun
kareleri reddedilir.

**Kat, "içeride miyim" sorularına da girer:** `BaseZone.Floor` bölgenin Y'sinden türer (kat
listesinin sürümüne göre önbellekli) ve bölge ancak `FloorState.Local` ona eşitken içeride sayar;
`StandingHeightState` boy örneğinden ofseti çıkarır, yoksa üst kata çıkan oyuncunun boyu kat
yüksekliği kadar uzun ölçülürdü.

**Operatör katları tek tek izler** (`AdminFloorControls` + `AdminSpectatorCamera` + `AdminSession.Floor`):
kat sayısı birden büyük arenada alt bardaki Başlat/Durdur düğmelerinin sağında "Zemin" · "1. kat" ·
"2. kat" … düğmeleri belirir; bir kata tıklamak `AdminSession.Floor`'u yazar ve kamerayı kuş bakışına
alır. Kuş bakışı o kipte **yalnız o katı** gösterir: kameranın near/far düzlemleri o katın zemininin
0,5 m altından bir üst katın zemininin 0,5 m altına kadar bir **dilim** keser (en üst katta tavan
sınırı, zemin katta alt sınır yoktur). Diğer katların gövdeleri ve zemin halkaları dilimin dışında
kaldığı için kendiliğinden görünmez olur; serbest ve POV kamerada dilim yoktur, yeni sahnede seçim
zemine döner. ⚠️ **Kat için layer ya da tag YOKTUR:** ayrım yalnız dünya Y'sidir (`BaseZone` ve
`FloorPortal` ile aynı ilke), yani üst kat plakası **gerçek yükseklikte gerçek geometri** olmalıdır —
göze doğru görünsün diye alçağa kurulmuş bir plaka operatörün kat görüntüsünü sessizce yalanlar.
...
| `NetIdentity` / `NetSpawnCatalog` | Sahne objesi kimliği (`sceneId`; aralığı `NET_ID_SCENE_MIN..NET_ID_SCENE_MAX`; sahne kaydında ve Play'e girerken `SceneIdGuard` bake'ler ve zorlar — Play öncesi onarım yalnız bellektedir, sunucu kimliği kayıtta yazılan listeden okur, sahne kaydedilmeden görmez; merge/kopya sonrası tüm arena sahneleri için `Configure All Build Elements` → *Ağ nesneleri* → **Onar** (`SceneIdBatchRepair`, açık sahne kirliyse çalışmaz, sonda açık sahneleri geri getirir)) ve id→prefab kataloğu. Prefabdan gelen objede `sceneId` prefabda 0 kalır, sahnedeki her kopya kimliğini **prefab override** olarak alır — override prefaba Apply edilmez (`Yapma-Listesi`). ⚠️ Oyuncu senkronu bunlardan geçmez, `playerId` ile gider. Katalogun tüketicisi **dinamik obje doğuşudur** (`NetObjectSpawner`): `kind` → prefab; kaydı olmayan `kind` gelirse obje doğmaz ve tür başına tek satır uyarı basılır. Katalog `Resources` altındadır (`_Shared/Data/Resources/NetSpawnCatalog.asset`) — hiçbir sahne onu referanslamaz, taşınırsa dinamik doğuş sessizce durur |
| `NetObjectKind` | Ağ nesnesinin **tür tanımı** (SO): telde taşınan `kind` + `maxHp` (`0` = hasar almaz, kimliği olan dekoratif nesne meşrudur — tutulabilir objeler tipik olarak böyledir) + **tutulabilirlik** (`Grab`: `None`/`Anyone`) + **izinli olay listesi** (`Events`, her biri `NetObjectEventRule`) + türe özel bayrakların adları. Tür kuralı içeriğe, kimlik listesi sahneye aittir — aynı tür on arenada geçer ve kuralı tek yerden değişir. ⚠️ **Türe özel bayraklar bit4+'tan başlar ve ADLA okunur** (`TryGetFlagBit`/`TryGetFlagMask`, `NetObject.HasKindFlag`): bit0-3 çekirdeğindir (`Held`/`Broken`/`Awake`/`HeldRight`) ve bit numarasıyla okuyan bir sunum, bir bit kaydığında hata vermeden yanlış objeyi çizer. Sunucuya `maps.json`'ın kök `kinds[]`'i olarak export edilir (`grab` + `events[]` dahil) |
| `NetObjectEventRule` | Türün kabul ettiği **tek bir olayın** kuralı (serialize edilen ikincil tip, kendi dosyasında): `Name` + `Policy` (`Anyone`/`Owner`) + `PhaseGate` (`Playing`/`Any`). ⚠️ Ad telde serbest metindir, **sunucuda değildir**: listede olmayan olay reddedilir — yani buradaki bir yazım hatası derleme hatası değil, "hiç gerçekleşmeyen etkileşim"dir |
| `NetObject` | Sahne objesini ağa bağlar (`NetIdentity` + `NetObjectKind`): `Hp`/`Flags` önbelleği ve **sunumun tek kancası** `StateChanged`. Olay durumun **kaynağını** da taşır (`NetStateOrigin`: `Live` = `object_state`, `Snapshot` = `world_state`) — tüketici efekt oynatıp oynatmayacağına buradan karar verir. ⚠️ Sunum burada YOKTUR — collider kapatmak, materyale `_DamageAmount` yazmak tüketicinin işidir; bileşen ne çizer ne karar verir. ⚠️ Durumu **istemci yazmaz**: vuran taraf yalnız `hit_report{targetNetId}` yollar, kırılma kararı sunucunundur. Sahiplik yüzeyi: `Owner` · `IsMine` · `IsHeld` · `HeldByRightHand` · `IsAwake` · `Stage` + dinlenme pozu (`RestPosition`/`RestRotation`/`HasRestPose`, **arena uzayında** — kalibrasyon arena kökünü kaydırdığı için dünya kopyası bayatlardı) + `ScenePosition`/`SceneRotation` (`Awake`'teki dünya pozu): sunucunun hiç kıpırdatmadığı bir objenin dinlenme pozu budur, yoksa "hiç hareket etmemiş" obje için soracak poz kalmaz. ⚠️ **`OwnerChanged`, `StateChanged`'den ÖNCE tetiklenir**: iyimser kavramanın geri alınması, sunum o duruma tepki vermeden yapılmalıdır. `EventReceived` sunucudan relay edilen kozmetik olayları taşır; `BindDynamicId` yalnız `NetObjectSpawner`'ındır (sahne objesinin kimliği bake'lidir). `Payload` = objenin **örnek verisi** (`object_state.s`) ve `TryGetPayloadValue(key, out value)` onu `k:v;k:v` biçiminde okur — ⚠️ **ağ katmanı içeriğini YORUMLAMAZ**, biçimi yazan modundur (`modeState` ile aynı sözleşme); bozuk/eksik dize istisna atmaz, anahtar bulunamamış sayılır. `ResetToAuthoredState()` durumu bake'li başlangıca döndürür (`Hp = MaxHp`, bayrak/sahip/aşama/`Payload` sıfır, `HasRestPose` düşer) ve `Snapshot` kaynağıyla yayınlar — çağıranı `NetObjectSpawner`'dır: yeni `world_state` **yalnız yer değiştirmiş** objenin pozunu taşıdığı için, önceki maçta kıpırdamış bir sahne objesinin dinlenme pozunu başka hiçbir mesaj düzeltmez |
| `NetObjectRegistry` | `netId → NetObject` statik defteri (`OnEnable`/`OnDisable` ile dolar/boşalır). Aynı kimlik ikinci kez kaydolursa **hata basar**: sahne bake'i bozuktur ve sessiz kalırsa sahada "yanlış obje kırılıyor" diye teşhis edilir |
| `NetObjectSync` | `object_state`'i `Live`, `world_state`'i **`Snapshot`** kaynağıyla uygular; kendini önyükler (her yeni arenaya elle kurulum adımı doğmasın). ⚠️ **`world_state`'i sahne yüklenene kadar TAMPONLAR** (yalnız sonuncusunu): mesaj sahne yüklemesinden önce varabilir ve o an `netId`'lerin karşılığı yoktur — tamponlanmazsa geç katılan oyuncu kırık örtüleri sağlam görür. Mesajın `sceneName`'i yüklü sahneden farklıysa uygulanmaz, beklenir. ⚠️ **Bilinmeyen dinamik `netId` yalnız `Snapshot` kaynağında doğar** (`object_spawn` ya da `world_state`); `Live` bir `object_state` bilinmeyen kimlikle gelirse **düşürülür**: sunucu tablosunu yeniden kurarken yola çıkmış bir mesaj `load_match`'ten sonra varabilir ve ondan obje yaratmak sunucunun satırı olmayan — dolayısıyla hiç `object_despawn` edilmeyecek — bir hayalet bırakır. **Yukarı yönün de tek kapısı burasıdır:** `SendGrab` · `SendRelease` (elden çıkış) · `SendRest` (durma) · `SendEvent`. ⚠️ **Doğuş/ölüm bu katmanda prefab ÇÖZMEZ** — `object_spawn`/`object_despawn` birer olaya (`SpawnRequested`/`DespawnRequested`) çevrilip Core'a bırakılır; Net katmanı oyun bilmez, katalog ve örnekleme Core'un işidir |
| `RemoteObjectRegistry` | Uzak obje pozunun halka tamponu (`RemotePlayerRegistry`'nin obje karşılığı): `0x05`'in obje bölümünü alım thread'inden yutar, `TryGetInterpolatedPose`/`IsStreaming` ile arena-uzayı pozu verir. Halkalar bağlantı koptuğunda **ve dünya sıfırlamasında** (`Clear()`, `NetObjectSpawner`) boşaltılır: girdi son paketten sonra ~1 sn daha kullanılabilir kalır, yani önceki maçın son uçuş pozu bake'li pozuna yeni oturtulmuş objeyi geri çekerdi. ⚠️ **Oyuncu pozuyla AYNI saatte interpole edilir** (`INTERP_DELAY_MS`) — ayrı bir zaman tabanı, elden fırlatılan objeyi atan oyuncunun elinden görünür biçimde ayırırdı. ⚠️ **Kendi sahip olduğun objede poz vermez:** o objenin kaynağı sensin, gelen akış kendi paketinin yankısıdır ve bir gecikme boyu geriden sürüklerdi |
...
| `SceneRouter` | `load_match` / `return_to_lobby` / geç katılım → sahne yükleme. **Rolden bağımsız** (admin de oyuncuların sahnesine gider); rol yalnız TEK yerde ayrışır — `set_ready` sadece player'dan gider (admin "hazır" görünmemeli). **Lobi sahnesi de sunucudan gelir** (§3.8.1): `LobbyScene` alanı `return_to_lobby`/`welcome`'dan beslenir, sahne bu build'in listesinde yoksa kabuk `Lobby`'ye düşer ve sebebini loglar. Lobi bir maç sahnesi olmadığı için `LastMatchScene` boş kalır → `set_ready` gönderilmez. **Yükleme asenkrondur** (`LoadSceneAsync`): geçiş boyunca oyun döngüsü aktığı için `LoadingOverlay` çizilebilir ve ilerleme gösterilebilir; `set_ready` kapısı DEĞİŞMEZ — `sceneLoaded` aktivasyon sırasında tetiklenir ve bildirim yine oradan gider (tek kapı). **Yükleme başlamadan önce başlık siyaha iner** (`SceneTransitionFade`, aşağıda): iniş yüklemeyi en çok 0.5 sn geciktirir, karartacak quad yoksa anında geçilir ve `set_ready` akışına dokunmaz. ⚠️ Asenkron yükleme **iptal edilemez**: yükleme sürerken gelen yeni hedef (ör. maç ortasında `load_match`) **sıraya alınır** ve mevcut yükleme biter bitmez yüklenir — hedef sessizce düşürülmez. ⚠️ Geçiş boyunca `Application.backgroundLoadingPriority` **`High`'a çekilir ve sonra eski değerine geri konur**: aksi hâlde asenkron yükleme senkrondan gözle görülür biçimde yavaştır (Tuzaklar: "`LoadSceneAsync` varsayılan ayarla yavaştır"). **Sunucusuz iki kapı:** `LoadLocalScene` oyuncunun kendi cihazında açılan yardımcı sahneyi yükler (mekan ölçümü, §3.11) — sunucuya hiçbir şey gitmez ve `LastMatchScene`/`LastModeId`'ye **dokunulmaz**, yani sonradan gelen `load_match` normal yolundan geçer; `ReturnToOpenScene` sunucunun **açık** sahnesine döner (koşan maçın arenası varsa o, yoksa lobi). ⚠️ İkincisi bir "geri" düğmesi değildir: nereden gelindiği hatırlanmaz, sunucunun o anki gerçeği neyse ona gidilir — ölçüm sürerken maç başlamış olabilir |
| `SceneTransitionFade` | **Sahne geçişinin siyah karartması** — yalnız oyuncu başlığında; kalıcı tekil, kaydı `AppSingletons`'ın rol koşulunun altındadır (masaüstü admin'de kamera kalıcıdır ve sahne onun altında değişir, ekran varyantı zaten scrim'lidir). İki rampa: yüklemeden **ÖNCE** 0.5 sn'de siyaha iner (`SceneRouter` bekler) ve her `sceneLoaded`'da alfa 1'den başlar, kısa bir süre siyahta tutulup 1 sn'de açılır — uygulama açılışı ve dev penceresinden doğrudan arena sahnesinde Play dahil, özel durum yoktur. ⚠️ **Kare adımı 1/30 sn'ye kırpılır ve aktivasyon karesi atlanır:** yükleme takılması bir karede saniyelerce `deltaTime` üretir, kırpılmazsa rampanın tamamı tek karede yanıp söner — oyuncu karartmayı hiç görmez, yalnız bir sıçrama görür. ⚠️ **Aktivasyondan sonra siyah kısa bir süre TUTULUR:** sahne etkinleştikten sonraki ilk kareler hâlâ takılır (shader ısınması, nesnelerin uyanması) ve rampayı oraya açmak arenayı daha otururken gösterir — geçiş yerine "pat" okunur. Çizdiği yüzey rig prefabındaki `CenterEyeAnchor/SceneTransitionFade` quad'ıdır (`HmdOverlayBuilder` kurar, materyalini de o üretir); baş `OVRCameraRig.centerEyeAnchor`'dan bulunur — ⚠️ `Camera.main` KULLANILMAZ (rig'in üç kamerası da `MainCamera` etiketli). Alfa `MaterialPropertyBlock` → `_BaseColor` ile yazılır, `renderer.enabled` alfaya bağlıdır ve zaman `Time.unscaledDeltaTime`'dır (sunum katmanı `timeScale`'den bağımsızdır). ⚠️ **Materyalin kuyruğu 2990'dır, yani UI'nin 3000'inin ALTINDA:** yükleme ve bağlantı kartları siyahın ÜSTÜNE çizilir, oyuncu karartma sürerken yükleme yüzdesini okumaya devam eder. Rig'siz sahnede **sessizdir** (kabuk açılışı normaldir); rig var ama quad yoksa geçiş karartmasız sürer ve eksiklik **maç sahnesinde hata, başka sahnede sahne adıyla uyarı** olarak basılır: oyuncunun maça karartmasız düşmesi kurulum hatasıdır, kabuk/araç sahnesinde ise normaldir — ikisi aynı seviyede basılsa gerçek hata uyarı gürültüsünde kaybolurdu. ⚠️ **`ScreenFade` hakemine kaynak DEĞİLDİR ve `OutOfBoundsFade` quad'ı paylaşılmaz:** hakemin çizeni sahnedeki `ArenaBoundary`'dir — kabuk lobisinde yoktur, sahne geçişi tam da o çizenin öldüğü andır ve kapatılan bir muhafaza quad'ı son değerinde donar; aynı renderer'a ikinci bir yazıcı koymak "iki otorite" tuzağıdır. Bu yüzden kendi quad'ı ve kendi sürücüsü vardır. ⚠️ **"Free-roam'da görüşü karartma" kuralıyla çelişmez:** o kural **açık uçlu** hâller içindir (kopan bağlantı, kamera bekleyen kart) ve kartların scrim'siz kalmasını söyler; geçiş karartması sınırlıdır (iniş + yükleme + kısa bekleme + açılış) ve zaten **donmuş** olan bir kareyi düzenler. ⚠️ Quad **sahnenindir**, kalıcı kökün altına taşınmaz (Tuzaklar) |
...
| `ConnectionOverlay` | **Bağlantı hata ekranı** — kalıcı tekil, `RuntimeInitializeOnLoadMethod(AfterSceneLoad)` ile kendini önyükler ve **görünümü prefabtan alır**: `Resources/UI/ConnectionOverlayScreen` (masaüstü) ya da `…World` (VR), seçimi XR aygıtı/platform belirler. Prefab sahneye KONMAZ (yeni arena eklerken unutulacak adım doğmasın). ~3 sn **grace** (anlık kopmada yanıp sönmesin; açılışı da maç ortasındaki kopmayı da kapsar). Durumlar: adres yok → "SUNUCU BULUNAMADI" (bu bir bağlantı değil **yapılandırma** sorunudur, ayrı dal); adres biliniyor + **oyuncu** → `RECONNECT_GRACE` dolana dek "BAĞLANTI KOPTU · çıkarılmana N sn / maç istatistiklerin korunuyor", sonra "OYUNDAN ÇIKARILDINIZ — yeniden bağlanılıyor"; adres biliniyor + **admin** → bugünkü "SUNUCUYA BAĞLANILAMIYOR". ⚠️ **Admin'de geri sayım YOKTUR** çünkü admin kaydı kopar kopmaz silinir (§2) — ona süre göstermek yalan olurdu. ⚠️ **Değişen yalnız SUNUM:** `ArenaClient` her iki hâlde de sonsuz backoff'la denemeyi sürdürür; süre sunucunun kaydı ne zaman düşüreceğini söyler, başlığın ne zaman pes edeceğini değil. Geri sayım istemcinin **kendi** kopuş anından sayılır (bağlantı yokken sunucudan gelemez) ve Wi-Fi sessizce öldüğünde sunucununkinden `HEARTBEAT_TIMEOUT` kadar erken biter — sapma bilerek bu yönde: ekran "çıkarıldın" derken kayıt hâlâ durabilir, tersi olamaz. Altta `N sn · M. deneme` + son hata. Rol'e göre ipucu (player: joystick 1 sn / admin: launcher). Masaüstü varyantı: screen-space + scrim + **"Yeniden Bağlan"** (adres yoksa devre dışı; `Disconnect()` otomatik denemeyi durdurduğu için tek kurtarma yolu). VR varyantı: world-space kart + `HudFollow`, scrim YOK (kopuş açık uçludur, süresiz karartma free-roam'da tehlikeli; sınırlı geçiş siyahı `SceneTransitionFade`'in işidir ve kart onun üstünde okunur), **düğme YOK** (o yüzden `_reconnectButton` alanı orada boştur — normaldir). ⚠️ **Alan dışında GİZLENMEZ:** bağlantısı kopan oyuncu bunu alanın neresinde olursa olsun öğrenmek zorundadır — gizlemek onu karartmanın içinde sebepsiz bırakırdı. Baskınlık çizim sırasıyla sağlanır: kartın canvas'ı kuyruk 3000 içinde `sortingOrder = 10` ile alan-dışı karartma quad'ının (0,5 m) üstüne çıkar, muhafazanın kırmızı karartması kartın dışında kalan her yerde baskın kalır. ⚠️ **Önünde dünya arayüzü açılırken bastırılır** (`SetSuppressed(istekçi, true)` — istekçi başına, ISDK ışını isteğiyle aynı desen ve aynı bırakma kuralı: kapanışta + `OnDisable`'da bırakılır): VR kartı tembel takiple kafanın tam önünde durur ve o arayüzün üstünü kapatır; bugünkü istekçi lobinin IP tuş takımı, yani ekranın bildirdiği hatanın tek çözüm yoludur (Tuzaklar: "kafayı takip eden bir overlay, önünde açılan dünya arayüzünün üstünü kapatır"). Bastırma yalnız **sunumu** susturur, `ArenaClient` denemeyi sürdürür; grace saati bastırma boyunca **durur**. **Gizlenme kısa bir solmadır, gösterim anlıktır:** kart bağlantı geri geldiği karede, çoğu zaman sahne geçişinin ortasında kalkar ve sert bir kesme kare düşmesi gibi okunur; canvas solma bitince kapatılır (gizliyken çizim maliyeti sıfır kalır), yeni bir gösterim süren solmayı iptal eder. ⚠️ **VR kartı duvarın arkasında kalmaz:** dünya kartı derinlik testine tabidir ve alanın sınırında duran oyuncunun 1,1 m önü çoğu zaman duvardır — kart orada tam görünmek zorundadır; bu yüzden dünya varyantı `HudFollow`'un **duvar kaçınmasını** koddan açar (kafa→kart ışını katı bir yüzeye çarparsa kart bakış çizgisi boyunca öne çekilir ve aynı oranda küçülür, görünen boyutu değişmez; en yakın 0,55 m). Prefab alanı yalnız başlangıç değeridir, bu kartta kod kazanır |
| `LoadingOverlay` | **Sahne geçişi yükleme ekranı** — `ConnectionOverlay` ile birebir aynı desen: kalıcı tekil, `RuntimeInitializeOnLoadMethod(AfterSceneLoad)` ile kendini önyükler, görünümü prefabtan alır (`Resources/UI/LoadingOverlayScreen` masaüstü / `…World` VR, seçimi XR aygıtı/platform belirler), prefab sahneye KONMAZ. **Açılışta önyüklenir, ilk gösterimde değil:** prefabı tam geçiş anında `Resources.Load` etmek yüklemenin en kötü karesinde takılma üretirdi. Yalnız `SceneRouter` sürer: `Show(sceneName)` / `SetProgress(0..1)` / `Hide()`. Bar dolumu `UiKit.SetBarFill` desenidir (`anchorMax.x`) ve hedefe **yumuşayarak** gider — yükleme kareleri düzensizdir, ham `progress` sıçrardı. Başlık ve ipucu metinleri prefabta SABİTTİR; kod yalnız sahne adını, yüzdeyi ve barı yazar. ⚠️ VR varyantında **scrim YOKTUR ve eklenmez** (`ConnectionOverlay` ile aynı güvenlik gerekçesi: kartın beklediği süre açık uçludur — free-roam'da görüşü süresiz karartmak tehlikeli). Geçişin **sınırlı** siyahı ayrı bir katmandır (`SceneTransitionFade`) ve kart onun ÜSTÜNE çizilir: karartma yüzdeyi yutmaz. ⚠️ **World-space kart kamera bulunana kadar ÇİZİLMEZ** ve kamera değişince `HudFollow` yeniden başlatılır (Tuzaklar: "dünya-uzayı overlay'i kamera yokken çizme") — sahne geçişi tam da kameranın öldüğü/doğduğu andır, bu kapı olmadan ekran masaüstünde görünüp VR'da hiç çizilmez. Prefab bulunamazsa hata loglanır ve ekran hiç çizilmez — **sahne geçişi yine de tamamlanır** |
...
### İstemci: `VortexArena.App.Survey` (mekan ölçümü — yalnız oyuncu)

Sahada kumandayla mekanın ölçüsünü çıkaran akış (§3.11; protokol tarafı
`Docs/ArenaNet-Protokol.md` §10.11). Rol kapısı burada değil `AppSingletons`'tadır.

> ⚠️ **Ölçüm sahnesi hiçbir özel bileşen TAŞIMAZ** — sahnede yalnız rig, ışık ve zemin durur,
> geri kalan her şeyi `VenueSurveyGesture` sahne yüklendiğinde kodla kurar. Gerekçe: sahneye
> konan bir bileşen seri referans ister, referans derleme ister ve ölçüm sahnesi bu zincirin
> tamamına bağımlı hâle gelirdi; ayrıca bu klasördeki hiçbir sınıf prefab/sahne düzenlemesi
> beklemez.
>
> ⚠️ **ProBuilder runtime'ı build'e YALNIZ buradan girer.** `VortexArena.App` asmdef'inin
> `Unity.ProBuilder` referansı bu rehber içindir; `VortexArena.Core`'a aynı referans **eklenmez**
> (§7).

| Sınıf | Görevi |
|---|---|
| `VenueSurveyGesture` | Kalıcı tekil (`SceneRouter.Install` deseni); **ölçüm sahnesinin dışında** jesti dinler: A+B üç saniye → bağlamı `ArenaBoundary.Active`'ten alıp yerel ölçüm sahnesini yükler. ⚠️ Ölçüm sahnesi **aktifken hiçbir şey okumaz** — girdinin sahibi orada denetleyicidir, iki dinleyici aynı tuşu iki kez tüketirdi. `SceneRouter.LastMatchScene` doluysa jest hata titreşimiyle reddedilir (maç sırasında ölçüm yok). Sahne yüklenince denetleyiciyi **kodla** yaratır; `venue_survey_result`'ı da bu tekil dinler (sonuç sahne kapandıktan sonra gelebilir) |
| `VenueSurveyController` | Ölçüm sahnesinin beyni: mod sırası (kalibrasyon → köşeler → kolonlar), nokta listeleri, doğrulama, geri alma ve gönderim. Sahnede yaratılır, **sahneyle ölür**; yarım kalan ölçüm kaydedilmez (sahne kapanırsa noktalar atılır — yarım bir alan dosyası, dosyanın hiç olmamasından kötüdür) |
| `VenueSurveyInput` | Jest çözümü (MonoBehaviour değil): basılı tutma süreleri, tık dizisi ve **kenar** bayrakları. ⚠️ Her jest bir kez ateşler ve yeniden silahlanmak için **iki tuşun da bırakılması** gerekir; birbirinin içine giren tutuşlar (A tutarken B'ye basmak) bozulmuş sayılır — aksi hâlde mod değiştirme tıkları aynı anda "geri al" olarak da okunurdu. Süreler `Time.unscaledTime` ile ölçülür |
| `VenueSurveyHaptics` | Titreşim sözlüğü (statik, `ArenaCalibrator.Pulse` kalıbı): 1 kısa = nokta alındı · 2 kısa = mod değişti · 3 kısa = reddedildi · 1 orta = geri alındı · 1 uzun = girildi/gönderildi. **Tek geri bildirim kanalı bu değildir ama en güvenilir olanıdır** — oyuncu ölçüm sırasında etikete değil duvara bakıyordur |
| `VenueSurveyContext` | Ölçüme **girerken** doldurulan statik bağlam: şablon var mı, şablonun A/B noktaları, mekan adı, varsayılan kolon yüksekliği, dönülecek sahne. ⚠️ Bağlam sahneden ÖNCE toplanır çünkü ölçüm sahnesinde `ArenaBoundary` yoktur — şablonun okunabileceği tek an çıkış anıdır |
| `VenueSurveyGuide` | Gözlükte görülen rehber geometri (ProBuilder): nokta küpleri, A/B kalibrasyon küpleri, köşelerden çıkarılan zemin+duvar hacmi (tavan kapağı silinir, yoksa oyuncu kendi ölçtüğü kutunun içinde kör kalır) ve kolon prizmaları. ⚠️ `CreateShapeFromPolygon` üçgenleme düşünce **sessizce boş mesh bırakır** — dönüş değeri ve yüz sayısı kontrol edilir, düşerse obje yok edilir ve hata loglanır (`DimensionMeshBuilder` ile aynı kapı) |
| `VenueSurveyLabel` | Kafaya bağlı TMP etiketi: bulunulan mod, sayaçlar, jest hatırlatması, süren tutuşun ilerlemesi ve son olay. Kameranın **anchor'ına** bağlanır (`Camera.main` kullanılmaz, §7) |
| `VenueSurveyExport` | Nokta listeleri → `ArenaDimensions`: çerçeve normalizasyonu (şablon varsa eski A/B'nin üstüne döndürme, yoksa A = orijin), milimetreye yuvarlama, protokol mesajına paketleme ve **yerel kopya** (`persistentDataPath/VenueSurvey/`). ⚠️ Yerel kopya sunucu bağlı olsa da yazılır: gönderim başarısız olursa alınmış bir ölçümün tek nüshasıdır |
...
tek yerde — bu bileşende — kalır. Çözülmüş planın kendisi `Plan` ile **salt okunur** verilir (plan
yoksa `null`); bugünkü tüketicisi mekan ölçümünün şablon çerçevesidir (§3.11). ⚠️ **Dönen nesne
kopya değildir, çağıran onu DEĞİŞTİRMEZ** — plan önbelleklidir, üstünde yapılan bir düzeltme
muhafazanın mesafe hesabını sessizce başka bir arenaya çevirirdi.
⚠️ **Gözlemci kipindeki muhafaza `IsOutOfBounds`'u `false`'a
...
`defaultColumnHeight` hiç çizilmeyen kolonlar demekti). Okuyucu `JsonUtility`'dir, **yazıcı
(`ToJson`) elle yazılmıştır**: `JsonUtility.ToJson` her float'ı 17 basamak basar (`3.17` →
`3.1699981689453127`) ve dosyayı elle okunamaz hâle getirirdi; elle yazıcı sayıları milimetreye
yuvarlar (`0.###`) ve `topViewHeight`'ı yalnız sıfırdan büyükse yazar. ⚠️ **Kolonun muhafazaya girip girmeyeceğini
...
geçici sahne kopyasından siler; sahne dosyası değişmez. ⚠️ **Gerekçesi boyut ya da "ProBuilder
build'e girmesin" DEĞİL, sahipliktir:** ProBuilder runtime'ı build'e zaten giriyor, ama yalnız
`VortexArena.App`'in mekan ölçüm rehberi üzerinden (`App\Scripts\Survey`, §3.11) — `VortexArena.Core`
o pakete referans VERMEZ ve verilmez. Maketteki `ProBuilderMesh` sanat değil **ölçü referansıdır**:
sahada çizilmesi gereken bir şey değildir ve Core'a bir paket bağımlılığı satın almaya değmez. **Editör Play kipinde** ise her şey
...
obje düşerdi.
**Kat ofsetinin tek sahibi de burasıdır** (§3.13): `SetFloorLift(metre)` rig kökünü **yalnız Y'de**
taşır, `FloorLiftMeters` o anki ofseti verir ve ofset her hizalama yolunun kuyruğunda yeniden
uygulanır; çapa kaydına yazılan zemin `VirtualFloorY + FloorLiftMeters`, `Awake` ise statik ofseti
sıfırlar. ⚠️ Rig kökünü oynatan **başka hiçbir kod yoktur** ve eklenmez (§7, "Rig'i/kamerayı asla
taşıma"). **Yerleri sahneden değil boyut dosyasından gelir**: `Start` işaretçileri
...
**Her kalibrasyonda zemin sapması ölçülür:** elle kalibrasyonda B noktası yakalandığında izleme
uzayının kendi zemin tahmini ile yakalanan zemin noktası, çapadan geri yüklemede aynı tahmin ile
çapanın kendisi karşılaştırılır ve fark `set_calibration.floorOffset` olarak bildirilir (§3.11) —
ölçüm rig'in **her** yeniden hizalanmasında tekrarlanır (elle B noktası · çapadan geri yükleme · takip bozulunca yapılan sessiz yeniden hizalama, sonuncusunda `source` `anchor` kalır), yani oturum içindeki kayma da satıra düşer. ⚠️ Ölçü **tracking-yerel** alınır — arena hizalamasından SONRAKİ dünya
...
⚠️ **Sayı tek başına iki arızayı ayırmaz:** "kumandanın ucu yakalamada yüksekteydi" (hizalama kaymıştır, oyuncunun ayakları zemine batar) ile "başlığın zemin tahmini kaymış" (hizalama doğrudur, sapma zararsızdır) aynı değeri üretir; **oturum içi hiçbir ölçüm de ayıramaz**: gereken bilgi oyuncunun gerçek göz yüksekliğidir, o da dışarıdan gelir. Gözlük yalnız karşılaştırılacak sayıyı basar — `[Kalibre] zemin ölçümü: kaynak=… nokta=… kafa=… sanalZemin=… köken=…` satırı bağlamı verir (`kaynak` yakalama elle mi çapadan mı; `köken` `Stage` değilse `nokta` zaten anlamsızdır), kararı ise onu izleyen `hizalama sonrası kafa yüksekliği` satırı taşır: hizalamadan sonraki birkaç saniyenin **en yüksek** değeri. ⚠️ Tek kare ölçüm **ters yönde** yanıltır — doğru yakalama işaretin üstüne eğilerek yapıldığı için o kare çömelmeyi ölçer, uç havadayken yapılan hatalı yakalama ise ayakta yapılır; aynı gerekçe `BodyScaleState`'in 10 sn beklemesinde de var. Kural: bu değer oyuncunun **gerçek göz hizasına** eşit olmak zorundadır — değilse hizalama yanlıştır, sapmanın büyüklüğü değil. Ayakta durulamayacak kadar düşük bir değer ayrıca uyarı basar. ⚠️ **İki satır da `client_log` ile sunucu günlüğüne aktarılır** (`info` seviyesinde; gözlüğün kendi günlüğü sahada okunamaz) — kafa yüksekliği **tel üzerinde taşınmaz**, yani yalnız gözlük ve sunucu konsolunda okunur; `kaynak` ile zemin sapması ise ayrıca roster'dan admin konsoluna düşer (`AdminRoster`).
...
⚠️ **Yakalama log'undaki `fiziksel` konum DÜNYA uzayıdır ve ön hizalama kaymasını içerir** — ~1 m'lik
bir Y tek başına hata kanıtı değildir. Zemini parantezdeki `zemin sapması` (tracking-local Y) söyler:
uç yerdeyse ~0, `CALIB_FLOOR_WARN_METERS`'ı aşarsa uç yere değmemiş ya da gözlüğün alan verisi bayat.
...
Her bildirimde son ölçülen zemin sapmasını da yollar ve değer **tazedir**, çünkü her hizalama onu yeniden ölçer (§3.11).
...
`ArenaNetCharacterBehaviour.SeedBodyHeightHint`). ⚠️ **İpucu tek denemede yerine oturmaz ve ret
NORMAL cevaptır:** çalışma zamanı öneriyi gövde takibi gerçekten koşana kadar reddeder, oysa karakter
ondan çok önce var olur — tek sessiz deneme hiçbir yere inmez. Kabul edilene kadar saniyede bir
tekrarlanır (pencere birkaç on saniyedir, izin penceresini yavaş cevaplayan oyuncuyu kapsar) ve
pencere dolarsa uyarı düşer; başarı da tek satırla yazılır, yoksa "ipucu verildi mi" sorusunun
cevabı hiçbir yerde olmaz.
...
ölçü görselden geldiği için denetim yolu gözle bakmaktır. **Bölgenin katı Y'sinden türer**
(`Floor`, kat listesinin sürümüne göre önbellekli) ve içeride sayılmak için oyuncunun katı ona eşit
olmalıdır — §3.13: kat yalnız bir dikey ofset olduğu için XZ testi tek başına alt kattaki şeridi
üst kattan da "içeride" sayardı),
`MapDefinition` / `ModeDefinition` / `GameCatalog` (içerik SO'ları; harita ve mod tanımının
ikisi de bir `gameType` alanı taşır — varsayılanı `QuickBattle`, yani mevcut asset'ler
dokunulmadan doğru değere düşer, §3.10. Harita tanımı ayrıca **harita başına köstebek yoğunluğunu**
(`moleDensity`) taşır: export'la `maps.json`'a çıkar, yalnız sunucudaki `MoleMode` okur — alanın
varsayılanı yeni bir mekanın başlayacağı yoğunluktur. Katalog sorguları: `MapsForMode(modeId)` bir modu
...
profilini eler, oyun tipini ve haritanın `supportedModeIds`'ini **birlikte** süzer),
...
zemini yanlış kurulduğunda yalan söyleyen tam olarak odur; ⚠️ örnekten **kat ofseti düşülür**
(§3.13), yoksa üst kata çıkan oyuncunun boyu kat yüksekliği kadar uzun ölçülür ve tavan bir daha
aşağı inmediği için bel çizgisi maçın kalanında ulaşılamaz kalır. Cihazda SAKLANMAZ: işletme gözlüğü
elden ele geçer. İkinci koşul silahın aşağı doğrultulmasıdır ve KUMANDADAN okunur (`anchor.forward`, yatayın 25°
...
atış sesini havuzlu çalar — namlu alevi **elindeki silahın kendi `VisualEffect` grafiğidir** (yerelde
`WeaponMuzzleVfx`'in gönderdiği aynı `OnFire` olayı; grafik `SterilizeVisual`'dan sağ çıkar çünkü
MonoBehaviour değildir), havuzdaki eski parçacık yalnız grafiği olmayan eşyanın yedeğidir, ikisi
birden oynamaz —, tracer'ı çizer ve silahın **geri tepmesini** tetikler —
...
takım tonu; collider yok, gölge yok, nabız yok) çizilir — yalnız gövde görünürken ve kat
farkı varken açık, ad etiketi " (ÜST KAT)" / " (ALT KAT)" ekini alır. Silüet **sunumdur**, vuruş
hattı onu tanımaz. Ghost materyali ya da gövde mesh'i eksikse bir kez uyarır ve kabuk kurulmaz.
**Görünümü ölü/kalibresiz hayaletle KARIŞTIRILMAZ:** silüet düz, **kenar parlaması olmadan**, alfa
**0,2** ile çizilir (hayalet kenar parlamalı ve alfa 0,28) — silüet canlı bir rakiptir, ceset değil.
İzleyenin katı yoksa hiç kurulmaz (`FloorState.ViewerHasFloor`, admin gözlemci). Kabuğun bounds payı
gövde ölçeğine **bölünür** (`SetBodyScale` silüeti yeniler): pay yerel uzaydadır, bölünmezse kısa
gövdede şişer, uzun gövdede kırpılır; `updateWhenOffscreen` kapalıdır — kemikler zaten her karede
yazılıyor, açık bırakmak her kare bir sınır hesabı satın alırdı.
...
şeydir** (kaynağın `localScale`'i = `BodyScale`, `ArenaNetCharacterBehaviour.ApplyArenaRoot` yazar) ve ayrıca çarpılır — ikisi karıştırılmaz. ⚠️ Kalçanın bind'e göre farkı `heightCalibration`'a **bölünerek** aktarılır: hedef kök o çarpanı zaten taşır, bölünmezse çömelmede kalça bacağın yetiştiğinden fazla iner ve ayak yere gömülür. Ağ yolu, MSDK retarget config'i ve
...
açıp kapatır, her kare açık tutmak onunla kavga ederdi. ⚠️ **Mesafeli kavrama görselleri (tüp +
retikül) ayrı bir istektir ve EL BAŞINA susturulur** (`SetGrabVisualsSuppressed(isteyen,
suppressLeft, suppressRight)`; kuralı `WeaponGranter` verir): görsel düğümün hangi ele ait olduğu
`WeaponGranter.ResolveControllerFromGameObject` ile çözülür — el çözülemezse düğüm iki elden biri
susturulduğunda gizlenir. Aynı geri bildirimi interactor'ın aday listesini süzerek almak YASAKTIR:
boş aday listesiyle ISDK basışı yer (Tuzaklar). ⚠️ **Oyuncunun gördüğü el rig'in kendi sentetik
...
| `ModeSelection` | **Henüz başlamamış** maçın seçili modu (`selection_state`, §5.3) — yalnız sunum. `ModeRuntime` ile karıştırılmaz: orası **koşan** maçın kuralıdır. Statik `HasValue`/`ModeId`/`IsTeamless`/`IsKidsGame` + `Changed`; besleyicisi `ModeRuntimePump`. `IsKidsGame` seçili modun oyun ailesini **yerel katalogdan** çözer (aile telde taşınmaz). ⚠️ Hiçbir kuralı/HUD'ı/loadout'u değiştirmez (maç türü `start_match`'i bekler) ve **tüketicisi olmayan alan eklenmez** — tüketicileri `BaseZoneVisibility` (takım kipi) ve `WeaponGranter` (çocuk oturumunda silah kapısı) |
...
| `UI/HudFollow` | HUD panelini kafanın önünde **tembel takiple** tutar (mesafe + dikey ofset, yaw ölü bölgesi, yumuşatma) — gerekçesi Tuzaklar'da ("can barı neden kafaya kilitli"). **Duvar kaçınma** (`avoidWalls`, prefab alanı + `AvoidWalls` özelliği; **varsayılan KAPALI**): kafa→panel ışını (trigger'lar, oyuncunun kendi rig'i ve panelin kendisi hariç) katı bir yüzeye çarparsa panel bakış çizgisi boyunca öne çekilir ve **aynı oranda küçülür** (`localScale`), görünen boyutu değişmez; `minDistance`'tan yakına gelmez, çarpılan yüzeyle arasında `wallClearance` payı kalır. ⚠️ Kapalı olmasının sebebi ölçekle oynamasıdır: kafaya kilitli çocuğu olan panelde (mod HUD'ı → `HealthHud`) açılırsa çocuk da ebeveynle küçülür. Bugün açan tek yer `ConnectionOverlay`'in dünya kartıdır |
...
| `Combat/SurfaceDefinition` | Bir yüzey ÇEŞİDİNİN çarpma sözleşmesi: parçacık prefabı, çarpma sesleri (+ seviye ve perde bandı), düğümün gizlenme süresi ve **o yüzeye çözülen materyaller**. Materyal listesi ana bağlama yoludur — bir materyali burada bağlamak onu kullanan her renderer'ı her arenada birden kapsar, yani yeni bir environment paketi sahne işi değil liste satırı maliyetindedir. ⚠️ **Materyal ASSET'i bağlanır:** çalışma anında `renderer.material` okumak KOPYA üretir ve sözlükte hiçbir zaman eşleşmez — kütüphane bu yüzden yalnız `sharedMaterial` okur, belirti olsaydı "her şey default yüzey" olurdu. ⚠️ **Çok materyalli mesh DESTEKLENMEZ** (yalnız ilk materyal çözülür): hangi submesh'e vurulduğunu bilmek `hit.triangleIndex` ister, o da mesh'te Read/Write (bellek iki katı) + `MeshCollider` şartı koyar; objeyi ikiye bölmek ya da `SurfaceTag` koymak yeterlidir. ⚠️ Ömür parçacığın kendi ömründen kısaysa efekt yarıda kesilir. **Klip yerleşimi:** `Assets/Audio/Impacts/SFX_Impact_<Yüzey>_NN`; seviye `AudioMix.Weapons` ile çarpılır (§4 `AudioMix`), prefabda `AudioSource` yoksa havuz düğümü kendi ekler |
| `Combat/SurfaceLibrary` | *(`_Shared/Data/Resources/SurfaceLibrary.asset`)* "Bu collider hangi yüzey" sorusunun **tek** cevabı. Çözüm sırası, ilk eşleşen kazanır: (1) collider'ın ÜSTÜNDE bir `SurfaceTag` — açık override; (2) renderer'ın `sharedMaterial`'i sözlükte; (3) `default` tanım. ⚠️ **Üçüncü basamak isteğe bağlı DEĞİLDİR:** eşlenmemiş bir yüzeyde hiçbir şey çıkmaması "efekt bozuk" diye okunur, jenerik bir toz ise dekor diye okunur. ⚠️ **Kimlik MATERYALDEN gelir, tag/layer'dan değil:** tag obje başına tek düz addır ve iki materyalli duvarı ayıramaz, layer fizik filtresinin kaynağıdır ve sayısı azdır (malzeme çeşidine harcanırsa gerçek bir fizik ihtiyacına yer kalmaz), materyal ise zaten görünüm başına yazılmıştır ve tek eşleme tüm arenayı kapsar. Aynı materyal iki tanıma bağlanırsa **ilk bağ** geçerlidir + tek uyarı: sonuncuyu kazandırmak efekti liste sırasına bağlar, aynı duvar bir build'de kıvılcım öbüründe talaş çıkarırdı. `WeaponCatalog` ile aynı gerekçeyle `Resources`'ta — tüketicisi kendini önyükleyen bir tekildir, bağlanacak referans alanı yoktur. Sözlük ilk sorguda kurulur (atış başına tanım listesini gezmek saniyede on kez tarama demekti) |
| `Combat/SurfaceTag` | Tek obje için açık yüzey override'ı; çalışma anı davranışı YOKTUR, yalnız mermi düştüğünde okunur. Materyal eşlemesinin yetmediği haller içindir: çok materyalli mesh, ya da materyalini başkalarıyla paylaştığı halde farklı ses vermesi gereken obje. **İstisnadır** — kütüphanedeki materyal eşlemesi bir arenayı tek satırda kapatır, bu bir objeyi kapatır ve hatırlanması gerekir. Collider'dan YUKARI aranır (`RemoteHitBox` şekli): bileşen ebeveynde dururken collider çocukta olabilir. ⚠️ **Dekor haritalı arenada istisna değil KURALDIR:** görsel harita mesh'i collider taşımaz, mermi görünmez sınır kutularına (`ArenaBoundry*`) ve kulübe duvar collider'larına çarpar; renderer'sız collider materyale hiç çözülmez ve `default`a düşer — yüzey o objelerin köküne konan etiketle verilir, **yeni sınır kutusu eklerken etiket de kopyalanır** |
| `Combat/SurfaceImpactFx` | Merminin değdiği yüzeyin parçacığını ve sesini oynatan **havuzlu, kendini önyükleyen DDOL tekil** (`HitMarker` kalıbı, `SurfaceImpactFx.Shared`) — sahneye konmaz, yeni arenaya kurulum adımı doğurmaz. Havuz **yüzey başınadır** ve o yüzeye ilk isabette kurulur; düğüm `hit.point + normal * ε` konumuna `LookRotation(hit.normal)` ile oturur, yani +Z'ye püskürtmek üzere yazılmış bir parçacık sistemi her açıda duvardan dışarı püskürür. ⚠️ **Atış başına `Instantiate`/`Destroy` YOK:** 600 RPM'de bu doğrudan GC dikenidir ve ateş eden oyuncu sayısınca çoğalır. ⚠️ Parçacıklar oynatılmadan önce `Clear`'lanır — geri dönüşen örnek bir önceki çarpmayı hâlâ oynatıyor olabilir ve temizlemeden başlatmak onun parçacıklarını bir kare eski duvarda bırakır. Ses `PlayOneShot`'tır (aynı düğüm hâlâ ötüyor olabilir, `Play` onu keserdi). Prefabı olmayan yüzey de düğüm alır: yalnız sesli yüzey hiçbir yerde özel dal açtırmaz. ⚠️ **Doğrudan çağrılmaz** — kapı `ArenaCombat.ReportImpact` |
| `Combat/BlastFxPool` | Patlama sunumunun (parçacık + tek atımlık ses + kısa flaş ışığı) **havuzlu, kendini önyükleyen DDOL tekili** (`SurfaceImpactFx` kalıbı, `BlastFxPool.Shared`) — sahneye konmaz, yeni arenaya kurulum adımı doğurmaz. Havuz **patlama prefabı başınadır**. ⚠️ **Patlama başına `Instantiate`/`Destroy` YOK:** Quest'te bu yalnız GC dikeni değil, **ilk patlamada shader/materyal ısınması** demektir — takılma tam da oyuncunun patlamaya baktığı anda düşer. ⚠️ `Prewarm` **fitil başında** çağrılır (`Throwable.Arm` → `ThrowableEffect.Prewarm`) ve tek anlamlı yer orasıdır: tetik anı stall'a en kapalı andır, fitil ise zaten ölü zamandır. Parçacıklar oynatılmadan önce `Clear`'lanır (geri dönüşen düğüm bir önceki patlamayı oynatıyor olabilir), ses `PlayOneShot`'tır. **Flaş ışığı kodda yaşar, prefabda değil:** düğüm kurulurken köke tek bir `Point` ışık eklenir ve `Update`'te doğrusal söner. ⚠️ Parçacık sisteminin **Lights modülü kullanılmaz** — parçacık başına gerçek zamanlı ışık Quest'te maliyetlidir ve görünümü platformun ek-ışık bütçesine bağlı olduğu için başlıkla admin arasında sessizce ayrışır. Ömür `Time.unscaledTime` ile sayılır (duraklamış maç patlamayı ekranda dondurmasın). ⚠️ **Doğrudan çağrılmaz** — kapı `BlastEffect` |
| `Combat/ArenaCombat` | **Oyun kodunun ağa açılan tek kapısı** (statik). `ReportShot` / `ReportThrow` / `ReportHit` / `ReportRaycastHit` / `ReportAreaHit` / `ReportAreaSelfHit` / `ReportImpact` + `TryGetTargetPlayerId` / `IsHeadshot` / `CanFire` / `LocalPlayerId`. ⚠️ **Kendine alan hasarı AYRI bir metottur ve olmak zorundadır:** `ReportAreaHit` oyuncuları `RemoteAvatar` listesinden gezer, yerel oyuncunun avatarı yoktur — kendi bombasının hasarı o yoldan asla çıkmaz. `ReportAreaSelfHit` **dost ateşi anahtarını kendisi okur** ve kapalıyken hiç rapor yollamaz (sunucu zaten reddederdi). ⚠️ **Oyuncu alan hasarında GÖVDE KAPSÜLÜYLE ölçülür, isabet kutularıyla DEĞİL** (`TryScoreBody`): oyuncunun katının arena zemininden kafa pozuna dikey bir kapsül (yarıçap koddaki `BodyProxyRadiusMeters`); uzaklık kapsülün **yüzeyine** ölçülür (merkez gövdenin içindeyse sıfır = tam hasar), siper ışını kapsülün ortasına atılır. Atan ve diğerleri **aynı** kapsülle puanlanır — nişancı oyunlarının çarpışma gövdesi kuralı. İsabet kutuları silah ışınının işidir: kemiklere asılıdır, gecikir ve zemine gömülür; kutu başına ölçüm bir adım ötedeki oyuncuda kafayı (≈1,7 m yukarıda) okuyup 150'lik bombayı 20-30'a düşürür. ⚠️ **Zemin oyuncunun katının ARENA zeminidir** (`ArenaFloors`), rig'in takip uzayı DEĞİL: takip kökeni göz seviyesindeyken o düzlem kafada durur, kapsül göze çöker ve ayağının dibindeki bomba iki metre uzaktaymış gibi puanlanır — çömelince artan, ayakta azalan hasar bunun belirtisidir. Sonucu: çömelmek hasarı değiştirmez, ayağının dibindeki bomba merkez hasarını verir. İkisinde de isteğe bağlı **siper kapısı** vardır (`requireLineOfSight`), böylece duvar arkası hasarı için elle `OverlapSphere` yazılmaz. ⚠️ **Siper kapısı açıkken engel iki sınıftır ve davranışları farklıdır:** kırılamayan her şey (arena geometrisi, `maxHp 0` ağ nesnesi) hasarı **tümden keser**; kırılabilir bir ağ nesnesi ise **kalan canı kadarını SOĞURUR** ve artanı arkasındakine geçirir — kırık olan (enkaz) hiçbir şey soğurmaz. ⚠️ **Sıra bağlayıcıdır — önce soğurma, sonra mesafe düşümü:** siper **merkez hasarından** düşülür, mesafe düşümü kalana uygulanır. Ters sıra sipere **zaten küçülmüş** bir sayı üzerinden bedel ödetir; sonucu, bir bombanın gözle görülür biçimde kırdığı siperin arkasındakine **hiç** hasar geçirememesidir — hata vermez, yalnız yanlış çalışır. Aynı sıra üç yolda da geçerlidir (uzak oyuncu, ağ nesnesi, kendine hasar), yoksa atan başkasından farklı bir eğriye tabi olur. Zincirdeki her siper ayrı ayrı sayılır, hedefin kendi collider'ı kendini gölgelemez. Soğurulan miktar istemcinin `NetObject.Hp` **aynasıdır**: hasarı istemci hesaplar, sunucu olduğu gibi uygular (§10.3), yani tek gidiş-dönüş içindeki iki patlama aynı canı okur ve ikincisi bilerek fazla deler. ⚠️ Sorgu yalnız `Obstacle` maskesini okur — arenanın **DIŞ** duvarı/zemini/tavanı o layer'da olmadığı için patlamayı durdurmaz. ⚠️ **Nesne arama sorgusu ayrı daraltılır:** alan etkisinin **ağ nesnesi** hedefleri `ArenaLayers.AreaTargetMask` (`Default` + `Obstacle`) üzerinde ve **tetikleyiciler yok sayılarak** aranır (oyuncular bu sorgudan değil avatar listesinden gelir). Maskesiz + tetikleyicili sorgu dekoru, kavrama hacimlerini ve etkileşim tetikleyicilerini aynı tampona doldurur ve **dolan tampon gerçek hedefleri sessizce düşürür** — daraltma bir maliyet kaygısı değil **doğruluk** kuralıdır. Buna dayanan değişmez: hasar alabilen her ağ nesnesi `Default` ya da `Obstacle` üzerinde ve **tetikleyici olmayan** bir collider taşır. ⚠️ `ReportShot`/`ReportThrow` **UDP olay kanalına** (`0x03`) gider, `ReportHit` **WS**'te kalır — kaybı kozmetik olan ile otoriter olanın kanalı ayrıdır (§10.3). Bir vuruşu doğru bildirmek dört şeyi bilmeyi gerektiriyor (arena uzayı, yön≠nokta, `RemoteHitBox` ile hedef çözme, hasarı istemcinin belirlemesi) — bunlar `Weapon` içinde gömülü kalsaydı ikinci bir hasar kaynağı yazan herkes aynı dördünü yeniden keşfederdi. `Weapon` de bu kapıyı kullanır (tek doğruluk kaynağı). Bağlantı yokken sessizce no-op. **Sunum işi de burada durur ve iki tanedir:** `ReportHit` gönderimden sonra `HitMarker`'ı tetikler, `ReportImpact` ise merminin DEĞDİĞİ yüzeyin efektini (`SurfaceImpactFx`) — kapının kendisiyle aynı gerekçe, yeni bir hasar kaynağı (ok, balta, patlama) ikisini de bedavaya alsın. ⚠️ İkisi **bağımsızdır**: çarpma efekti dekorda da çıkar (hasar yok), gösterge ise yalnız raporlanan vuruşta — kan/kıvılcım seçimini artık çağıran yapmaz, yüzey çözer. Reçeteler: `Gelistirici/Yemek-Kitabi.md` |
| `Combat/WeaponFrame` | Sahnedeki silahın **çerçevesi** — `VA_WeaponFrame` prefabı olarak her `WPN_*` kökünün çocuğudur (`WeaponKitBuilder` bağlar). ⚠️ **Yalnız `GrabPath == DistanceGrab` eşyada vardır:** başka bir alma yolu yazılmışsa çerçeve `Awake`'te kendini kapatır, prefabdaki ISDK **mesafeli kavrama bileşenlerini temizler** ve hata basar — filtreyle "aday değil" demek yetmez, boş aday listesiyle bile interactor hover'a girer ve basışı yer (Tuzaklar). Kaynak silahı **dondurur** (Rigidbody kinematik + yerçekimsiz, `Grabbable`/`GrabInteractable`/`HandGrabInteractable`/`DistanceHandGrabInteractable` kapatılır) → yakın kavrama tümden kapalıdır, silah **yalnız uzaktan** alınır ve çerçeveden hiç ayrılmaz (ön kabza göstergesi için ek iş yok: `Weapon` onu yalnız TUTULAN silahta çizer). ⚠️ **İki mesafe-kavrama bileşeni birden taşır ve ikisini de dinler** (`DistanceGrabInteractable` = kumanda hattı, `DistanceHandGrabInteractable` = el hattı): hangisinin koşacağını ISDK rig'i "el izleniyor mu" sorusuna göre seçiyor (§7). ⚠️ El hattının **`Hand Alignment`'ı `None`'dır**: `AlignOnGrab` ISDK'ya kavrama boyunca sentetik elin bileğini kavranan nesneye kilitletir ve çerçeve yerinde dondurulduğu için oyuncunun eli sahnedeki silaha yapışır (§7). Pointer olaylarında `Select` gelince `WeaponGranter.SelectWeapon(...)` çağrılır — yani oyuncunun eline giden şey kaynağın bir **KLONU**dur. Nişan geri bildirimi **ISDK'nın kendi mesafe-kavrama göstergesinden** gelir (tüp + reticle); çerçevenin kendi `LineRenderer` ışını `isRayVisible` ile **kapalıdır** — ikisi birden açıkken elde iki ışın görünür ve kapatmak menzil bilgisini kaybettirmez, çünkü ISDK'nın aday listesi her adayı `CanBeSelectedBy` ile (yani `WeaponFrame.Filter` ile) süzer: menzil dışındaki çerçeve aday bile olmaz. Aynı zamanda bir `IGameObjectFilter`'dır (mesafe kapısı `_interactorFilters`'a bağlanır). ⚠️ **`Filter` üç şey sorar: kalibreli · hayatta · menzilde** — ölü oyuncu çerçeveden silah SEÇEMEZ; vermeyi zaten `WeaponGranter.CanHoldWeapon` kapatıyor, buradaki kapı onun yerine geçmez, aday listesinden düşürerek doğru geri bildirimi verir (ışın/reticle hiç çizilmez). Yalnız basış boyunca **değişmeyen** koşullar buraya konur ("elde silah var mı" konmaz, §7). ⚠️ Prefabda çerçeve **MODELİ yoktur** — çerçeve görünmeyen bir seçim hacmidir (`frameVisual` boş, `isFrameVisible` işaretlense de çizilecek görsel bulunmaz); görsel isteniyorsa prefaba model kökü eklenir — reçete `Gelistirici/Yemek-Kitabi.md`. ⚠️ **Çerçeve yalnız silah SABİT dururken vardır:** `Weapon.HeldChanged` dinlenir ve silah hangi yoldan tutulursa tutulsun (ele verildi ya da ISDK ile kavrandı) çerçevenin GameObject'i kapanır, bırakılınca geri gelir. Kural olayda durur, çağrı noktalarında değil — "silahı ele alan" birden çok yol var ve her birine ayrı ayrı "çerçeveyi de kapat" eklemek yeni bir yol açıldığında unutulacak bir adım olurdu. Abonelik bu yüzden `Awake`/`OnDestroy`'dadır: `OnDisable`'da olsaydı çerçeve kendini kapattığı anda "bırakıldı" sinyalini duyamaz ve bir daha geri gelmezdi |
| `Combat/WeaponGranter` | Silahın ele geldiği **tek nokta** (§3.9). **Kendini önyükleyen kalıcı tekil** — sahneye konmaz, bu yüzden yeni arenaya ek kurulum adımı doğurmaz. İki kaynağı vardır: (a) **`RandomGrant`** kuralında grip basılıyken o elde rastgele bir silah durur, bırakılınca **yok olur**, tekrar basınca **yenisi** gelir (`Disposable`); (b) **`WeaponCanvas`** kuralında `WeaponFrame`'in çağırdığı `SelectWeapon` ile seçilen silahın **kalıcı klonu** tutulur — grip bırakılınca yalnız gizlenir, tekrar basınca arenanın neresinde olursa olsun aynı silah aynı mermiyle geri gelir (`Persistent`). Sahnedeki silah **tükenmez**, sınırsız kez seçilir. Seçim ancak başka bir çerçeveden alınarak değişir ve **harita başına** sıfırlanır. **Kaç klon:** seçilen tanım `TwoHand` ise oyuncu başına **tek** (ikinci el ön kabzaya adaydır), `OneHand` ise **el başına bir tane** (çift tabanca; her klon kendi şarjörünü taşır). ⚠️ **"Aynı anda ikinci silah tutulamaz" kuralının yeri `RandomGrant` yoludur** (`TickHand`): bir eldeki silah çift kavramalıysa öteki ele ikinci bir silah verilmez, o el ön kabzaya adaydır — yoksa oyuncu FFA'da iki tüfek tutar ve tüfeği iki elle sabitlemenin hiçbir yolu kalmazdı. ⚠️ **Çerçeve yolunda böyle bir kapı YOKTUR ve eklenmez:** orada kural yapısaldır (çift elli seçimde oyuncu başına zaten TEK klon var), seçim değiştirmek ikinci silah üretmez — yalnız rafta silah değiştirir. Seçime kapı koymak tam da bunu kırar ve belirtisi teşhisi zordur (§7). ⚠️ **Verilen silahta kavrama algısının TEK kaynağı bu sınıftır:** `Disposable` de `Persistent` klon da el anchor'ının değil bu tekilin (DDOL kökü) altında durur ve pozunu `Weapon.ApplyCanonicalGrip` her karede sürer — anchor'ın çocuğu olsaydı çözücünün yazdığı dünya pozu parent dönüşümüyle çakışırdı. Klonun ISDK kavrama bileşenleri bu yüzden **açılmaz** (`PrepareSummonedClone`); ikinci el grip'e basıp kumandasını ön kabza soketinin (kabul küresinin) içine getirince (`Weapon.IsHandOnSecondaryGrip` — kural silahta) `Weapon.SetSecondaryHand` ile yazılır. İki algı yolu birden açık olsaydı aynı el iki ayrı kaynaktan "tutuyor" görünürdü. ⚠️ **Ön kabza bağının KURULMASI ile SÜRDÜRÜLMESİ ayrı kurallardır** (`ResolveSecondaryHand`, silah örneğine bağlı bir kilit): kurulma mesafeye bakar (kumanda görülen soket küresinin içindeyse — küre neyi vaat ediyorsa o), **sürdürme yalnız grip tuşuna** — ne mesafe ne açı yoklanır, oyuncu tuşu bırakana kadar bağ durur. Sürdürmeye mesafe kapısı **geri konmaz**: silah ikinci ele doğru **KAYMAZ** (yalnız nişanlanır — ana kavrama noktası ana avuçta kalır, `ItemGripSolver`), yani soketin ikinci elin kumandasından uzaklığı her zaman *|ellerin arası − silahın kavrama arası|* kadardır ve oyuncu kolunu uzatıp topladıkça bu fark kabul yarıçapını (~0.10 m) kendiliğinden aşar — mesafeye bakan bir sürdürme kuralı, nişan alma/eğilme/dönme gibi normal hareketlerde bağı oyuncu tuşu bırakmadan koparır. Kilit silah **örneğine** bağlı olduğu için grip bırakılıp yeni silah gelince kendiliğinden düşer, yani bir silahtan ötekine sızmaz. **Sahne süpürmesi** (arenadaki tezgâhların gizlenmesi — ⚠️ **taban bölgeleri BURAYA AİT DEĞİL**, onlar `BaseZoneVisibility`'dedir) tek bir kapıdan geçer: `ModeDistributesWeapons` = kaynak `random` **ve** ortada **kurulmuş bir maç var**. ⚠️ "Kaynak random mı" sorusu tek başına bu sorunun cevabı DEĞİLDİR (§7): sahnelenen arena lobi profiliyle koşar ve orada kaynak da `random`'dır — kapı yalnız kaynağa baksaydı maç kurulmadan tezgâhlar gizlenir, oyuncu bekleme boyunca ne silah alabilir ne serbest atış yapabilirdi. Ayrım `modeId`'den DEĞİL kuraldan okunur (§10.5): serbest alanı benzersiz kılan bileşim `random` + `fireWhilePaused`'dur, koşan FFA de `random`'dır ama serbest atışı yoktur. **Serbest alanda iki yol birden açıktır:** oyuncu tezgâhtan seçerse o silah gelir (seçildiği anda rastgele verilenler geri alınır), seçmezse loadout'tan rastgele biri. ⚠️ Sahnelemeden FFA'ya geçişte kaynak `random` kaldığı için "kaynak değişti mi" yetmez, "mod dağıtıyor mu" da izlenir — izlenmeseydi sahnelemede seçilen silah maça taşınır ve modun rastgele dağıtımı sessizce delinirdi. ⚠️ **(b) yolunda silahı sahneye koyan bileşen YOKTUR ve yazılmaz** — yerleşim arena kararıdır, harita tasarlanırken elle konur (`BaseZone` gibi prefab örneği olarak); silah konmamış bir arenada bu yol sessizce boş döner. ⚠️ **Silahı elde tutmanın kapısı kalibrasyon, hayatta olmak VE çocuk oturumu olmamaktır** (`CanHoldWeapon`): **ölü oyuncuya silah verilmez** — grip bırakılıp yeniden basılması da geri getirmez, çerçeveden de alınamaz (aynı kapı seçimde: `WeaponFrame.Filter`). Ölüm ayrıca eldekini **bir kez** süpürür (`HandleAliveChanged`) — VR'da yere düşecek bir üçüncü şahıs bedeni olmadığı için ölümü okunur kılan şey boşalan eldir. Kapının **kabul edilmiş bedeli**: canlanması olmayan tur tabanlı modda hayalet oyuncu tur kapanana kadar eli boş bekler ve canlandıktan sonra tezgâha yeniden yürür. El başına **mandal** (`IsGripHeldSinceDeath`, bırakışta açılır) kapıyla birlikte KALIR: ölüm boyunca basılı tutulan grip, kapı açılır açılmaz kendi basışı olmadan silah almasın. Süpürme iki kaynağı da bulur ama **farklı işler**: tek kullanımlık silah (ve park edilmiş ikizi) yok edilir, çerçeve klonu yalnız gizlenir ve **seçim korunur** — canlanıp tekrar basınca aynı silah aynı mermisiyle döner. **Hasar kapısı bundan etkilenmez, yeri değişmedi:** tetiği `PlayerCombatState.CanFire` kapatır (hayatta + faz/`fireWhilePaused`) ve ateş edilse bile sunucu `playing` dışında `hit_report` işlemez (§10.3) — *tutmak sunumdur, hasar kapısı başka yerdedir*. Kalibresizlik kapı olarak KALIR (§10.6): o oyuncu hiçbir fazda ateş edemez ve canlanamaz da, silah elinde hiç canlanmazdı. Canlanmada seçim korunup silah `RefillFull` ile tam şarjör + rezervle döner — dolum yeri burasıdır. **İkinci dolum kapısı `countdown` mesajıdır:** her geri sayımın başında eldeki silah dolar. Tek başına canlanma yetmiyor — tur tabanlı modda turu **sağ bitiren** oyuncu canlanmaz ve yarım şarjörle yeni tura girerdi (§3.8.2). Kapı geri sayım olduğu için mod-agnostiktir. Admin'de rig kapalı olduğu için silah verme yolu kendiliğinden kapalı, süpürme ise çalışır. Dağıtım normalde rastgeledir; **yalnız editörde** `SequentialGrant` bayrağı (dev sandbox yazar, `#if UNITY_EDITOR`) onu loadout sırasına çevirir — bütün silahları tek tek gözden geçirmek için. Üretim davranışı değişmez. **Askıya alma/geri verme kapısı da burasıdır** (`StowHeld`/`RestoreStowed`): bileklikten bomba alınırken o eldeki silah yere DÜŞMEZ ve yeniden seçilmez — aynı örnek park edilir, atış bitince aynı ele döner. Atılabilir tutan **elin kendisi** raftan silah alamaz ve nişan görseli (tüp + retikül, çerçeve ışını) o elde gizlidir; öbür el — iki elde de **SİLAH** yoksa — rafı görür ve seçebilir (`CanSelectWith` kilidi atılabiliri saymaz, görsel cevap el başına `IsHandFreeForRack`). Seçim oyuncu başınadır: yeni seçim park edilen örneği de yok eder, atıştan sonra bomba eli yeni seçimi alır. İki elli silah bomba elinde park edilmişken boştaki elin **TAZE** basışı aynı örneği (aynı şarjörle) o ele alır; bomba alınırken zaten basılı olan kavrama (ön kabza) almaz. ⚠️ **Çocuk oturumu ayrı bir kapıdır** (`IsKidsPlayground` = lobi profili işlerken `ModeSelection.IsKidsGame`): seçili mod çocuk ailesindense silah verilmez ve tezgâhlar süpürülür — lobi sahnesi askeri bir harita olabilir ve sunucunun sahneye bakan çocuk lobi profili (§10.7) o durumu göremez. Kapı **yalnız lobi profilinde** okunur: maç kurulduğunda cevabı koşan kuralın silah kaynağı verir, yoksa operatörün sonraki tur seçimi koşan maçın ellerini boşaltırdı |
| `Combat/ThrowableDefinition` (+ `ThrowableTrigger`) | Atılabilir eşyanın DAVRANIŞ tanımı (`ItemDefinition`'dan türer; kimlik/prefab/kavrama tabanda kalır — `WeaponDefinition` ile aynı ayrım): tetik kipi (`Fuse` = fitil, `Impact` = ilk temas), fitil süresi, **kılıf dolum süresi**, atış hızı ölçeği + tavanı, yönden türetilen dönüş hızı, çarpışmadan muaf layer'lar, patlama yarıçapı/hasarı/kenar çarpanı, görüş hattı anahtarı, patlama prefabı/sesi. ⚠️ **Yeni tür telde bedavadır:** atma olayı `itemId` taşıdığı için bomba, molotof, flashbang ve sis aynı 9 bayttan geçer — yeni tür **protokol sürümü artırmaz** ve sunucuya iş çıkarmaz (§6.4) |
| `Combat/Throwable` | Atılan eşyanın **uçuşu**: nicelikleme → poz/dönüş → rigidbody → avatar muafiyeti → yetişme → fitil. ⚠️ **Determinizm burada yaşar** (§7): yön ve hız telin kendi oktahedral/cm-sn yuvarlamasından geçirilir (**atan da** kendi niceliklenmiş değerlerinden simüle eder, yoksa tek ayrıksı kopya onunkidir), dönüş ve açısal hız yönden türetilir (rastgele fırıl kopyaları ayrıştırır), uçuş boyunca **her avatar collider'ı muaf tutulur** (0.5 sn'de bir yeniden taranır — uçuş sırasında avatar doğabilir). Yetişme analitiktir + tek küre taraması; `Physics.Simulate` KULLANILMAZ (global, sahnenin tamamını adımlar). **Geometriye ilk temasta sönüm yükselir** (`LandedLinearDamping`/`LandedAngularDamping`): PhysX'te yuvarlanma direnci yoktur, küre collider'lı bomba yoksa fitile kadar yuvarlanır; uçuşta prefabın değerleri kalır (türetilmiş fırıl uçuşu tamamlasın). Kural her kopyada aynı → ek ayrışma yok. ⚠️ **Yerel oyuncunun ölmesi fitili durdurmaz** — `CanFire` atmayı kapatır, havadaki eşyayı değil (sunucudaki ölüm sonrası penceresi, §10.3) |
| `Combat/ThrowableEffect` + `BlastEffect` | Tetiklendiğinde koşan etki ailesi. **Hasarı yalnız atanın kopyası raporlar** (`LocalOwner`); uzak kopyalar yalnız FX oynatır. Hasarsız türler (flashbang, sis) hiç `hit_report` üretmez — her istemci **yalnız kendi oyuncusunu** değerlendirir, sunucu onları hiç görmez. `BlastEffect` bugünkü tek üyedir: FX + ses, sonra uzak hedeflere `ReportAreaHit` ve kendine `ReportAreaSelfHit`. Oyuncuya hasarın mesafesi **gövde kapsülüne** ölçülür (arena zemini→kafa; atan ve diğerleri için aynı kapsül, isabet kutuları değil); çömelmek hasarı değiştirmez. Sunum `BlastFxPool` üzerinden gider — patlama başına `Instantiate` yoktur. **`ThrowableEffect.Prewarm` ailenin ikinci kancasıdır:** `Throwable.Arm` çağırır, varsayılanı no-op'tur, `BlastEffect` onunla FX havuzunu fitil boyunca kurar. ⚠️ `ReportAreaHit`'e `ArenaLayers.AreaTargetMask` geçer — maskesiz sorgu her kavrama hacmini ve tetikleyiciyi tamponun içine çeker |
| `Combat/WristHolster` | **Sol bilek kılıfı** — rig'in sol el anchor'ına oturur (rig keşfi yine `WeaponGranter` kapısından; ikinci yol açılmaz). Alan el **sağ eldir** (sol el kendi bileğine ulaşamaz, yani kural kendiliğinden tek anlamlı). ⚠️ **Yakınlığı kendisi ölçmez, `GripSocket`'e devreder** (`Configure` ile kendi yarıçapını/göstergesini iterek) — ikinci bir yakınlık uygulaması, aynı jestin iki yerde farklı yarıçapla çalışması demekti. Taşıdığı eşyanın alma yolu `WristHolster` olmak zorundadır; kural eşyanın tanımında durur, bu bileşende değil. Alırken sağ eldeki silahı askıya aldırır, bırakınca **her zaman atar** (kılıfa geri koyma yoktur; sıfır hızda ayağın dibine düşer). ⚠️ **Fitil bırakma anından, dolum PATLAMADAN sonra sayılır** — yani kılıf uçuş + fitil boyunca da boştur ve iki atış arası en az fitil + dolum kadardır; mermi sınırı yoktur. ⚠️ **Ölü oyuncunun bileği BOŞTUR:** ölümde elde ve bilekteki örnek yok edilir (eldeki **olay yollamadan** — patlamayan bomba: az önce düelloyu kazananı öldüren bir el olmaz), süren dolum ölü beklerken **tamamlanmaz**. Kılıfın dolu sayılması (`_ready`) ile bilekteki örneğin var olması ayrı şeylerdir: canlanınca örnek kendiliğinden geri gelir — dolu ise hemen, değilse dolum tamamlandığında. **Silahsız modda (`ModeRuntime.IsWeaponless`) bilek BOŞTUR:** iki örnek de yok edilir, el geri verilir, `_ready` ve süren dolum korunur — tetik kapısı yalnız almayı reddeder, bileği boşaltmaz. Taşınan örnek (bilekte ve elde) **kinematik + interpolasyonsuz + çarpışmasızdır** (`Deactivate` → `RigidbodyDrive`); bunları geri açan tek yer `Throwable.Arm`'dır, kılıfın `Reactivate`'i yalnız collider'ları açar |
| `RigidbodyDrive` (`Core/`) | Rigidbody'nin **"kim sürüyor" çiftini** tek yerden yazar: `SetKinematic(body, kinematic)` kinematik bayrağıyla birlikte interpolasyonu da çevirir (kinematik → `None`, simüle → `Interpolate`). ⚠️ Transform'la sürülen (avuç, kılıf, akan poz) kinematik gövdede interpolasyon AÇIK kalırsa fizik her karede transform'u son iki fizik pozundan yeniden yazar: collider zamanında, görsel elin çevresinde sürüklenir ve titrer (§7 "Transform'la sürülen Rigidbody"). Çağıranlar: `WristHolster.Deactivate`, `Throwable.Arm`, `NetObjectBody`, `NetObjectGrabBridge` — `isKinematic` doğrudan yazılmaz |
| `Combat/BreakableObject` | Kırılabilir ağ nesnesinin **hazır sunumu** (`NetObject`'in yanına eklenir, `StateChanged`'i dinler): hasar oranını (`1 - hp/maxHp`) `MaterialPropertyBlock` ile `_DamageAmount`'a yazar, `Broken` olunca collider'ları kapatıp sağlam kökten enkaz köküne geçer, kırılma efekti/sesini **yalnız canlı değişimde** (`NetStateOrigin.Live`) oynatır. Alanlar: `damageRenderers`, `hitColliders` (boş bırakılırsa alt ağaçtan kendisi toplar), `intactRoot`, `brokenRoot`, `breakFxPrefab`, `breakFxLifetime`, `breakClip`, `breakVolume`. ⚠️ Can tutmaz, karar vermez — durumu yazan tek taraf sunucudur. Hasar oranını **görünür** kılan hazır yüzey shader'ı `VortexArena/BreakableSurface`'tir (`_Shared/Shaders/`): albedoyu `_DamageColor`'a karartır ve nesne uzayında prosedürel çatlak açar; `_DamageAmount` sıfırken yüzey pürüzsüzdür. Aydınlatma URP'nin kendi PBR'ıdır (kendi ışık matematiği YOK), yani hasarsız prop kaynak `Lit` materyaliyle birebir aynı görünür — kırılabilir yapılan bir dekorun rengi değişmez. Kurulu örnekler `_Shared/World/Prefabs/` (`NO_BreakableCover` · `NO_TargetBoard`), ortak kırılma efekti `_Shared/FX/FX_BreakDebris`. ⚠️ **Bu özelliği taşımayan bir shader'da yazım sessizce yok sayılır** — obje kırılır ama "hasarlı ama ayakta" görünümü hiç oluşmaz. ⚠️ `MaterialPropertyBlock` materyali **SRP Batcher dışına** düşürür: bu shader dekorasyona değil, sayısı bilinçli olarak az tutulan kırılabilir objelere konur |
| `Combat/GripSocket` | **Yakınlık kavrama soketi**: eşyanın NEREDEN alındığı (prefabdaki yer + kabul yarıçapı + yaklaşınca beliren gösterge küresi). Projedeki **tek** yakınlık uygulamasıdır — `WristHolster` kendi kopyasını taşımaz, `Configure` ile bunu sürer. ⚠️ **Soket bir kavrama KAYDI DEĞİLDİR:** soket "nereden alınır"ı, kavrama kaydı (`ItemDefinition`) "alınınca el nasıl durur"u söyler; ikisini karıştırmak elin sığmadığı bir yerden alınabilen eşya üretir. ⚠️ **Oyuncunun gördüğü küre kabul hacminin KENDİSİDİR** (gösterge prefabı 1 m çapında yazılır, ölçeği yarıçaptan verilir): ayrı sayılar "içindeyim ama almıyor" üretir ve bu sahada bozuk kavrama olarak okunur. Ölçülen nokta kumanda **anchor'ıdır**, bilek değil — bilek santimlerce ötede olduğu için ölçüm oradan yapılsaydı oyuncuya "girdin" denip kavrama reddedilirdi. Yarıçapın 1 cm tabanı korunur: sıfır yarıçap soketi matematiksel olarak ulaşılmaz yapar ve hata vermez. **Yakalama kipi** (`CatchMode`, köprü uçuşta açar): kabul yarıçapı koddaki `CatchRadius`'a büyür (küre de büyür — gördüğün yine kabul hacmidir) ve ölçüm tek karelik nokta mesafesi değil, **karenin süpürmesidir** — soketin ele göre bir kare önceki konumundan bu kareki konumuna çizilen doğru parçasının en yakın noktası; 12 m/sn'de obje karede ~17 cm gider, 12 cm'lik küreyi iki örnek arasında atlardı. Göreli ölçüldüğü için objeye savrulan el de sayılır. Bir karede 1 m'den uzun süpürme (dinlenme pozuna sıçrama, `Return`) ışınlanmadır, nokta olarak ölçülür. Yarıçap prefab başına DEĞİL koddadır: prefabdaki yarıçap duran objenin hacmidir, yakalama tek bir his olarak ayarlanır. **Gösterge susturulabilir** (`ItemDefinition.ShowGrabIndicator` — dağıtıcıda kendi alanı): ⚠️ susturma **yalnız görseli** kapatır, kabul yarıçapına ve alma kapısına DOKUNMAZ — küre ile kabul hacminin aynı sayı olduğu değişmezi korunur, yoksa "içindeyim ama almıyor" üretilirdi. Küreleri kapatmak yoğun soketli bir düzende (üst üste duran eşyalar, kendi dağıttığı malzemenin arasındaki dağıtıcı) sahneyi okunur tutar |
| `Combat/NetObjectGrabBridge` | `WorldSingle` bir **ağ nesnesini** ele koyan ve orada tutan bileşen (`WristHolster`'ın ağ nesnesi karşılığı). Kavrama **iyimserdir**: basış objeyi hemen yerelde alır + `object_grab` gider, cevap yayınlanan `object_state.owner`'dır ve sahip başkası çıkarsa kavrama `NetObject.OwnerChanged`'den **geri alınır** (olayın `StateChanged`'den önce tetiklenmesinin sebebi tam olarak budur). ⚠️ **Sunucu da kendiliğinden ele verebilir** — dağıtıcının `take` olayı objeyi `owner` + `Held` ile doğurur (§10.10); o yolda kimse bir tuşa basmadığı için el, durumun bit3'ünden **üstlenilir** ve `object_grab` gönderilmez (o mesaj istemcinin *istemesidir*, sunucu çoktan karar vermiştir). Üstlenilmezse obje sahibi görünen ama hiçbir elin sürmediği hâlde havada kalırdı. Üstlenme de basışla alınan kavrama gibi kısa bir **titreşimle** onaylanır: o yolda oyuncu bir şeye basmıştır ama dağıtıcının göstergesi gizli olabilir ve ele düşen malzemeden başka karşılık yoktur. `IsHeld` boyunca objeyi sahibin eline — yerel ya da uzak — **kanonik kavrama poziyle** bağlar (`ItemGripSolver`; duruş telde gitmez, iki uç aynı çözücüyü koşar). El bırakınca `object_release` yollar ve **dikiş orada biter**: uçuş penceresini `NetObjectPoseSender` (poz akışı + `object_rest`) ve `NetObjectBody` (serbest yerleşim + fizik otoritesi) devralır. Bırakma ekseni `Return` ise obje kendi başlangıç pozuna oturur (poz **arena uzayında** saklanır — kalibrasyon arena kökünü kaydırır, dünya kopyası artık var olmayan bir yere geri gönderirdi), `Physics` ise rigidbody serbest kalır. Eşya tanımını **kendi prefabındaki** serialize alandan okur; tanım yoksa ya da `WorldSingle` değilse hata basar. `LateUpdate` **varsayılan sırada** koşar (`Weapon`/`WristHolster` gibi): `HandGripPoser` order 100'de eli buraya yazılana kilitler, daha geç yazan bir kaynakta el objenin bir kare gerisinde kalırdı. ⚠️ **`CarryAnchor` yazılıyken transform'a HİÇ dokunmaz** — objeyi avuca değil taşıyıcı yerleştirir, ve sunucunun elden verdiği kavramayı da (`AdoptServerGrab`) devralmaz: tek transform'a iki yazar derleme hatası değil, görünür titremedir. **Havada yakalama:** uçuştaki objede (`Awake`, `Held` değil) soketi yakalama kipine alır; **boş** ve kavraması **sıkılı** bir el objeye değince kenar tetik beklemeden alır (gerçek yakalama hareketi "önce kapan, sonra gelsin"dir; 30 ms'lik geçişe basış zamanlanamaz), basış kenarı her zaman çalışır. Duran objede kenar kuralı korunur — yoksa sıkılı elle yanından geçilen her eşya ele yapışırdı; dolu el (silah/eşya) uçan objeyi de yalnız basışla alır, yanından uçan spatula tüfeği askıya almasın. **Onay süresi:** `OBJECT_GRAB_CONFIRM_SECONDS` içinde sahip + `Held` gelmezse iyimser kavrama geri alınır (onay "sahip" değil "sahip VE elde"dir — kendi attığını geri yakalayan zaten sahiptir); sunucunun elden verdiği kavrama süre tutmaz. ⚠️ **Basış doğrudan kavramaz:** `IGrabClaimant` uygulanır ve girdi yolu yalnız `GrabArbiter.Submit` ile **adaylık bildirir**; kavramayı hakemin geri çağırdığı `CommitGrab` yapar — kendi soketini tek başına okuyan bir bileşen, soketi çakışan komşusunu göremez. ⚠️ **`HeldItems.Report` reddi TAM GERİ ALMADIR** (`ReleaseLocal`), uyarı logu değil: slot parmak pozunu besliyor, zorla geçirilen eşya elin hiç kapanmadığı bir avuçta durur, üstelik obje çoktan tele girmiş olur. `Report` **stow'dan SONRA** çağrılır (`WristHolster` ile aynı sıra) — park edilen silah eli kendi olayıyla boşaltır, öncesinde yapılan talep her zaman reddedilirdi. Soketin göstergesi `TickSocket`'te `ItemDefinition.ShowGrabIndicator` ile AND'lenir (yarıçapa dokunulmaz). ⚠️ **Odak kaybı ayrı bir kod istemez:** Meta menüsü (güç tuşu da uykudan önce odağı düşürür) OpenXR eylemlerini pasifleştirir, kavrama 0 okunur ve tutulan obje normal el açılışı gibi bırakılır — kural budur, odak dinleyen ikinci bir yol yazılmaz. Uyku/duraklatmada ise istemci hiçbir şey gönderemez: objeyi sunucunun poz sessizliği kapısı indirir, uyanan istemcinin yerel tutuşu gelen `object_state` ile sessizce düşer |
| `Combat/GrabArbiter` (+ `GrabArbiterPump` · `IGrabClaimant`) | **Yakınlık grip basışının tek hakemi:** talep sahipleri `Submit(claimant, rightHand, distance)` ile aday olur, `Resolve()` **el başına TEK kazanan** seçip `CommitGrab`'ini çağırır. Ölçüt **EN YAKIN** — mesafe `GripSocket.TryMeasure`'dan gelir, yani alma kapısının ve oyuncunun gördüğü kürenin okuduğu **aynı sayı**. ⚠️ **Var olma sebebi:** her talep sahibi ham grip eksenini bağımsız okuyup yalnız kendi soketine sorarsa, soketlerin çakıştığı her yerde (üst üste duran eşyalar, dağıttığı malzemenin arasındaki dağıtıcı) tek basışa birden çok taraf cevap verir: tek avuca birkaç obje, telde birkaç mesaj, tek ele birkaç yazar — ve hata vermez. Tek soket gören bir bileşen bunu çözemez, karar **soketlerin üstünde** verilmek zorundadır. Beraberlikte önceki talep kalır (kazanan kareler arası salınmasın); önceki kareden kalan talep **bayattır ve karşılaştırılmadan ezilir** — koşmayan bir `Resolve` eli maç boyunca kapalı bırakmasın. Canlılık `is UnityEngine.Object` ile ayrıca sınanır: arayüz referansı Unity'nin yok-edilmiş-obje null'undan geçmez. **`GrabArbiterPump`** kendini önyükleyen DDOL tekildir (`ModeRuntimePump` kalıbı — sahneye konmaz, yeni arenaya kurulum adımı doğurmaz) ve `LateUpdate`'te `Resolve()` çağırır. ⚠️ **`[DefaultExecutionOrder(40)]` kuralın kendisidir:** talep sahipleri varsayılan sırada (0) yazar, `HandGripPoser` 100'de eli kilitler; 40 ikisinin arasıdır, yani kazanan **aynı karede** kavranır. ⚠️ 50 DEĞİL 40: `BurgerCarrier` 50'dedir ve aynı sıra değerini paylaşan iki bileşenin koşma düzeni tanımsızdır — kargosunu kavramadan önce yerleştiren taşıyıcı bir önceki karenin cevabına göre dizer. Bu aralığın dışına taşınırsa kavrama bir kare geç düşer ve obje ele gözle görülür biçimde sıçrar. ⚠️ **Hakem YALNIZ basış yolunu kapsar:** `BurgerCarrier.Claim` (kargo hacmindeki her malzemeyi birden talep etmesi) **kasıtlı çoklu kavramadır** ve hakemden geçmez — geçseydi tepsi tek malzemeyle kalkardı |
...
| `Combat/WeaponMuzzleVfx` | **VFX Graph namlu alevini atış başına bir kez tetikler:** `Weapon.Fired`'a abone olur ve grafiğe adlandırılmış olayı (`fireEventName`, varsayılan `OnFire`) yollar. ⚠️ **Bu köprü zorunludur, süs değil:** `Weapon`'ın kendi `muzzleFlash` alanı `ParticleSystem` tipindedir — bir `VisualEffect` oraya BAĞLANAMAZ, dolayısıyla grafiği ateşleyecek başka hiçbir yol yoktur. ⚠️ **Grafik tarafının sözleşmesi:** spawner örtük `OnPlay` akışında BIRAKILMAZ, adlandırılmış olay bağlamına bağlanır ve **sonlu tek döngüye** alınır (`Loop Duration` × `Loop Count` = 1); örtük `OnPlay`'de kalan bir spawner nesne etkinleşir etkinleşmez çalışmaya başlar ve **hiç durmaz** — "efekt kendi kendine dönüyor" belirtisinin tek sebebi budur. Döngü süresi silahın atış aralığından belirgin biçimde KISA olmalıdır (aksi hâlde spawner hâlâ koşarken gelen olay yutulur ve o atış alevsiz kalır). Bileşen silahın kökünde değil, **`VisualEffect`'in kendi nesnesinde** durur: kök `WeaponKitBuilder`'ın yönettiği alandır, bu bileşen ise silaha özeldir ve kit koşusuyla dağıtılmaz | 
...
| `Player/ArenaNetCharacterBehaviour` | Movement SDK'nın ağ katmanı ile ArenaNet arasındaki **tek köprü** (§6.9/6.10). SDK'nın `INetworkCharacterBehaviour`'ını uygular. **Gönderen:** SDK'nın gönderim kancası (`ReceiveStreamData`) çağrılınca SDK baytlarını atar, canlı kemiklerden (`JointPairs[idx].Joint.localRotation` + kalça `localPosition` + en alçak bacak ekleminin kök yerel yüksekliği) `SkeletonWire.Write` ile kendi blob'unu yazar ve `0x07` olarak yollar — ölçek ve kemik uzunluğu gitmez. ⚠️ **Kalça ORAN UZAYINDA gider:** ham `localPosition` gerçek metredir (SDK oyuncunun kendi oranlarını yazar) ve alıcı aynı `bodyScale`'i köke uyguladığı için gönderen onu kendi `bodyScale`'ine (`BodyScaleState.ServerScale`) **böler**; bölünmezse boy iki kez ölçeklenir. `footY` bölünmez, gerçek metredir. **Alıcı:** `LateUpdate`'te önce karakterin kökünü arena uzayına oturtur (`ApplyArenaRoot`), sonra `ApplyInterpolatedBones` ile kemikleri yazar (`RemoteSkeletonRegistry.TryGetInterpolatedBones`: kök ofsetiyle aynı gecikmeli saat ve aynı iki örnek; indeks 0'ın rotasyonu yazılmaz, iskelet ölüyken kemiklere dokunulmaz); rotasyon ve kalçadan **sonra** en alçak bacak ekleminin çizilen yüksekliğini ölçüp farkı (kök ölçeğine bölünmüş, ±0,5 m kırpılmış) kalçanın y'sine ekleyerek ayağı `footY`'ye oturtur — sıra bağlayıcıdır, ayağın yerini rotasyonlar belirler. ⚠️ **Hedef zemine kırpılır, yalnız yukarı:** `footY` bir eklemin yüksekliğidir, tabanın değil — derin çömelmede gönderenin parmak eklemi kendi zeminine iner ve olduğu gibi kopyalanırsa mesh tabanı yere gömülür; hedef oyuncunun kat zemini + `SoleClearanceMeters` × ölçekten aşağı inmez (pay modele özgüdür, model değişirse yeniden ölçülür). ⚠️ **Sağlıklı çözümde bu fark ~0,1 m'yi geçmez (derin çömelme ve çocuk oranları bu kadarını meşru olarak ister); büyüğü ve kırpmaya dayananı sunucu günlüğüne uyarı olarak düşer** (gövde başına kısıtlı): gömülü ayak, kırpılmış düzeltme ve yanlış gövde ölçeği başlıkta birbirinin aynı görünür, ayıran tek şey o satırdır — ⚠️ **satır YALNIZ yeni ya da DEĞİŞEN arıza için basılır:** eşik üstünde kalan aynı düzeltme (fark birkaç santimden az) 5 sn'lik alt sınır dolsa bile tekrar etmez, düzeltme eşiğin altına inince kural yeniden kurulur ve sonraki arıza yine bildirilir; süren tek arızanın her 5 sn'de kendini tekrarlaması sunucu günlüğündeki diğer satırları gömerdi — kırpma doyuma ulaştığında düzeltme sabitlenir ve kalça çömelmeye de cevap vermez. SDK'nın alıcı yolu (`NetworkCharacterHandler.ReceiveData`, SDK interpolasyonu) çağrılmaz; handler ve retargeter uzak prefabda yalnız `JointPairs` ve bind pozu kaynağıdır. **Rol ayrımının uygulandığı TEK yer**: `HasInputAuthority` yerelde `true` (sensör kaynağı `MetaSourceDataProvider` açık, gövde body tracking'den çözülür ve akar), uzakta `false` (kaynak KAPATILIR — açık bırakılsaydı her uzak avatar aynı yerel sensörü okurdu). ⚠️ Kaynak bileşen prefabdan **silinmez, yalnız kapatılır**: `CharacterRetargeter.Awake` onu kendi GameObject'inden `GetComponent` ile arıyor ve yoksa assert atıyor — tek prefabın hem yerel hem uzak çalışabilmesi bileşenin orada durmasına bağlı. ⚠️ **Kökü SDK değil bu sınıf yazar**: blob'un 0. eklemi gönderenin dünya uzayındadır ve telde yalnız yerel rotasyonu durur, dünya yerleşimini taşımaz; o yüzden kök ayrıca taşınır — yaw + poz kanalı kafasına göre ofset olarak (§6.9), alıcı onu kendi interpolasyonlu kafasının üstüne kurar. **T-poz yedeğini de bu sınıf üretir** (`RequestTPoseFallback`, istek `LocalBodyAvatar`'dan): hedef iskeletin **referans T-pozunu** (`SkeletonRetargeter.TargetReferencePoseLocal`'dan bind rotasyonları + bind kalça konumu + `footY = 0`) aynı kadansta `SkeletonWire.Write` ile yazıp yollar (`SerializeSkeletonAndFace` kullanılmaz), kök HMD'nin zemine izdüşümünden türetilir (§6.9). ⚠️ **Üç tetikleyici de AYNI referans T-pozunu gönderir**, karakterin o anki kemiklerini DEĞİL: bir arızadan sonra o kemikler SDK'nın son uyguladığı donuk pozdur ve kıpırdamayan gövde sahada "ağ bozuk" diye okunur. ⚠️ **Kapı "hiç poz üretmedi" ile SINIRLI DEĞİLDİR:** oturum boyunca hiç poz uygulanmamış olması (`AppliedPose` kapısı — `RetargeterValid` DEĞİL) yalnız birinci tetikleyicidir; gövde çözümü oyun ortasında bozulduğunda (çöp kök, bayat kare) yedek yine devreye girer, çünkü **bozuk bir gövde ile hiç gövde arasındaki fark uzak ekranda yoktur** — ikisi de "oyuncu kayboldu" gibi görünür ve arıza teşhis edilemez. Üçüncü tetikleyici **tel eklemlerinin çözülememesidir** (`JointPairs` `SkeletonWire` indekslerini karşılamıyor — prefab hatası): SDK poz uygulamaya devam ettiği için diğer ikisi silahlanmaz; ⚠️ çözülemeyen kare **kare denetimine hiç sokulmaz**, sokulsa akış damgasını tazeler ve yedek hiç açılmaz — oyuncu tek bir hata loguyla görünmez kalır. Giriş/çıkış **histerezislidir**: eşiği anlık geçen tek bir kare yedeği açıp kapatsaydı avatar gerçek poz ile T-poz arasında çırpınır, sinyalin kendisi okunamazdı — aynı sebeple çıkış girişten daha isteklidir (ilk sağlam kare dizisiyle yedek susar ve avatar canlı poza döner). ⚠️ **Yedek bayrağı (`IsTPoseFallbackStreaming`) yalnız SDK yolu gerçekten döndüğünde silinir** — histerezis dolana kadar sağlam kareler gönderilmiyor ve T-poz akmaya devam ediyor, bayrağı her sağlam karede silmek admin ekranındaki gövde durumunu saniyede onlarca kez `G:+` ile `G:T` arasında çırpıtırdı. ⚠️ **Kare denetimi üç bacaklıdır ve üçü de gereklidir** (§6.9): **kök makullüğü** (kafaya yatay uzaklık + kök yüksekliği), **eklem makullüğü** (bind pozundan ÖLÇÜLEN kemik oranları, ayağın kök zeminine göre yeri, iki kare arası eklem sıçraması; ⚠️ **eklem İÇEREN segment yalnız YUKARIDAN sınırlanır** — kalça→bilek dizin üstünden geçer ve çömelmede meşru olarak kısalır, alt sınır her derin çömelmeyi bozuk sayardı; gövde gibi tek kemikli segmentte iki sınır da durur. Ayağın "kalçanın üstünde" sayılması için de pay bırakılır: derin çömelmede ve diz üstünde kalça bilek hizasına iner. ⚠️ **Sıçrama testi rig'in MEŞRU taşınmasını yok sayar** — `ArenaCalibrator.CalibrationGeneration` değişince saklanan referans düşer (kök emniyetiyle aynı kapı) ve **o kare de referans olmaz** — rig, retargeter matrisi okuduktan SONRA taşınmış olabilir (hizalama `await`/coroutine sonrasında koşar), yani o karenin eklemleri hâlâ eski rig konumundadır ve referans alınsaydı bir sonraki gönderim metrelerce "sıçrama" sayılıp gövdeyi T-poz yedeğine atardı; kök emniyeti de aynı kareyi geçirir ama saklamaz. **Reddedilen karenin eklemleri referans olarak SAKLANMAZ**, yoksa bir sonraki sağlam kare geri sıçrama gibi okunurdu) ve **donmuş akış** (eklem-yerel pozun bit-birebir tekrarı). Yalnız köke bakmak yetmez — merdivende ya da örtülmede kök kafanın altında makul kalırken gövde katlanır, yani en görünür bozulma tek bacaklı bir kapıdan sessizce geçer. Donma **kök üzerinden ölçülemez**: kök HMD'yi izlediği için donmuş izlemede bile oynar. Üçüncü bacağın var olma sebebi `OVRBody`'nin sözleşmesidir — `GetBodyState4` başarılı olduğu sürece geçerlilik bayrağı **koşulsuz** yazılır, güven değeri hesaba girmez ve geçersiz eklem **önceki karenin değerini korur**; yani *"geçerli"* ile *"taze"* aynı şey değildir ve bayrağa güvenen bir kapı, gövde takibini sistem ayarlarından kapatan oyuncuda hiç açılmaz. Kemik uzunlukları asset'ten okunmaz, **bind pozundan ölçülür**: sabit metre gömmek boyu farklı oyuncuda sağlam kareleri elerdi. ⚠️ **İzin REDDEDİLMİŞSE sabır penceresi hiç işlemez** — yedek anında kurulur, çünkü T-poz operatöre "bu başlıkta izleme yok" diyen tek sinyaldir ve **lobide** görülmelidir; beklemek izinsiz oyuncuyu lobide sağlam gösterip arızayı maçın ortasına ertelerdi. T-poz bir görsel değil bir **teşhistir**: gören operatör o başlıkta gövde takibinin arızalı olduğunu bilir (`Docs/Kullanim-Kilavuzu.md` bakım prosedürü) ve oyuncunun kendi ekranında hiçbir belirti olmaz. ⚠️ **Kemikler kökle TEK saatte interpole edilir** (`Environment.TickCount − INTERP_DELAY_MS`, kök halkasının aynı iki örneği): SDK'nın `NetworkTime`/`RenderTime` ekseni çizime girmez — ayrı bir saat, gövdeyi kökünden farklı bir ana ait duruşla çizerdi. ⚠️ `ReceiveStreamAck` **bilerek boştur** — ack yalnız delta sıkıştırma içindir ve delta kapalıdır (§6.9). ⚠️ **Kök emniyeti (`GuardRootJump`)**: iskelet kökü bir gönderimde `RootJumpLimitMeters` (1,5 m) üstünde sıçrarsa son kök gönderilmeye devam eder ve yeni kök ancak `RootHoldMaxSends` (24) gönderim ısrar edince kabul edilir — emniyet **yalnız bir el tutuluyorken silahlıdır** (kumanda kaybı gövde çözümünü çökertip kökü fırlatıyor, §7 "`OVRCameraRig` el anchor'larını KOŞULSUZ yazar"), `ArenaCalibrator.CalibrationGeneration` değişince saklanan kök düşer: rig'in gerçekten taşındığı an meşru bir sıçramadır. ⚠️ **Kökün KONUMU her zaman poz kanalının kafasına bağlıdır (§6.9); iskeletten yalnız yaw + ofset gelir ve hangi kipin çizdiği `IsPoseDriven` ile dışarı açılır** (§6.11): iskelet akışı **canlıyken** (`RemoteSkeletonRegistry.GetRootAgeMs` ile yaşı sorulur) telden gelen ofset ile gövde yaw'ı kafanın zemin izdüşümüne eklenir; ölü akışta ofset düşer, yaw kafadan türetilir — gönderenin T-poz yolunun kullandığı formülün aynısı (ikisi ayrışırsa akış dönünce gövde sıçrar). Tazelik testi zorunludur çünkü kayıt defterinde örnekler eskimez: sorulmazsa susmuş bir akış gövdeyi son kökünde dondurur, hiç örnek gelmemişse gövde sıfır ölçekle kaybolur ve **kemiklere asılı çarpma kutuları bir noktaya çöktüğü için oyuncu vurulamaz olur**. ⚠️ **`RepairBodyTracking()` gövde izlemesini yeniden başlatır** (§6.11): kaynak bileşen kapatılıp açılır — `MetaSourceDataProvider : OVRBody` olduğu için bu gerçek bir `StopBodyTracking`/`StartBodyTracking2` çiftidir, bileşen sıfırlaması değil. Sıra bilerek şudur: **önce yeniden başlat, sonra bayat kalibrasyonu sil, en son boy ipucunu ver** — silme başlatmadan önce yapılsa ölmekte olan oturum onu geri benimser, ipucu silmeden önce verilse silme onu süpürür. ⚠️ Boy ipucu `BodyScale` DEĞİLDİR (o bir oran, §10.8) ve **arena uzayında** ölçülür: gözlüğün kendi zemini bu arızada tam da güvenilmez olandır. **Kat ofseti üç yerde geçer** (§3.13): alıcı kök zeminini `ArenaFloors.HeightOf(FloorState.PlayerFloor(id))`'den alır (kafa onun altındaysa `FloorAt`'a düşer), gönderici T-poz kökünü `FloorState.LiftMeters`'a yazar, SDK kare kabul kapısı da `arenaRoot.y − LiftMeters` karşılaştırır — ⚠️ kapıdan ofset düşülmezse üst kata çıkan oyuncunun kareleri reddedilir ve gövdesi T-poza düşer |
...
| `Player/LocalBodyAvatar` | Oyuncunun gövdesi — **yalnız ağ kaynağı**, hiç çizilmez. ⚠️ Oyuncu kendi gövdesinden HİÇBİR ŞEY görmez (gövde de kol da el de); gözlükte gördüğü eller **rig'in sentetik elleridir** ve bu sınıfın onlarla ilgisi yoktur. **Görünmezlik yalnız çizimdedir; telde tam gövde gider.** Uzak avatarla **aynı FBX ve aynı retarget config'ini** `Owner = Host` olarak kurar (`ArenaNetCharacterBehaviour.Initialize(playerId, hasInputAuthority: true)`), yani "başkalarının gördüğü gövde" tek doğruluk kaynağıdır. **Kendini önyükleyen kalıcı tekil** (`WeaponGranter` kalıbı): prefabı `Resources.Load("LocalBodyAvatar")` ile yükleyip sahne köküne kurar, sahneye elle KONMAZ → yeni arena bir kurulum adımı doğurmaz. Gövde ancak **iki koşul** birden sağlanınca kurulur: etkin bir `OVRCameraRig` (yani rol gerçekten oyuncu) ve sunucudan alınmış bir `playerId` (blob onunla etiketleniyor, §6.9). Kurulumda `HideAllRenderers()` alt ağaçtaki **tüm** Renderer'ları istisnasız kapatır ve bir daha çalışmaz — gövdeye sonradan renderer eklenmiyor; prefabda da hepsi kapalı gelir, çalışma anındaki geçiş yalnız garantidir. ⚠️ **Obje YAŞAMAYA devam eder ve yıkılmaz** — görünmez olması "gereksiz" demek değildir: bu obje giderse oyuncu ağa hiç iskelet göndermez ve **diğer oyuncular onu göremez**. Artık yerelde tek bir pikseli bile çizilmediği için bu refleks daha da tehlikelidir: sileninin ekranında hiçbir şey değişmez. ⚠️ Görünmezlik **renderer düzeyinde** yapılır, obje kapatılarak DEĞİL (§7, `OVRBody` kendini kalıcı kapatabilir) ve **kemik transformuyla da DEĞİL** (§7, SDK'nın iskelet serileştirmesi `localScale` dahil canlı transformu okur; tele giden rotasyon ve kalça konumu da canlı transformdan okunur; kalça tele girerken gönderenin `bodyScale`'ine bölünür, ayak yüksekliği ayrı gerçek-metre alanıyla gider). **Gövde izlemesi bekçisi buradadır** (§6.11): `RetargeterValid` **kesintisiz** tanıma süresi boyunca düşük kalırsa bir kez `LogError` basılır, T-poz yedeği devreye sokulur (`ArenaNetCharacterBehaviour.RequestTPoseFallback`) ve ardından izleme **artan aralıklarla yeniden başlatılır** (`RepairBodyTracking`). ⚠️ **Tek atışlık DEĞİLDİR ve olamaz:** izleme oyun ortasında da ölüyor, sahanın gerçekten çarptığı durum o — ilk raporda kilitlenen bir kontrol oturumun geri kalanını korumasız bırakır. Gövde dönünce bütün mandallar **temizlenir**, yoksa ikinci arıza hızlı ilk denemesini kaybeder. ⚠️ **Kesintisiz süre şartı gevşetilemez:** `RetargeterValid` sahne yüklemesinde ve kaynak anlık göz kaybettiğinde bir kare düşer; ona bakıp yeniden başlatmak onarmaya çalıştığı arızayı ÜRETİR. ⚠️ **Tek istisnası izin reddidir:** `BODY_TRACKING` izni işletim sisteminden **doğrudan** sorulur ve verilmemişse sabır penceresi sıfırlanır, yedek anında kurulur — bekleyecek bir şey yoktur ve T-poz **lobide** görünmelidir (Android dışında izin modeli olmadığı için soru "evet" döner, sabır penceresi olduğu gibi işler). ⚠️ **İzin `RetargeterValid`'DEN ÖNCE karara girer, sonra değil:** izinsiz başlıkta SDK elindeki iskeleti tutmaya devam ettiği için bayrak `true` kalabilir, yani "sağlıklı" sayılan başlık hiç yedeğe düşmez ve arıza yalnız başkalarının ekranında görünür (Tuzaklar: geçerli ≠ izinli). ⚠️ **İzinsizken onarım DENENMEZ:** yeniden başlatmak izin veremez, bedeli ise gerçek bir izleme kesintisidir — yedek silahlı bırakılır ve hata satırı sebebi izin olarak yazar, yoksa operatör bozuk bir gözlük arar. ⚠️ **Odak kaybı da bildirilir** (`OnApplicationFocus`): odak giderken sensör ölür ama uygulama bir süre daha koşar, bildirim yedeği o pencerede tele koyar; uygulama gerçekten duraklayınca zaten hiçbir şey gönderilmez — kanca yalnız geçişi kısaltır, ikinci bir tetik bayrağı değildir (bayrak olsaydı aynı karede iki yoldan gönderim yapılabilir, "yedek ile SDK yolu aynı anda asla göndermez" kuralı kırılırdı). Operatör aynı onarımı `restart_body_tracking` ile de tetikler (`RepairBodyTrackingNow`, aralığı sıfırlar). Bekçinin kararı `BodyTrackingState` ile dışarı verilir ve `status.body` olarak sunucuya gider (§5.1). ⚠️ **Hata satırı yereldeki TEK sinyaldir**: arıza yerelde hiçbir iz bırakmaz (eller rig'den geliyor), görünür hâli (T-poz) yalnız başkalarının ekranındadır (§7). **Gövde kalibrasyonunun tetikleyicisi de buradadır** ve ⚠️ **varsayılan KAPALIDIR** (`calibrateBodyProportions = false`): kapalıyken `CharacterRetargeter.Calibrate()` hiç çağrılmaz ve herkes prefabın oranlarını kullanır — açılırsa **uzak gövde bozulur**, çünkü gönderenin rotasyonları ve kalça yüksekliği değişen oranlara göre çözülür ama alıcı prefabın bind uzunluklarıyla çizer (§7). ⚠️ Alanın **yerel karşılığı YOKTUR**: gövde çizilmiyor, eller rig'den geliyor — yani açmanın oyuncunun kendi ekranında hiçbir kazancı yok, geriye yalnız bedeli kalıyor. Alan açıldığında yol şudur: `ArenaCalibrator.CalibrationGeneration` değişince 3 sn sonra `Calibrate()` çağrılır — gecikme zorunludur, oyuncu arena kalibrasyonunu zemine EĞİLEREK yapıyor ve o andaki poza sabitlenen gövde oranı maçın kalanı boyunca yanlış boy demektir. ⚠️ Sürenin dolması tek başına yetmez: `Calibrate()` geçerli poz yokken **sessizce hiçbir şey yapmaz** (§7), bu yüzden çağrı koşul sağlanana dek her karede yeniden denenir. Kalibrasyondan sonra uygulanan gövde ölçeği bir kez raporlanır; `ScaleRange` sınırına dayanmışsa uyarıya döner — sınıra dayanmış ölçekte **diğer oyuncular** bu oyuncuyu yanlış boyda görür; sonucu yalnız onlar görür, oyuncunun kendi ekranında iz bırakmaz. ⚠️ Sınıf `[DefaultExecutionOrder(30000)]`'dir: `Calibrate()` o karenin **uygulanmış** pozunu ölçmeli, yani SDK'nın retarget döngüsünden ve iskelet gönderim kancasını çağıran `NetworkCharacterHandler`'dan (order 100) sonra koşmalıdır. ⚠️ Avatar **sahne kökünde** durur, rig'in altına konmaz (§7, "retarget avatarı hareket eden kökün altına konmaz"). ⚠️ Gövdede **collider yoktur** — `Weapon`'ın atış raycast'i maskesiz, kendi gövden kendi atışını yerdi. Admin'de kurulmaz ve bu rol kontrolüyle DEĞİL, etkin rig yoksa hiç çalışmayarak sağlanır (`AppSession` App asmdef'indedir, Core onu göremez) |
| `World/NetObjectSpawner` | `object_spawn`/`object_despawn`'in **tek dinleyicisi**: `kind`'ı `NetSpawnCatalog`'dan çözer, arena pozunu dünyaya çevirir, örneği yaratır ve sunucunun tahsis ettiği kimliği ona bağlar. **Sahneye KONMAZ** (`NetObjectSync` deseni; dinamik obje geç katılanın `world_state`'iyle de gelebilir → kendini önyükler, `DontDestroyOnLoad`). Kataloğu `Resources`'tan yükler. ⚠️ **Yalnız kendi yarattığını yok eder** — sahne objesinin kimliği bake'lidir, onu silmek hiçbir sıfırlamanın geri getiremeyeceği bir boşluk bırakırdı. Katalogda karşılığı olmayan `kind` → obje doğmaz + **tür başına tek** uyarı (saniyede bir doğuran bir dağıtıcı aksi hâlde konsolu boğar). Bağlantı kopunca ve sahne değişince defter temizlenir. **İstemci dünya sıfırlaması da burasıdır:** `load_match` / lobiye dönüş mesajında tüm dinamik örnekleri yok eder (önce `SetActive(false)` — `Destroy` kare sonunda işlediği için aksi hâlde aynı karedeki `world_state` ölü objeyi defterde bulurdu), sahne objelerini `ResetToAuthoredState()` + bake'li poz ile geri alır ve `RemoteObjectRegistry`'nin poz halkalarını boşaltır (bayatlamamış bir halka objeyi hemen ardından geri çekerdi). Kanca **sahne yüklemesi değil mesajdır**: aynı harita ikinci kez sahnelenirken sahne yeniden yüklenmez, ve dinamik kimlik havuzu her sahnelemede baştan başladığı için temizlik yeni `world_state`'ten önce bitmek zorundadır (§10.10) |
| `World/NetObjectBody` | **Serbest** (tutulmayan) ağ nesnesini yerine koyar: akış varsa `RemoteObjectRegistry`'nin interpole pozu, akış yoksa sunucunun dinlenme pozu, o da yoksa **sahnedeki yeri** (hiç kıpırdamamış bake'li objenin telde pozu yoktur; yazmak onu orijine ışınlardı). ⚠️ **Sahipsiz objede (`Owner == 0`) akış hiç okunmaz:** akış yalnız BAŞKA bir istemcinin sahiplenip uçurduğu obje için vardır, sahiplik `object_rest`'e kadar sürer. Kapı olmasa önceki maçtan kalan bir halka objeyi sıfırlamanın oturttuğu pozdan çekerdi. Fizik otoritesi burada durur: `isKinematic = !IsMine` — iki başlıkta birden simüle edilen obje aynı bıçağın iki ayrı yerde olması demektir. ⚠️ **`IsHeld` iken hiçbir şey yazmaz:** o an transform'un sahibi kavrama köprüsüdür ve tek transform'a iki yazar derleme hatası değil, **görünür bir titremedir**. Yerel elde olan obje (`HeldItems.Holds`) de tutulan sayılır: iyimser kavrama penceresinde `Held` biti henüz gelmemişken akan poz elle yarışmasın, yakalanan bıçak yakalayan elin yanından uçmaya devam etmesin |
| `World/NetObjectPoseSender` | Sahip olduğumuz objenin pozunu `OBJECT_POSE_RATE_HZ` ile akıtır (`0x09`) ve durduğunda `object_rest` yollar. **Kapı üçü birden:** `IsMine && IsAwake && !IsHeld` — yani `object_release` ile `object_rest` arasındaki **uçuş penceresi**. Yerel elde olan obje (`HeldItems.Holds`) kapıyı ayrıca kapatır: kendi attığını havada geri yakalayan oyuncu `Held` onayı gelene kadar zaten sahiptir, el pozu akmasın ve avuçta duran obje "durdu" diye `object_rest` üretip kendini serbest bırakmasın. ⚠️ **Durmayı istemci ölçer** (`OBJECT_REST_SPEED` altında kesintisiz `OBJECT_REST_SECONDS`): sunucunun ne fiziği ne metresi var. Tek karelik "durdum" yetmez — hız sekmenin tepe noktasında da sıfırlanır, obje havada donardı. ⚠️ **Tutulan bir ağ nesnesinin üstünde durmak da "durdu" sayılmaz:** temas eden collider'ın `NetObject`'i tutuluyorsa sayaç sıfırlanır (temas hafızası 0,25 sn — fizik her karede adım atmaz). Yoksa elde tutulan tabağa bırakılan bıçak dinlenmeye geçer, sunucu onu o pozda dondurur ve tabak altından çekilince bıçak havada asılı kalır. ⚠️ **Zemin altı kapısı sahiptedir:** objeyi `OBJECT_KILL_Y`'nin (arena y) altında gören sahip onu ev pozuna (sunucunun bildiği dinlenme pozu, yoksa objenin doğduğu poz) oturtur, hızını sıfırlar ve o pozla `object_rest` yollar. Düşen obje **kendiliğinden hiç durmaz** — hızı eşiğin altına inmez, yani `object_rest` hiç gitmez ve zeminden kaçan malzeme sahipli kalıp maç boyunca sahada eksilir. Sunucuda yedek kapı yoktur (metre bilmez). `object_rest` uçuş başına **bir kez** gider (kapıyı sunucunun `Awake`'i düşürmesi kapatır; her kare yeniden yollamak güvenilir kuyruğu doldururdu). `RestSent` olayı yalnız **gönderen** istemcide tetiklenir ve bu yüzden bir sunum bileşeninin "bu objeyi ben durdurdum" sorusuna verilebilecek tek dürüst cevaptır: `policy:"anyone"` bir olayı herkes gönderebildiği için (bir köftenin ızgaraya konması gibi) seçici gerekir, yoksa sunucu aynı sayacı oyuncu sayısınca başlatır |
...
| `GameType` | `QuickBattle` / `Kids` — haritanın ve modun ait olduğu **oyun ailesi** (§3.10); operatörün ilk seçimidir, tur tipi (`modeId`) onun altında durur. `MapDefinition`/`ModeDefinition` serialize eder → ⚠️ **yeni değer SONA eklenir**. Telde karşılığı `GameTypeIds.ToWire` ile üretilen `"quickbattle"` / `"kids"` string'idir; enum'un kendisi tele girmez |
...
Çok katlı arenada üst kat plakası da bu kökün altına alınabilir — kuş bakışında üst kat gizlenmezse
admin alt katta hiçbir şey görmez.

**`ArenaFloors`** (statik): arenanın kat seviyelerinin **tek kaynağı** (§3.13). Kat 0 = dünya y 0;
diğer seviyeler sahnedeki `FloorPortal` örneklerinden türer — kökü kat `k` zeminine
`LevelToleranceMeters` (0,25 m) içinde oturan, `upperHeight` = h olan portal kat `k+1`'i
`kat k + h` diye tanımlar. ⚠️ **Elle girilen bir seviye listesi YOKTUR ve eklenmez:** sayı listesi
mesh'ten sessizce sapar ve oyuncu altında zemin olmayan bir yüksekliğe kaldırılır — `BaseZone`'un
ölçüsünü şeritten almasıyla aynı kural. Aynı kat çifti için 0,05 m'den fazla sapan ikinci bir ölçü
uyarı basar ve **ilk değer geçerli** kalır (iki ölçümü karşılaştırdığı için tolerans yerleştirme
toleransından dardır); hiçbir kat zeminine oturmayan portal listeye girmez ve kendini kapatır.
⚠️ **Diskleri aynı X/Z'ye düşen portal çiftini de uyarır:** bir portaldan gelen oyuncu doğrudan
diğerinin dolumunun içine iner (Tuzaklar: "Portal disklerini aynı X/Z'ye koyma").
`MarkDirty` sonrası (portal açılıp kapandıkça, sahne yüklenip boşaldıkça) ilk sorguda **tembel**
yeniden kurulur — tüketici başına değil değişiklik başına bir kez yürünür; `Version` tüketicilerin
türettiklerini önbelleğe almasını sağlar. `Count`/`HeightOf(floor)`/`FloorAt(worldY)`. Kat sayısı
birden büyükse konsola `[ArenaFloors] n kat: 0.00 m / 3.00 m` düşer — kurulumun tek denetim yolu bu
satırdır.

**`FloorState`** (kendini önyükleyen kalıcı tekil; sahneye KONMAZ — `CalibrationState` kalıbı):
yerel oyuncunun katı + roster aynası. `Request(floor, reason)` aralığı denetler, ofseti **hemen**
uygular (`ArenaCalibrator.SetFloorLift`) ve `set_floor` gönderir; `FLOOR_CONFIRM_SECONDS` içinde
kendi roster satırında yankı gelmezse **yerel katı geri alır** — reddedilen ya da düşen bir bildirim
oyuncuyu kendini yanlış katta sanırken bırakmaz. `MoveWithFade` aynı işi `ScreenFade`'in "floor"
kaynağıyla karartmanın arkasında yapar ve **geçiş olacak mı sorusunu `bool` olarak döndürür**
(`false` = geçersiz kat ya da zaten o kat; çağıran sesini/durumunu buna göre harcar). Süren bir geçiş
varken gelen ikinci istek: karartmanın **tepesine ulaşılmadıysa hedef güncellenir**, ulaşıldıysa
**kuyruğa alınır** ve geçiş bitince uygulanır — istek yok sayılırsa portalın üstüne düşen ölüm
inişi kaybolur ve oyuncu yanlış katta canlanır. Sınırlarda uzlaşma: yeniden bağlanınca
yerel kat 0 değilse yeniden bildirir (sunucu `hello`'da sıfırlıyor), kat 0'a dönülen sahne yüklemesi
süren karartmayı durdurup roster aynasını temizler ve **göndermez** (sunucu `load_match`'te
sıfırlıyor), bekleyen istek yokken sunucu farklı bir kat yazarsa yerel kat **karartmayla** ona uyar
(ve ağa geri gönderilmez). `ApplyLocal` sunucudan geleni arenanın kat sayısına karşı denetler:
aralık dışı kat **0'a düşürülür ve loglanır** — sunucu defter tutar, geometriyi bilmez. Ölüm dalı
buradadır (§3.7/§3.13): `PlayerCombatState`'in açık taban katına ~1 sn'lik karartmayla iner, hedef
bulunduğu kattan yukarı çıkmaz. `ViewerHasFloor` "izleyenin bir katı var mı" sorusudur ve admin
gözlemcide `false`'tur — kat silüeti ve " (ÜST KAT)" soneki oradan geçmez. Logları `[Kat] …`
önekiyle akar.

**`FloorPortal`** (prefabı `_Shared/App/Prefabs/VA_FloorPortal.prefab`, kiti editör aracı üretir):
iki kat arasındaki geçiş noktası. Kök = **alt kat** çemberi, `Üst` çemberi `OnValidate` ile
`(0, upperHeight, 0)`'a oturur; Inspector'da yalnız `upperHeight` (kat yüksekliğini TANIMLAYAN ölçü)
ve üç ses kancası vardır (dolum döngüsü · geçiş anı · meşgul uyarısı — boş bırakılanı sessizdir).
Kural: yerel oyuncunun **kafası** kendi katındaki çemberin merkezine XZ'de 0,25 m içinde kesintisiz
2 sn kalırsa kısa bir karartmayla diğer kata geçilir; dolum çemberi büyür, HUD ortasında kalan
saniye yazar. **Aynı anda tek oyuncu geçer:** iki kattaki çemberlerin içindeki kafalar arasında en
düşük `playerId` sahiptir, diğerinin çemberi kırmızıya döner ve "Portal meşgul" yazar. Geçişten
sonra çemberden çıkılmadan yeniden tetiklenmez; ölü oyuncu kullanabilir, **kalibre olmayan
kullanamaz** (ofset hizalanmamış bir rig'i anlamsız bir yüksekliğe taşır).
⚠️ **Kat DEĞİŞTİĞİ her durumda portal kendini kurulmamış yapar ve dolumu sıfırlar** — kendi geçişi,
başka bir portal, ölüm inişi, sunucunun sıfırlaması: yeni katta zaten diskin içinde duran oyuncu aksi
hâlde anında geri ışınlanır; kurulma ancak kafa diskten çıkınca geri gelir. Geçiş gerçekleşmediyse
(`FloorState.MoveWithFade` `false`) ışınlanma sesi çalmaz ve portal harcanmaz.
⚠️ **İkinci portalın diski aynı X/Z'ye konmaz** (§7) — gelen oyuncu doğrudan diğerinin dolumuna
girer. HUD araması (`ModeHudBase`) 0,5 sn aralıkla **tekrarlanır**: sahne değişmeden mod değişince HUD
yeniden doğar ve tek seferlik arama portalı sessizce sayaçsız bırakırdı. Çemberler Scene view'de
Gizmo olarak çizilir — yerleşimin denetim yolu gözle bakmaktır. ⚠️ **Çemberlere collider konmaz**
(§7): kafa XZ testi kapıyı kendi çözüyor, collider ise atış izini ve engel ölçümünü yalanlar.
...
eğriyle sönen radyal çerçeve. ⚠️ **Ölüyken çerçeve sönmez, GRİYE düşer** (`deathColor` · `deathAlpha`):
sabit bir gri zemin çizilir ve öldüren vuruş onun üstünde bir parlama olarak görünür, sönerken zemine
oturur. Gerekçe iki yönlüdür — kırmızı *"taze hasar"* demektir ve ölü bedende yanıltıcıdır; çerçeveyi
tümden söndürmek ise **öldüren vuruşu görünmez kılar**, çünkü o vuruşun `health_update`'i zarfı
silahlandırdığı karede atılırdı (yön göstergesi ve titreşim çalıştığı için arıza "tutarsız" diye
okunur). ⚠️ **Parlamanın KENDİ RENGİ vardır, yalnız opaklığı değil** (`deathFlareSeconds` ·
`deathFlareFadeSeconds`): öldüren vuruş kırmızı çakar, ardından griye yürür ve ölü kalındığı sürece
gride durur. Zarfı ölüm grisiyle çizmek onu **görünmez** kılar — zemin zaten `deathAlpha`'dadır,
üstüne binen gri bir parlama birkaç birimlik opaklık farkından ibarettir ve ölen oyuncu kendisini
öldüren vuruşu hiç görmez. ⚠️ **Ölü dalına düşük can nabzı ve engel erimesi KARIŞMAZ:** `LowHpAlpha` can sıfırdayken
sonsuza kadar nabız atar, o dala sokulsaydı gri çerçeve durmadan yanıp sönerdi.
Can eşiğin altına düşünce çerçeve **tamamen sönmez**,
düşük opaklıkta yavaşça nabız atar. Renk, opaklık, süreler, eşik ve **ana anahtar** Inspector'dadır;
sayılar burada tutulmaz. ⚠️ Vuruşun **yoğunluk tabanı** vardır (`minHitIntensity`): hafif bir mermi
de çerçeveyi görünür kıldırır — çerçeve yön göstergesi dışındaki tek "vuruldun" kanalıdır.
⚠️ **Engel erimesi ayrı bir kanaldır** (`drainAlpha` · `drainPulseHz`): ~4 Hz gelen 3-5 HP'lik
damlalar vuruş zarfından geçirilseydi küçücük alfada seğirir, siyahın üstünde görünmez ya da titreme
gibi okunurdu. Engel bayrağı açık ve son can düşüşü tazeyken (`DrainHoldSeconds`) çerçeve yüksek
opaklıkta düz bir nabızla çizilir; toleransta düşüş olmadığı için görünmez, erime durunca söner.
...
**`ScreenFade`** (statik hakem): HMD karartma quad'ını isteyen birden çok sistem var (muhafaza ·
engel ihlali · kat geçişi, §3.13) ama quad tektir. Kaynaklar alfalarını her karede bildirir, **en yüksek** olan çizilir;
...
⚠️ **Sahne geçişinin karartması da buraya kaynak DEĞİLDİR** (`SceneTransitionFade`, §4): hakemin
çizeni sahnedeki `ArenaBoundary`'dir — kabuk lobisinde yoktur ve geçiş tam da o çizenin öldüğü
andır, üstelik kapanan muhafaza quad'ı son değerinde donar; bu yüzden geçişin kendi quad'ı ve
...
`RemoteShotFx`, `WeaponAudio` ve `SurfaceImpactFx` (`Weapons` — çarpma sesi atışın sonucudur,
atışla aynı kanaldan kısılır; parçacık kanaldan bağımsız çizilir). ⚠️ **Kanal ile gözlemci ses odağı (`SpectatorAudioFocus`)
...
alanını boş bırakmaktır — **varsayılan kurulum tam olarak budur:** `adminViolation` alanı
bilinçli olarak boştur, admin PC'si çoğu işletmede seyirci hoparlörüne bağlıdır ve her ihlalde
bip gitmesi istenmez. Kanca ve tercih satırı duruyor; klip atanırsa çalışır. ⚠️ Oyuncuya özel sesler (öldürme/ölüm/canlanma) yerel oyuncu kimliği
çözülemiyorsa —
...
değişen** duyuru sesleri. Bir kural satırı beş şeyi bağlar: `modeId` (boş = her mod) · `sceneName`
(boş = her harita) · `gameType` (her tip / Hızlı Savaş / Çocuk Oyunları) · `ModeAudioEvent` ·
**klip listesi** (biri rastgele seçilir, tek klip de geçerlidir) + seviye ve uyarı eşiği. ⚠️ **Seçimde bir önceki klip elenir** (liste ikiden az dolu
...
eşleşmesi 4, harita 2, oyun tipi 1 puan — aynı arena birden çok modda oynandığı için mod ağır
basar; harita zaten tek bir aileye ait olduğu için tip en altta), eşitlikte listedeki ilki. Çalan
yer yine `GameAudio`'dur (`PlayModeEvent`); aynı an için hem kayıtta hem bankada klip varsa **kayıt
bankayı ezer**, yoksa iki duyuru üst üste binerdi.
...
klibi her maçın sonunda yalan söylerdi. Kural yoksa o mod **sessiz** biter; sesi olacaksa yeri
`MatchEnd` satırıdır.
...
| `DimensionMeshReader` | `DimensionMesh'i JSON'a Çevir`: maketi okuyup **kaynak dosyanın üstüne** yazar (hedef sorulmaz, maketin işaretçisinden gelir). Ayak izi çıkarımı: yatay yüzler (`\|normal.y\| > 0.9`) Y seviyesine göre gruplanır, **en alt** grup alınır (prizmada alt yüz kazanır), kenarlar XZ'ye izdüşürülüp kaynaştırılır, **yalnız bir kez geçen** kenar sınır sayılır, halka yürünür ve doğrusal ara köşeler ayıklanır. Noktalar dünya üstünden kök uzayına çevrilir — kolonu sürüklemek/döndürmek doğru yazılır. ⚠️ Kenarlar köşe **indeksiyle değil konumla** anahtarlanır: ProBuilder sert normaller için köşeleri yüz başına ayırıyor, indeksle bakan tespit tüm mesh'i sınır sanar. Kalibrasyon noktaları `DimensionAnchor` küplerinin transformundan okunur; ⚠️ küp yoksa dosyadaki `calibration` **KORUNUR** (sıfırlanmaz — eski bir maketi çevirmek mekanın zemin bandı ölçüsünü silerdi). Yazmadan önce çıktı geri ayrıştırılır; doğrulanamazsa dosyaya **dokunulmaz**. ⚠️ Gidiş-dönüşün dosyayı bozmaması dört korumaya dayanır: sayılar milimetreye yuvarlanır (`ArenaDimensions.ToJson`; `JsonUtility.ToJson` 17 basamak basar, diff okunmaz olurdu), ölçülen kolon yüksekliği `defaultColumnHeight` ile aynıysa alan **0** bırakılır ("varsayılanı kullan" satırı sabit kalır) ve okunan halkalar kaynak dosyadaki halkaya (yön + ilk köşe) hizalanır — `WalkRing` sözlük sırasının en küçük köşesinden rastgele yönde başladığı için dokunulmamış kolonlar aksi hâlde her çevirmede yeniden sıralanırdı. Dördüncüsü dosyanın **yazım biçimidir**: nokta bloklarının açık mı tek satır mı olduğu ve satır sonunun CRLF mi LF mi olduğu kaynak dosyadan aynalanır — ölçüm aracı noktayı üç satırda ve CRLF ile yazar, yazıcı tek satırda ve LF ile yazsaydı ilk üç koruma tutsa bile dokunulmamış bir maketi çevirmek dosyanın **her satırını** değişmiş gösterirdi. Kaynak yoksa ya da okunamıyorsa ölçüm aracının biçimi kanonik sayılır (açık noktalar + CRLF); tek satır biçim yalnız **korunur**, hiçbir zaman seçilmez. Kolon eşleşmesi önce ad, adsızsa en yakın merkezdir; dosyada boş bırakılmış kolon adı boş kalır |
...
| `TemplateBasicsLoader` | `Template Temellerini Yükle`: aktif sahneye altyapıyı **prefab örneği** olarak koyar (`VA_ArenaBoundary`, `VA_CameraRig`, `VA_PoseSync`, `VA_CalibrationManager`; seçime bağlı `VA_ModeHud` · taban bölgeleri), `ArenaCalibrator`'ın sahneye bakan alanlarını bağlar, `ArenaBoundary`'nin rig'e bakan alanlarını (`head`/`fadeRenderer`/`warningText`) `VA_CameraRig` içinden bağlar ve mekanın boyut dosyasını `ArenaBoundary.dimensionsJson`'a takar. ⚠️ **Kalibrasyon işaretçisi koymaz ve yerleştirmez** — onların tek kaynağı ölçü maketidir (`DimensionMeshBuilder`); ikinci bir üretici hangisinin geçerli olduğunu belirsizleştirirdi. Taban bölgelerini takım malzemesiyle boyar (tek `VA_BaseZone` prefabı iki takıma da hizmet ediyor; şerit rengini çalışma anında kimse yazmıyor). Penceredeki **Kat sayısı** (1–4) 1'den büyükken her ek kat için bir `VA_FloorPortal` örneği koyar (`Portal_Kat{k}_{k+1}`, kabaca o katın zemin yüksekliğinde ve **X'te 2 m aralıkla** dizili: `Portal_Kat0_1` (0,0,0), `Portal_Kat1_2` (2,3,0) …) — ⚠️ aralık zorunludur, disklerin X/Z'si çakışırsa bir portaldan gelen oyuncu doğrudan diğerinin dolumuna iner; yerini ve `upperHeight`'ını tasarımcı ayarlar, üst kat plakasını da o portalın tanımladığı yüksekliğe kendisi kurar (§3.13). ⚠️ Sahnede **zaten bir portal varsa hiç dokunmaz** — kat düzeni bir kez elle yerleştirilmiştir ve ikinci koşu onu ezerdi; prefab yoksa raporda kiti üretme adımına yönlendirir. **İdempotent** — var olan örneği asset yoluyla tanır ve atlar; dolu bir alanın üstüne YAZMAZ |
| `FloorPortalKitBuilder` | `Kat Portalı Kitini Üret` (`Tools > VortexArena > Arena`): kat geçişinin **malzemesini ve prefabını** üretir — `M_FloorPortal` + `M_FloorPortalFill` (URP Unlit, saydam) ve `VA_FloorPortal.prefab` (kök = alt kat çemberi `Alt` + `Dolum` çocuğu, `Üst` çemberi `upperHeight` yüksekliğinde; `FloorPortal` alanları bağlı). **Yeniden çalıştırılabilir:** var olan materyal/prefab güncellenir, sahnedeki örnekler bağını korur. ⚠️ **Çemberlerde collider yoktur ve eklenmez** (§7) — kapıyı kafanın XZ mesafesi çözüyor, collider atış izini ve engel ölçümünü yalanlar |
| `SceneBudgetWindow` | `Sahne Bütçesini Ölç` (`Tools > VortexArena > Arena`): bir tenant'ın (ya da hepsinin) arena sahnelerini **ek olarak açar, ölçer, kapatır** — sahne başına LOD0 üçgen/vertex, LOD0 renderer (aynı anda çizilebilen: LOD0 + LOD'suz; tüm LOD'lar dahil sayı detay satırında), static flag'li renderer, skinned, materyal, terrain, particle, ışık sayıları ve en ağır mesh'ler; eşiği aşan hücre kırmızı (1M+ üçgen · 1500+ renderer · birden çok terrain · 200+ renderer'lı sahnede static flag yok). Zaten açık sahneye dokunmaz, hiçbir şeyi kaydetmez, hiçbir şeyi düzeltmez; "Konsola yaz" tabloyu tek log'da markdown basar. ⚠️ **Kamera taşıyan kök objeleri saymaz:** oyuncu rig'i her sahnede aynıdır ve Meta el prefabı editörde yüzlerce debug küresi taşır (Tuzaklar: "Rig'in el küreleri editörde çizilir, Quest'te çizilmez") — sayılsa her sahne aynı sabitle kirlenir ve küçük arenada dekorun kendisi görünmez olur. Üçgen index buffer'dan sayılır (`GetIndexCount`); `mesh.triangles` KULLANILMAZ — Read/Write kapalı mesh'te patlar, açıkta tam kopya ayırır. ⚠️ Renderer eşiği LOD1+ renderer'ları **saymaz** (üçgen gibi): her an bir LOD seviyesi çizilir, hepsi sayılırsa eşik gerçek yükü değil LOD seviyesi sayısını ölçer ve yanlış alarm verir. Modal dialog yoktur (ana thread'i kilitler) |
| `SurfaceAssignWindow` | `Tools > VortexArena > Arena > Yüzey Atama` — seçili objenin hangi yüzeye çözüldüğünü (etiket / materyal / varsayılan) ve nedenini gösterir; `SurfaceTag` koyar-kaldırır, materyali bir `SurfaceDefinition`'a bağlar (bağlarken materyali diğer tanımlardan söker) ve materyali kullanan objeleri açık sahnelerde seçer. Collider'sız obje ve çok materyalli renderer uyarı olarak işaretlenir. ⚠️ Eşleme **elle** yapılır, tool hiçbir şeyi kendiliğinden atamaz: aynı atlas dokusunu paylaşan tahta ve metal objeler tek materyalde birleşebilir, göz kararı toplu eşleme ancak o yüzeye sıkılınca fark edilen yanlış bağlar üretir |
| `ObstacleLayerAuditor` | `Engel Hacimlerini Denetle`: açık sahnelerde `Obstacle` layer'ını tarar ve **konveks olmayan** collider'ları, trigger'ları, collider'sız damgalı objeleri ve **görünen yüzeyden şişkin** collider'ları raporlar (dialog + tıklanabilir konsol satırları). Collider'sız raporu alt ağacın **yalnız en üstteki damgalı objesini** sayar — damgalı bir grubun altındaki çocuklar ayrı satır üretmez, yoksa tek bir hacim onlarca satıra bölünür ve gerçek eksikler listede kaybolur; liste ayrıca konsola da yazılır (dialog uzun listede kırpar). Şişkinlik testi çalışma anındaki testin **aynısıdır** (`ClosestPoint` ile nokta-içeride; ikinci bir matematik ikinci bir doğruluk kaynağı olurdu) ve kalan geometrik tuzağı yakalar: konveks işaretlenmiş **içbükey** bir mesh'te hull çukuru doldurur, collider görünenden büyür ve oyuncu **boşlukta** ceza alır — çizilen mesh doğru olduğu için gözle görülmez. Üçgen ağırlık merkezleri yüzeyin **iki yanına** 2 cm taşınır ve ölçüt "ikisi de içeride"dir: tek yana bakmak üçgen sarım yönüne güvenmek olurdu ve yön ters okunsaydı araç HER objeyi şişkin raporlardı. ⚠️ Yalnız `MeshCollider` denetlenir (kaynak: collider'ın kendi mesh'i) — Box/Capsule zaten bilinçli bir kabalaştırmadır ve aracın **kendi önerdiği** çözümdür; onu hata saymak olurdu. Mesh okunamıyorsa (Read/Write kapalı) ya da collider/obje kapalıysa test yapılmaz, obje sorunlu diye raporlanmaz. **Var olma sebebi tek bir kuraldır ve gözle denetlenemez:** non-convex bir `MeshCollider` bu layer'a girerse `Collider.ClosestPoint` girdi noktasını aynen döndürür → nokta-içeride testi her zaman "içeride" der → **o sahnedeki herkes anında ölmeye başlar**. Çalışma anında `ObstacleVolumes` böyle bir collider'ı eliyor ama o satır ancak Play'e girilince görülür; bu araç aynı soruyu sahne kaydedilmeden önce sorar. ⚠️ Hiçbir şeyi DÜZELTMEZ — otomatik convex işaretlemek sanatçının bilerek yaptığı seçimi sessizce ezerdi |
| `HmdOverlayBuilder` | `HMD Katmanlarını Kur`: `VA_CameraRig.prefab` içindeki `CenterEyeAnchor`'a **iki uyarı yazısını** — engelin içi (`ObstacleWarningText` + `ObstacleWarningOverlay`) ve alanın dışı (`BoundaryWarningText`, sürücüsü sahnedeki `ArenaBoundary`) —, hasar vinyetini (`DamageVignette` + `M_DamageVignette`) ve **sahne geçişi karartmasının quad'ını** (`SceneTransitionFade` + `M_SceneTransitionFade`; bileşen taşımaz, sürücüsü kalıcı tekildir) kurar — dört katman. **İdempotent.** ⚠️ **Bileşeni ekler, alan değerlerini YAZMAZ:** `DamageVignette`'in ayarları (`maxAlpha`, süreler, eşikler) prefabın kendisindedir; koddaki `[SerializeField]` varsayılanı yalnız bileşenin ilk eklendiği anda geçer, sonrası prefabın sözüdür — kod varsayılanını değiştirmek çalışan ayarı değiştirmez, prefab ve kod birlikte güncellenir. İkisi de `CenterEyeAnchor`'ın çocuğu olduğu için **kafaya kilitlidir**: oyuncunun baktığı yerde, görüşün ortasında durur — takip/yumuşatma bileşeni yoktur ve eklenmez (`HudFollow`'un tembel takibi HUD panelleri içindir; ihlal uyarısı okunana kadar kaçmamalıdır). ⚠️ **Fontu araç bağlar**: `AddComponent<TextMesh>` font atamıyor ve fontsuz `TextMesh` hiç mesh üretmiyor — yazı hata vermeden, sessizce hiç çizilmez. Yerleşik fontun adı sürümle değişti (2022.2'den beri `LegacyRuntime.ttf`, öncesi `Arial.ttf`) ve bulunamayan ad null döner, o yüzden ikisi de denenir. ⚠️ **Yazının büyüklüğü `WarningCharacterSize`'dan ayarlanır, font boyundan değil**: satır yüksekliği ≈ `FontSize × CharacterSize / 10 × Scale` ve atlas bu mesafede zaten ~6× fazla örneklenmiş, yani font boyunu büyütmek yalnız atlas belleği harcar. ⚠️ **İki yazı aynı anda açık olabilir** (muhafaza sahnedeki `ArenaObstacle`'ları da alan dışı sayar), bu yüzden üst üste değil **dikey istiflenirler** — engel uyarısı merkezin üstünde, alan-dışı altında. ⚠️ **Vinyetin materyalini araç ÜRETİR** — shader import edilmeden GUID'i bilinemediği için elle yazılmış bir `.mat` sessizce boş shader referansıyla açılırdı; materyal bir asset olduğu ve prefabtan referanslandığı için de build'den strip edilmez (`Shader.Find` ile çalışma anında üretilseydi Quest'te pembe çizilirdi). ⚠️ **Geçiş karartmasının materyalini de araç üretir** — `M_ScreenFade`'in kopyasıdır (`AssetDatabase.CopyAsset`; URP/Unlit · Transparent · siyah) ve üstüne `_QueueOffset = -10` + kuyruk 2990 yazılır: kuyruk bilerek UI'nin (3000) ALTINDADIR, yükleme ve bağlantı kartları siyahın üstünde okunsun diye. **Denetim de dört katmanı kapsar** (`CheckOverlays`/build hazırlık paneli): geçiş quad'ı için obje var · materyali dolu · collider yok. ⚠️ **Çizim sırası prefabdaki MESAFEDEN gelir** (yazılar 0.42 · vinyet 0.44 · geçiş quad'ı 0.5 · alan-dışı karartma quad'ı 0.5) — sayılar değişirse sıra korunmalı. Araç çalıştırılmadıkça davranış eskisi gibi: karartma çalışır, yazı ve vinyet hiç çizilmez |
| `BuildElementsConfigurator` | `Configure All Build Elements`: kayıt listelerini **klasör ağacından eşitler** — `Venues/*/Scenes/*/` taranır ve klasör tek doğruluk kaynağı sayılır. Düğme **TEKTİR** (*Hepsini Çalıştır*) ve her zaman etkindir: aktif sahne bir arena kutusuysa önce onun `MapDefinition`'ını yazar/günceller, sonra HER durumda eşitler. Sahne açık olmadan da koşar — silinmiş bir arenanın kalıntısını temizlemenin yolu budur; o durumda yalnız `MapDefinition` adımı atlanır ve rapora tek satır düşer. İki ayrı düğme, kullanıcıya "hangisi yeterli" diye sormaktı; build öncesi tek adım bırakmak o soruyu ortadan kaldırır. Eşitleme: eksik olan **uyarı** üretir (kutuda sahne yok / birden çok sahne var / sahne adı klasör adıyla uyuşmuyor / `Data/<Sahne>.asset` MapDefinition yok ya da yanlış yerde / mekan kökünde `Art,Data,Prefabs,Scenes` dışında klasör), fazla olan **silinir** (Build Settings'te mekan ağacında olmayan ya da diskte bulunmayan satırlar; `GameCatalog.maps` ve `ModeDefinition.maps` içindeki ölü ve artık taranmayan referanslar). `Boot.unity` index 0'da kalır, mekan-dışı sahneler (`_Shared/Scenes/*`) korunur, `Template/` sahneleri listeye hiç girmez. `ModeDefinition.maps` **boşsa** dokunulmaz (boş = kısıtsız); doluysa o modu destekleyen haritalarla birebir eşitlenir — hedef küme boş çıkarsa liste boşaltılmaz (boş liste "kısıtsız" demek olurdu), yalnız uyarı basılır. Aynı eşitleme **silah kitini** (`WeaponKitBuilder.BuildAll`: `WD_*` asset'leri, `WPN_*` bağları + temizliği, uzak atış FX'i, ön kabza göstergesi, `WeaponCatalog`) ve **net eşya kataloğunu** (`NetItemIdGuard.Rebuild`) de koşturur; hemen ardından **rastgele silah havuzlarını** (`SyncModeLoadouts`: `weaponSource:"random"` olan her modun `loadout`'u = `WeaponCatalog`'un tamamı). ⚠️ Havuz eşitlemesi kit koşusundan **SONRA** gelir — kaynağı o koşunun yazdığı katalogtur; önce gelseydi yeni bir silah havuza ancak ikinci eşitlemede girerdi. ⚠️ `loadout`'ta **boş liste doldurulur** (`maps`'in tersine): `WeaponGranter` için boş havuzun karşılığı "kısıtsız" değil "hiç silah yok"tur. ⚠️ **Ayrı menü öğesi yoktur ve açılmaz:** elle çalıştırılan bir kit adımı unutulur ve bedeli sahada "silah kavranmıyor" olarak ödenir. Sonda `ServerConfigExporter.Export(false)` + **sağlık raporu**: `ArenaBoundary` var mı · `dimensionsJson` dolu mu · muhafaza dünya orijinine yakın mı (arena uzayı = dünya uzayı) · ölçü maketi `EditorOnly` etiketli mi (etiketliyse build'e girmez ve kalibrasyon işaretçileri onunla birlikte silinir). Hiçbiri işi durdurmaz, hepsi rapora satır düşer. Kontroller **aktif sahneye** bakar, yani sahne bir kutuda değilse hiç koşmaz. ⚠️ **MapDefinition kendiliğinden ÜRETİLMEZ:** `supportedModeIds` boş bırakmak "kısıtsız" demek olduğu için üretilen boş bir tanım lobiyi sessizce her modda oynanır kılardı — sahne açılıp modlar araçtan seçilir. Ayrı bir "Arena Id" alanı yoktur: MapDefinition'ın adı sahne adıdır. Pencerede ayrıca **Hazırlık** bölümü vardır (`BuildReadiness`): build almadan önce çalıştırılmış olması gereken araçların durumu tek ekranda listelenir — **koşum sırasıyla**: arena kayıtları · silah kiti · rastgele silah havuzları · net eşya kataloğu · **eşya alma yolu ↔ prefab** · iskelet eklem listesi · ağ nesneleri · `maps.json` · HMD katmanları. Satırlar **yalnız okur**; hepsini tek düğme koşturduğu için düğmeli satır bir istisnadır (`maps.json` export'u ve HMD katmanları, tek başına tekrarlanabilsin diye; **ağ nesneleri → Onar** ise tek düğmede HİÇ koşmaz — her arena sahnesini açıp `sceneId` onarır (`SceneIdBatchRepair`) ve kullanıcının çalıştığı sahneyi her eşitlemede elinden almak kabul edilemez; merge/kopya sonrası elle basılır, açık sahne kirliyse çalışmaz). Ağ nesneleri denetimi ayrıca export edilmiş listede **yinelenen `sceneId`** arar — bu, sahne dosyasının merge'de çakıştığının açılmadan görülebilen tek izidir. **HMD katmanları** tek düğmede de koşar ama **yalnız bayatken**: o araç paylaşılan `VA_CameraRig` prefabına yazar ve her koşuda yeniden serialize edilseydi git diff'i gürültüyle dolar, aynı prefabda çalışan iki geliştirici sürekli çakışırdı. Kalan ✗ **insan adımıdır** (kavraması yazılmamış silah, ateş sesi atanmamış silah, atanmamış `netItemId`) ve düğmeyle geçmez. ⚠️ **Arena kayıtları ve silah havuzu denetimleri eşitlemenin KENDİ gövdesini `dryRun` ile koşar** — ikinci bir "güncel mi" mantığı yazılmaz, yazılsaydı asıl eşitlemeden sessizce sapardı. Her satırın **tooltip'i ne zaman gerektiğini** yazar (başlığın yanındaki `?`). Satırlar pencere odaklandığında toplanır (OnGUI'de denetim koşmaz) |
| `ItemGrabPathGuard` | **Kural ↔ prefab** karşılaştırması: alma yolu `DistanceGrab` **olmayan** bir eşyanın prefabında mesafeli kavrama bileşeni (`DistanceGrabInteractable` / `DistanceHandGrabInteractable`) bulunmamalıdır. ⚠️ **Neden bileşenin YOKLUĞU, filtre değil:** ISDK'da aday listesini kapatmak objeyi alınamaz yapmaz — boş aday listesiyle bile interactor hover'a girer, `Select()` kavrama basışını hiçbir şey seçmeden **kuyruktan düşürür** ve basış sessizce yenir; oyuncunun gördüğü "kavrama bazen çalışmıyor"dur. `WeaponFrame`'in filtresi bir MESAFE kuralıdır, "alınabilir mi" kuralı değil. ⚠️ **HİÇBİR ŞEY YAZMAZ** — düzeltme insan adımıdır (bileşeni prefabdan kaldır ya da tanımdaki yolu düzelt); bileşen soyan bir araç, bilerek uzaktan alınan bir silahı sessizce alınamaz yapardı. Hazırlık satırı olarak çizilir (`NetItemIdGuard` deseni) |
| `SkeletonStreamGuard` | İskelet akışının (§6.9) **eklem listesi denetimi**, gönderen (`LocalBodyAvatar`) ile alıcı (`RemoteAvatar`) prefablarında: (1) tel listesi `SkeletonWire.JOINT_INDICES`'teki her indeks iki prefabın `_jointPairs`'inde dolu ve **aynı kemik adına** çözülüyor; (2) `_jointPairs[0]` retargeter'ın kendi transformu; (3) `SkeletonWire.HIPS_INDEX` "Hips" adlı kemik; (4) `_objectsToHideUntilValid` boş (§7 "`_objectsToHideUntilValid` uzak gövdede nesneyi kalıcı gizler"); (5) `NetworkCharacterRetargeter`'ın `_bodyIndicesToSync`/`_bodyIndicesToSend` listeleri — dördü birebir aynı ve **boş değil** (boş liste SDK'da "tüm eklemler" demektir). Bu son liste tele etki etmez, yalnız SDK'nın atılan iç serileştirmesini besler; denetim o yol derlendiği sürece durur. ⚠️ Denetim gerekiyor çünkü indeks bir kemik adı değil `_jointPairs` sırasıdır: bir prefabda eşleşme kayınca kare alıcının doğrulamasından geçer, hiçbir yerde hata çıkmaz, yalnız uzak gövdeler bozuk çizilir. `VortexArena.Core.Editor` bu yüzden `VortexArena.Protocol`'e referans verir. ⚠️ **Hiçbir şey YAZMAZ ve liste runtime'da HESAPLANMAZ** — bu bir hesap değil iki prefabta serialize edilmiş veridir; çalışma anında ad/hiyerarşi tarayıp yeniden üretmek listeyi yazan ikinci bir taraf açar ve tam da bu denetimin engellediği ayrışmayı üretirdi. Bileşen tipiyle değil ADIYLA bulunur ve alanlar `SerializedObject` ile okunur: denetim uğruna editör derlemesine Movement SDK referansı eklenmez; ad saparsa satır ✗ olup gerekçesini yazar |
...
Modun **tek satır durumu yoktur**: aşağıdakilerin hepsi sunumdur ya da olay bildiricisidir, oyunun
gerçeği sunucudadır (`BurgerMode`). İki kural bütün kutuyu yönetir:

- ⚠️ **Bir olayı KİM bildirir?** Olayların `policy`'si `anyone`'dır (gönderen, hedef objenin sahibi
  değildir — ekmek tahtada, köfte ızgarada serbest durur), ama aynı gerçeği N istemcinin bildirmesi
  sunucuda aynı sayacı N kez başlatır. Seçici sahnede aranır ve **tek kişiyi** göstermelidir: objeyi
  **durduran** istemci (`NetObjectPoseSender.RestSent`) ya da aleti **elinde tutan** istemci.
- ⚠️ **Sunum sahnede kalır, telde taşınmaz:** yürüyüş, slot konumu, balonun içeriği yereldir; telde
  yalnız `stage` ve `s` gider. Böyle bir objeye `NetObjectBody`/`NetObjectPoseSender` **eklenmez** —
  hem yoldan hem ağdan sürülen bir transform, iki başlıkta iki yer demektir.

| Sınıf | Görevi |
|---|---|
| `BurgerKinds` | İstemcinin **tek sözlüğü**: tür adları, olay adları, `stage` sayıları, `s` anahtarları ve müşteri yürüyüş süresi. ⚠️ `Docs/ArenaNet-Protokol.md` §10.5'in `burger` tablosunun birebir aynasıdır — çağrı yerine elle yazılan bir literal sunucuda **sessizce** reddedilir (`kinds[].events[]` doğrulaması) ve belirtisi "etkileşim hiçbir şey yapmıyor" olur. Yürüyüş süresi sunucunun sabitinin aynasıdır: ayrışırsa müşteri bankoya varmadan sipariş verir ya da varınca yerinde durur. Sabır süresi ise **yaklaşık** bir aynadır (sunucu `server.json → burger` ile vardiya boyunca düşürür, istemci bilmez) — yalnız balonun rengini sürer, kural üretmez. Tür→Türkçe ad tablosu da buradadır (balon ve red teşhisi aynı adı yazsın diye) |
| `BurgerClientController` | Burger HUD'ının kökü (`ModeHudBase` alt sınıfı) — yalnız **moda özgü** olanı ekler: kooperatif skor satırı (ortak toplam `match_state.scoreRed`'den, kendi katkısı `PlayerInfo.score`'dan) ve mutlu/mutsuz müşteri sayaçları (`modeState`). Silah/can/ölüm/kill-feed alanları prefabda **bilerek boş** — bu modda karşılıkları yok, taban atanmamış alanı çizmez. Faz etiketi "MAÇ" yerine "VARDİYA"; maç sonu başlığı kazanan değil vardiyanın bitişidir. ⚠️ `modeState` ayrıştırması **hoşgörülüdür**: boş/yarım/bozuk dize sayaçları sıfırda bırakır, bilinmeyen anahtar atlanır — alan sonradan yeni sayaç kazanacaktır |
| `BurgerDispenser` | Elin soketin içinde kavramaya basması `take` olayını yollar — ⚠️ basış **doğrudan değil hakemden geçer**: `IGrabClaimant` uygulanır, girdi yalnız `GrabArbiter.Submit` ile aday olur ve olayı hakemin çağırdığı `CommitGrab` yollar. Dağıtıcının soketi kendi dağıttığı serbest malzemelerin soketleriyle çakışıyor; hakemsiz tek basış hem malzemeyi alır hem yeni malzeme istetirdi. ⚠️ **İki el de ayrı ayrı aday olur**, soketin çözdüğü "yakın olan el" değil: bankoda ikinci el çoğu zaman doludur (spatula, tahta, alınmış malzeme) ve çoğu zaman daha yakın durur — tek el çözülseydi basışı o el cevaplar, dolu bulunup reddedilir, birkaç santim geride bekleyen boş ele hiç bakılmazdı; belirtisi "dağıtıcı hiçbir şey vermiyor" olur. Aynı karede iki el birden kazanabilir; tek basıştan iki malzeme çıkmasını **bekleme süresi** engeller (`CommitGrab` de ona bakar). **Göstergesi kapatılabilir** (`hideGrabIndicator` — dağıtıcının `ItemDefinition`'ı yok, kendi soketini kendisi sürer); susturma yalnız görseldir, kabul yarıçapı aynı kalır. ⚠️ Tür `grab:"none"` ve objede `NetObjectGrabBridge` **yoktur** — dağıtıcının kendisi hiç alınmaz, yalnız olay üretir; malzeme **sunucuda, doğrudan elde** doğar (`object_spawn` + `owner` + `Held`), yerelde hiçbir şey üretilmez. ⚠️ Basış kenarı **her karede el başına** izlenir, sokete girişte örneklenmez: zaten sıkılı bir el hacme sürüklenerek malzeme alırdı. **Boşa çıkan istek gözlükte de loglanır** (`[BurgerDispenser] … el dolu` / `take istendi`) ve sunucu tarafındaki karşılığıyla (`[world] object_event reddedildi (… → netId …): faz …`) eşleşir: "bastım, hiçbir şey olmadı" şikâyetinde eli mi dolu yoksa fazın mı kapalı olduğu ancak iki satır birlikte okunarak ayrılır |
| `BurgerKnife` | Bıçağın ağzı **serbest** bütün ekmeğe değince `cut` yollar; kesmeyi sunucu yapar (bütünü despawn, aynı pozda iki yarım). Birinin elindeki ekmek kesilmez — modun kapısı "hedef serbest olmalı"dır |
| `BurgerGrill` | Izgara **hacmi**: köfte girince `grill` başlat, çıkınca durdur. Pişme sayacı **sunucunundur**, ızgara yalnız "girdi/çıktı" der. Bildiren, köfteyi ızgaraya **koyan** oyuncudur (`RestSent`) |
| `BurgerPatty` | Köftenin **pişme sunumu** (`NO_patty`): `stage`'i (çiğ/pişmiş/yanmış) renge çevirir (`MaterialPropertyBlock`, materyal paylaşımı bozulmaz), relay edilen `grill` olayıyla cızırtıyı açıp kapar, pişmişe geçişte tek vuruş ses çalar. Sayaç sunucudadır; bu bileşen yalnız görünür kılar. ⚠️ Olmadan köfte üç aşamada da aynı görünür ve sunucunun çiğ/yanmış reddi çocuğa "neden olmadı" diye kalır. Ses alanı boşsa sessiz |
| `BurgerCarrier` | Elde tutulan bir taşıyıcının üstünde (spatulanın bıçağı) malzeme taşır; yalnız `NO_spatula` prefabındadır — servis tahtası tezgâhta sabit bir iş istasyonudur, taşıyıcı değildir. ⚠️ **Taşıma ilişkisi telde YOKTUR, türetilir:** malzeme sıradan `object_grab` ile taşıyıcıyı tutan elin malı olur, yani telde yalnız `owner` + `held` + `heldRight` durur; her istemci aynı kuralı uygular — *taşıyıcıyla aynı elde tutulan malzeme, o taşıyıcının ankorunda durur*. Ayrı bir alan ya da mesaj eklenmez. ⚠️ **Yığın sırası dinlenme pozunun YÜKSEKLİĞİNDEN okunur**, geliş sırasından değil: geliş sırası maçın ortasında bağlanan başlıkta yoktur (`world_state` sırasızdır) — hamburger o başlıkta karışır ve tarif alttan üste okunduğu için yanlış servis olurdu. Taşınan malzeme **poz paketi göndermez** (`held` olduğu için `0x09` kapısı zaten kapalı): pozu taşıyıcıdan, taşıyıcınınki elden gelir. **Yalnız sahibi talep eder ve bırakır;** diğer başlıklar aynı durumdan aynı yerleşimi türetir. Yük, taşıyıcı `spillAngle` derecesinden fazla yatınca ya da taşıyıcı elden bırakılınca `object_release` ile dökülür — oradan sonrası sıradan uçuş penceresidir (fizik + poz akışı + `object_rest`), yani ızgara ve tahta kancaları değişmeden çalışır. Kapasite alandan gelir: spatula tek malzeme. ⚠️ **İki şerit vardır:** malzeme türleri ankorda **yığılır**; malzeme olmayan tutulabilir nesneler (bıçak, spatula, bütün ekmek) taşıyıcı kaldırılırken üstünde duruyorsa **gevşek** biner (kapasitesi `looseCapacity`, `0` = hiç binmez ve **varsayılan budur**; kurulu hiçbir prefab açmaz — spatulada açık olsaydı hedeflenen köftenin yanındaki bıçağı da kaldırırdı). ⚠️ **Açmadan önce bir tür allow-list'i gerekir:** şeridin tek filtresi "kavrama köprüsü var mı"dır, yani hacme giren tutulabilir her nesne biner. Gevşek yükün yerleşimi iki **dinlenme pozunun** farkıdır — nesnenin taşıyıcı üstündeki dinlenme pozu ile taşıyıcının tezgâhtaki dinlenme pozu; sunucunun hiç hareket ettirmediği bir sahne nesnesi için `NetObject.ScenePosition`/`SceneRotation` kullanılır — yani ofseti her başlık aynı türetir ve telde yeni alan yoktur. ⚠️ **Gevşek yük yalnız kaldırışın ilk anlarında alınır:** sonrasında taşıyıcı dinlenme pozundan uzaklaşmıştır ve aynı fark nesneyi havada oturturdu. ⚠️ **Başka bir taşıyıcının üstünde giden taşıyıcı UYUR:** talep etmez, kendi yükünü oturtmaz — aynı elin yükünü iki taşıyıcı yazsaydı tek transforma iki yazıcı düşerdi. ⚠️ **Hacme giren her serbest malzeme alınmaz:** aday dinlenmiş olmalı (`awake` değil — uçan malzeme kapılmaz) ve merkezi yığının tepesinden prefabdaki temas bandı içinde durmalı (`0` = bant yok, hacmin tamamı). Bir hamburger boyu yüksek bir kargo hacmi bant olmadan üstünden geçtiği her şeyi (ızgaradaki köfteleri) süpürür; spatulanın tek slotu bant istemez. ⚠️ **Taşıma hacmi TETİK collider'dır** ve taşıyan yüzeyin ölçüsünde durur: tetik değilse propun kendi `Rigidbody`'sinin bileşik çarpışmasına katılır (prop görünmeyen bir kutuya çarpar, yerde yanlış durur), yüzeyinden büyükse malzeme yüzeyin epey yukarısındayken kabul edilir ve havada asılı görünür — ikisi de hata vermez |
| `BurgerCarryHaptics` | Elinde malzeme tutan yerel oyuncuya o elde **seyrek, hafif bir kalp atışı** verir (çift vuruş, ilki kavrama anında; periyot `HeartbeatPeriodSeconds`) — ağırlıksız malzemede "hâlâ elimde" diyen başka işaret yoktur. ⚠️ Sürekli ya da sık titreşim birkaç saniyede rahatsız eder; atış seyrek tutulur. Kendini önyükleyen kalıcı tekildir (sahneye/prefaba konmaz, yeni arena kurulum adımı doğurmaz); yerelde tutulan malzeme yoksa hiç rapor yazmaz. ⚠️ Genlik uyarı darbesinin ve onay burst'ünün ALTINDADIR: `ControllerHaptics` en yükseği seçer, güçlü bir taşıma titreşimi engel uyarısını maskelerdi. El boşalınca açık `0` yazılır — heartbeat zaman aşımını beklemek bırakıştan sonra titreşimi uzatır ve "kumanda takıldı" diye okunur |
| `BurgerServingBoard` | Servis tahtası: tezgâhta **sabit bir iş istasyonu** — kavranamaz ve alınamaz (`KIND_board` → `grab: none`, `ITEM_BurgerBoard` → `grabPath: None`; istemcide tek alan hem almayı hem kavrama küresini kapatır, çünkü `NetObjectGrabBridge.IsTakeable` yalnız `ProximitySocket`'te doğrudur). Hamburger başka yerde kurulup taşınmaz, **müşterinin önünde yerinde** kurulur. Sahiplenilemediği için **kalıcı kinematiktir**: `NetObjectBody` kinematikliği yalnız sahipte kaldırır, sahipsiz obje kinematik kalır ve sunucudan pozu gelmediği için sahnenin koyduğu yerde durur — fizik tahtayı yerinden oynatamaz. Slotta duran tahta üstündeki yığını `serve` ile bekleyen müşteriye bildirir (yığın **dünya Y'sine göre alttan üste** sıralanır — sunucunun karşılaştırdığı sıra budur). **Tek tetikleyici, üst ekmeğin tahtaya oturmasıdır** ve hamburger KAPALI olmalıdır (en üstte `bun_top`): kapalılık şartı her malzemede yeniden reddedilmeyi önler. ⚠️ **Tahtayı bir yuvaya bağlayan tek şey KONUMUDUR** — `ResolveSlot()` slot numarası değil HACİM arar (`BurgerCounterSlot.Contains`); hiçbir yuvanın içinde durmayan tahta malzeme kabul eder ama hiç servis yapmaz (Tuzaklar). ⚠️ **Reddin kendi mesajı yoktur:** doğru servis `object_state` üretir, yanlış tarif `serve` olayının kendisi olarak geri gelir — yani gelen `serve` **reddin ta kendisidir**. Red gelince **sebebi yerelde teşhis eder** (olaydaki yığın + müşterinin tarifi → alt/üst ekmek yeri, köfte aşaması, eksik/fazla tür) ve müşterinin balonuna yazdırır. ⚠️ Teşhis sunucunun servis kuralının **aynasıdır**; kural değişince burası da değişir, yoksa balon yalan söyler. Kabul edilen servis yalnız servis edende kısa haptik verir. **Yığına yeni katman oturduğu an kısa geri bildirim verir** (`stackSound` atanmışsa ses + katmanın görsel çocuklarında kısa ölçek pop'u + katmanı KOYAN başlıkta tek darbe haptik). ⚠️ Katman tetik girişiyle değil **yoklanarak** bulunur (10 Hz `CollectStack`; aday elde olmayan ve dinlenmiş malzemedir) — oturan malzeme hacme girip çıkar ve her giriş aynı katmanı yeniden patlatırdı; tahtadan çıkan katmanın kaydı düşer. ⚠️ İlk yoklama sessizdir: maç ortasında bağlanan başlık hazır burgerin her katmanını duyururdu. ⚠️ Pop **kökü değil** collider'sız görsel çocukları büyütür — kök büyüse collider da büyür ve yığındaki komşu katmanı iterdi. **Engellenen her servis tek satır uyarı basar** (tahta/ağ · bekleme süresi · yuva dışı · yığın kapalı değil · yuvada bekleyen müşteri yok), engel başına oran sınırlı: sessiz `return` sahada "servis gözlükten hiç çıkmıyor" diye görünür. **Müşterisi olmadığı için düşen servis BEKLEMEDE kalır** ve müşteri yuvaya gelince yeniden gönderilir — kapanış hareketi (üst ekmeğin oturması) ikinci kez olmaz; yoklanarak izlenir, çünkü müşterinin "beklemeye başladı" olayı yoktur. ⚠️ Yeniden deneme yalnız **isteği** tekrarlar, kararı yine sunucu verir |
...
Burger kutusunun iki kuralı burada da geçerlidir (sunum sahnede kalır; olayı tek kişi bildirir), ama
...
| `MoleHole` | Deliğin sunumu: `stage`/`s`'ten köstebeği yükseltir, bekletir, ezer, indirir; rengi `c:`'den alır. `IsHittable`, ayaktaki köstebeğin yanında **inişteki** köstebeği de vurulabilir sayar (§10.5 payı; yalnız ayakta→gizli geçişinde açılır, ezilip inende açılmaz) — istemci bu payı sunucununkinden uzun tutarsa rapor sessizce düşer. **Prefab iki katmandır:** pivot (`Mole`) collider'ı taşır, altındaki `Model` yalnız görüntüyü — model değişince **sadece `Model` değişir**, ayakları pivotun orijinine (yükselince zemin hizası) oturtulur ve takım rengini alacak görseller `teamRenderers`'a sürüklenir (göz/burun gibi sabit renkli parçalar dışarıda kalır; liste boş bırakılırsa hepsi boyanır). ⚠️ **Yalnız `teamMaterial` slotları boyanır** (bugün kask): tek mesh'li atlas modelinde renkli parça ayrı alt-mesh + ayrı materyal olmalıdır — atlas dokusuna renk çarpmak koyu kürkü siyaha çevirir. Kask alt-mesh'i `MSH_Mole_HelmetSplit` üretilmiş mesh'idir (FBX'in UV adalarından ayrıldı); FBX değişirse yeniden üretilmelidir. Renk `_BaseColor`'a `baseTint` payıyla, ayrıca `glow` gücüyle **ışıma** olarak yazılır — materyalde Emission açık olmalıdır. Doğru/yanlış ezilmede delik altındaki `correctFx`/`wrongFx` partikülleri oynar (sesle aynı aşama değişimi kapısı, snapshot'ta oynamaz). **Köstebeğin collider'ı balyozun hedefidir** ve alt ağacın herhangi bir yerinde olabilir; balyoz köke `GetComponentInParent` ile ulaşır. **Hareketin iki sürücüsü vardır ve yalnız biri çalışır:** alt ağaçta `Animator` varsa yükseliş/bekleme/iniş **kliplerindir**, bileşen yalnız aşamayı `stage` adlı int parametreye yazar (0 gizli · 1 ayakta · 2 ezik) ve pivotu hiç kıpırdatmaz; animatör yoksa hareket koddan (`riseHeight`/`riseSeconds`) lerp'lenir. ⚠️ İkisi aynı anda açık olamaz — tek transform'a iki yazıcı köstebeği titretir ya da tepeye hiç vardırmaz; bu yüzden kod yolu tamamen devre dışı kalır, harmanlanmaz. ⚠️ **Klip yolunda köstebek objesi kapatılmaz**, dolayısıyla model **kendi deliğini getirmek zorundadır**: free-roam zemini düzdür (fiziksel çukur yoktur), saklanacak yeri olmayan köstebek aşağıdayken açıkta durur. Yalnız `hideWhenDown` görselleri (gövde; boşsa elle atanmış `teamRenderers`) animatör `Hidden` durumuna **vardıktan sonra** kapanır — delikte bekleyen köstebek her karede boşuna skin'lenmesin; iniş görünür kalır. Delik/taş gibi hep görünen parçalar bu listeye konmaz. Modelin tek kemiğe bağlı katı parçaları (delik, taş, zemin) `SkinnedMeshRenderer` değil, kemiğin altındaki `MeshRenderer`'dır — animasyonu aynı izler, skinning maliyeti ödemez. Kod yolunda ise aşağıdaki köstebek **kapatılır**, yalnız indirilmez — collider'ı da onunla kapandığı için gizli köstebek zaten vurulamaz. ⚠️ Anlık uygulama ile animasyon `NetStateOrigin` ile ayrılır: snapshot'ta (geç katılan) köstebek yerine **ışınlanır**, çoktan başlamış bir çıkışı baştan oynamaz. Ezik köstebek **ezik inerek** kaybolur: `0` aşaması iniş sürerken gelir, ezikliği aşamadan değil son ezilmeden okumak köstebeğin inmeden önce eski boyuna şişmesini önler |
| `MoleHammer` | Balyozun bileşeni: **başın kendi collider'ının** dokunduğu **ayakta** köstebeğe `whack{n}` yollar. Temas **her karede** aranır (önce çakışma, sonra başın bir önceki konumundan süpürme; temas tamponu 32). Vuruş sayılması için iki koşul birlikte gerekir: bu karede temas **ve** son `speedWindowSeconds` (varsayılan 0,1 sn) içindeki **tepe** baş hızının `minSwingSpeed`'i (1,5 m/s) aşması. ⚠️ **İlk temas yavaşsa o ÇIKIŞ kilitlenir:** köstebek duran balyozun içine çıktıysa ya da balyoz yavaşça bastırıldıysa, o çıkış ancak balyoz köstebekten `embeddedExitMargin` (5 cm) kadar çıkıp yeniden hızlı girerse ya da içerideyken `embeddedSwingSpeed` (3 m/s) sert salınım yapılırsa sayılır — vurulma kapsülü sabit ve ayaktaki zarfı kapladığı için duran balyoz köstebek çıktığı an zaten "temas"tadır, içerideki küçük kıpırdama/titreme vuruş değildir. ⚠️ Anlık hız değil pencere tepesi okunur: çarpma anında yavaşlayan sallama yine sayılır, yavaş itme sayılmaz — "dokunarak ezme" böyle kapanır. ⚠️ Rapor (delik, sayaç) çiftiyle **tekilleştirilir** — bir sallama birçok kare sürer ve her karede gönderilseydi aynı olay sunucuya beş kez giderdi. Vuruş küresinin yarıçapı/konumu Inspector'dan görsel ayarlanır, kodda ölçü yoktur. ⚠️ **Köstebeğin vurulma kapsülü `Mole` pivotunda sabittir ve ayaktaki pozun tüm hareket zarfını kaplar** (Idle klibinin sallanması dahil, kafa tepesi ve yan yüzler içeride): klip kemikleri oynatır, pivotu değil — kapsül yalnız gövdeyi sararsa görünen kafaya inen vuruş ıskalanır. Vuruş yalnız `stage` 1'de sayıldığı için gizliyken havada kalan kapsül sorun değildir; model değişince kapsül yeni modelin ayaktaki sınırlarına göre yeniden boyutlanır. |
| `MoleHammerGranter` | Live'da iki ele birer balyoz verir, düşerse **anında** geri verir, balyoz tanımının `teamTintChildren` adlı parçalarını (vuruş yüzü göstergeleri) oyuncunun takım rengine boyar; uzak oyuncunun balyozunu aynı listeyle `RemoteAvatar` boyar. ⚠️ Balyoz `WeaponDefinition` DEĞİLDİR: bu mod `weaponSource:"none"`dır ve silah olarak vermek ailenin baştan kapattığı hasar hattını açardı. Mod HUD prefabında yaşar — ömrü tam olarak modun maçı kadardır, her köstebek arenasında elle konulacak bir sahne objesi gerektirmez |
| `MoleClientController` | Mole HUD'ının kökü (`ModeHudBase` alt sınıfı) — yalnız **moda özgü** olanı ekler: `TeamScorePanel`'i besler ve oyuncunun kendi doğru/yanlış sayısını (`modeState`) yeşil `D` / kırmızı `Y` olarak **yerel oyuncunun takımının** skor tarafına yazar (`redSideHitsText`/`blueSideHitsText`; takım `ArenaCombat.LocalTeam`, değişimi `PlayerCombatState.LocalTeamChanged`; `Neutral`'da ikisi de boş). Can/ölüm/kill-feed alanları prefabda **bilerek boş**; iç içe `HealthHud` örneğinde `Backdrop` kapalıdır. ⚠️ `modeState` ayrıştırması **hoşgörülüdür**: bozuk/eksik dize sayaçları sıfırda bırakır, bilinmeyen token atlanır |
...
| `StateHost` | UDP kaydı, poz alımı, 20 Hz snapshot yayını (16 girdiden fazlası MTU'ya sığan parçalara bölünür; olay varsa ve sığıyorsa `0x05` ile tek datagramda birleşir), `0x06` RTT echo'su. **Obje pozu da buradan geçer:** `0x09` alımında sahiplik **kilitsiz** doğrulanır ve obje başına `seq` ile eski paket düşürülür; tik başına toplanan pozlar `0x05`'in **obje bölümüne** yazılır (geri düşüş yolunda — `0x02`/`0x04` — obje bölümü yoktur ve açılmaz). ⚠️ Obje `seq` defteri bayatlayınca **düşürülür**: obje kimlikleri tur/sahne sıfırlamasında yeniden kullanılıyor, kalıcı defter yeni turun sıfırdan başlayan `seq`'ini eski turun yüksek değerinin arkasında kilitler ve objenin pozu hiç akmazdı. **Telemetriyi burada üretir:** saniyelik `[state]` satırı — gerçek bayt-sn/paket-sn, tik kayması, uplink jitter + poz/olay kaybı, iskelet batch'inin (`0x08`) kendi paket-sn ve kB-sn'si (bant genişliği bütçesindeki `skeleton (sunucu TX)` kalemini doğrudan ölçer); eşiği aşan oyuncu için ek `[net]` satırı |
| `PlayerRegistry` | Oyuncu listesi, `playerId` tahsisi (1..255), `devices.json` ile kalıcı **kimlik** (ad + forma numarası), bağlantı durumu ve **maç katılımcısı defteri**. ⚠️ **"Çevrimdışı" diye bir durum YOKTUR** (§2): `Connected` → soket düştü ya da `HEARTBEAT_TIMEOUT` doldu → `Reconnecting` (kayıt durur, maç kapılarına girmez) → `RECONNECT_GRACE` da dolarsa oyuncu **çıkarılır**: koşan maçın katılımcısıysa `Left` olarak maç sonuna kadar durur, değilse kayıt silinir ve playerId havuza döner. **Maç defteri** (`MatchParticipant` → `inMatch`) istatistik satırını bağlantıdan bağımsız kılar: `left` bir kayıt yalnız bu bayrak yüzünden yaşar. ⚠️ Defter **lobiye dönerken** kapanır, `match_end`'de DEĞİL — erken temizlik maç sonu tablosunu tam da okunduğu anda boşaltırdı. ⚠️ `Left` kayıtlar `_players`'ta durduğu için playerId'leri `NextFreePlayerIdLocked` tarafından zaten atlanır; ayrı bir rezervasyon defteri gerekmez. ⚠️ Toplu bayrak yazımı (maç kurulumu/kapanışı) **tek bir** `Updated` yayınlar — kayıt başına yayın N tam roster JSON'u demek olurdu. ⚠️ Soket alanının adı `Socket`'tir; `Connection` **durumu** taşır (aynı isimde iki kavram olamazdı). **Kimlik:** ilk bağlantıda ad 20'lik havuzdan rastgele (kullanılmayanlar arasından), numara 1'den itibaren ilk boş (1..99); `set_identity` ikisini de değiştirir. Adlar tekrar edebilir, **numara tüm KAYITLI cihazlar arasında benzersizdir** — sahiplik sorgusu `_players`'a değil `_devices`'a bakar (hiç bağlanmamış cihaz da numara tutar). Çevrimiçi sahipten numara istenirse reddedilir; çevrimdışı sahip **aynı anda** yeniden numaralanır. **Rol başına kalıcılık farkı:** oyuncu kaydı yukarıdaki iki aşamadan geçer (deviceId kalıcı, geri dönen aynı satıra oturur — ad, numara, takım, `kills/deaths/score` korunur); **`hello`'da takım olduğu gibi kalır** — takımsız bir maç kuruluyken (`MatchDirector.IsTeamlessMatchSetUp`, hello yolunda registry kilidinin DIŞINDA sorulur) katılana da dönene de takım atanmaz (`""`), aksi hâlde yalnız **boş** takım az kişili takıma (`SmallerTeam`) yazılır; boş takımı koşulsuz "atanmamış" saymak takımsız maçta dönen oyuncuyu kırmızı gövdeye geçirir. Bağlı ama takımsız kalmış oyuncuya takımı `MatchDirector` iki anda yazar: takımlı bir `start_match`'te (`BalanceTeams` — **tek oyuncu da**; yalnız yarı-taşıma 2+ ister) ve `set_selection` takımlı moda geçtiğinde (`AssignTeamlessForMode` — takımı olanlara dokunmaz); ikisi olmadan takımsız maçtan dönen oyuncu yeniden bağlanana kadar beyaz etiketle kalır. **Admin kaydı ilk adımda tümüyle SİLİNİR** ve `Reconnecting` durumuna hiç girmez (deviceId oturumluk — geri gelen admin yeni bir kimlikle gelir, o satır asla eşleşemezdi; ayrıca her açıp kapatma roster'da hayalet satır ve tükenen playerId bırakırdı) ve admin adı diske yazılmaz. Aynı PC'de iki admin varsa ad " (2)" ile ayrıştırılır. **Atma bunun istisnasıdır** (`RemoveByPlayerId`): kayıt anında silinir ve katılımcılıktan da düşer — kopma satırı bırakır (maç sonu tablosunda görünür), atma bırakmaz; `devices.json`'a dokunulmaz, yani atılan cihaz geri bağlanırsa adını/numarasını korur (§5.4) |
| `PlayerState.Floor` | Oyuncunun katı — **yalnız defter** (`set_floor` → `PlayerRegistry.SetFloor` → roster). Sunucu geometriyi bilmediği için karar veremez; işi `calibrated` ile birebir aynıdır: aralık kapısı (`0..MAX_FLOOR_INDEX`, dışını loglayıp yok sayar), değişmedi-kapısı ve `hello`/`load_match`/lobiye dönüşte `0`'a çekme (§3.13; kural `ArenaNet-Protokol.md` §10.6 "Kat modeli") |
| `LobbyService` | Roster yayını (`lobby_state`) — **tek yayıncı döngüden**, kirli bayrakla birleştirilerek, her yayında `version` artarak (Tuzaklar: "ateşle-unut yayın sıra garantisi vermez"); `status.rosterVersion` geride kalan istemciye yalnız ona tam snapshot yollatır. Ayrıca ready/takım/kick/`set_identity` + **adminler arası ortak durumun sahibi**: mod/harita seçimi burada yaşar, `set_selection` ile değişir, `admin_state` ile yalnız adminlere yayılır. Her admin komutu "kim ne yaptı" duyurusu üretir. **Mekan ölçümü yüklemesi de buradan geçer** (`venue_survey` → `VenueSurveyStore` → `venue_survey_result` yalnız gönderene): dosya adının mekanını **sunucunun seçili mekanı** verir, mesajdaki `name` değil — ad bir istemci alanı olsaydı yanlış yazılmış bir ad sahada ikinci bir dosya açardı |
| `MatchDirector` | **Faz makinesi (10 Hz tick), vuruş hattı, can/skor, canlanma.** Mod kaydı tek yerde (`RegisterModes()` — yeni mod buraya bir satır). **Skor defteri:** `AddScore(team,…)` (takım) + `AddPlayerScore/ScoreOf/TryGetLeader` (bireysel); modlar skoru YALNIZ buradan yazar. **Mod komutları** (§3.8.2): `TryPauseForMode` / `TryReviveRosterForMode` / `SetModeState` / `TryStartRound` / `TryCancelCountdownForMode` — modun fazı doğrudan yazmasını (ikinci otorite) ve kendi mesajını yollamasını (ikinci gönderici) gereksiz kılar. **Dost ateşi anahtarının da sahibi burasıdır** (§3.9): açılışta kapalı, yalnız `set_friendly_fire` çevirir, yürürlükteki kural şekline `ApplyRulesLocked` damgalar. ⚠️ **Takımdaş VE kendini öldürmede `OnKill` çağrılmaz** — ödül yazılmaz; kill feed işler, `deaths` artar. Takımdaş öldürmede öldürene `kills` −1 / `score` −1 yazılır (Counter-Strike kuralı, `KillPlayerLocked`; takım skoru değişmez). İki test **ayrıdır**: boş takım takımdaş sayılmadığı için takımsız modda kendini öldürme "takımdaş" testinden geçemez. Kendini öldürmede `kills` de artmaz, ceza da yoktur (öldüren ile ölen aynı kayıt). ⚠️ **Vuruş hattında atıcının canlı olması şart değildir:** elden çıkmış hasar kaynağı (havadaki bomba) sahibi öldükten sonra da sayılır — ölçüt ölümden itibaren bir **pencere**dir, penceresi dolmuş atıcının raporu reddedilir (yeniden bağlanıp kendini canlı sanan istemciye karşı emniyet). **Canlandırmanın tek yolunun sahibi burasıdır** (§3.7): `HandleReviveRequestAsync` (oyuncunun talebi) → `RevivePlayerLocked`. ⚠️ **Operatörün elle canlandırması YOKTUR** — ikinci bir yol açılırsa §3.7'deki yasakların hepsi orada da tekrarlanmak zorundadır. **İhlal defteri de burada tutulur** (§10.9): oyuncu **ve tür** başına (`obstacle` / `out_of_bounds`) sayı + toplam süre, kenarlar `violation` mesajı olarak yalnız adminlere yayılır ve defter `return_to_lobby`'de skorla birlikte sıfırlanır. ⚠️ Kenarın başlangıcı `VIOLATION_MIN_SECONDS` dolana kadar **bekletilir** — sınırda salınan oyuncu akışı kullanılamaz hâle getirirdi; eşik yalnız akış/defter içindir, ceza ilk kareden itibaren işler. ⚠️ **Alan-dışı bayrağı can eritmez** — tazelik kapısı (`OBSTACLE_FLAG_STALE_MS`) yine her iki türe de uygulanır, yoksa susmuş bir istemci akışta sonsuza kadar açık bir ihlal bırakırdı. ⚠️ Canlandırma `ResetMatchStateLocked` ile YAPILMAZ: o sunucudaki alanları yazar ama istemciye hiçbir şey göndermez, oyuncu ölüm ekranında donar. **Ağ nesnesi mesajlarının üç dalı da burada koşar** (`object_grab`/`object_release`/`object_rest` + `object_event`) ve **modun obje API'si de burasıdır**: `SpawnObject`/`DespawnObject` (doğuş/ölüm) · `SetObjectStage`/`SetObjectFlags`/`SetObjectPayload` (aşama/bayrak/örnek verisi) · `TryReadObject` ve `ObjectIdsOfKind` (okuma; ikincisi modun **süreceği objeleri** öğrenme yoludur — tabloyu baştan sona tarar, o yüzden kurulum yolunun aracıdır, tikin değil). ⚠️ **Yazan duyurur:** yukarıdaki yazma çağrılarının her biri kendi `object_state`'ini yayınlar, çünkü tek bir olay birkaç objeyi birden değiştirebilir; `OnObjectEvent`'in dönüş değeri yayını değil **relay'i** belirler (`true` = işledim, relay etme). ⚠️ **Sahiplik aynası ve son bilinen poz KİLİTSİZ tutulur** (`netId → (playerId, held)` + poz slotu): `0x09` doğrulaması UDP alım thread'inde, maç kilidine hiç girmeden yapılır (`Alive` deseni). Ölümde ve kopmada o oyuncunun objeleri serbest bırakılır |
| `MapTable` | `maps.json` (Unity export'undan) — sunucunun okuduğu tek içerik tablosu. Girdi başına `sceneName` + `gameType` + `modes` + `moleDensity` (harita başına köstebek yoğunluğu, yalnız `MoleMode` okur) + o sahnenin ağ nesneleri (`objects[]`); **arena ÖLÇÜSÜ yoktur** (sunucu metre kullanmaz, §7.30). `gameType` boş gelen eski bir export yüklenirken `"quickbattle"`'a normalize edilir, böylece kapılar tek bir değeri okur; `MatchesGameType(map, modeGameType)` da `start_match`'in üçüncü kapısıdır — modun tipi haritanınkiyle uyuşmazsa komut reddedilir ve konsola sebep yazılır (§3.10) |
| `KindTable` | `maps.json`'ın kök `kinds[]`'i: ağ nesnesi **türünün** kuralları (`kind`, `maxHp`, `grab`, `events[]`). Yüklemede normalize edilir — boş/eksik alan eski export demektir ve `grab:"none"` · `policy:"anyone"` · `phaseGate:"playing"` sayılır, yani bugünkü arenalar tek satır iş gerektirmez. Sorgu yolları sahiplik ve olay kapılarını besler ("bu tür tutulabilir mi", "bu ad bu türde izinli mi ve hangi politikayla"). Kimlik listesinden ayrıdır çünkü sahibi farklıdır — kimlik sahneye, kural içeriğe aittir; aynı tür on arenada geçer ve canı tek yerden değişir. Bilinmeyen `kind` → obje tabloya alınmaz + tek satır log |
| `World/WorldObjectTable` | Ağ nesnelerinin sunucu-otoriter defteri: `netId → {kind, hp, maxHp, flags, owner, stage, dinlenme pozu, dinamik mi}`; hasar, kırılma, **sahiplik** ve tur/sahne sıfırlaması. Sahiplik üç adımda yürür: kavrama (boştaysa sahip yazılır, `Held` +gerekirse `HeldRight` kalkar), elden çıkış (`Held` düşer, `Awake` kalkar, ⚠️ **sahiplik KORUNUR** — uçuş penceresi tam olarak budur), durma (`Awake` düşer, `owner = 0`, bildirilen poz dinlenme pozu olur). Sahibi kopan, ölen **ya da poz kanalı `OBJECT_OWNER_SILENCE_MS` (1 sn) boyunca susan** oyuncunun objelerini serbest bırakır; bu kapı olmadan bir kopma objeyi tur sonuna kadar kilitler, sessizlik sorulmazsa da uyuyan/ağı giden gözlük heartbeat dolana dek `connected` göründüğü için obje o süre boyunca donmuş elde ya da bırakıldığı noktada havada asılı kalır. Dinlenme pozunu **çağıran çözer** (`MatchDirector`): **ölümde ve bağlantı kopmasında** son bilinen pozun tam altındaki zemin (`OBJECT_DROP_CLEARANCE`) — elde tutulan obje poz akıtmadığı için son bilinen poz orada **sahibin o eli**dir. Kopuş `left`'i beklemez: bekleseydi obje `RECONNECT_GRACE` boyunca havada asılı kalırdı. **Dinamik kimliği de burası tahsis eder** (`NET_ID_DYNAMIC_*`): tahsis tur içinde **monotondur** — despawn edilen kimlik tur bitmeden geri verilmez, yoksa yolda olan bir poz paketi ya da olay kimliği devralan yeni objeye yazar ve hiçbir yerde hata görünmez. ⚠️ **Kendi kilidi YOKTUR, `MatchDirector`'ın `_gate`'i altında yaşar** — ikinci kilit deadlock adayıdır; bu yüzden metotları `…Locked` adlanır ve kilidi tutmak çağıranın sözleşmesidir |
| `Modes/IGameMode` + `TdmMode` + `FfaMode` | Mod kuralları: skor, kazanma koşulu, tur süresi. Yeni kancalar **varsayılan gövdeyle** eklenir (default interface method) → mevcut modların hiçbiri değişmez; **tüketicisi olmayan kanca EKLENMEZ**. `FfaMode` yüzeyin ilk tüketicisidir: takımsız + bireysel skor + sabit durma canlanması, `MatchDirector`'a tek satır kayıt dışında hiçbir dokunuş yok. `OnRoundStart` ikinci örnektir: Live'a HER girişte çağrılır, tur kavramı olmayan modlar hiç yazmaz. `OnMatchEnd` de aynı düzendedir: `finished`'a girişte **maç başına bir kez**, kilit dışında ve bir sonraki tick'te çağrılır (`EnterEndLocked` bayrağı kurar, `TickAsync` tüketir; yeni maç, lobi ya da sahneleme bayrağı siler — eski modun `netId`'leri yeni sahnenin objelerine denk gelebilir). `finished`'da `OnTick` çağrılmadığı için sahada kalan nesneyi toplamanın tek yeri burasıdır. ⚠️ **`OnTick` yalnız Live'da değil, geri sayımda ve mod duraklatmasında da çağrılır** (tur tabanlı mod geri sayımı iptal edebilmek için yoklar) — yalnız Live'da iş yapması gereken mod fazı kendisi kontrol eder; iç durumunu `OnMatchStart`'ta temizleyen mod bu kapıyı koymak **zorundadır**, çünkü geri sayım boyunca elindeki `netId`'ler önceki maçındır ve yeni obje tablosu aynı kimlikleri çoktan dağıtmıştır. `InitialModeState` durum yüzeyinin küçük kardeşidir: `start_match` `modeState`'i temizler ve `OnMatchStart` ancak geri sayım bitince düşer, yani sayacı HUD'da olan mod sıfır durumunu buradan verir (varsayılan: boş) — dizeyi üreten **tek** biçimlendirici moddadır, ikinci bir literal iki durumu ayırırdı. `OnObjectEvent` üçüncüsüdür: ⚠️ **dönüş değeri TEK bir soruyu cevaplar — olay relay edilecek mi?** `true` = "işledim, relay etme" (gerçeği, yaptığın yazmaların yayınladığı `object_state` duyurur), `false` = kozmetik (aynı `object_event` herkese relay edilir). İşlenmiş bir olayı relay etmek aynı gerçeği iki kez duyurur ve istemcide sunum iki kez oynar. Mod tabloya hiç dokunmaz, yazma yolu direktörün API'sidir |
| `Modes/TournamentMode` | **Tur tabanlı takım elemesi** (§3.8.2). Kural olarak TDM'den tek farkı `Revive = None`'dır; turun tamamı bu sınıfın iç durumudur (`_round`, `_roundLive`, `_matchOver`). Eleme `OnKill`'de değil **`OnTick` taramasında** ölçülür — takım bağlantı kopmasıyla da boşalır ve o yolda `OnKill` çağrılmaz; tek tarama tek doğruluk kaynağıdır. ⚠️ **Turu bitiren TEK şey elemedir**, sayaç değil: `roundSeconds` burada da MAÇIN bütçesidir (`_matchRemaining`, turlar arası taşınır, `SetTimeRemaining` ile geri yazılır) ve sıfıra inmesi koşan turu **kesmez** — yalnız onu son tur yapar, karar o tur kapanınca `EndRound`'da verilir. Bunun bedeli, "ayakta"nın tek anlamı olmasıdır: **savaşabilir** = canlı **ve** kalibreli, sahadan düşen de ölü sayılır — yoksa kimsenin öldüremediği bir oyuncu turu ve onunla maçı süresiz açık tutardı. Boşalan takım iki farklı şey demektir ve `_roundContested` ikisini ayırır: hiç çatışmaya dönüşmemiş tur (harita önizlemesi) beklemede kalır, çatışmanın ortasında boşalan takım turu kaybeder. Toplanma kapısı `set_ready` bayrağını yeniden kullanır ve **zaman aşımı yoktur**: tur herkes tabanına girmeden başlamaz, geri sayım her koşulda iptal edilebilir, çıkış operatörün `kick`/`end_match`/`abort_match` komutudur. Bekleme uzarsa 30 sn'de bir konsola teşhis satırı basar (tur başlatmaz). Toplanma sayacının `<toplam>`'ı yalnız **kalibre** oyuncuları sayar (savaşabilirin tanımıyla aynı): kalibresiz oyuncu toplanmayı bekletmez. İstemci ucu `Modes/Tournament/TournamentRegroupReporter`'dır: taban bayrağını **kenar tetikli** yollar ve son kenarı **iki** tetikte unutup mevcut durumu tekrar yollar — yeniden bağlanma (`OnConnected`) ve `lobby_state`'te kendi satırının `ready:false` görünmesi (sunucu `hello`'da ve tur sonunda bayrağı sıfırlar; §7 "kenar tetikli bildirim"). Başlıktaki yazı iki basamağı **ayırır**: incelemede (`roundend:`) merkez uyarısı boş kalır ve durum satırı tabanda olmayanı **hemen** tabana yollar (bayraklar toplanmaya girerken zaten sıfırlanır; erken yürümek toplanmayı kısaltır, beklemek kazandırmaz); toplanmada (`regroup:`) tabana dönme yönergesi çizilir ve kapının `h/t` sayacı hem merkez satırının hem durum satırının sonunda yazılır — HUD köşesindeki faz etiketi aynı sayıyı taşır ama oyuncu köşeye bakmaz. ⚠️ `IsMatchOver`'da `TimeRemaining <= 0` dalı YOKTUR — sayaç maçın olsa da kararı hep `EndRound` verir |
| `Modes/BurgerMode` | **Vardiyanın tamamının otoritesi** (§10.5): tarif üretimi, müşteri yaşam döngüsü, sabır ve pişme sayaçları, servis doğrulaması, ortak+bireysel puan (`AddSharedScore`) ve `modeState` yazımı (`h:<mutlu>;u:<mutsuz>`; sayaç `InitialModeState` ile **sahnelemeden itibaren** sıfır değerleriyle teldedir, ilk müşteriyi beklemez). `OnTick` **yalnız `playing` fazında** iş yapar: geri sayımda müşteri gelmemeli, üstelik müşteri/ızgara/malzeme defterleri `OnMatchStart`'a kadar önceki maçın kimliklerini tutar. Müşteri **dinamik ağ nesnesidir**: `stage` (geliyor/bekliyor/mutlu/mutsuz) + `s` (`slot:<n>;r:<tarif>`) taşır, **pozu telde YOKTUR** — yürüyüşü her istemci sahnedeki yoldan aynı şekilde türetir, sunucu metre bilmez. **Tarif doğrulaması:** alt ekmek en altta, üst ekmek en üstte, **ortadaki sıra serbest**, malzeme çokluğu birebir (fazla/eksik = red) ve her köfte **pişmiş** olmalı (çiğ ya da yanmış = red). Kabul → puan + müşteri mutlu; red → `serve` olayının kendisi relay edilir (ayrı bir `rejected` mesajı yok). **Sabır bir sunucu sabitidir**: dolunca müşteri mutsuz gider, sayacı artar ve **puan cezası YOKTUR** — çocuk oyununda ceza, oynamayı bırakma sebebidir. **Yere düşen malzeme birikir**, çünkü objeleri yalnız doğru servis siler: mod canlı malzeme sayısına **tavan** koyar ve taşınca en eski **SERBEST** malzemeyi despawn eder — ⚠️ elde olana asla dokunulmaz, yani tavan bir garanti değil eğilimdir. Kuralın şekli ve varsayılan süre/limit → `Server/README.md` mod tablosu |
| `Modes/MoleMode` | **Köstebek turunun tamamının otoritesi** (§10.5): çıkış zamanlayıcısı (hangi delik, hangi renk, ne zaman), havada kalma ve ezik pencereleri, `whack` doğrulaması, takım + oyuncu puanı ve `modeState` yazımı. Delikleri **maç başında** `MatchDirector.ObjectIdsOfKind` ile öğrenir; kendi durumu delik başına (aşama, sayaç, renk, süre) tutulur — çekirdek bir objenin aşamasında ne kadar durduğunu saymaz. **Renk dengeli desteden çekilir**, çıkış başına rastgeleden değil: kısa bir turda saf rastgelelik hedeflerin çoğunu bir takıma verebilir. **Doğrulama yalnız DURUMDUR** — köstebek ayakta mı ve sayaç tutuyor mu; mesafe/fizik yargılanmaz (sunucu metre bilmez). ⚠️ **Süresi dolup inen köstebek kısa bir PAY boyunca aynı nonce ile vurulabilir kalır** (istemcideki iniş animasyonunun görünür süresi kadar; ezilip inen köstebekte pay YOKTUR, maç bitince silinir) — pay içindeki vuruş normal vuruşla birebir aynı puanı ve `2` aşamasını üretir. Bayat sallama **sessizce** düşer: ceza da yoktur, red mesajı da. ⚠️ **Payı süren delik yeni çıkış için seçilmez** — yeni nonce paydaki geç sallamayı sessizce düşürür, yükseliş de iniş ortasından başlardı. ⚠️ **Takım skoru 0'da kıstırılır, oyuncu skoru kıstırılmaz** — eksi takım skoru çocuk kitlesine anlatılamaz, ama yanlış vuranın katkısı eksiye inemezse hiç görünmezdi; iki kanal bu yüzden birbirinin toplamı değildir. **Kaçırmanın cezası YOKTUR** (vurulmayan köstebek sessizce iner): çocuk oyununda ceza, sallamayı bırakma sebebidir. ⚠️ **"Yan yana iki delikten aynı anda çıkarma" buradan YAPILAMAZ** — sunucu delik konumunu bilmez; elindeki tek kaldıraç yoğunluktur, gerisi yerleşim kararıdır. **Yoğunluk harita verisidir** (`maps.json → moleDensity`, oradan da `MapDefinition`'dan): maç başında bir kez okunur, kırpılır ve tik boyunca sabit kalır; alan yazılmamışsa modun kendi varsayılanı işler. ⚠️ **Çarpan tavandan önce ÇIKIŞ SIKLIĞINA işler** — köstebek sabit bir süre ayakta kaldığı için sahadaki sayı ≈ *ayakta kalma süresi ÷ çıkış aralığı*'dır ve tavan çoğu zaman hiç dolmaz; yalnız tavanı büyütmek sahada hiçbir şeyi değiştirmez. Bu yüzden çarpan aralığı böler, tavanı çarpar; tavanın kendisi ayrıca oyuncu sayısına bağlıdır (kalabalıkta bol, tek çocukta az). ⚠️ **`OnTick` yalnız `playing`'de iş yapar:** çekirdek geri sayımda da tick'ler ve mod örneği önceki maçın deliklerini `OnMatchStart`'a kadar tutar — kapı olmasa köstebek "başla"dan önce çıkardı. **`OnMatchEnd`'de her delik aşama `0`'a iner** (önce aşama, sonra boş yük — tersi hâlâ ayakta olan köstebeği renksiz boyardı); sayaçlar (`modeState`) korunur, bekletilen sonuç tablosunu besler. Kuralın şekli ve varsayılan süre/limit → `Server/README.md` mod tablosu |
...
| `VenueSurveyStore` | Mekan ölçümünü diske yazan statik kapı (§10.11): doğrulama (zemin köşe sayısı, kalibrasyon açıklığı, sayıların sonluluğu; eksik noktalı kolon uyarıyla atılır), dosya adının temizlenmesi ve **exe'nin yanına** `<Mekan>_dimensions.json` yazımı. ⚠️ Çıktı Unity'nin `ArenaDimensions.Parse`'ı ile okunabilir olmak zorundadır — yazılmayan `topViewHeight` düğümü de bu yüzden çıkarılır (Unity'nin kendi yazımı da onu yazmaz). Var olan dosya üzerine yazılmadan önce `.prev.json`'a kopyalanır, yazımın kendisi geçici dosya üzerinden atomiktir: yarım yazılmış bir ölçü dosyası, dosyanın hiç olmamasından kötüdür. ⚠️ Sunucu dosyayı **bir daha okumaz** — ölçünün tüketicisi Unity'dir |
| `ServiceShutdown` (internal) | Dört uzun ömürlü servisin ortak kapanış adımı: iptal edilen döngüyü **tek ortak zaman aşımıyla** bekler, süre dolarsa `[kapanış] <servis> … durmadı — zorla.` satırını basıp geçer, bittiyse görevi `await` ederek istisnasını gözlemler (gözlemlenmemiş görev bırakmaz). Servis başına tavan Windows'un konsol kapatma işleyicisine tanıdığı süreden türetilmiştir |

**`StopAsync()` sözleşmesi** (`BeaconService` · `StateHost` · `LobbyService` · `MatchDirector`):
**iptal → döngünün bitmesini BEKLE (`ServiceShutdown`) → sonra kaynağı kapat** (UDP soketi, timer) →
`CancellationTokenSource.Dispose()`. Alanlar önce yerelde yakalanıp `null`lanır, böylece ikinci çağrı
no-op'tur. `MatchDirector` drain'den sonra bekleyen çıktısını (`_pendingOutbox`) kendi kilidi altında
atar — kapanışta yayın yoktur. Sıra tek yerdedir (`Program.ShutdownAsync`): lobi → director →
stateHost → beacon → control → registry; her üretici, yazdığı kanal kapanmadan önce susturulur ve
**"Kapandı." son satırdır**. Kapanışı üç yol tetikler ve üçü de aynı idempotent yolu çağırır: Ctrl+C,
konsol penceresinin kapatılması ve süreç çıkışı.
...
`sceneId` atar. Prefabdan gelen objede bileşen prefabda durur, `sceneId` 0 kalır; sahnedeki her kopya
kendi kimliğini **prefab override** olarak alır — sahneyi kaydetmek yeter. Sahne kaydedilirken ve
Play'e girerken `SceneIdGuard` 0 kalan/çakışan id'leri onarır (`Ctrl+D` ile çoğaltılan obje,
kopyalanan sahne); Play öncesi onarım yalnız bellektedir, sunucu için sahne yine kaydedilir. Sahne
dosyası merge/kopya ile değiştiyse `Configure All Build Elements` → *Ağ nesneleri* → **Onar** tüm
arena sahnelerini açıp onarır. ⚠️ `sceneId` override'ı prefaba **Apply edilmez** (`Yapma-Listesi`).
Bu altyapı dinamik obje senkronu (kapı, pickup) içindir; oyuncular `playerId` ile senkronlanır,
`NetIdentity` gerekmez.
...
3. **Rig'i/kamerayı asla taşıma** — free-roam'da oyuncu fiziksel; canlanma durum değişimidir.
   **Tek istisna `ArenaCalibrator.SetFloorLift` ile uygulanan DİKEY sanal ofsettir** (kat geçişi,
   §3.13): başka hiçbir kod rig kökünü oynatmaz ve yatayda hiçbir kod oynatmaz.
...
17. **Serialize edilen enum'a yeni değer SONA eklenir.** Unity enum'ları sayısal indeksle saklar:
    ...
    Eşyanın üç ekseni de (`ItemGrabPath`/`ItemInstancing`/`ItemReleaseMode`) aynı kuraldadır ve
    **0. indeksleri bugünkü davranıştır** (`DistanceGrab`/`PerViewerClone`/`Return`): alanlar var
    olmadan yazılmış her asset `0` okuyor, sıra bozulursa arsenalin tamamı sessizce başka bir şeye
    döner.
...
54. **Sıfır quaternion telde meşru görünür, Unity'de geçersizdir.** Dört sıfır bayt geçerli bir
    ...
    ⚠️ Telin kendisi bu vektörü artık kapatır: poz bloğu tamsayı kodludur ve çözümü tanım gereği
    birim quaternion üretir (NaN/∞ da taşıyamaz), iskelet kökü ise quaternion değil yalnız yaw
    taşır (§6.9) — alıcıdaki denetim yine KALIR, çünkü aynı Transform zincirine teli hiç görmeyen
    pozlar da giriyor (yerel sensör, editör araçları). ⚠️ Sunucu poz İÇERİĞİNİ doğrulamaz ve
    doğrulamayacak (istemci-otoriter, §3.2) — denetim çizen tarafın işidir.
...
56. **Movement SDK retarget avatarı hareket eden bir kökün altına KONMAZ — çıktı dünya
    ...
    **Kural bugün de bağlayıcıdır ve tasarımın kendisi oldu:** karakter — yerel de uzak da — sahne
    ...
    KENDİ kök eklemi **kullanılmaz** (gönderenin dünya uzayındadır; telde yalnız yerel rotasyonu durur) — kök arena
...
57. **"Mesajı yolla, sonra soketi kopar" mesajı YOLLAMAMAKLA aynı şey olabilir.** Atma yolu
    ...
    `HEARTBEAT_TIMEOUT` temizliği) — yoksa istemci kendini atılmış sanıp uygulamayı kapatır.
...
59. **Uzak sunum davranışı eşya prefabına BİLEŞEN eklenerek yazılmaz — `SterilizeVisual` tüm
    ...
    tarafında durur.** Eşyanın takım rengi de böyledir: boyanacak parçalar prefabda bileşen
    ...
    başına bilinçli bir karardır. ⚠️ İkinci yarısı da aynı derste: "yerelde görünen ama uzakta görünmeyen"
...
91. **Gövde ORANINI değiştirmek ağ formatını bozar; BOYU taşımanın yolu ayrı bir alandır.**
    ...
    tele yalnız eklem rotasyonu + kalça konumu gider ve alıcı prefabın **bind uzunluklarıyla** çizer
    (ArenaNet-Protokol §6.9), yani değişen oranlara göre çözülmüş rotasyon ve kalça yüksekliği uzak
    gövdeye oturmaz ve uzak avatar **bozuk duruşlara** girer. Teşhisi zorlaştıran şey, arızanın yalnız KARŞI
...
92. **Ağa giden iskelet kemiklerin CANLI Unity transformlarından okunur — ama yalnız rotasyon +
    kalça; ölçek ve kemik uzunluğu tele GİTMEZ.** Blob'u VortexArena yazar (`SkeletonWire`,
    ArenaNet-Protokol §6.9): gönderim kancasında `JointPairs` eklemlerinin `localRotation`'ı ve
    kalçanın `localPosition`'ı okunur, alıcı kendi bind uzunluklarıyla çizer. SDK'nın kendi
    serileştirme yolu (`NetworkCharacterHandler` → `GetCurrentBodyPose` → `SkeletonJobs.GetPoseJob`,
    `localScale` dahil) **hâlâ derleniyor ve koşuyor ama sonucu atılır, tele girmez** — onu yeniden
    tele bağlayan bir değişiklik kemik ölçeğini de geri taşır. Sonuç: gövde üstünde yapılan her
    "yalnız görsel" kemik hilesinin rotasyon ve kalça bileşeni **telde gider** — bir kemiği döndürmek
    ya da kalçayı kaydırmak başkalarının gördüğü gövdeyi değiştirir. ⚠️ Teşhisi zorlaştıran şey,
    hatanın **kendini gizleyen** tarafta hiç görünmemesidir: gizleyen oyuncu o uzvu zaten görmüyordur.
    **Kural: gizlemeyi kemik transformuyla YAPMA** (ölçek dahil) — görünürlük `Renderer.enabled` ile
    yönetilir, o transformlara hiç dokunmaz ve telde tam gövde gider. Kemik transformuna yazmak zorunda kalınırsa yazı gönderimden
...
96. **Retarget edilen bir transforma `+=` ile ofset yazmak, ancak SDK'nın o kare pozu GERÇEKTEN
    ...
    sınıfın geri kalanıyla **aynı kapıdan** geçer (`RetargeterValid`). ⚠️ `AppliedPose` bir **latch
    değildir**: yalnız retarget hesabı koştuğunda yazılır, kaynak geçersizken hesap hiç koşmadığı
    için bayrak **son değerinde kalır** — "bir kez doğru olduysa bir daha yanlışa dönmez" diye
    okunamaz.
...
101. **Gövde oranı kalibrasyonu YERELDİR ama sonucu TELDEN gider — bu yüzden varsayılan
    KAPALIDIR.** Tele eklem rotasyonu + kalça konumu gider ve alıcı prefabın bind uzunluklarıyla
    çizer: gönderen `CharacterRetargeter.Calibrate()` ile kendi gövde oranlarını değiştirdiğinde
    rotasyon ve kalça yüksekliği o oranlara göre çözülür, uzak gövdenin uzunluklarıyla uyuşmaz ve
    uzak avatar bozuk duruşlara girer. ⚠️ Anahtarın **yerel görünümle ilgisi yoktur**
    ...
    tele giden veri o ayarın sonucunu taşıyorsa o ayar artık yerel değildir.
...
116. **İkinci bir görsel temsil, ikinci bir ağ tüketicisiyle sürülmez — uygulanmış sonucu oku.**
    Aynı gövdeyi ikinci bir modelle çizmek gerektiğinde ilk refleks ikinci bir retargeter ya da
    ikinci bir iskelet okuyucusu kurmaktır; oysa `RemoteSkeletonRegistry` halkasındaki kare bir
    modelin `JointPairs` **indeksleriyle** yazılmıştır — ikinci model kendi eşleşmesini, bind
    uzunluklarını ve retarget config'ini ister, interpolasyon ve ölü-akış kararı da ikinci kez
    verilir ve iki gövde birbirinden ayrışabilir. Doğru hamle **zaten uygulanmış olan sonucu okumaktır**: ikinci model
...
124. **`OVRCameraRig` el anchor'larını KOŞULSUZ yazar — anchor'dan okunan poz "geçerli" demek
    ...
    ⚠️ Belirti bir **paket/ofset** sorunu gibi okunur ama değildir: `0x01` sabit 41 B'dir, alan
...
138. **İki ayrı SENSÖRDEN doğan iki poz, aynı uzayda olsalar bile aynı yeri göstermez — çizimde
    ...
    **kumandanın** pozu, çizilen bilek ise iskelet blob'undaki **body tracking** çözümü. İkisi de
    ...
    (`Calibrate()` uzak gövdenin bind uzunluklarıyla uyuşmazdı, §10.8), yani model prefab oranlarını
...
146. **Movement SDK retargeter'ında `ApplyRootScale` KAPALI kalır — açıkken karakter kökünün
    ...
    **Belirtinin imzası:** kafa/eller doğru yerdedir (ikisi de rig'in ÇOCUĞUDUR, kalibrasyonu
    ...
    kutuları gövdeyle birlikte gittiği için kimse kimseyi vuramaz. Teşhis tek ölçüdür:
    `RemoteSkeletonRegistry.TryGetInterpolatedRoot`'un döndürdüğü kök OFSETİ okunur (kök telde
    kafaya göre ofset taşır, §6.9). Sağlıklı ofset kafa-kalça izdüşüm farkıdır — küçüktür ve
    arenanın her yerinde benzer; ölçek açıkken ofset oyuncunun ORİJİNDEN UZAKLIĞIYLA orantılı
    büyür (x ve z'de aynı katsayı, oyuncu orijinden uzaklaştıkça artar). Ofsetin konuma oranı
    `1/rootScale − 1`'dir; buradan `rootScale` çözülür.
...
181. **Kafayı takip eden bir overlay, önünde açılan dünya arayüzünün ÜSTÜNÜ kapatır — açan taraf
    ...
    **sunumu** susturur — `ArenaClient` denemeyi sürdürür; grace saati bastırma boyunca durur,
    ...
    ⚠️ **Alan dışı bir bastırma sebebi DEĞİLDİR:** karartmanın içinde kalan oyuncu bağlantısının
    koptuğunu da bilmek zorundadır. Çakışma gizlemeyle değil çizim sırasıyla çözülür — kart kuyruk
    3000 içinde `sortingOrder = 10` ile karartma quad'ının üstündedir, kırmızı karartma kartın
    çevresinde baskın kalır.
...
183. **Döngüyü BEKLEMEDEN kaynak kapatma.** `Cancel()` çağırmak döngünün bittiği anlamına gelmez:
    `ReceiveAsync`/`WaitForNextTickAsync` içindeki bir döngünün altından soketi ya da timer'ı çekmek
    ya `ObjectDisposedException` üretir ya da "Kapandı." satırından **sonra** bir tik daha koşup
    konsola yazar; beklenmeyen görevin iptal istisnası da gözlemlenmemiş görev olarak kalır. Kural:
    **iptal → drain (zaman aşımlı) → kaynağı kapat → CTS dispose** (§4 `StopAsync` sözleşmesi).
    Kapanışın tetikleyicisi de tek değildir — Ctrl+C, konsol penceresinin kapatılması ve süreç
    çıkışı aynı **idempotent** yolu çağırır; pencere kapanışını yakalayan P/Invoke delegate'i
    **statik alanda** tutulur, yoksa GC onu toplar ve handler tam kapanış anında çöker.
184. **Atılabilir avatarla ÇARPIŞMAZ, ve bu bir denge kararı değil determinizm şartıdır.** Atma
    olayı telde yalnız yön + ilk hızdır (çıkış noktası bile yok); her istemci kendi kopyasını
    simüle eder. Uzak gövdeler her ekranda interpolasyondan dolayı farklı yerdedir, dolayısıyla
    bir avatara çarpan kopya ötekilerden ayrılır ve bomba başka yerde patlar. Kural: atılabilir
    **yalnız statik geometriyle** çarpışır (avatar collider'ları uçuş boyunca muaf tutulur), telde
    olmayan her başlangıç büyüklüğü (dönüş, açısal hız) **yönden deterministik türetilir**, ve
    **atan da kendi niceliklenmiş değerlerinden** simüle eder — yoksa tek ayrıksı kopya atanınkidir.
    Sekme katsayısı da bu yüzden düşük tutulur: ayrışma sekme sayısıyla büyür. Ama SIFIR değil —
    sıfır sekmeli küre yere yapışıp fitile kadar yuvarlanır; `PM_Bomba` düşük sekme + `Maximum`
    birleşim (zemin materyali ne olursa olsun bir kez seker, ikinci sekme `bounceThreshold`'un
    altında kalır) ve ilk temasta yükselen sönüm (`Throwable`) onu kısa sürede durdurur.
185. **Yön, KONUM kapısından geçirilmez.** `ArenaSpace` bugün özdeş dönüşüm döndürüyor, bu yüzden
    bir yönü `ArenaToWorld(Vector3)`'ten geçirmek hata vermez — dönüşüm bir gün gerçekleştiğinde
    yöne ötelenme uygulanır ve **hiçbir şey bağırmaz**. Yönün kendi kapısı vardır
    (`WorldToArenaDirection` / `ArenaToWorldDirection`) ve normalleştirme sözleşmesi de oradadır.
186. **Ağ nesnesi sunucuya EXPORT'la ulaşır; unutulursa sessizce kırılmaz olur.** Sahneye `NetObject`
    koymak yetmez: kimlik listesi sahne KAYDINDA `Data/<SahneAdı>_objects.json`'a, oradan da
    `Export Server Config` ile `maps.json`'a girer. Sahne kaydedilmemişse ya da export koşulmamışsa
    sunucu o `netId`'yi tanımaz, `hit_report` sessizce reddedilir ve sahadaki belirti "mermi
    geçiyor ama obje kırılmıyor" olur — yani sorunun kaynağı silahta aranır. Aynı sebeple bir
    sahnenin `objects[]`'inde geçip `kinds[]`'te bulunmayan tür de aynı belirtiyi verir; hazırlık
    paneli ikisini de listeler.
187. **ISDK'da "alınamaz" bir FİLTREYLE ifade edilmez, bileşenin YOKLUĞUYLA ifade edilir.** Aday
    listesini kapatmak objeyi alınamaz yapmaz: interactor hover'a `HasCandidate ||
    ComputeShouldSelect` ile girer, yani **boş aday listesiyle bile** hover'dadır ve `Select()`
    kavrama basışını hiçbir şey seçmeden **kuyruktan düşürür**. Basış sessizce yenir; oyuncunun
    gördüğü "kavrama tuşu bazen çalışmıyor"dur ve sebebi kavranmak istenen objede değil,
    yakınlardaki başka bir objededir. Doğrusu prefabda mesafeli kavrama bileşenini
    **bulundurmamaktır**; kural eşyanın alma yolu ekseninde durur ve `ItemGrabPathGuard` ikisini
    karşılaştırır. `WeaponFrame`'in filtresi bu kuralın istisnası değildir — o bir MESAFE kapısıdır,
    "alınabilir mi" kapısı değil.
188. **Tutulan bir ağ nesnesi eşya baytını KULLANMAZ.** Aynı obje hem `itemL`/`itemR` baytıyla hem
    obje durumuyla taşınırsa uzak elde **iki kopya** çizilir: biri ağ nesnesinin kendi örneği, biri
    baytdan katalogdan üretilmiş klon. İkisi aynı yerde başlar, gecikmede ayrışır ve belirti "elinde
    iki bıçak var" değil çoğu zaman "bıçak elinin yanında titriyor" olur. Bayt **kaynağında**
    bastırılır (`HeldItems` slotu, `ItemDefinition.IsWorldSingle`) — her tüketiciye ayrı ayrı
    eklenen bir istisna, ilk unutulduğu yerde geri gelir.
189. **TMP yedek fontunun glif tablosu bir ÖNBELLEKTİR, içerik değil — merge'de elle birleştirilmez.**
    Yedek font asset'i `Dynamic` doldurma kipindedir (`m_AtlasPopulationMode: 1`): eksik karakteri
    çalışma anında kaynak `.ttf`'den kendisi üretir, tablodaki glifler yalnız o ana kadar üretilmiş
    olanlardır ve `m_ClearDynamicDataOnBuild` build'de zaten hepsini siler. Bu yüzden dosya iki
    geliştiricinin editöründe **farklı** kaydolur ve merge'de çakışır. ⚠️ İki tarafı birleştirmek
    hem gereksizdir hem tehlikelidir: glif dikdörtgenleri iki ayrı atlas üretimine bakar, karışığı
    yanlış yerden harf çizer. **Çakışan yedek font asset'inde bir taraf BÜTÜN olarak alınır** (dosya
    kendi atlasını da taşıdığı için tek taraf her zaman kendi içinde tutarlıdır). Aynı sebeple
    tabloda karakter sayısının azalması "karakter kayboldu" demek değildir. ⚠️ Hangi asset'in
    kullanıldığı `TMP Settings`'in yedek listesinden okunur — TMP'nin kendi
    `TextMesh Pro/Resources/` klasöründe duran bir kopya **kullanılıyor demek değildir**; kullanılmayan
    ikinci kopya yalnız çakışma ve "merge conflicts" hatası üretir.
...
191. **Lobi profili tek değildir: `kids` haritası sahnelenince SİLAHSIZ profil geçerlidir.**
    Normal lobi profili grip'e basana rastgele silah verir; çocuk oyununda bekleme süresinde de
    silah olmamalıdır. Kararın **tek noktası** `MatchDirector.LobbyProfileForSceneLocked`'dir —
    açık sahnenin ailesi profili belirler. Sahne ve kural yazan yeni bir yol açılırsa (yeni bir
    sahneleme/lobiye dönüş kapısı) profili oradan almak zorundadır; `ModeRules.LobbyProfile`'ı
    doğrudan uygulayan bir satır çocuk haritasında sessizce silah dağıtır.
192. **İç engel cezası `weaponSource:"none"` modlarda KAPALIDIR.** O modlarda canlanma yoktur
    (`reviveAnchor:"none"`), yani banko/ızgara gibi `Obstacle` katmanındaki bir geometriye kafasını
    uzatan oyuncu ölür ve **vardiya sonuna kadar ölü kalır**. Kapı `TickObstacleLocked`'in tek
    koşulundadır; karartma, uyarı ve admin halkası kalır — operatör ihlali görmeye devam eder,
    yalnız can gitmez.
193. **Ölçümün referansı ÖLÇÜLENE bağlı olamaz — takibin sürdüğü karakter, sürüldüğü şeyi
    ölçemez.** Gövde ölçeği "oyuncunun gözü / karakterin gözü" diye yazılmıştı ve karakterin
    kafasını retargeter oyuncunun kafasına sürdüğü için oran gözlüğü kim takarsa taksın `1` çıkar;
    belirti "ölçüm biraz hassasiyetsiz" gibi görünür (`0.99–1.01`), "ölçüm hiçbir şey ölçmüyor" gibi
    değil, yani teşhis edilmez. Referans, ölçülen şeyle birlikte hareket etmeyen bir sabit
...
194. **Transform'la sürülen Rigidbody'de interpolasyon KAPALI olmalıdır; taşınırken kapatılan
    ...
    belirti "gösterge küresi yerinde ama eşya elin çevresinde sağa sola kayıyor, kafa dönünce yer
    değiştiriyor" olur ve **yalnız yerel oyuncuda** görünür — uzak taraf prefabın Rigidbody'sini
    zaten siler (`RemoteAvatar.SterilizeVisual`), yani admin ekranı sorunu hiç göstermez. Kural
    `RigidbodyDrive.SetKinematic`'tedir: bayrakla interpolasyon birlikte çevrilir, `isKinematic`
    elle yazılmaz. İkinci yüzü: taşınırken kapatılan `detectCollisions` fırlatmada geri açılmazsa
    bomba **yerin içinden geçer** ve yalnız fitil onu bitirir — `detectCollisions` `isKinematic`'ten
    **bağımsızdır**, gövdeyi dinamiğe döndürmek onu geri açmaz ve collider'ların açık olması hiçbir
    şey değiştirmez: temas hiç üretilmez, hata da verilmez. Bu yüzden `Throwable.Arm` gövdeyi olduğu
    gibi kabul etmez, ihtiyacı olan her alanı (kinematik, çarpışma, interpolasyon, yerçekimi) kendi
    yazar — uzak kopya taze prefabdan doğduğu için bu da yalnız atanın kopyasında görülür.
195. **Sessiz Wi-Fi ölümünü TCP fark etmez; ölü bağlantıyı istemcinin kendisi düşürür.** Gözlüğün
    Wi-Fi'ı kesilince sokete RST gelmez: `status` gönderimleri TCP'nin yeniden gönderim kuyruğunda
    "başarıyla" beklerken `ReceiveAsync` sonsuza dek bekler, hiçbir hata oluşmaz ve sonsuz
    yeniden bağlanma döngüsü **hiç başlamaz** — belirti "Wi-Fi geri gelince de bağlanmıyor,
    uygulamayı kapatıp açmak gerekiyor"dur. Sunucu tarafı bunu görmez (o `HEARTBEAT_TIMEOUT` ile
    zaten düşürür), yani hata yalnız gözlükte yaşar. Savunma `ArenaClient.LinkWatchdogAsync`'tir:
    sunucu her `status`'a `heartbeat` yollar, `HEARTBEAT_TIMEOUT` boyunca hiçbir WS çerçevesi
    gelmezse istemci soketi kendisi düşürür (§8). Aynı hastalığın ikinci yüzü **bağlanma
    denemesidir:** ağ yokken `ConnectAsync` ne iptal belirtecini ne `Abort`'u dinler, deneme askıda
    kalır ve döngü **1. denemede donar** — belirtisi ekranda saniye sayacının artıp deneme sayacının
    1'de kalmasıdır. Bu yüzden hem bağlanma hem gönderim `ArenaClient.AwaitBoundedAsync` ile bir
    zamanlayıcıya karşı yarıştırılır: süre (`CONNECT_TIMEOUT` / `SEND_TIMEOUT`) dolunca işlem terk
    edilir, soket `Abort` edilir ve döngü bir sonraki denemeye geçer.
196. **Kenar tetikli bildirim yeniden bağlanmada TEKRAR yollanır.** Sunucu kopuşta ve `hello`'da
    `ready`'yi sıfırlar; "değişince yolla" diye yazılmış istemci ise son kenarı hatırlar ve tekrar
    yollamaz. Tabanında dururken Wi-Fi'ı kopup gelen oyuncu sunucuda süresiz "tabanına dönmedi"
    görünür, toplanma geri sayımı hiç başlamaz ve operatör tüm oyuncuları tabanda görürken sebebi
    hiçbir ekranda okuyamaz. Kural: kenar tetikli her rapor `NetEvents.OnConnected`'da son kenarı
    unutur (`TournamentRegroupReporter.HandleConnected`); `CalibrationState` ve `SceneRouter`
    aynı kalıbı zaten uygular. İkinci tetik roster'ın kendisidir: `lobby_state`'te **kendi** satırı
    `ready:false` görünüyorsa istemci kenarı yeniden yollar — kopuş dışında (tur sonunda yapılan
    temizlik) da bayrak sunucuda düşer ve `welcome` gelmez.
197. **ProBuilder runtime'ı build'e YALNIZ `VortexArena.App/Survey` üzerinden girer — `VortexArena.Core`'a
    referans eklenmez.** Ölçüm rehberi (küpler, zemin+duvar hacmi, kolon prizmaları) sahada çalışma
    anında üretilmek zorundadır, yani App asmdef'i `Unity.ProBuilder`'ı referanslar ve paket APK'ya
    girer. Buradan "madem zaten giriyor, Core da referanslasın" **çıkmaz**: Core her sahnede, her
    modda, admin build'inde de derlenir ve arena sahnelerindeki ölçü maketi (`ArenaDimensionMesh`)
    sanat değil **ölçü referansıdır** — o yüzden `DimensionMeshBuildStripper` maketin görsel dalını
    build'e giden sahne kopyasından ayıklamaya devam eder. Referansı Core'a taşımak sahada hiç
    çizilmeyen bir mesh ailesini kalıcı hâle getirir ve tek yönlü katman kuralını (Core, App'i
    bilmez) tersinden delerdi.
198. **Ölçüm sahnesi BOŞTUR — içine elle bileşen konmaz.** `VenueSurvey` sahnesinde yalnız rig, ışık
    ve zemin durur; denetleyiciyi, rehber geometriyi ve etiketi sahne yüklendiğinde `VenueSurveyGesture`
    kodla kurar. Sahneye konan bir bileşen seri alan ister, seri alan referans ister ve o referans
    prefab/sahne düzenlemesiyle bakılan bir bağ olurdu: sahne mekana ait değildir, `Configure All
    Build Elements` ona dokunmaz ve arena kurulum reçetelerinin hiçbiri onu kapsamaz — yani bozulduğu
    gün kimsenin bakmayı düşüneceği bir yer değildir. Kodla kurulan bir sahnenin bozulma biçimi
    derleme hatasıdır, sessiz bir boş referans değil.

199. **Bir sensör SDK'sının "geçerli" bayrağı "taze" demek DEĞİLDİR — donmuş veri geçerli
    raporlanır.** `OVRBody`'de `GetBodyState4` başarılı olduğu sürece geçerlilik bayrağı **koşulsuz**
    yazılır: güven değeri hesaba girmez ve **geçersiz bir eklem önceki karenin değerini korur**. Yani
    kaynak susmadan donabilir ve bayrağa bakan her kapı kapalı kalır. Belirtisi teşhis edilemez bir
    ayrımdır: aynı arıza bir yoldan (kaynağı gerçekten düşüren bir kesinti) beklenen yedeği tetiklerken
    başka bir yoldan (çalışma zamanı cevap vermeye devam ederken izlemenin kapatılması) hiçbir şey
    tetiklemez — "bazen çalışıyor" diye raporlanır. Kural: **tazelik, verinin KENDİSİNDEN ölçülür**;
    kaynağın beyanından değil. Ölçüm de verinin donmayan bir bileşeninden yapılamaz — burada kök,
    HMD'yi izlediği için izleme donsa bile oynamaya devam eder ve tam da aranan arızayı gizler; ölçüm
    eklem-yerel poz üzerindedir. Aynı kural bayat kare sayacına da işler: "kare gelmiyor" ile "aynı
    kare geliyor" iki ayrı arızadır ve ikincisini yalnız içerik karşılaştırması yakalar.

200. **Kendi thread'inde soket kapatma: Mono'da `ClientWebSocket`'in `Abort`/`Dispose`'u ve
    `ConnectAsync`/`CloseAsync`'in SENKRON başlangıcı ağ yokken BLOKLAR.** Bir zaman aşımı yarışı bu
    çağrıların hiçbirini kurtarmaz, çünkü sayaç ancak `await`'e girildikten sonra işler: senkron
    başlangıç ondan öncedir, `Dispose` ise `using` bloğundan çıkarken zaman aşımı fırlatılırken
    koşar. Sonuç, döngünün **tek bir denemenin içinde sonsuza kadar parklanmasıdır** ve dışarıdan
    görünen tek belirti sayacın ilerlememesidir: geçen süre artmaya devam eder, "kaçıncı deneme"
    sabit kalır, kullanıcı uygulamayı yeniden başlatana kadar dönmez. Kural: bağlantı döngüsünde
    `using` kullanma, terk etmeyi ayrı bir thread'e bırak, engelleyebilecek her çağrıyı `LongRunning`
    bir görevde başlat. Aynı tuzak ana thread'de daha beter: `Disconnect()` içinden çağrılan bir
    `Abort` Unity'nin kendisini dondurur. **Teşhis kuralı:** "süre artıyor ama deneme sayacı
    artmıyor" bir ağ sorunu değil, **bloklanmış bir thread** işaretidir.

201. **Bir SDK'nın "geçerli" bayrağı "izinli" demek de DEĞİLDİR — işletim sistemi izni bayrağın
    ÖNÜNDE sorulur.** 199'un ikinci yüzüdür: orada bayrak *taze* olmayı, burada *izinli* olmayı
    garanti etmiyor. Gövde takibi izni verilmemiş bir başlıkta çalışma zamanı elindeki iskeleti
    tutmaya devam eder ve geçerlilik bayrağı `true` kalır; bayrağa önce bakan bir bekçi o başlığı
    **sağlıklı** sayar, yani yedek tam da en çok gerektiği durumda hiç kurulmaz. Belirti yerelde
    hiç yoktur (eller rig'den geliyor) ve uzakta "bazı gözlüklerde gövde gelmiyor" diye okunur.
    Kural: izin, sağlık kapısının **ilk** koşuludur — sonra bakılan bir izin hiç bakılmamış izindir.
    İkinci yarısı onarımdır: izin reddi bir arıza değil bir **karardır**, kaynağı yeniden
    başlatmak onu döndüremez; denemek yalnız gerçek bir izleme kesintisi ödetir. İzinsizken
    yapılacak tek şey yedeği silahlı tutmak ve sebebi hata satırına yazmaktır.
202. **Sahne geçişini çizen quad SAHNENİNDİR — sürücüsü kalıcı olsa da o `DontDestroyOnLoad`
    kökün altına ALINMAZ.** Karartmayı süren tekil oturum boyu yaşar, çizdiği quad ise rig
    prefabının parçasıdır ve sahneyle ölür; sürücü her `sceneLoaded`'da yenisini bulur. Quad'ı
    kalıcı kökün altına taşımak iki arıza doğurur: (1) sahneyle ölmediği için yeni sahnede
    **elle gizlenmesi** gerekir — unutulduğu an oyuncu siyah ekranda kalır ve hiçbir hata basılmaz;
    (2) eski sahnede duran quad taşındığı için **inişi çizecek yüzey kalmaz**, yani karartma tam da
    yükleme başlarken kaybolur. Kural: ömrü sahneye bağlı olan **çizim yüzeyi** sahnede kalır,
    kalıcı olan yalnız onu **süren** taraftır.

203. **`_objectsToHideUntilValid` uzak gövdede nesneyi kalıcı gizler — avatar prefabının
    retargeter'ında bu liste BOŞ kalır.** `NetworkCharacterRetargeter` listedeki nesneleri ilk
    geçerli poza kadar gizler; görünür yapan SDK'nın `ReceiveData` yoludur. Uzak gövdede o yol hiç
    çağrılmaz — kemikleri `ArenaNetCharacterBehaviour` `SkeletonWire` karesinden yazar
    (ArenaNet-Protokol §6.9) — yani listeye konan nesne uzak ekranlarda **hiç görünmez** ve hata
    basılmaz; yerel gövde zaten çizilmediği için sahibinin ekranında da iz yoktur. `SkeletonStreamGuard`
    listeyi iki prefabda boş ister. Yüz/şekil verisi (`_shapePoseData`) de tele girmez.

204. **SDK gönderende eklem KONUMLARINI oyuncunun gerçek oranlarıyla yazar; kalça telde oran uzayına
    bölünmeden gönderilirse boy iki kez ölçeklenir.** Meta retargeter'ı "Source Body Proportions" ile
    çözdüğü için kalçanın `localPosition`'ı gerçek metredir, prefab oranı değil; alıcı ise aynı
    `bodyScale`'i kök transforma uygular. Bölmeden göndermek kısa oyuncuyu daha kısa, uzun oyuncuyu
    daha uzun çizer ve ayakları batırır/havada bırakır. Kalça gönderende `bodyScale`'e bölünür, ayak
    yüksekliği ayrı gerçek-metre alanıyla (`footY`) gider ve alıcı kalçayı öteleyerek ayağı oraya
    oturtur — hedef oyuncunun zemini + taban payından aşağı inmez.
205. **Üst kat plakası `Obstacle` layer'ına KONMAZ, ama collider'ı `Default`'ta KALIR.** İki yarısı
    ayrı ayrı bağlayıcıdır: `Obstacle` "kafa girerse ceza" sözleşmesidir
    (`ObstacleViolationProbe`) — plaka oraya damgalanırsa üst katta duran **herkes** sürekli
    ihlalde sayılır ve ekranı kararır. Collider'ı sökmek ya da plakasız bir üst kat kurmak ise ters
    yönden kırar: atış izi maskesizdir (`ArenaCombat.TraceShot`), yani katlar arasında hiçbir şey
    durmaz ve alt kattaki oyuncu üst kattakini tavanın içinden vurur.
206. **Kat yüksekliği hiçbir yere elle yazılmaz — onu portal TANIMLAR.** `k`. katın zemini, o kata
    çıkan `FloorPortal`'ın `upperHeight`'ının söylediği yüksekliktedir (`ArenaFloors`, §3.13);
    ikinci bir sayı listesi mesh'ten sessizce sapar ve oyuncu altında zemin olmayan bir yüksekliğe
    kaldırılır. Bunun yükümlülüğü yerleştirmededir: portalın kökü kendi katının zeminine 0,25 m
    içinde oturmalıdır, yoksa portal kat listesine girmez ve **kendini kapatır** (konsolda uyarı).
207. **Kat portalı çemberlerine collider KONMAZ.** Kapıyı kafanın çember merkezine XZ mesafesi
    çözüyor; konan bir collider atış izini (maskesiz) ve engel/kavrama ölçümlerini yalanlar — geçiş
    noktası mermi yiyen görünmez bir disk olurdu.
208. **Üst kat plakası admin kuş bakışını kapatır.** Plaka `ArenaRoof` kökünün altına alınmazsa
    operatör tepeden yalnız üst katı görür, alt kattaki hiç kimseyi göremez — çatı gizleme yolu
    zaten vardır (§4 `ArenaRoof`), ikinci bir gizleme mekanizması yazılmaz.
209. **Alıcıda iskelet köküne kat yüksekliği EKLENMEZ.** Telin `dy`'si kökün **mutlak** arena
    yüksekliğidir (kafa izdüşümünün y'si 0, ArenaNet-Protokol §6.9), yani kat bilgisi zaten
    içindedir; üstüne `ArenaFloors.HeightOf(kat)` eklemek üst kattaki gövdeyi **bir kat daha**
    yukarıda çizer — tavanın içinde yüzen bir rakip, hata basılmadan. Kat defteri yalnız iskelet
    akışı yokken kullanılan yedek yola aittir.
210. **İki kat portalının diskleri aynı X/Z'ye KONMAZ.** Geçiş oyuncuyu yatayda kıpırdatmadığı için
    üst kata çıkan kafa doğrudan diğer portalın dolum çemberinin içine iner: portal geçiş sonrası
    kendini kurulmamış yapsa bile oyuncu diskten çıkmadıkça kat değiştirmeye devam eden bir zincire
    girer. `ArenaFloors` çakışan diskleri uyarır, şablon aracı portalları X'te 2 m aralıkla dizer.
211. **Kat değişiminde portal kurulu KALMAZ.** Herhangi bir kat değişimi (kendi geçişi, başka portal,
    ölüm inişi, sunucunun sıfırlaması) portalı kurulmamış yapar ve dolumu sıfırlar; yeniden kurulma
    ancak kafa diskten çıkınca gelir — yoksa yeni katta diskin içinde duran oyuncu anında geri
    ışınlanır. Aynı sebeple geçiş gerçekleşmediğinde (`FloorState.MoveWithFade` `false`) ses çalmaz ve
    portal harcanmaz.
212. **Arena dekoru static flag'siz bırakılmaz.** İşaretsiz renderer üç şeyden birden düşer: static
    batching'e girmez (her obje ayrı draw call — Quest'in tile tabanlı GPU'sunda draw call sayısı
    doğrudan kare süresidir), occlusion culling'e girmez, lightmap almaz (bake koşsa bile sonuç
    hiçbir renderer'a yazılmaz; sahne klasöründeki lightmap dosyaları yanıltır). Hata yoktur, sahne
    yalnız Quest'te kasar. Hareketsiz dekora Contribute GI + Batching + Occludee + Reflection Probe;
    **Occluder yalnız kapalı yüzeye** (duvar, taş, prop) — yapraklı vegetasyon ve çim örtmez, Umbra
    bake'ini şişirir. `Animator`/`Rigidbody` altındaki obje işaretlenmez: batching transform'u
    dondurur, obje görünürde kımıldamaz. Rüzgâr shader'ı dünya konumu + vertex renginden
    hesaplıyorsa batching güvenlidir; pivot/object-space'ten hesaplayan shader'da `BatchingStatic`
    verilmez (batch sonrası object-space = world-space, eğim yanlış noktadan başlar). Denetim:
    `SceneBudgetWindow` (§4) — "static flag yok".
213. **Rig'in el küreleri editörde çizilir, Quest'te çizilmez.** `VA_CameraRig` içindeki Meta
    `HandSphereMap` prefabı parmak başına `sphere` MeshRenderer'ları taşır (yüzlerce, `DefaultMaterial`,
    gölge açık); `HandSphereMap.Start()` bunları okuyup `SetActive(false)` yapar. Editör
    istatistiğine ve sahne ölçümüne girer, çalışma anına girmez — bu yüzden `SceneBudgetWindow`
    kamera taşıyan kökleri saymaz. Renderer'ını kapatmak zararsız ama kazançsızdır; **GameObject'i
    kapatılmaz** — script yalnız `activeSelf` küreleri okur, kapatılan küre kavrama haritasından düşer.
214. **Servis tahtasını yuvasına bağlayan tek şey KONUMUDUR.** `BurgerServingBoard.ResolveSlot()` bir
    slot numarası ya da referans okumaz, **hacim** arar (`BurgerCounterSlot.Contains`). Hiçbir yuva
    collider'ının içinde durmayan tahta malzemeleri kabul eder, yığını dizer, hiçbir hata vermez —
    ama **hiç servis yapmaz**: müşteri sabrı dolana kadar bekler. Tek işaret konsoldur (engellenen
    servis sebebini yazar), oyuncunun gördüğü hiçbir şey değişmez.
...
216. **Quest 3S'te `BODY_TRACKING` kurulumda verilen normal izindir — izinsiz yol build'siz test
    edilemez.** `pm revoke` çalışmaz, gözlük ayarlarında anahtarı yoktur ve `bodyapiservice`'i zorla
    durdurmak oyunu da öldürür. İzin reddedilmiş hâli denemenin tek yolu, manifestinde bu izin
    **olmayan** ayrı bir APK'dır.

217. **"Susmuş ama `connected`" — bağlantı durumu canlılık ölçüsü DEĞİLDİR.** Uyuyan (güç tuşu),
    duraklatılmış ya da ağı giden bir gözlük heartbeat kapısı dolana kadar 10–20 sn `connected`
    görünür; sahada fiziksel bir şeyi (obje sahipliği gibi) yalnız `connection`'a bağlayan kapı o
    süre boyunca donmuş durumu herkesin ekranında tutar — obje donmuş elde ya da bırakıldığı
    noktada havada asılı kalır. Kural: böyle bir kapıda poz sessizliği de sorulur
    (`OBJECT_OWNER_SILENCE_MS`, sahiplik için `MatchDirector.TickObjectOwnersLocked`).
...
(faz makinesi, vuruş hattı, free-roam canlanma, kill-feed/HUD) + **ağ nesneleri** (oyuncu olmayan
varlıkların canı sunucuda; kırılma herkeste aynı anda görünür — §3.6b) · **üç oyun modu** — `tdm` (Takım
...
**çok katlı arena** (kat portalı; kat = rig kökünün dikey sanal ofseti, başka kattaki rakip
izleyenin katına düşen silüet + kendi katında vurulabilir gerçek gövde — §3.13) ·
admin **sahne-içi gözlemci** (üç kamera kipi + sahne üstü yönetim HUD'ı,
çoklu admin, çok katlı arenada **kat seçici** — kuş bakışı yalnız seçili katı gösterir) · geliştirici araç seti (`Tools > VortexArena > Development > Dev`, `dev-targets.json`,
...
**Planlanmamış ufuk:** yeni modlar (bölge kontrolü, turnuva, silah yarışı, zombi), Meta colocation/paylaşımlı anchor araştırması
