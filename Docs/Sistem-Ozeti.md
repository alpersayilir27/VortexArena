...
149. **Body tracking iskeleti Quest'in KENDİ zemin tahminine basar — A/B kalibrasyonu RİG'i
    ...
    **Düzeltme gönderendedir:** `ArenaNetCharacterBehaviour.HeadPinOffset` — kök tele konmadan önce
    karakterin göz işaretçisi (`EyeAnchor`) rig'in `centerEyeAnchor`'ına sabitlenir; iki referans
    aynı fiziksel nokta (oyuncunun gözü) olduğu için fark o karedeki toplam iskelet kaymasını verir
    ve kök o vektörle ötelenir (salt konum; blob köke göreli olduğundan iskelet tek parça taşınır).
    ⚠️ Bu sabitleme KALDIRILMAZ ve yerel karakterin transformuna TAŞINMAZ — yerel karakter §10.8
    boy ölçümünün referansıdır.
