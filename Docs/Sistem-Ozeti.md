...
kat 0'a). Fiziksel konumu değişmez, yalnız dikey ofseti değişir — üst katta ölen oyuncu aksi hâlde
`base` şartını hiç sağlayamazdı. Zaten o kattaysa hiçbir şey olmaz.
...
zeminine oturmayan portal listeye girmez ve kendini kapatır. Liste portal açılıp kapandıkça ve sahne
...
anında. Gerekçe §3.7'dedir: tabanına yürüyerek canlanan oyuncunun tabanı başka kattaysa şartı hiç
sağlayamazdı.
...
izdüşürülmüş bir **hayalet kopya** çizilir (`RemoteAvatar`: aynı kemikleri paylaşan ikinci bir
`SkinnedMeshRenderer` kabuğu, `AvatarGhost` materyali, `_FloorShift` = iki kat yüksekliğinin farkı;
collider yok, gölge yok, nabız yok) ve ad etiketine " (ÜST KAT)" / " (ALT KAT)" eki gelir. Hayalet
yalnız bir sunumdur: vuruş hattı onu tanımaz. ⚠️ `ProximityWarning` bilinçle Y'yi yok sayar — iki
oyuncu farklı katlarda da olsa **fiziksel olarak** aynı noktada duruyor olabilir.

**Uzak iskeletin kökü** kat defterinden gelir (`ArenaNetCharacterBehaviour`): alıcı kök zeminini
`ArenaFloors.HeightOf(FloorState.PlayerFloor(id))` alır (kafa onun altında kalıyorsa `FloorAt`'a
düşer), gönderici T-poz kökünü `FloorState.LiftMeters`'a yazar ve SDK kare kabul kapısı
...
takımın hayalet rengi; collider yok, gölge yok, nabız yok) çizilir — yalnız gövde görünürken ve kat
farkı varken açık, ad etiketi " (ÜST KAT)" / " (ALT KAT)" ekini alır. Hayalet **sunumdur**, vuruş
...
kaynağıyla karartmanın arkasında yapar; süren bir geçiş varken ikinci çağrı **yok sayılır** (fadeyi
yeniden başlatmak arenayı yanlış kattan bir kare gösterirdi). Sınırlarda uzlaşma: yeniden bağlanınca
yerel kat 0 değilse yeniden bildirir (sunucu `hello`'da sıfırlıyor), sahne yüklenince kat 0'a iner
ama **göndermez** (sunucu `load_match`'te sıfırlıyor), bekleyen istek yokken sunucu farklı bir kat
yazarsa yerel kat ona uyar. Ölüm dalı buradadır (§3.7/§3.13): `PlayerCombatState`'in açık taban
katına ~1 sn'lik karartmayla iner. Logları `[Kat] …` önekiyle akar.
...
kullanamaz** (ofset hizalanmamış bir rig'i anlamsız bir yüksekliğe taşır). Çemberler Scene view'de
...
hayalet kopya + kendi katında vurulabilir gerçek gövde — §3.13) ·
