...
| `BurgerDispenser` | Elin soketin içinde kavramaya basması `take` olayını yollar. ⚠️ Tür `grab:"none"` ve objede `NetObjectGrabBridge` **yoktur** — dağıtıcının kendisi hiç alınmaz, yalnız olay üretir; malzeme **sunucuda, doğrudan elde** doğar (`object_spawn` + `owner` + `Held`), yerelde hiçbir şey üretilmez. ⚠️ Basış kenarı **her karede el başına** izlenir, sokete girişte örneklenmez: zaten sıkılı bir el hacme sürüklenerek malzeme alırdı. **Boşa çıkan istek gözlükte de loglanır** (`[BurgerDispenser] … el dolu` / `take istendi`) ve sunucu tarafındaki karşılığıyla (`[world] object_event reddedildi (… → netId …): faz …`) eşleşir: "bastım, hiçbir şey olmadı" şikâyetinde eli mi dolu yoksa fazın mı kapalı olduğu ancak iki satır birlikte okunarak ayrılır |
