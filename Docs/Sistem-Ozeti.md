...
| `Combat/ItemGripPose` | **Bir kavrama kaydı** (serialize edilen struct): `authored` + kumanda anchor'ının eşyaya göre yerel konumu/euler'i + parmak `preset`'i. `LocalPose` anchor'ın eşyaya göre pozunu, `InverseLocalPose` aynı ölçünün ters yönünü (eşyanın ANCHOR'a göre pozu) verir — ana el ikincisini kullanır. ⚠️ **Konum METREdir ve eşyanın görsel ölçeğiyle BÜYÜTÜLMEZ:** geri bileşim her zaman `item.position + item.rotation * position` ile yazılır, `Transform.TransformPoint` ile DEĞİL — `WPN_*` kökleri 0.8 ölçekli, `TransformPoint` aynı ölçüyü ikinci kez uygular ve el silahın yanında yüzer. Stüdyo da aynı simetrik yolla yazar, iki uç tek sözleşmede kalsın. ⚠️ **Kayıt gözlükle yakalanmaz, stüdyoda yazılır:** yakalanan kayıt kaçınılmaz olarak o anki kumanda eğikliğini içerir ve o eğiklik eşyanın kendi dönüşünü sürdüğü için doğrudan namluya taşınırdı |
