...
skeleton   (sunucu RX) = SKELETON_RATE_HZ × (14 + B) × N            ← B = blob boyu
...
`⌈(11+B)×N / 1200⌉` ile böler. `B` bilinmeden hesap yapılmaz — **ölçülür:** gönderen ilk karede blob
boyunu konsola basar (`İskelet blob'u … B`), avatar sahneyle kapanırken adet/en küçük/en büyük/ortalama
özeti düşer; sunucu konsolundaki `[state]` satırı ise batch'lenmiş çıkışı doğrudan verir
(`iskelet … p/s … kB/s` — paket N oyuncunun batch'i olduğu için ortalama blob buradan türetilmez).
Kanal ayrıca poz kanalından **daha düşük hızda** akar; oradaki
...
| **Toplam** | | **~1,0 Mbps** | **~630** | **~1.050** |

Sakin durumda (lobi, atış yok) `0x02` + poz + yoklama kalır: **~0,8 Mbps, ~450 paket/sn**.

**1 Gbps'lik bir AP'ye göre bant %0,1'dir.** Buna karşılık ~730 çerçeve/sn (TCP ACK'leri dahil),
