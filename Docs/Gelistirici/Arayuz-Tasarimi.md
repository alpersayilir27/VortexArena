...
(kafaya kilitli şerit: can barı + durum satırı + tur/skor paneli + tur sonucu) ve `DeathHud` (ölüm
ekranı) üçünde de, `RoundNoticeHud` (merkez bildirimi) yalnız tur tabanlı HUD'da. `ModeHudBase`'in
geri kalan alanları (`phaseText` · `timeText` · `scoreText` · `killFeedText` · `standingsText`)
