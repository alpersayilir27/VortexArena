...
Üçünün de içi **bilerek boştur**: taşıdıkları tek şey nested prefab örnekleridir — `HealthHud` (can
barı + durum satırı) ve `DeathHud` (ölüm ekranı) üçünde de, `RoundNoticeHud` (merkez bildirimi)
yalnız tur tabanlı HUD'da. `ModeHudBase`'in geri kalan alanları (`phaseText` · `timeText` ·
`scoreText` · `killFeedText` · `standingsText`) **bağlanmamıştır**; taban bağlanmayan alanı çizmez.
Görüş alanına yeni bir öge eklenecekse yeri bu prefablardır, alan `ModeHudBase`'de zaten hazır.
