...
Üçünün de içi **bilerek boştur**: yalnız iki nested prefab örneği taşırlar — `HealthHud` (can barı +
durum satırı) ve `DeathHud` (ölüm ekranı). `ModeHudBase`'in geri kalan alanları (`phaseText` ·
`timeText` · `scoreText` · `killFeedText` · `standingsText`) **bağlanmamıştır**; taban bağlanmayan
alanı çizmez. Görüş alanına yeni bir öge eklenecekse yeri bu prefablardır, alan `ModeHudBase`'de
zaten hazır.
