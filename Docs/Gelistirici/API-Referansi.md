...
| `Tools > VortexArena > Weapons > Kavrama Pozu Stüdyosu` | Silahın elde nasıl duracağını **gözlüksüz** yazar (`GripPoseStudio`). `WPN_*`'ı prefab kipinde aç → *Ana/Ön Kabza Ellerini Oluştur* → kumanda çerçevelerini (ve çocukları olan hayalet elleri) kabzalara oturt → elin Inspector'ından parmak preset'i → **Kaydet**. Kayıt `WD_*.asset`'e gider (kumanda anchor'ının eşyaya göre pozu + preset); prefaba hiçbir şey yazılmaz, eller stage'in ayrı kökleridir. ⚠️ Kökü yalnız taşımak silahı kumandayla hizalı bırakır, çevirmek ana elde silahı döndürür |
