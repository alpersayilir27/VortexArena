...
| `Tools > VortexArena > Weapons > Kavrama Pozu Stüdyosu` | Silahın elde nasıl duracağını **gözlüksüz** yazar (`GripPoseStudio`). `WPN_*`'ı prefab kipinde aç → *Ana/Ön Kabza Ellerini Oluştur* → kumanda çerçevelerini (ve çocukları olan hayalet elleri) kabzalara oturt → elin Inspector'ından parmak preset'i → **Kaydet**. Kayıt `WD_*.asset`'e gider (kumanda anchor'ının eşyaya göre KONUMU + preset); prefaba hiçbir şey yazılmaz, eller stage'in ayrı kökleridir. ⚠️ Kök yalnız TAŞINIR — kayıt dönüş taşımaz, silah her zaman kumandayla hizalıdır (çevrilen kök geri hizalanır) |
