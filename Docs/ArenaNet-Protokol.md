...
**Duruş telde gitmez.** Eşyanın ele göre duruşu her istemcinin **kendi APK'sındaki** eşya verisinden gelir: el başına yazılmış kavrama kaydından (`ItemGripPose` — **kumanda anchor'ının** eşyaya göre yerel KONUMU; ⚠️ **kayıtta DÖNÜŞ YOKTUR**, eşyanın dönüşü her zaman ana kumandanınkidir). Kayıt telde giden el pozuyla **aynı uzaydadır**, yani `ItemGripSolver` onu doğrudan okur (`ItemDefinition.PrimaryGripPosition`); ön kabza kaydı (`secondaryGrip`) ise yalnız ikinci elin GÖRSELİNİN yapışacağı yeri söyler, eşyayı ne kaydırır ne döndürür. ⚠️ **Kavrama kaydı tel formatını DEĞİŞTİRMEZ:** iki uç aynı `WD_*`'ı okur ve arada çevrilecek bir şey yoktur — rig'i olmayan izleyici (admin) de eşyayı oyuncuyla birebir aynı çizer. Ön koşulu **kanonik kavramadır**: her eşyanın eline denk gelen noktası sabittir (serbest kavrama = keyfi ofset = uzak tarafta yanlış duruş). Aynı sebeple namlu yönü de telde gitmez — `muzzle` çocuğu prefabdadır.
