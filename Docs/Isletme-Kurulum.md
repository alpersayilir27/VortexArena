...
> **Boy ölçekleme kalibrasyondan AYRI bir adımdır ve operatörün işidir** (`Docs/ArenaNet-Protokol.md` §10.8). Kalibrasyon bittikten sonra oyuncu **ayakta ve dik** dururken admin panelinde satırındaki **ÖLÇ** düğmesine (ya da tercihler panelindeki **TÜM OYUNCULARI ÖLÇEKLE**'ye) basılır; avatar o oyuncunun boyuna ölçeklenir ve düğmenin etiketi çarpanı gösterir (`×1.04`). Ölçüm anında oyuncu eğilir/yürürse ölçüm **reddedilir** ve eski değer durur — tekrar basmak yeter. ⚠️ Ölçüm kalibrasyondan otomatik tetiklenmez: oyuncu o sırada kumandayı zemine değdirmek için eğilmiş durumdadır. ⚠️ Kalibrasyon sıfırlanınca ölçek de sıfırlanır.
>
