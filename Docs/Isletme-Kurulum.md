...
> **Boy ölçekleme kalibrasyondan AYRI bir adımdır ve operatörün işidir** (`Docs/ArenaNet-Protokol.md` §10.8). Kalibrasyon bittikten sonra oyuncu **ayakta ve dik** dururken admin panelinde satırındaki **ÖLÇ** düğmesine (ya da istatistik panelinin alt şeridindeki **TÜMÜNÜ ÖLÇEKLENDİR** düğmesine) basılır; avatar o oyuncunun boyuna ölçeklenir ve düğmenin etiketi çarpanı gösterir (`×1.04`). Ölçüm anında oyuncu eğilir/yürürse ölçüm **reddedilir** ve eski değer durur — tekrar basmak yeter. ⚠️ Ölçüm kalibrasyondan otomatik tetiklenmez: oyuncu o sırada kumandayı zemine değdirmek için eğilmiş durumdadır. ⚠️ Kalibrasyon sıfırlanınca ölçek de sıfırlanır.
>
...
- [ ] **12.** Ölen oyuncu ölüm ekranını gördü → **5 sn** sonra kendi takımının taban bölgesine (`BaseZone` — renkli şerit) fiziken girdi → **canlandı** (`canlandı: Gözlük NN`). Bölgeye girdiği hâlde satır düşmüyorsa taban/ölüm akışını gözden geçir — oyuncunun kendi kendine canlanmasının başka bir yolu yoktur (kalan tek çare operatörün satırdaki **CAN** düğmesidir, `operatör canlandırdı: Gözlük NN`).
...
| Bir oyuncunun avatarı diğerlerinde **olduğundan kısa/uzun** görünüyor | Boy hiç ölçülmemiş ya da ölçüm eğilmişken alınmış | Oyuncuyu **ayakta ve dik** durdur, admin panelinde satırındaki **ÖLÇ** düğmesine bas; etiket çarpanı yazınca ölçüm oturmuştur. Düğme pasifse oyuncu kalibresizdir — önce kalibrasyon |
