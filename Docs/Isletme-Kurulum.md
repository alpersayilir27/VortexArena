...
4. Hata sinyalleri (ikisinde de ikinci çapa hiç belirmez):
   ...
   - **Tek titreşim (0,6 sn)** = iki noktanın ölçülen zemin yüksekliği 10 cm'den fazla ayrışıyor; bir noktada kumandanın ucu zemine değmemiş (ya da zemin eğimli). Tekrar al.
...
> **Kumandayı nasıl tutacağın önemli DEĞİL:** ölçülen nokta kumanda pivotunun sabit bir miktar **altıdır** ve bu ofset dünyaya göre uygulanır — eğik tutmak, çevirmek, gözlüğün nereye baktığı sonucu değiştirmez. Tek koşul **kumandanın ucunun zemin işaretine değmesidir**.
>
> **Ofset neden var:** başlığın takip ettiği nokta kumandanın **ucu değil gövdesinin içindeki pivottur**. Yazılım bu farkı `ArenaCalibrator.floorProbeDropMeters` ile telafi eder; alan `VA_CalibrationManager` **prefabında** yaşar (tüm arenalar ve lobi aynı örnekten gelir, sahne başına kopya tutulmaz).
>
...
>
> `R` başlıktan başlığa ve oturumdan oturuma **belirgin biçimde** oynuyorsa tek bir sabit onu yutamaz: o zaman başlıkların zemin yüksekliği ayarı düzeltilmeli ve gözlük **yerde dururken** takılmalıdır (havada açılan gözlük yanlış tahminle başlar).
>
> Gözle doğrulaması: kalibrasyondan sonra sanal çapa işaretçileri **bir saniye** görünür olur — o pencerede kumandanın ucu çapa küpünün **merkeziyle** aynı hizada olmalı (küpün yarısı zeminin altındadır, tek Y sözleşmesi budur). Pencere kısadır çünkü işaretçiler kurulum aracıdır, arena dekoru değil; süresi `ArenaCalibrator.markerVisibleSeconds` alanından uzatılabilir.
...
> **Boy ölçekleme kalibrasyondan 10 saniye sonra kendiliğinden alınır; operatör her an yeniden alabilir** (`Docs/ArenaNet-Protokol.md` §10.8). Ölçü, o andaki **gözlük yüksekliğidir**: gözlüğü kafasında ve ayakta olan oyuncu doğru ölçülür, eğilmiş ya da gözlüğü elinde tutan oyuncu yanlış — sistem duruşu sorgulamaz. Ölçüm gelene kadar herkes **1,80 m** sayılır; sonuç 1,00–2,20 m'ye kırpılır. Yanlış çıkarsa oyuncuyu **ayakta ve dik** durdurup admin panelinde satırındaki **ÖLÇ** düğmesine (ya da istatistik panelinin alt şeridindeki **TÜMÜNÜ ÖLÇEKLENDİR** düğmesine) bas; düğmenin etiketi çarpanı gösterir (`×1.04`). ⚠️ Kalibrasyon sıfırlanınca ölçek de sıfırlanır; uygulama yeniden açılınca ölçü hatırlanmaz (1,80 m'den başlar).
>
> **Kalibrasyon durumu sunucuda tutulur ve operatör tarafından sıfırlanabilir** (`Docs/ArenaNet-Protokol.md` §10.6). Bir başlığın hizalaması sahada kayarsa operatör admin ekranındaki **KAL** düğmesiyle o oyuncuyu savaş dışı bırakır (ateş edemez, hasar yemez, canlanamaz; avatarı diğerlerinin ekranında parlar), oyuncu yeniden kalibre olunca kaldığı yerden devam eder. **Kalibre durumdayken elle kalibrasyon kilitlidir** — oyuncu kendi hizalamasını kazara bozamaz. Sahanın tamamı için istatistik panelinin alt şeridinde iki düğme vardır: **TÜMÜNÜ KALİBRE ET** kimseyi savaş dışı bırakmadan herkesin gözlüğüne kayıtlı çapadan hizalamayı yeniden yükletir; **HİZALAMALARI SIFIRLA** herkesi savaş dışı bırakır. ⚠️ **Sıfırlama her yerde TEK düğmedir ve şiddeti basış süresi seçer:** kısa basış gözlüklerdeki kaydı **korur** (ardından TÜMÜNÜ KALİBRE ET çalışır — seans devrinin normal yolu budur), **1 saniye basılı tutmak** kaydı da yok eder (ardından TÜMÜNÜ KALİBRE ET iş görmez ve herkes elle A/B almak zorunda kalır — yalnız zemin bantları yer değiştirdiğinde). Aynı kural oyuncu başına **KAL** düğmesinde de geçerlidir. Operatör yönergesi: `Docs/Kullanim-Kilavuzu.md` §4.1.
...
- [ ] **7b.** Geçişte iki başlık da **kısa süre karardı ve arena siyahtan açıldı**; karartma sürerken yükleme yazısı/yüzdesi okunabiliyordu ve arenaya girildiğinde ekran açık kaldı (siyahta takılı kalan başlık yok).
- [ ] **8.** **Her iki başlıkta da kalibrasyon yapıldı** (Bölüm 3 prosedürü, A ve B işaretleri üzerinde); ardından her ikisi **dik dururken** admin panelinden **ÖLÇ** ile ölçüldü → karşıdakinin avatarı gerçek boyuyla uyumlu görünüyor.
...
- [ ] **12.** Ölen oyuncu ölüm ekranını gördü → **5 sn** sonra kendi takımının taban bölgesine (`BaseZone` — renkli şerit) fiziken girdi → **canlandı** (`canlandı: Gözlük NN`). Bölgeye girdiği hâlde satır düşmüyorsa taban/ölüm akışını gözden geçir — ⚠️ canlanmanın **tek yolu** budur, operatörün elle canlandırma düğmesi yoktur.
- [ ] **13.** Maç süresi/skor limiti dolunca `maç sonu — kazanan: …` yayınlandı; kazanan ekranı **kendiliğinden kapanmadı**, admin'den lobiye dönülünce tüm başlıklar **lobiye döndü**.
...
| Bağlanıyor ama roster'da "yeniden bağlanıyor" düşüyor (45 sn dönmezse "ayrıldı") | 15 sn boyunca status gelmedi (Wi-Fi zayıf, başlık uykuya geçti) | AP kapsamasını/kanalı kontrol et; başlıkta uyku süresini uzat. Cihaz geri gelince aynı satıra oturur |
...
| Avatarlar doğru yerde ama **yükseklikleri yanlış** (yere gömük / havada) | Kalibrasyonda kumandanın ucu yere değmemiş; ya da `ArenaCalibrator.floorProbeDropMeters` o kumanda modeli için henüz ölçülmemiş (varsayılan 0,08 m tahmindir) | O başlıkta kumandanın **ucunu zemine değdirerek** yeniden kalibre et (nasıl tuttuğun önemli değil). Gömülmenin gerçek olup olmadığını kalibrasyondan birkaç saniye sonraki `[Kalibre] hizalama sonrası kafa yüksekliği` satırı söyler (admin konsolunda **görünmez** — gözlük logunda ve sunucu konsolunda çıkar; admin konsolu yalnız `[Kalibre] <oyuncu>: kaynak=… zemin=…` satırını gösterir) — o sayı oyuncunun gerçek göz yüksekliğine eşit olmalı; "zemin sapması" uyarısı tek başına gömülme kanıtı DEĞİLDİR (başlığın zemin tahmini bayatken de çıkar, hizalama doğruyken). Tüm başlıklarda aynı yönde sapma varsa ofset yanlıştır: §3'teki reçeteyle ölç, alanı güncelle, APK'yı yeniden al |
| Bir oyuncunun avatarı diğerlerinde **olduğundan kısa/uzun** görünüyor | Kalibrasyondan 10 sn sonraki otomatik ölçümde oyuncu eğilmiş/çömelmiş ya da gözlük kafasında değildi; ya da hiç kalibre olmadı (1,80 m varsayımı) | Oyuncuyu **ayakta ve dik** durdur, admin panelinde satırındaki **ÖLÇ** düğmesine bas; etiket çarpanı yazınca ölçüm oturmuştur. Düğme pasifse oyuncu kalibresizdir — önce kalibrasyon, ölçüm 10 sn sonra kendiliğinden gelir |
...
| Ölen oyuncu canlanmıyor | 5 sn gecikme dolmadı; oyuncu **kendi takımının** taban bölgesine fiziken girmedi | Oyuncuya doğru tabana (takım rengi) yürümesi söylenir — **kendiliğinden canlanmaz**, tabana girmeden canlanma yoktur |
