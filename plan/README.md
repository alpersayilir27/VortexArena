...
| Elde tutulan eşya + atış olayları: **Faz 0–3 + soket kavrama + olay zamanlaması bitti**. Kalan: ⚠️ **altı silahın kavramasını yazmak** (araç hazır — prefabı prefab kipinde aç → `Kavrama Pozu Stüdyosu` → elleri oluştur, kabzalara oturt → Kaydet) · tracer/soket değerleri · bomba | `elde-tutulan-esya-ve-atis-olaylari.md` |
...
| **İskelet konum sabitlemesi** (alıcıda, gövde poz kanalının kafasına yumuşatılmış ofsetle sabitlenir — OS zemin tahmini sapması + body tracking'in app-açılışında tutmaması arızalarına karşı; Sistem-Ozeti §7): kod **bitti** (`UpdateHeadPin`). Kalan: editör + saha doğrulaması, gözlük reboot'lu Antep tekrarı, `bodyScale≠1` turu. ⚠️ Yeni APK + admin build ister | `iskelet-zemin-duzeltmesi.md` |
