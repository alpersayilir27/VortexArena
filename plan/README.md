...
| Planlanmış iş | Dosya |
|---|---|
| **Meta Movement SDK ile full body avatar**: kod, protokol (`0x07`/`0x08`), doküman ve **prefab kurulumu bitti** (iki avatar da Ch15 retarget config'i + `ArenaNetCharacterBehaviour` ile kurulu; 66 eklem eşleşmiş; tele giden liste `SkeletonWire.JOINT_INDICES`). Kalan: ayar maddeleri ve açık riskler | `meta-movement-full-body.md` |
...
| **Kavrama: avuç çerçevesi + ikinci el**: kod (`HandGripPivot`, `ItemGripSolver`), doküman ve 13 silahın dört kavrama kaydı **bitti**; başlıktaki davranış doğrulandı. Kalan: karşı taraftan (uzak avatar / admin) görünüm doğrulaması | `iki-elli-kavrama.md` |
...
| **Kalibre modu + T-poz sağlamlaştırma + ölçek geri bildirimi**: kod (sunucu, istemci, admin paneli üç mod düğmesi) ve doküman **bitti**. Kalan: zemin sapması uyarısının ve T-poz yedeğinin doğrulaması | `kalibre-modu-ve-olcek.md` |
