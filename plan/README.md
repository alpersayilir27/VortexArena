...
| **Çocuk Oyunları — Köstebek Ezme** (ailenin ilk yarışmalı oyunu): kırmızı–mavi takımlar, iki elde balyoz, zemindeki deliklerden 2 sn'liğine çıkan köstebekler; kendi rengini ezmek takıma +puan, rakip renge vurmak kendi takımından −puan; ölüm/taban yok, süre bitince yüksek skor kazanır, oyuncu başına doğru/yanlış/katkı tutulur. Mod, oyun mantığı, doküman ve prototip içerik kurulumu **bitti** — köstebek arenası, delikler, tür asset'i, balyoz ve HUD yerinde; protokol sürümü değişmedi; ses kancaları ve yanlış vuruş rengi `MoleHole`'da hazır. Kalan: gerçek modeller/animasyonlar, ses klipleri, balyozun kavrama pozu, doğrulama | `cocuk-oyunlari-kostebek-ezme.md` |
...
| **Bağlantı kopması**: kod, protokol (v8) ve doküman **bitti** — "çevrimdışı" kalktı, yerine `connected`/`reconnecting`/`left` + maç katılımcısı defteri geldi. Kalan: doğrulama listesi (⚠️ v8 tüm başlıklara yeni APK ister) | `baglanti-kopmasi-ve-mac-katilimcilari.md` |
...
| **Yüzey çarpma efektleri** (kar duvarda kar, tahtada talaş, metalde kıvılcım): yüzey kimliği **materyalden** çözülür (tag/layer değil) + `SurfaceTag` override; havuzlu prefab, `ArenaCombat.ReportImpact` tek kapısı. Protokole ekleme YOK — uzak taraf çarpma noktasını atış olayından türetiyor. Kod ve doküman **bitti** (`SurfaceDefinition` · `SurfaceLibrary` · `SurfaceTag` · `SurfaceImpactFx`; `Weapon.hitEffectPrefab` kalktı). Kalan: yüzey seti + parçacık/ses içeriği, materyal eşlemesi, doğrulama | `yuzey-carpma-efektleri.md` |
...
| **Engel ihlali** (kafası iç engele giren oyuncu kör kalır, 3 sn sonra canı erimeye başlar, 8. sn'de ölür; namlusu engelin içinden geçen silah ateş etmez): kod, protokol (v11) ve doküman **bitti** — sözleşme `Obstacle` layer'ı (⚠️ collider KONVEKS olmalı), bayrak `SnapshotEntry` bit5. Kural **yalnız kafayı** yargılar (el/kol/gövde ceza üretmez, oran kuralı yok), karartma içerideyken koşulsuz tam siyahtır, can kaybının kırmızısı karartmanın **üstünde** ayrı bir katmandır ve **ölümü tek kod yazar** (`KillPlayerLocked`; engelde canlanma yok). Kalan: layer'ı hâlâ eksik arenalar (sahne başına elle, açmadan görülemez — ⚠️ oralarda iş "layer değiştir" değil **konveks collider koymak**: environment paketleri konkav `MeshCollider` ile geliyor ve öylesi çalışma anında eleniyor) + kesişim VFX'i + doğrulama. ⚠️ Yeni APK ister | `engel-ihlali.md` |
| **Mekan boyut maketi**: boyut dosyaları, prefablar, sahneler ve doküman **bitti**. Kalan: maketten JSON'a geri yazımın dosyayı bozmaması (sayı biçimi · `height` 0 koruması · köşe sırası) · muhafazayı taşımaya çağıran editör metinleri · sağlık raporunda orijin bekçisi · doğrulama | `mekan-boyut-maketi.md` |
| **Kalibre modu + T-poz sağlamlaştırma + ölçek geri bildirimi**: kod (protokol **v13**, sunucu, istemci, admin paneli üç mod düğmesi) ve doküman **bitti**. Kalan: doğrulama listesi. ⚠️ v13 tüm başlıklara yeni APK ister | `kalibre-modu-ve-olcek.md` |
| **Admin ihlal görünürlüğü** (engel + alan dışı halkada renk/frekans, sunucu-kaynaklı ihlal akışı ve defteri, alan dışında ateş kapısı): kod, protokol (**v14** — rezervdeki bit7 `FLAG_OUT_OF_BOUNDS`, tel formatı ve paket boyu DEĞİŞMEDİ) ve doküman **bitti**. Kalan: doğrulama listesi (uyarı sesi bilinçli olarak yok). ⚠️ Silah/namlu engel durumu tele girmez — kaynağında zaten engelleniyor. ⚠️ v14 tüm başlıklara yeni APK ister | `admin-ihlal-gorunurlugu.md` |
