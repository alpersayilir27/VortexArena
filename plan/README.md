...
| **Engel ihlali** (kafası iç engele giren oyuncu kör kalır, 3 sn sonra canı erimeye başlar, 8. sn'de ölür; namlusu engelin içinden geçen silah ateş etmez): kod, protokol (v11) ve doküman **bitti** — sözleşme `Obstacle` layer'ı (⚠️ collider KONVEKS olmalı), bayrak `SnapshotEntry` bit5. Kural **yalnız kafayı** yargılar (el/kol/gövde ceza üretmez, oran kuralı yok), karartma içerideyken koşulsuz tam siyahtır, can kaybının kırmızısı karartmanın **üstünde** ayrı bir katmandır ve **ölümü tek kod yazar** (`KillPlayerLocked`; engelde canlanma yok). Kalan: kesişim VFX'i · kırmızı çerçevenin yoğunluğu ve öldüren vuruşta da çizilmesi (`DamageVignette`) · doğrulama. ⚠️ Yeni APK ister | `engel-ihlali.md` |
| **Mekan boyut maketi**: boyut dosyaları, prefablar, sahneler ve doküman **bitti**. Kalan: muhafazayı taşımaya çağıran editör metinleri · sağlık raporunda orijin bekçisi · geri yazımda dosya biçiminin korunması (`ToJson` kaynağın nokta biçimini aynalar) · kopya boyut dosyasının silinmesi · doğrulama | `mekan-boyut-maketi.md` |
| **Kalibre modu + T-poz sağlamlaştırma + ölçek geri bildirimi**: kod (protokol **v13**, sunucu, istemci, admin paneli üç mod düğmesi) ve doküman **bitti**. Kalan: T-poz yedeğinin dört kaçağı (lobide kurulmaması · Ayarlar kapatmasında donmuş veri · arka planda kol IK'sının T-pozu bozması · eklem makullüğü denetimi yok — protokol değişmez, göndericide + `RemotePoseBody`'de) · doğrulama listesi. ⚠️ v13 tüm başlıklara yeni APK ister | `kalibre-modu-ve-olcek.md` |
| **Admin ihlal görünürlüğü** (engel + alan dışı halkada renk/frekans, sunucu-kaynaklı ihlal akışı ve defteri, alan dışında ateş kapısı): kod, protokol (**v14** — rezervdeki bit7 `FLAG_OUT_OF_BOUNDS`, tel formatı ve paket boyu DEĞİŞMEDİ) ve doküman **bitti**. Kalan: geç bağlanan admine açık ihlal tekrarının `lobby_state`'ten ÖNCE varması (satır adsız, sayaçlar kayıp — sunucu sırası + sayaçların `PlayerInfo`'ya taşınması) · doğrulama listesi (uyarı sesi bilinçli olarak yok). ⚠️ Silah/namlu engel durumu tele girmez — kaynağında zaten engelleniyor. ⚠️ v14 tüm başlıklara yeni APK ister | `admin-ihlal-gorunurlugu.md` |
