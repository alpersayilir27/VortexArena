...
| Elde tutulan eşya + atış olayları: **Faz 0–3 + soket kavrama + olay zamanlaması bitti**. Kalan: ⚠️ **13 silahın kavramasını yakalamak** (araç hazır — Dev penceresi → rol *Silah* → silahı seç → Play → pinch + 5 sn sayaç, el başına dört ölçü) · tracer/soket değerleri · bomba | `elde-tutulan-esya-ve-atis-olaylari.md` |
| **Kavrama: avuç çerçevesi + ikinci el**: kod (`HandGripPivot`, `ItemGripSolver`) ve doküman **bitti**. Kalan: avuç ofsetini başlıkta ölçmek + 13 silahın kavramasını Dev penceresi > rol *Silah* akışıyla yakalamak (yakalanmadan iki elli çözüm koşmaz: ön kabza ekseni oluşmaz) + doğrulama | `iki-elli-kavrama.md` |
...
| **Admin ihlal görünürlüğü** (engel + alan dışı halkada renk/frekans, sunucu-kaynaklı ihlal akışı ve defteri, alan dışında ateş kapısı): kod, protokol (**v14** — rezervdeki bit7 `FLAG_OUT_OF_BOUNDS`, tel formatı ve paket boyu DEĞİŞMEDİ) ve doküman **bitti**. Kalan: üç prefab bağı (tercih paneli satırı · istatistik kolonu · uyarı ses klibi) + doğrulama listesi. ⚠️ Silah/namlu engel durumu tele girmez — kaynağında zaten engelleniyor. ⚠️ v14 tüm başlıklara yeni APK ister | `admin-ihlal-gorunurlugu.md` |
