...
| Elde tutulan eşya + atış olayları: **Faz 0–3 + ön kabza kapısı/göstergesi + olay zamanlaması bitti**. Kalan: ⚠️ **13 silahın kavramasını yazmak** (araç hazır — `Kavrama Pozu Stüdyosu`, prefab kipinde, el başına poz + el modelinin kumanda üstündeki yerleşimi + silaha özel parmak rigi) · tracer/gösterge değerleri · bomba (→ `bomba.md`) | `elde-tutulan-esya-ve-atis-olaylari.md` |
| **Yol haritası** — bomba · ağ nesnesi modeli · kırılabilir objeler · oyun tipleri · Hamburgerci: sıra, bağımlılık, iki protokol sürüm grubu; kararlar verildi ve dosyalara işlendi | `yol-haritasi.md` |
| **Bomba ve atılabilirler**: sunucu, istemci kodu ve doküman **bitti** — protokol sürümü DEĞİŞMEDİ (sol bilek kılıfı · silah askıya alma · 5 sn fitil · patlamadan 3 sn sonra dolum · mermi sınırı yok · ağ nesnesi değil, yerel deterministik balistik · kendine hasar = dost ateşi anahtarı · atıcı ölse de hasar verir). Molotof/flashbang/sis aynı zemine oturur, **yeni tür sürüm artırmaz**. Prefab, tanım asset'i ve rig'deki kılıf kuruldu. Kalan: kavrama kaydı + katalog senkronu + model/FX/ses içeriği + sunucu derlemesi + doğrulama | `bomba.md` |
...
| **Çocuk Oyunları — Hamburgerci**: mod ve **oyun mantığı yazıldı** (sunucuda `BurgerMode` + tarif/sabır/pişme/servis, `object_state.s` örnek verisi, "duyuran = yazan" sözleşmesi, sahipli `SpawnObject`, `AddSharedScore`; istemcide dokuz Burger bileşeni; 14×16 kutusunda arena + prototip dükkân; admin "Oyun tipi" satırı). içerik kurulumu da yapıldı — 24 tür asset'i, 24 prototip prefab, `Resources/NetSpawnCatalog`, sahne yerleşimi, admin müşteri sayacı satırı, export. Kalan: gerçek modeller/sesler, dört propun kavrama pozu, doğrulama | `cocuk-oyunlari-hamburgerci.md` |
...
| **Kavrama: avuç çerçevesi + ikinci el**: kod (`HandGripPivot`, `ItemGripSolver`) ve doküman **bitti**. Kalan: avuç ofsetini başlıkta ölçmek + 13 silahın kavramasını stüdyoda yazmak (yazılmadan iki elli çözüm koşmaz: ön kabza ekseni oluşmaz) + doğrulama | `iki-elli-kavrama.md` |
