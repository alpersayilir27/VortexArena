...
| **Oyuncu boy ölçeği** (operatör düğmesiyle ölçülen üniform `bodyScale`, `lobby_state` ile bir kez taşınır; kalibrasyonda uç ofseti kumanda rotasyonundan bağımsızlaştı): kod, protokol (v9) ve doküman **bitti**. Kalan: üç prefabın kurulumu (⚠️ derlemeden sonra) + doğrulama. ⚠️ v9 tüm başlıklara yeni APK ister | `oyuncu-boy-olcekleme.md` |
| **Engel ihlali** (oyuncu İÇ engelin içine girince karartma + saniyede 30 HP + admin halkasının kırmızı yanıp sönmesi): kod, protokol (v11) ve doküman **bitti** — sözleşme `Obstacle` layer'ı (⚠️ collider KONVEKS olmalı), tespit ağırlıklı nokta örneklemesiyle, bayrak `SnapshotEntry` bit5. Kalan: engellerin layer'a alınması + kesişim VFX'i + doğrulama. ⚠️ v11 tüm başlıklara yeni APK ister | `engel-ihlali.md` |
