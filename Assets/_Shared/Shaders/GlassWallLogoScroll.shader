// Şeffaf cam / enerji duvarı + üstüne basılı, YATAYDA KAYAN marka logosu.
// `VortexArena/GlassWallLogo`'nun kayan varyantıdır: tek farkı logonun sabit hızla sola
// yürümesidir, cam/kenar/kesme davranışının tamamı aynıdır. İki shader AYRI durur çünkü
// duvarların bir kısmı sabit logo istiyor; hız alanını mevcut shader'a eklemek her materyalde
// sessizce bir animasyon riski taşırdı.
//
...
//
// ⚠️ Kayma DUVARIN KENDİSİNDE periyodiktir: merkez kipinde de sol kenardan çıkan logo sağ
// kenardan geri girer (döşeme kipinde zaten frac ile sürekli). Bunun görünür bir yan etkisi
// vardır — logo merkezi kenara yakın kurulup taşacak kadar büyütülürse taşan parça karşı
// kenarda görünür; kayma hızı 0 olsa bile bu geçerlidir.
Shader "VortexArena/GlassWallLogoScroll"
{
    Properties
    {
        ...
        [Header(Kaydirma)]
        // Birim: saniyede LOGO GENİŞLİĞİ. 0.5 = logo iki saniyede kendi boyu kadar sola yürür.
        // Ölçü logo birimindedir, böylece _LogoSize değiştirilince kayma hissi bozulmaz.
        // Artı = sola, eksi = sağa, 0 = sabit (o hâlde sabit logolu shader'la aynı görüntü).
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 LogoUV(float2 uv, float aspect, out float box)
            {
                ...
                // Kayma: ÖRNEKLENEN x büyüdükçe görüntü SOLA gider — bu yüzden artı hız sola akar.
                // Ofset logo biriminde eklenir (sizeUV'ye bölünmüş uzayda), yani hız _LogoSize'dan
                // bağımsızdır. ⚠️ _Time.y oyun boyunca büyür; frac'ın hassasiyeti çok uzun
                // oturumlarda düşer (kayma titreyebilir) — çare hızı büyütmek değil, gerekirse
                // materyali yeniden kurmaktır.
                ...
                return frac(luv);
            #else
                // Merkez kipinde DUVARIN KENDİSİ periyodiktir: sol kenardan çıkan logo sağ kenardan
                // geri girsin diye x, duvarın logo birimindeki genişliği (1 / sizeUV.x) kadarlık bir
                // aralığa katlanır. Aralık merkezden simetrik seçilir; böylece hız 0 ve logo merkezde
                // iken katlama hiçbir şey yapmaz ve görüntü sabit logolu shader'la birebir aynı olur.
                float period = 1.0 / max(sizeUV.x, 1e-4);
                float d = luv.x - 0.5;
                d = frac(d / period + 0.5) * period - period * 0.5;
                luv.x = d + 0.5;
